                  ...
                  DWORD dwFlags)
{
    ...
    sizel.cx = (long) MulDiv(w, 2540, iLogPixelsX);
    sizel.cy = (long) MulDiv(h, 2540, iLogPixelsY);
    pObj->pOleObj->lpVtbl->SetExtent(pObj->pOleObj,
                                     DVASPECT_CONTENT,
                                     &sizel);
...
void DeleteObj(POBJECT pObj)
{
    ...
    hResult = gpStorage->lpVtbl->DestroyElement(gpStorage,
                                                pObj->szStorageName);
...
BOOL GetObjData(POBJECT pObj, LPFORMATETC pFmt, LPSTGMEDIUM pMed, BOOL bHere)
{
    ...
    if (pFmt->cfFormat == CF_TEXT) {
        ...
        //
        // For DRAGON the pointl value is the mouse vector 
        // within the object in HIMETRIC units.
        // 

        pointl.x = (long) pObj->iXOffset;
        pointl.y = (long) pObj->iYOffset;
        ...
        if (!bHere) {
            ...
            //
            // Modify the object descriptor to have the size of the 
            // current rectangle we are displaying it in so it won't
            // apparently change size as we drag it
            //
            ...
            if (lpOD) {
                ...
                lpOD->sizel.cx = MulDiv(pObj->rc.right - pObj->rc.left,
                                        2540,
                                        iLogPixelsX);
                lpOD->sizel.cy = MulDiv(pObj->rc.bottom - pObj->rc.top,
                                        2540,
                                        iLogPixelsY);
                GlobalUnlock(hGlobal);
            }
