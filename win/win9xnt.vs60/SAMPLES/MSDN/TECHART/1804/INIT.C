...
BOOL InitCurrentInstance(HANDLE hInstance, LPSTR lpszCmdLine, int cmdShow)
{
    ...
    //
    // Say we do drag and drop (this is the regular Windows file drop)
    //

    DragAcceptFiles(ghwndMain, TRUE);

    //
    // Enable the OLE drop support
    //

    dprintf3("Registering for drag & drop");
    RegisterDragDrop(ghwndMain, 
                     (LPDROPTARGET) &(gDocInfo.DocIDropTarget));
    // Another bogus API call is required here:
...
void Terminate()
{
    ...
    RevokeDragDrop(ghwndMain);
    CoLockObjectExternal((LPUNKNOWN) &(gDocInfo.DocIDropTarget), FALSE, TRUE);
