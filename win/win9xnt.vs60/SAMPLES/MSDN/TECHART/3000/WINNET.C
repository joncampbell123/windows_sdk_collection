...

...

    {
      ...
      for (i=0; i<dwEntriesToGet; i++)
      {
        ...
        { case RESOURCEUSAGE_CONNECTABLE :
          ...
          case (RESOURCEUSAGE_CONTAINER | RESOURCEUSAGE_RESERVED) : // Top-level container
  ...
  PERR("WNetEnumResource - impossible to drop out of infinite loop");
  return(1);
}

/****************************************************************************\
  ...
  { case        NO_ERROR                  :
    ...
    case        ERROR_EXTENDED_ERROR      :
      { LPDWORD lpdwErrorCode;
        ...
        if (NO_ERROR != WNetGetLastError(lpdwErrorCode,
                                           (LPTSTR)ucErrorBuf,
                                            (DWORD)SZ_ERROR_BUF,
        ...
        else
        {
          ...
          sprintf(ucMsgBuf,
            "WNetGetLastError code = %d, Msg=[%s], Provider =[%s] line %d file %s",
            lpdwErrorCode,ucErrorBuf,ucProviderNameBuf,Line,File);
          PERR(ucMsgBuf);
        }
      }
      break;
