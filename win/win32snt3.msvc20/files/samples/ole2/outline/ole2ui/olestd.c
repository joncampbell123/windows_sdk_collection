...
// REVIEW: added IDS_OLE2UILINKEDTYPE to strings.rc
   ...
   if (fIsLink && lpszFullUserTypeName) {
      ...
      lpszBuf = OleStdMalloc(
            OLESTRLEN(lpszFullUserTypeName)+OLESTRLEN(szLinkedTypeFmt)+1);
      if (lpszBuf) {
         W2A (lpszBuf, lpszAnsiBuf, 256);
         W2A (szLinkedTypeFmt, szAnsiLinkedTypeFmt, 80);
...
)
{
   ...
   if (!IsEqualCLSID(rClsid,&CLSID_NULL) && CoIsOle1Class(rClsid) &&
      ...
      if (lpszProgID == NULL)
         goto Cont1;
      lpszBuf = OleStdMalloc(
            (ULONG)OLESTRLEN(lpszUserName)+OLESTRLEN(lpszProgID)+3);
      if (lpszBuf == NULL) {
         if (lpszProgID)
