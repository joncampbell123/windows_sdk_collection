...
void UpdateCVClassIcon(HWND hDlg, LPCONVERT lpCV, HWND hList)
   ...
   W2A (lpCV->lpszIconSource, lpszAnsiIconSource, 256);
   hIcon = HIconAndSourceFromClass(&clsid, lpszAnsiIconSource, &(lpCV->IconIndex));

   if (NULL == hIcon)  // Use Vanilla Document
   {
      lstrcpy(lpszAnsiIconSource, szOLE2DLL);
      A2W (lpszAnsiIconSource, lpCV->lpszIconSource, 256);
   ...
   if (lpCV->lpOCV->lpszDefLabel) {
      ...
      if ((cch = OleStdGetAuxUserType(&clsid, AUXUSERTYPE_SHORTNAME,
         ...
         if ((cch = OleStdGetUserTypeOfClass(&clsid, (LPOLESTR)szUniLabel,
            ...
            LoadString(ghInst,(UINT)IDS_DEFICONLABEL,szLabel,OLEUI_CCHLABELMAX);
            //cch = lstrlen(szLabel);
         }
      }
   }
