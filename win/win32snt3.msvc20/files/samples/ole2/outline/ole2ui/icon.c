...
BOOL CALLBACK EXPORT ChangeIconDialogProc(HWND hDlg, UINT iMsg
   ...
   UINT                    uRet=0;
   LPOLESTR                psz;
   char                    szTemp[OLEUI_CCHPATHMAX], tmp[OLEUI_CCHPATHMAX];
   OLECHAR                 szUniTemp[OLEUI_CCHPATHMAX];
         ...
         switch (wID)
            ...
            case ID_LABELEDIT:
               ...
               if (EN_KILLFOCUS==wCode)
                  {
                  GetDlgItemText(hDlg, ID_LABELEDIT, szTemp, OLEUI_CCHPATHMAX);
            ...
            case ID_FROMFILEEDIT:
               ...
               GetDlgItemText(hDlg, ID_FROMFILEEDIT, szTemp, OLEUI_CCHPATHMAX);

               W2A (lpCI->szFile, tmp, OLEUI_CCHPATHMAX);
               if (lpCI && lstrcmpi(szTemp, tmp))
                  {
            ...
            case ID_ICONLIST:
               switch (wCode)
                  ...
                  case LBN_SETFOCUS:
                     ...
                     W2A (lpCI->szFile, tmp, OLEUI_CCHPATHMAX);
                     if (lpCI && 0!=lstrcmpi(szTemp, tmp))
                        {
                        A2W (szTemp, lpCI->szFile, OLEUI_CCHPATHMAX);
                        UFillIconList(hDlg, ID_ICONLIST, szTemp);
            ...
            case ID_BROWSE:
            {
               ...
               //Copy for reference
               W2A (lpCI->szFile, szTemp, OLEUI_CCHPATHMAX);
               ...
               if (0==uRet)
                  uRet=(BOOL)Browse(hDlg, szTemp, NULL, OLEUI_CCHPATHMAX, (UINT)IDS_ICONFILTERS, dwOfnFlags);
                ...
                */
               W2A (lpCI->szFile, tmp, OLEUI_CCHPATHMAX);
               if (0!=uRet && 0!=lstrcmpi(szTemp, tmp))
               {
                  CheckRadioButton(hDlg, ID_CURRENT, ID_FROMFILE, ID_FROMFILE);
                  SetDlgItemText(hDlg, ID_FROMFILEEDIT, tmp);
                  UFillIconList(hDlg, ID_ICONLIST, tmp);
                  UpdateResultIcon(lpCI, hDlg, ID_FROMFILE);
               }
            }
            break;
            ...
            case IDOK:
               ...
               if (ID_FROMFILE & lpCI->dwFlags)
                  ...
                  W2A (lpCI->szFile, tmp, OLEUI_CCHPATHMAX);
                  if (HFILE_ERROR==DoesFileExist(tmp, &of))
                     {
                     OpenFileError(hDlg, of.nErrCode, tmp);
               ...
               if (lpCI->dwFlags & CIF_SELECTFROMFILE)
                  {
                  GetDlgItemText(hDlg, ID_FROMFILEEDIT, tmp, OLEUI_CCHPATHMAX);
                  A2W (tmp, psz, OLEUI_CCHPATHMAX);
               ...
               A2W (szTemp, szUniTemp, OLEUI_CCHPATHMAX);
               hMetaPict=OleUIMetafilePictFromIconAndLabel(hIcon
                  , szUniTemp, psz, lpCI->iIcon);
...
BOOL FChangeIconInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
   ...
   W2A (lpCI->szFile, szAnsiStr, 256);
   SetDlgItemText(hDlg, ID_FROMFILEEDIT, szAnsiStr);

   //Copy the label text into the edit and static controls.
   W2A (lpCI->szLabel, szAnsiStr, 256);
   SetDlgItemText(hDlg, ID_LABELEDIT,   szAnsiStr);
   SetDlgItemText(hDlg, ID_RESULTLABEL, szAnsiStr);
   ...
   if (lpCI->dwFlags & CIF_USEICONEXE)
   {
      W2A (lpCI->lpOCI->szIconExe, szAnsiStr, 256);
      lpCI->hDefIcon = ExtractIcon(ghInst, szAnsiStr, 0);

      if (NULL != lpCI->hDefIcon)
      {
       OLESTRCPY (lpCI->szDefIconFile, lpCI->lpOCI->szIconExe);
       lpCI->iDefIcon = 0;
      }
   }
   ...
   W2A (lpCI->szFile, szAnsiStr, 256);
   if (0!=UFillIconList(hDlg, ID_ICONLIST, szAnsiStr))
      {
      //If szFile worked, then select the source icon in the listbox.
      SendDlgItemMessage(hDlg, ID_ICONLIST, LB_SETCURSEL, (WPARAM) szAnsiStr, 0L);
      }
