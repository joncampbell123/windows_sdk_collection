...
typedef struct tagLINKINFO
  ...
  LPOLESTR  lpszDisplayName;    // file based part of name
  LPOLESTR  lpszItemName;       // object part of name
  LPOLESTR  lpszShortFileName;  // filename without path
  LPOLESTR  lpszShortLinkType;  // Short link type - progID
  LPOLESTR  lpszFullLinkType;   // Full link type - user friendly name
  LPOLESTR  lpszAMX;          // Is the link auto (A) man (M) or dead (X)
