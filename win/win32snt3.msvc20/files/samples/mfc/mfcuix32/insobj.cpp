...
STDAPI_(UINT) OleUIInsertObject(LPOLEUIINSERTOBJECT lpIO)
{
	...
	if (NULL != lpIO->lpszFile
		&& (lpIO->cchFile <= 0 || lpIO->cchFile > MAX_PATH_SIZE))
	{
			...
			switch (wID)
			{
					...
					if (fCheck)
					{
						if (lpIO->dwFlags & IOF_SELECTCREATEFROMFILE)
						{
							...
							else
							{
								...
								lpIO->fFileDirty = FALSE;
								lpIO->fFileValid = FALSE;
								SendDlgItemMessage(hDlg, ID_IO_ICONDISPLAY,
									IBXM_IMAGESET, 0, 0L);
								UpdateClassType(hDlg, lpIO, FALSE);
				...
				case ID_IO_CHANGEICON:
				{
					...
					else  // IOF_SELECTCREATEFROMFILE
					{
						...
						GetDlgItemText(hDlg, ID_IO_FILEDISPLAY, szFileName, MAX_PATH);
						...
						if (NOERROR != GetClassFile(szFileName, &ci.clsid))
						{
							...
							lpszExtension = szFileName + istrlen -1;
				...
				case ID_IO_FILE:
				{
					...
					nChars = GetDlgItemText(hDlg, ID_IO_FILEDISPLAY, szTemp, MAX_PATH);

					if (FValidateInsertFile(hDlg, FALSE, &lpIO->nErrCode))
					{
						int istrlen;

						GetFileTitle(szTemp, lpIO->szFile, MAX_PATH);
						  ...
						  // valid directory
					{
						...
						lstrcpyn(szBuffer, szTemp, MAX_PATH-1);
					...
					uRet=UStandardHook(lpIO, hDlg, uMsgBrowse,
						MAX_PATH_SIZE, (LPARAM)(LPSTR)lpIO->szFile);
					...
					if (0==uRet)
						uRet=(UINT)Browse(hDlg,
										  lpIO->szFile,
										  fUseInitialDir ? szInitialDir : NULL,
										  MAX_PATH_SIZE,
										  IDS_FILTERS,
										  dwOfnFlags);
					...
					else  // IOF_SELECTCREATEFROMFILE
					{
						if (lpIO->dwFlags & IOF_CHECKDISPLAYASICON)
						{
							...
							lpOIO->hMetaPict = (HGLOBAL)SendDlgItemMessage(
								hDlg, ID_IO_ICONDISPLAY, IBXM_IMAGEGET, 0, 0L);
						}
						else
							lpOIO->hMetaPict = (HGLOBAL)NULL;
					}
...
BOOL FInsertObjectInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
{
	...
	if ('\0' == *(lpIO->szFile))
	{
		...
		if (nLen != 0 && szCurDir[nLen-1] != '\\')
			lstrcat(szCurDir, _T("\\"));
		SetDlgItemText(hDlg, ID_IO_FILEDISPLAY, szCurDir);
...
UINT UFillClassList(HWND hList, UINT cIDEx, LPCLSID lpIDEx, BOOL fVerify)
{
	...
	pszExec=(LPTSTR)OleStdMalloc(OLEUI_CCHKEYMAX_SIZE*4);
	if (NULL==pszExec)
		return (UINT)-1;
	...
	while (TRUE)
	{
		...
		else
		{
			...
			pszID = (LPTSTR)OleStdMalloc(OLEUI_CCHKEYMAX_SIZE);
			if (NULL==pszID)
				continue;
			...
			*(pszClass+cch)=0;  // set back to rootkey
			...
			if ((LONG)ERROR_SUCCESS!=lRet)
				goto cleanup_continue;   // CLSID subkey not found
			...
			if (fVerify && !DoesFileExist(pszKey))
				goto cleanup_continue;
...
BOOL FToggleObjectSource(HWND hDlg, LPINSERTOBJECT lpIO, DWORD dwOption)
{
	...
	else
	{
		...
		{
		   SendDlgItemMessage(hDlg, ID_IO_ICONDISPLAY, IBXM_IMAGESET, 0,
...
void UpdateClassType(HWND hDlg, LPINSERTOBJECT lpIO, BOOL fSet)
{
   ...
   *szFileType = '\0';
   ...
   SetDlgItemText(hDlg, ID_IO_FILETYPE, szFileType);
}
...
void UpdateClassIcon(HWND hDlg, LPINSERTOBJECT lpIO, HWND hList)
{
	...
	pszName = (LPTSTR)OleStdMalloc((cb+1)*sizeof(TCHAR));
	...
	*pszName=0;
	...
	if (lpIO->dwFlags & IOF_SELECTCREATENEW)
	{
		...
		pszTemp = CharPrev(pszName, pszCLSID);
		*pszTemp = '\0';
	...
	else
	{
		...
		GetDlgItemText(hDlg, ID_IO_FILEDISPLAY, pszName, MAX_PATH);
...
void SetInsertObjectResults(HWND hDlg, LPINSERTOBJECT lpIO)
{
	...
	pszTemp=(LPTSTR)OleStdMalloc((DWORD)(4*cch)*sizeof(TCHAR));
	...
	*psz1=0;
	...
	if (0!=LoadString(_g_hOleStdInst, iString1, psz1, cch))
	{
		// Load second string, if necessary
		if (   (0 != iString2)
			&& (0 != LoadString(_g_hOleStdInst, iString2, psz4, cch)) )
		{
...
BOOL FValidateInsertFile(HWND hDlg, BOOL fTellUser, UINT FAR* lpnErrCode)
{
	...
	*lpnErrCode = 0;
	 ...
	 */
	if (0==GetDlgItemText(hDlg, ID_IO_FILEDISPLAY, szFile, MAX_PATH))
	...
	GetFullPathName(szFile, sizeof(tszPath)/sizeof(TCHAR), tszPath, &lpszDummy);
	SetDlgItemText(hDlg, ID_IO_FILEDISPLAY, tszPath);
