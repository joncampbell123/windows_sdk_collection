		...
		case WM_COMMAND:
			switch (wID)
			{
				...
				case ID_BROWSE:
				{
					...
					if (0==uRet)
						uRet=(BOOL)Browse(hDlg, lpCI->szFile, NULL, MAX_PATH_SIZE, IDS_ICONFILTERS, dwOfnFlags);
...
//                    if (ID_FROMFILE & lpCI->dwFlags)
					...
					{
						GetDlgItemText(hDlg, ID_FROMFILEEDIT, psz, MAX_PATH);
					...
					hMetaPict=OleMetafilePictFromIconAndLabel(hIcon,
...
BOOL FChangeIconInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
{
	...
	if (NULL!=hFont)
		SendDlgItemMessage(hDlg, ID_RESULTLABEL, WM_SETFONT, (WPARAM)hFont, 0L);
	...
	SendDlgItemMessage(hDlg, ID_FROMFILEEDIT, EM_LIMITTEXT, MAX_PATH,  0L);
	SetDlgItemText(hDlg, ID_FROMFILEEDIT, lpCI->szFile);
	...
	if (lpCI->dwFlags & CIF_USEICONEXE)
	{
		lpCI->hDefIcon = ExtractIcon(_g_hOleStdInst, lpCI->lpOCI->szIconExe, 0);
	...
	if (NULL == lpCI->hDefIcon)
	{
		...
		OleUIMetafilePictExtractIconSource(hMetaPict,
			lpCI->szDefIconFile, &lpCI->iDefIcon);
...
UINT UFillIconList(HWND hDlg, UINT idList, LPTSTR pszFile)
{
	...
	if (SearchPath(NULL, pszFile, NULL, MAX_PATH, szPathName,
		&lpszFilePart) != 0) //exe found
	{
		//REVIEW: I'm sure ExtractIcon works under Win32, but
		//  what about Win32s -- probably so, since it would have
		//  to be modified for dealing with 32-bit EXEs.
	   #ifdef EXTRACTICONWORKS
		//Get the icon count for this file.
		cIcons=(UINT)ExtractIcon(_g_hOleStdInst, szPathName, (UINT)-1);
	   #else
		/*
		 ...
		 */
		cIcons = 0xFFFF;
		hIcon = ExtractIcon(_g_hOleStdInst, szPathName, 0);
		...
		if (0!=cIcons)
		{
			...
			for (i=0; i<cIcons; i++)
			{
				hIcon=ExtractIcon(_g_hOleStdInst, szPathName, i);
				if (hIcon != NULL)
			...
			ErrorWithFile(hDlg, _g_hOleStdInst, IDS_CINOICONSINFILE, szPathName, MB_OK);
	}
	else
		OpenFileError(hDlg, ERROR_FILE_NOT_FOUND, pszFile);
	...
	switch (uID)
	{
		...
		case ID_FROMFILE:
			...
			if ((UINT)LB_ERR == iSel)
				...
				lTemp = SendDlgItemMessage(hDlg, ID_ICONLIST, LB_GETITEMDATA, iSel, 0);
			break;
	}
	SendDlgItemMessage(hDlg, ID_RESULTICON, STM_SETICON, lTemp, 0L);
	CheckRadioButton(hDlg, ID_CURRENT, ID_FROMFILE, uID);
}
