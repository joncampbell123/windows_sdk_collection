	...
	DLGPROC lpDlgProc, LPOLEUISTANDARD lpUI, HGLOBAL hMemDlg, LPTSTR lpszStdTemplate)
{
	...
	if (NULL==hTemplate)
	{
		hRes=FindResource(_g_hOleStdInst, (LPCTSTR) lpszStdTemplate, RT_DIALOG);

		if (NULL==hRes)
			return OLEUI_ERR_FINDTEMPLATEFAILURE;

		hTemplate=LoadResource(_g_hOleStdInst, hRes);
	...
	iRet=DialogBoxIndirectParam(_g_hOleStdInst, (LPCDLGTEMPLATE)hTemplate,
