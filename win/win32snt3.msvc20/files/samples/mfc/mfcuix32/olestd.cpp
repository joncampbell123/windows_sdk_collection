	...
	LPADVISESINK lpAdviseSink, BOOL fCreate)
{
	...
	hrErr = QUERYINTERFACE(lpOleObject, IViewObject, &lpViewObject);

	/* Setup View advise */
	if (hrErr == NOERROR)
	{
		OLEDBG_BEGIN2(TEXT("IViewObject::SetAdvise called\r\n"))
		lpViewObject->SetAdvise(
				dwDrawAspect,
	...
	hrErr = lpOleObject->SetHostNames(
			lpszContainerApp,
			lpszContainerObj
	);
	OLEDBG_END2
	...
	BOOL FAR*       lpfMustUpdate)
{
	...
	// query for IOleCache
	hrErr = QUERYINTERFACE(lpOleObj, IOleCache, &lpOleCache);
	if (hrErr != NOERROR)
	...
	*/
	...
	if (fSetupViewAdvise && lpAdviseSink)
	{
		...
		hrErr = QUERYINTERFACE(lpOleObj, IViewObject, &lpViewObj);
		if (hrErr == NOERROR)
		{
			OLEDBG_BEGIN2(TEXT("IViewObject::SetAdvise called\r\n"))
			lpViewObj->SetAdvise(
	...
	LPTSTR FAR*     lplpszType)
{
	...
	*lplpszType = NULL;
	...
	if (hrErr != NOERROR || IsEqualCLSID(*lpclsid, CLSID_NULL) ||
...
STDAPI OleStdDoTreatAsClass(LPCTSTR lpszUserType, REFCLSID rclsid, REFCLSID rclsidNew)
{
	...
	if ((hrErr != NOERROR) && lpszUserType)
	{
		lRet = RegOpenKey(HKEY_CLASSES_ROOT, (LPCTSTR) TEXT("CLSID"),
				(HKEY FAR *)&hKey);
		StringFromCLSID(rclsid, (LPTSTR FAR*)&lpszCLSID);
...
STDAPI_(LPTSTR) OleStdCopyString(LPTSTR lpszSrc, LPMALLOC lpMalloc)
{
	...
	lpszDest = (LPTSTR)lpMalloc->Alloc((lSize+1)*sizeof(TCHAR));
	if (lpszDest)
	...
	BOOL                fUseMemory)
{
	...
	hrErr = lpPStg->SaveCompleted(NULL);
	OLEDBG_END2
	...
	LPSIZEL         lpSizelHim)
{
	...
	// query for IOleLink
	QUERYINTERFACE(lpOleObj, IOleLink, &lpOleLink);
	fIsLink = (lpOleLink ? TRUE : FALSE);

	// query for IViewObject2
	QUERYINTERFACE(lpOleObj, IViewObject2, &lpViewObj2);

	// Get CLSID
	OLEDBG_BEGIN2(TEXT("IOleObject::GetUserClassID called\r\n"))
	hrErr = lpOleObj->GetUserClassID(&clsid);
	OLEDBG_END2
	if (hrErr != NOERROR)
		clsid = CLSID_NULL;
	...
	{
		if (0 == LoadString(_g_hOleStdResInst, IDS_OLE2UIPASTELINKEDTYPE,
				(LPTSTR)szLinkedTypeFmt, sizeof(szLinkedTypeFmt)/sizeof(TCHAR)))
			lstrcpy(szLinkedTypeFmt, (LPTSTR) TEXT("Linked %s"));
		lpszBuf = (LPTSTR)OleStdMalloc(
			(lstrlen(lpszFullUserTypeName)+lstrlen(szLinkedTypeFmt)+1)*sizeof(TCHAR));
		if (lpszBuf)
		{
	...
	*/
	if (fIsLink)
	{
		...
		hrErr = lpOleLink->GetSourceDisplayName(&lpszSrcOfCopy);
		OLEDBG_END2
		fFreeSrcOfCopy = TRUE;
	}
	else
	{
		if (lpszSrcOfCopy == NULL)
		{
			...
			if (hrErr == NOERROR)
			{
				...
				lpSrcMonikerOfCopy->GetDisplayName(pbc, NULL, &lpszSrcOfCopy);
				pbc->Release();
				fFreeSrcOfCopy = TRUE;
			}
		}
	...
	CLIPFORMAT FAR* lpcfFmt)
{
				...
				lpmedium))
	{
				...
				lpmedium))
	 {
		 ...
		 */
		 if (hrErr != NOERROR)
			CLSIDFromProgID(TEXT("Package"), &clsid);
		 sizelHim.cx = sizelHim.cy = 0;
		 pointl.x = pointl.y = 0;

		 szBuf = (LPTSTR)OleStdMalloc((DWORD)OLEUI_CCHKEYMAX_SIZE);
		 if (NULL == szBuf)
			goto error;
	...
	DVTARGETDEVICE FAR* ptd)
{
	...
	hrErr = QUERYINTERFACE(lpOleObj, IViewObject2, &lpViewObj2);
	if (hrErr != NOERROR)
		return NULL;
	...
	if (hrErr == NOERROR)
	{
		...
		*/
		if (lpmkFull)
		{
			...
			lpROT->Register(0, lpUnk,lpmkFull,lpdwRegister);
			OLEDBG_END2
...
#ifdef _DEBUG
			{
				...
				lpmkFull->GetDisplayName(
						lpbc,
						NULL,
						(LPTSTR FAR*)&lpszDisplay
				);
...
STDAPI_(void) OleStdNoteFileChangeTime(LPTSTR lpszFileName, DWORD dwRegister)
{
	if (dwRegister != 0)
	{
		...
		{
			lprot->NoteChangeTime( dwRegister, &filetime );
			lprot->Release();
...
STDAPI_(ULONG) OleStdGetLenFilePrefixOfMoniker(LPMONIKER lpmk)
{
	...
	if (lpmkFirst)
	{
		if ( (lpmkFirst->IsSystemMoniker((LPDWORD)&dwMksys) == NOERROR)
	...
	LPMONIKER FAR*  lplpmk)
{
		...
		lpszUserName[0] != '@')
	{
		...
		if (lpszProgID == NULL)
			goto Cont1;
		lpszBuf = (LPTSTR)OleStdMalloc(((ULONG)lstrlen(lpszUserName)+
					   lstrlen(lpszProgID)+3)*sizeof(TCHAR));
		if (lpszBuf == NULL)
		{
			if (lpszProgID)
		...
		for (j = 0; j < cFetched; j++)
		{
			for (i = 0; i < cEntries; i++)
			{
				if (!lpPriorityList[i].dwScratchSpace &&
					OleStdCompareFormatEtc(&lpPriorityList[i].fmtetc,
						&rgfmtetc[j]) <= 0)
				{
					lpPriorityList[i].dwScratchSpace = TRUE;
				}
...
STDAPI_(ULONG) OleStdGetItemToken(LPTSTR lpszSrc, LPTSTR lpszDst, int nMaxChars)
{
	...
	while (*lpszSrc && --nMaxChars > 0 &&
		(*lpszSrc=='/' || *lpszSrc=='\\' || *lpszSrc=='!' || *lpszSrc==':'))
	{
		*lpszSrc++;
		chEaten++;
	}
...
STDAPI_(LPSTORAGE) OleStdCreateRootStorage(LPTSTR lpszStgName, DWORD grfMode)
{
	...
	if (0 == LoadString(_g_hOleStdResInst, IDS_OLESTDNOCREATEFILE, (LPTSTR)szMsg, 64))
	  return NULL;
...
STDAPI_(LPSTORAGE) OleStdOpenRootStorage(LPTSTR lpszStgName, DWORD grfMode)
{
	...
	if (0 == LoadString(_g_hOleStdResInst, IDS_OLESTDNOOPENFILE, szMsg, 64))
	  return NULL;
...
STDAPI_(LPSTORAGE) OleStdCreateChildStorage(LPSTORAGE lpStg, LPTSTR lpszStgName)
{
	if (lpStg != NULL)
	{
		...
		HRESULT hrErr = lpStg->CreateStorage(
				lpszStgName,
				grfMode,
				reserved,
				reserved,
...
STDAPI_(BOOL) OleStdCommitStorage(LPSTORAGE lpStg)
{
	...
	if (GetScode(hrErr) == STG_E_MEDIUMFULL)
	{
		...
		hrErr = lpStg->Commit(STGC_OVERWRITE);
	}

	if (hrErr != NOERROR)
	{
		TCHAR szMsg[64];
		if (0 == LoadString(_g_hOleStdResInst, IDS_OLESTDDISKFULL, (LPTSTR)szMsg, 64))
		   return FALSE;
...
STDAPI OleStdDestroyAllElements(LPSTORAGE lpStg)
{
	...
	hrErr = lpStg->EnumElements(0, NULL, 0, &lpEnum);
	...
	while (1)
	{
		if (lpEnum->Next(1, &sstg, NULL) != NOERROR)
			break;
		lpStg->DestroyElement(sstg.pwcsName);
