				...
				case IDOK:
				{
					...
					*/
					...
					lpOPS->hMetaPict=(HGLOBAL)SendDlgItemMessage(hDlg,
						ID_PS_ICONDISPLAY, IBXM_IMAGEGET, 0, 0L);
...
BOOL FPasteSpecialInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
{
	...
	n = LoadString(_g_hOleStdInst, IDS_PSUNKNOWNTYPE, lpPS->szUnknownType, PS_UNKNOWNSTRLEN);
	if (n)
		n = LoadString(_g_hOleStdInst, IDS_PSUNKNOWNSRC, lpPS->szUnknownSource, PS_UNKNOWNSTRLEN);
	if (!n)
	{
	...
	{
		lpOD = (LPOBJECTDESCRIPTOR)GlobalLock(lpPS->hObjDesc);

		// Get FullUserTypeName, SourceOfCopy and CLSID
		if (lpOD->dwFullUserTypeName)
			lpPS->szFullUserTypeNameOD = (LPTSTR)((LPBYTE)lpOD+lpOD->dwFullUserTypeName);
		else
			lpPS->szFullUserTypeNameOD = lpPS->szUnknownType;

		if (lpOD->dwSrcOfCopy)
		{
			lpPS->szSourceOfDataOD = (LPTSTR)((LPBYTE)lpOD+lpOD->dwSrcOfCopy);
			...
			if (cfFormat == _g_cfFileName)
				lpPS->szSourceOfDataOD = ChopText(GetDlgItem(hDlg, ID_PS_SOURCETEXT), 0, lpPS->szSourceOfDataOD, 0);
		}
		else
			lpPS->szSourceOfDataOD = lpPS->szUnknownSource;
		...
		if (0 == OleStdGetAuxUserType(lpPS->clsidOD, 3, lpPS->szAppName,
			OLEUI_CCHKEYMAX_SIZE, NULL))
		{
			 // Use "the application which created it" as the name of the application
			 if (0==LoadString(_g_hOleStdInst, IDS_PSUNKNOWNAPP, lpPS->szAppName, PS_UNKNOWNSTRLEN))
			 {
				 PostMessage(hDlg, uMsgEndDialog, OLEUI_ERR_LOADSTRING, 0L);
				 return FALSE;
			 }
	...
	{
		// Get FullUserTypeName, SourceOfCopy and CLSID
		lpLSD = (LPLINKSRCDESCRIPTOR)GlobalLock(lpPS->hLinkSrcDesc);
		if (lpLSD->dwFullUserTypeName)
			lpPS->szFullUserTypeNameLSD = (LPTSTR)((LPBYTE)lpLSD+lpLSD->dwFullUserTypeName);
		else
			lpPS->szFullUserTypeNameLSD = lpPS->szUnknownType;

		if (lpLSD->dwSrcOfCopy)
			lpPS->szSourceOfDataLSD = (LPTSTR)((LPBYTE)lpLSD+lpLSD->dwSrcOfCopy);
		else
			lpPS->szSourceOfDataLSD = lpPS->szUnknownSource;
		...
		if (NULL == lpPS->hMetaPictLSD)
		{
			...
			RECT IconRect;
			int  nWidth;
			LPTSTR lpszLabel, lpszChopLabel;

			hIconWnd = GetDlgItem(hDlg, ID_PS_ICONDISPLAY);
			GetClientRect(hIconWnd, &IconRect);

			lpszLabel = new TCHAR[lstrlen(lpPS->szSourceOfDataLSD)+1];
			lstrcpy(lpszLabel, lpPS->szSourceOfDataLSD);
			// width is 2 times width of iconbox because it can wrap
			nWidth = (IconRect.right-IconRect.left) * 2;
			// limit text to the width or max characters
			lpszChopLabel = ChopText(hIconWnd, nWidth, lpszLabel,
				OLEUI_CCHLABELMAX);
			lpszChopLabel[OLEUI_CCHLABELMAX-1] = '\0'; // just to make sure
			lpPS->hMetaPictLSD =
...
void ChangeIcon(HWND hDlg, LPPASTESPECIAL lpPS)
{
	...
	if (0!=uRet)
	{
		...
		* metafilepict again.
		*/
		SendDlgItemMessage(hDlg, ID_PS_ICONDISPLAY,
...
void SetPasteSpecialHelpResults(HWND hDlg, LPPASTESPECIAL lpPS)
{
	...
	//   result text.
	cch = (UINT)GlobalSize(lpPS->hBuff) / (4 * sizeof(TCHAR));
	...
	*psz1 = 0;
	...
	if (0 != LoadString(_g_hOleStdInst, iString, psz1, cch))
	{
