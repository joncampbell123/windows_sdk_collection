...
STDAPI_(UINT) OleStdGetUserTypeOfClass(REFCLSID rclsid, LPTSTR lpszUserType, UINT cch, HKEY hKey)
{
	...
	*lpszUserType = '\0';
	...
	if ((LONG)ERROR_SUCCESS!=lRet)
	{
		...
		dw = (LONG)LoadString(_g_hOleStdResInst, IDS_PSUNKNOWNTYPE, lpszUserType,
			cch/sizeof(TCHAR));
	}
   ...
   if (fFreeProgID)
		OleStdFree((LPVOID)lpszProgID);

   OleStdFree((LPVOID)lpszCLSID);
 ...
 *
 */
STDAPI_(UINT) OleStdGetAuxUserType(
	REFCLSID rclsid,
	WORD     wAuxUserType,
...
STDAPI_(BOOL) OleStdGetMiscStatusOfClass(REFCLSID rclsid, HKEY hKey, DWORD FAR * lpdwValue)
{
	...
	lRet = RegQueryValue(hKey, szKey, (LPTSTR)szMiscStatus, (LONG*)&dw);
