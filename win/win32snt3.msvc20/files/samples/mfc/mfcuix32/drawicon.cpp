	...
	METARECORD FAR *pMFR, int cObj, LPICONEXTRACT pIE)
{
	...
	UNALIGNED BITMAPINFOHEADER* lpBH;
	LPBYTE              lpbSrc;
	LPBYTE              lpbDst;
	UINT                uWidth, uHeight;
	DWORD               cb;
	...
	// copy into aligned temporary because SetDIBits doesn't handle
	// unaligned data
	size_t nCount = lpbSrc - (LPBYTE)lpBI;
	LPBITMAPINFO lpBITemp = (LPBITMAPINFO)_alloca(nCount);
	memcpy(lpBITemp, lpBI, nCount);
	...
	if (!hBmp || !SetDIBits(hDC, hBmp, 0, (UINT)lpBH->biHeight, (LPVOID)lpbSrc, lpBITemp, DIB_RGB_COLORS))
	{
		if (!pIE->fAND)
	...
	else
	{
		...
		pIE->hIcon = CreateIcon(_g_hOleStdInst,
							  uWidth,
							  uHeight,
							  (BYTE)bm.bmPlanes,
							  (BYTE)bm.bmBitsPixel,
