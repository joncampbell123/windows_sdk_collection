...
void CTestWindow::OnTestSpinEdit()
{
	...
	// DLLs currently not supported on the mac.
	GetMenu()->EnableMenuItem(IDM_TEST_SPIN_EDIT, MF_DISABLED|MF_GRAYED);
	MessageBox("Feature not supported on Macintosh");
#else

	HINSTANCE hLibrary;
	if ( !(hLibrary = LoadLibrary(_T("MUSCRL32.DLL"))) )
	{
		CString strMsg;
		strMsg.LoadString(IDS_MUSCRL32_DLL_REQUIRED);
		MessageBox(strMsg);
