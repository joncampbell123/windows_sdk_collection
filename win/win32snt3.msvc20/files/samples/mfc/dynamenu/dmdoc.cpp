...
{
	{ _T("Black"),  ID_COLOR1, RGB(0,    0, 0) },   //REVIEW: move to str table
	{ _T("Red"),    ID_COLOR2, RGB(128, 0, 0) },
	{ _T("Purple"), ID_COLOR3, RGB(128, 0, 128) },
	{ _T("Blue"),   ID_COLOR4, RGB(0, 0, 128) },
};
...
BOOL CDynaMenuDoc::GetMessageString(UINT nID, CString& strMessage)
{
	for (int i = 0; i < NUM_TEXTCOLOR; i++)
	{
		...
		{
			AfxFormatString1(strMessage, IDS_COLORHELPFMT,
							 m_aColorDef[i].m_szColor);
			return TRUE;
		}
	}
	return FALSE;
...
void CDynaMenuDoc::OnColorOptions()
{
	...
	if (dlg.DoModal())
	{
		...
		ASSERT(pwnd->IsKindOf(RUNTIME_CLASS(CDynaMDIChildWnd)));
