	...
	COleDataObject* pDataObject, CPoint* pPoint, CRectItem* pItem)
{
	...
	if (pPoint != NULL)
		pItem->m_rect.OffsetRect(*pPoint - pItem->m_rect.TopLeft());
}
	...
	CPoint* pPoint, CRectItem* pItem, CLIPFORMAT cfFormat)
{
	...
	if (GetObjectInfo(pDataObject, &size, NULL) && size.cx != 0 && size.cy != 0)
	{
		...
		pItem->m_rect.right = pItem->m_rect.left + MulDiv(size.cx, 10, 254);
		pItem->m_rect.bottom = pItem->m_rect.top - MulDiv(size.cy, 10, 254);
		// set the base extent of the item
		pItem->GetCachedExtent(&pItem->m_extent);
	}
	else
	{
...
void CMainView::OnEditClone()
{
	...
	TRY
	{
		...
		pItem->m_rect = m_pSelection->m_rect;
		CRect rect(20, 20, 0, 0);
		ClientToDoc(rect);
		pItem->m_rect.OffsetRect(rect.TopLeft());
		ASSERT_VALID(pItem);
	}
	CATCH_ALL(e)
	{
