...
void CMainDoc::OnEditClearAll()
{
	...
	while (pos != NULL)
	{
		...
		ASSERT(pItem->IsKindOf(RUNTIME_CLASS(CRectItem)));
		pItem->Delete();
	}
...
void CMainDoc::AdjustItemPosition(CRectItem* pItem)
{
	...
	while (pos != NULL)
	{
		...
		ASSERT(pRectItem->IsKindOf(RUNTIME_CLASS(CRectItem)));
		if (pRectItem != pItem && pRectItem->m_rect == pItem->m_rect)
		{
			pItem->m_rect.left += 10;
			pItem->m_rect.right += 10;
			pItem->m_rect.top -= 10;
			pItem->m_rect.bottom -= 10;
