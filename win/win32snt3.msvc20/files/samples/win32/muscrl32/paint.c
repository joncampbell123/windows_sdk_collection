...
LONG PASCAL LMicroScrollPaint(HWND hWnd, PMUSCROLL pMS)
{
    ...
    for (nColor=0; nColor < CCOLORS; nColor++)
    {
        if (-1L==pMS->rgCr[nColor])
        {
            //HACK:  Windows 3.0 has no system color for button highlight
            if (-1==rgColorDef[nColor])
                rgCr[nColor]=RGB(255, 255, 255);
            else
                rgCr[nColor]=GetSysColor(rgColorDef[nColor]);
        }
        else
            rgCr[nColor]=pMS->rgCr[nColor];

        rgHPen[nColor]=CreatePen(PS_SOLID, 1, rgCr[nColor]);
    }
