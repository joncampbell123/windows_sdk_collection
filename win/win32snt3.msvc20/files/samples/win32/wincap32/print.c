...
// File change history:
// Date      Author             Reason
// 12/07/92  Mark Bader         Ported to Win32
//  1/26/94  Hung H. Nguyen     finished port & addes strict type checking
//
//
 ...
 *     2/1/94    Hung Nguyen    Changed Escape() to printing APIs
 *     2/4/94    Hung Nguyen    Change printing so that banding is used only
 *                              under Win32s
 *
 ********************************************************************/
 ...
 *     Date      Author       Reason
 *     9/15/91   Mark Bader   Created based on DIBVIEW
 *     2/1/94    Hung Nguyen  Changed to use PrintDlg common dialog
 ***********************************************************************/
...
int CALLBACK PrintAbortDlg(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg)
    {
        case WM_INITDIALOG:
        {
            ...
            wsprintf(szBuffer, "to %s on %s", (LPSTR)gszDevice,
                    (LPSTR)gszOutput);
