...
// File change history:
// Date     Author              Reason
// 12/10/92 Mark Bader          Ported to Win32
//  1/26/94 Hung H. Nguyen      Finished Win32 port and clean up
//
...
//  02/15/94    Hung Nguyen     Changed WinMain's parameter declaration
//
//************************************************************************
        ...
        int nCmdShow)
{
    ...
    if (!FindWindow(szAppName, "Screen Capture"))
    {
        wndclass.style = 0;
        ...
        hWnd = CreateWindow(szAppName, "Screen Capture", WS_OVERLAPPEDWINDOW,
...
//  01/15/94    Hung Nguyen     Added some small stuff
//  01/28/94    Hung Nguyen     Implemented IDM_EDITCOPY
//  01/29/94    Hung Nguyen     Added highliting of selection window
//
//*********************************************************************
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
                ...
                case IDM_VIEWCLEAR:
                {
                    ...
                    if (gbSave)
                    {
                        wRet = MessageBox(hWnd, "Do you want to save the bitmap?",
...
void DoCapture(HWND hWnd, WORD wCommand)
{
    ...
    switch (wCommand)
    {
        ...
        default:
            MessageBox(NULL, "INTERNAL ERROR 23: Invalid command", szAppName,
                    MB_ICONHAND | MB_OK);
            break;
    }
...
// History:  Date       Author          Reason
//
//           01/29/94   Hung Nguyen     added this code from WinView 3.0 sample
//                                      in WVBROWSE.C, SelectWindow() function
//
//***************************************************************************
