    ...
    )
{
    ...
    if ( argc > 1 ) {
        ...
        while (--argc > 0 ) {
            ...
            if (*p == '/' || *p == '-') {
                ...
                case 'G':
                    ...
                    if ( (DriveName = *argv ) && *DriveName && 
                        ...
                        fShowGeometry = TRUE;
                                                ...
                                                {
                        printf( "FLOPPY: Missing drive letter after -G\n" );
                                    DriveName = NULL;
                        fUsage = TRUE;
                        }
                            break;
                ...
                default:
                    printf("FLOPPY: Invalid switch - /%c\n", c );
                    fUsage = TRUE;
                    break;
                    }
                }
    ...
    if ( fShowGeometry ) {
        ...
        if ( hDrive == INVALID_HANDLE_VALUE ) {
            printf("FLOPPY: Open %s failed %d\n",DriveName,GetLastError());
            ExitProcess(1);
            }

        if ( LockVolume(hDrive) == FALSE ) {
            printf("FLOPPY:Locking volume %s failed %d\n", DriveName, GetLastError());
            ExitProcess(1);
            }

        if ( !GetDiskGeometry(hDrive,&Geometry) ) {
            printf("FLOPPY: GetDiskGeometry %s failed %d\n",DriveName,GetLastError());
            ExitProcess(1);
            }
        PrintGeometry(DriveName,&Geometry);

        if ( !GetSupportedGeometrys(hDrive) ) {
            printf("FLOPPY: GetSupportedGeometrys %s failed %d\n",DriveName,GetLastError());
            ExitProcess(1);
            }
    ...
    if ( fDiskImage ) {
        ...
        if ( Destination[strlen(Destination)-1] == ':' ) {
            if ( SourceIsDrive ) {
                printf("FLOPPY: Source and Destination cannot both be drives\n");
                ExitProcess(1);
                }
            SourceIsDrive = FALSE;
            ...
            if ( !SourceIsDrive ) {
                printf("FLOPPY: Either Source or Destination must be a drive\n");
                ExitProcess(1);
                }
            }
        ...
        if ( LockVolume(hDrive) == FALSE ) {
            printf("FLOPPY: LockVolume %s failed %d\n",DriveName,GetLastError());
            ExitProcess(1);
            }
        ...
        //
        {
            ...
            if ( SourceIsDrive ) {
                ...
                if ( !IoBuffer ) {
                    printf("FLOPPY: Buffer Allocation Failed\n");
                    ExitProcess(1);
                    }
                ...
                for ( ;NumBufs > 0; NumBufs-- )
                    ...
                    if (b && BytesRead){
                        ...
                        if ( !b || ( BytesRead != BytesWritten ) ) {
                            printf("FLOPPY: Fatal Write Error %d\n",GetLastError());
                            ExitProcess(1);
                            }
                        }
                    else {
                        printf("FLOPPY: Fatal Read Error %d\n",GetLastError());
                        ExitProcess(1);
                        }
                    }
                }
                ...
                if ( !b ) {
                    printf("FLOPPY: FileSize %d is not supported on drive %s\n",FileSize,DriveName);
                    ExitProcess(1);
                    }
                }
        }
