...
LDefault:
	...
	case CFieldExchange::MarkForUpdate:
		if (value.GetSize() != 1 || value[0] != AFX_RFX_INT_PSEUDO_NULL)
			pFX->m_prs->ClearFieldFlags(nField,
				AFX_SQL_FIELD_FLAG_NULL, pFX->m_nFieldType);
		goto LDefault;
	...
	int* pnScale, int* pnNullable)
{
	...
	if (pcbColumn != NULL)
	{
		// Only LONGVARBINARY,LONGVARCHAR (and sometimes VARCHAR) > 254 bytes
		ASSERT(dwTemp <= 255 || SQL_VARCHAR ||
			nSqlType == SQL_LONGVARBINARY || nSqlType == SQL_LONGVARCHAR);
