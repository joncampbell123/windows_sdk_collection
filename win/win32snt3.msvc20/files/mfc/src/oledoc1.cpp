...
LPMONIKER COleDocument::GetMoniker(OLEGETMONIKER /*nAssign*/)
{
	...
	LPMONIKER lpMoniker;
	CreateFileMoniker((LPTSTR)(LPCTSTR)m_strPathName, &lpMoniker);
	return lpMoniker;
}
...
BOOL COleDocument::OnOpenDocument(LPCTSTR lpszPathName)
{
	...
	TRY
	{
		if (m_lpRootStg == NULL)
		{
			...
			LPSTORAGE lpStorage;
			SCODE sc = StgOpenStorage(lpszPathName, NULL,
	...
	// lpszPathName must be fully qualified
{
	...
	TRY
	{
		...
		if (lpszPathName != NULL && !m_bSameAsLoad)
		{
			...
			LPSTORAGE lpStorage;
			SCODE sc = ::StgCreateDocfile(lpszPathName,
				STGM_READWRITE|STGM_TRANSACTED|STGM_SHARE_EXCLUSIVE|STGM_CREATE,
				0, &lpStorage);
			if (sc != NOERROR)
				AfxThrowOleException(sc);
