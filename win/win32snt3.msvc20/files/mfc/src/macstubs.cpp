...
{
	ProcessSerialNumber psnCur;

	SetActiveWindow(hWnd);

	psnCur.highLongOfPSN = 0;
	psnCur.lowLongOfPSN = kCurrentProcess;
	return SetFrontProcess(&psnCur) == noErr;
}
...
HANDLE WINAPI AfxGetCurrentProcess(VOID)
{
	return (HANDLE) 0x1;
}
