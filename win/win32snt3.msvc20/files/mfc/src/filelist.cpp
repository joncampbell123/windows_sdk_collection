	...
	LPCTSTR lpszCurDir, int nCurDir, BOOL bAtLeastName) const
{
	...
	int nLenDir = lstrlen(lpch) - (AfxGetFileTitle(lpch, NULL, 0) - 1);
	BOOL bSameDir = FALSE;
	if (nLenDir == nCurDir)
	{
	...
	AfxGetFileTitle(m_arrNames[nIndex], lpch, _MAX_PATH - 3);
#endif
...
static void AbbreviateName(LPTSTR lpszCanon, int cchMax, BOOL bAtLeastName)
{
	...
	cchFileName = AfxGetFileTitle(lpszCanon, NULL, 0) - 1;
