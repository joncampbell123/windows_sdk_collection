...
static const TCHAR sz14[] = _T("CLSID\\%1\\DefaultIcon\0") _T("%3,0");
	...
	OLE_APPTYPE nAppType, LPCTSTR* rglpszRegister, LPCTSTR* rglpszOverwrite)
{
	...
	BOOL bResult;
	bResult = AfxOleRegisterHelper(rglpszRegister, rglpszSymbols, 6, FALSE);
	if (bResult && rglpszOverwrite != NULL)
		bResult = AfxOleRegisterHelper(rglpszOverwrite, rglpszSymbols, 6, TRUE);
