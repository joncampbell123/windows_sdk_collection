...
void CDocument::SetPathName(LPCTSTR lpszPathName, BOOL bAddToMRU)
{
	...
	if (AfxGetFileTitle(m_strPathName, szTitle, _MAX_FNAME) == 0)
		SetTitle(szTitle);

	// add it to the file MRU list
	if (bAddToMRU)
		AfxGetApp()->AddToRecentFileList(szFullPath);
...
BOOL CDocument::OnOpenDocument(LPCTSTR lpszPathName)
{
	...
	HFINDFILE hFind = FindFirstFile(lpszPathName, &fileData);
	ASSERT(hFind != INVALID_HFINDFILE);
