...
void COleServerDoc::OnShowDocument(BOOL bShow)
{
	...
	{
		// hide has semantics of UIDeactivate is the item is active
		if (m_pInPlaceFrame != NULL && m_pInPlaceFrame->m_bUIActive)
		{
...
BOOL COleServerDoc::ActivateInPlace()
{
	...
	// get document type (used in SetActiveObject calls)
	CString strFileType;
	if (!GetFileTypeString(strFileType))
		return FALSE;
	...
	LPOLEINPLACESITE lpInPlaceSite =
		WRAPINTERFACE(m_lpClientSite, IOleInPlaceSite);
	...
	pFrameWnd->m_lpFrame->SetActiveObject(lpActiveObject, strFileType);
	if (pFrameWnd->m_lpDocFrame != NULL)
		pFrameWnd->m_lpDocFrame->SetActiveObject(lpActiveObject, strFileType);
	...
	LPCRECT lpRectBorder, LPOLEINPLACEUIWINDOW lpUIWindow, BOOL bFrame)
{
	...
	else
	{
		...
		if (lpUIWindow->GetBorder(&rectBorder) != NOERROR)
		{
	...
	if ((!rectRequest.IsRectNull() ||
		...
		lpUIWindow->RequestBorderSpace(&rectTemp) == NOERROR)
	{
	...
	if (pThis->m_lpRootStg != NULL)
	{
		...
		while ((pItem = pThis->GetNextClientItem(pos)) != NULL)
		{
			...
			LPPERSISTSTORAGE lpPersistStorage =
				WRAPINTERFACE(pItem->m_lpObject, IPersistStorage);
...
STDMETHODIMP COleServerDoc::XOleObject::SetHostNames(
	LPCTSTR lpszContainerApp, LPCTSTR lpszContainerObj)
{
	METHOD_PROLOGUE_EX(COleServerDoc, OleObject)
	ASSERT_VALID(pThis);
	...
	TRY
	{
		pThis->OnSetHostNames(lpszContainerApp, lpszContainerObj);
	}
	END_TRY
	...
	DWORD dwAssign, DWORD /*dwWhichMoniker*/, LPMONIKER* ppMoniker)
{
	...
	*ppMoniker = pThis->GetMoniker((OLEGETMONIKER)dwAssign);
	return *ppMoniker == NULL ? E_FAIL : NOERROR;
}
...
STDMETHODIMP COleServerDoc::XOleObject::GetUserType(
	DWORD dwFormOfType, LPTSTR* ppszUserType)
{
	METHOD_PROLOGUE_EX(COleServerDoc, OleObject)
	ASSERT_VALID(pThis);
