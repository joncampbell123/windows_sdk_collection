...
CWinThread::~CWinThread()
{
	...
	if (pThreadState->m_pCurrentWinThread == this)
	{
		...
		pThreadState->m_mapHWND.m_permanentMap.RemoveAll();
		pThreadState->m_mapHMENU.m_permanentMap.RemoveAll();
		pThreadState->m_mapHDC.m_permanentMap.RemoveAll();
		pThreadState->m_mapHGDIOBJ.m_permanentMap.RemoveAll();
	}
}
...
int CWinThread::Run()
{
	...
	for (;;)
	{
		...
		do
		{
			...
			if (!bIdle && IsIdleMessage(&m_msgCur))
				bIdle = TRUE;
