...
void COleServerItem::AddOtherClipboardData(COleDataSource* pDataSource)
{
	...
	if (lpDataObject->EnumFormatEtc(DATADIR_GET, &lpEnumFORMATETC) != NOERROR)
		return;
	ASSERT(lpEnumFORMATETC != NULL);

	// get all formats that the object will give us
	FORMATETC formatEtc;
	while (lpEnumFORMATETC->Next(1, &formatEtc, NULL) == NOERROR)
	{
		STGMEDIUM stgMedium;
		if (lpDataObject->GetData(&formatEtc, &stgMedium) != NOERROR)
		{
			// data is not available
			AfxFreeTaskMem(formatEtc.ptd);
		}
	...
	LPPOINT lpOffset, LPSIZE lpSize, LPSTGMEDIUM lpStgMedium)
{
	...
	++m_dwRef;  // protect against destruction during this call
	HGLOBAL hGlobal = _AfxOleGetObjectDescriptorData(
		lpOleObject, NULL, DVASPECT_CONTENT, pointl, lpsizel);
	--m_dwRef;
...
STDMETHODIMP COleServerItem::XOleObject::SetHostNames(
	LPCTSTR /*szContainerApp*/, LPCTSTR /*szContainerObj*/)
{
	...
	case OLEWHICHMK_OBJREL:
		{
			...
			case OLEGETMONIKER_FORCEASSIGN:
				...
				CreateItemMoniker(OLESTDDELIM,
					(LPTSTR)(LPCTSTR)pThis->m_strItemName, ppMoniker);
				break;
...
{
	METHOD_PROLOGUE_EX(COleServerItem, DataObject)
	return pThis->ExternalAddRef();
}
	...
	LPADVISESINK pAdvSink, DWORD* pdwConnection)
{
	...
	*pdwConnection = 0;
	...
	if (!(pFormatEtc->cfFormat == 0 && pFormatEtc->ptd == NULL &&
		  ...
		  pFormatEtc->tymed == -1) &&
		pThis->GetDataObject()->QueryGetData(&formatEtc) != NOERROR)
	{
	...
	if (pThis->m_lpDataAdviseHolder == NULL &&
		CreateDataAdviseHolder(&pThis->m_lpDataAdviseHolder) != NOERROR)
	{
		return E_OUTOFMEMORY;
	}
