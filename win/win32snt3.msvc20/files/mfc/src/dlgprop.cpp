...
static HBITMAP hbmScrollLeft;
static HBITMAP hbmScrollRight;
static HBITMAP hbmScrollLeftDis;
static HBITMAP hbmScrollRightDis;
...
struct _AFX_DLGPROP_TERM
{
	~_AFX_DLGPROP_TERM()
	{
		...
		AfxDeleteObject((HGDIOBJ*)&hbmScrollLeft);
		AfxDeleteObject((HGDIOBJ*)&hbmScrollRight);
		AfxDeleteObject((HGDIOBJ*)&hbmScrollLeftDis);
		AfxDeleteObject((HGDIOBJ*)&hbmScrollRightDis);
	}
};
...
{
	ASSERT(AfxIsValidString(szCaption, FALSE));
...
BOOL CPropertyPage::PreTranslateKeyDown(MSG* pMsg)
{
	...
	BOOL bResult = FALSE;
	if (pMsg->wParam == VK_TAB && ProcessTab(pMsg))
		bResult = TRUE;
	...
	{
		if ((::SendMessage(::GetFocus(), WM_GETDLGCODE, 0, 0) &
			(DLGC_WANTALLKEYS|DLGC_WANTMESSAGE)) == 0)
		{
...
CTabControl::CTabControl()
{
	...
	if (hbmScroll == NULL)
	{
		// Note: If one of these LoadBitmap calls fail, it is likely that
		...
		VERIFY(hbmScrollLeft =
			LoadBitmap(hInst, MAKEINTRESOURCE(AFX_IDB_SCROLL_LEFT)));
		VERIFY(hbmScrollRight =
			LoadBitmap(hInst, MAKEINTRESOURCE(AFX_IDB_SCROLL_RIGHT)));
		VERIFY(hbmScrollLeftDis =
			LoadBitmap(hInst, MAKEINTRESOURCE(AFX_IDB_SCROLL_LEFT_DIS)));
		VERIFY(hbmScrollRightDis =
			LoadBitmap(hInst, MAKEINTRESOURCE(AFX_IDB_SCROLL_RIGHT_DIS)));

		BITMAP bmStruct;
		VERIFY(GetObject(hbmScroll, sizeof(BITMAP), &bmStruct));
		sizeScroll.cx = bmStruct.bmWidth;
	...
	UINT nID)
{
	...
	return CreateEx(0,
		...
		&afxChNil, dwStyle, rect.left, rect.top, rect.right-rect.left,
...
void CTabControl::DrawScrollers(CDC* pDC)
{
	...
	HBITMAP hbm = hbmScroll;

	// Choose bitmap depending on scroll state
	if (IsTabVisible(0, TRUE))
		hbm = hbmScrollLeftDis;
	else if (IsTabVisible(GetItemCount()-1, TRUE))
		hbm = hbmScrollRightDis;
	...
	hbm = (HBITMAP)::SelectObject(dcTemp.m_hDC,hbm);

	pDC->BitBlt(m_rectScroll.left, m_rectScroll.top, m_rectScroll.Width(),
				m_rectScroll.Height(), &dcTemp, 0, 0, SRCCOPY);

	::SelectObject(dcTemp.m_hDC, hbm);
}
...
int CPropertySheet::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	rect.right = 48;    // slightly smaller buttons than normal
	rect.bottom = 14;
...
void CPropertySheet::RecalcLayout()
{
	...
	// buttonGap is m_cxButtonGap
	// button size is m_sizeButton
	nWidth = max(nWidth, 2*m_sizeTabMargin.cx + 4*m_sizeButton.cx + 3*m_cxButtonGap);
	...
	if (!m_bModeless)
	{
		...
		for (int i = 0; i < _countof(standardButtons); i++)
		{
			...
			if (hWnd != NULL)
			{
				::MoveWindow(hWnd, x, y, m_sizeButton.cx, m_sizeButton.cy, TRUE);
