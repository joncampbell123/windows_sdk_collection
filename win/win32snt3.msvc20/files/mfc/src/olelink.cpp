...
BOOL COleLinkingDoc::Register(COleObjectFactory* pFactory, LPCTSTR lpszPathName)
{
	...
	{
		if (CreateFileMoniker((LPTSTR)(LPCTSTR)lpszPathName, &lpMoniker) != NOERROR)
			bResult = FALSE;
	}
	...
	if (lpMoniker != NULL)
	{
		...
		ASSERT(lpROT != NULL);
		if (lpROT->IsRunning(lpMoniker) == NOERROR)
		{
...
BOOL COleLinkingDoc::OnOpenDocument(LPCTSTR lpszPathName)
{
	...
	Revoke();
	if (!RegisterIfServerAttached(lpszPathName, TRUE))
		return FALSE;
...
STDMETHODIMP COleLinkingDoc::XPersistFile::Load(
	LPCTSTR lpszFileName, DWORD /*dwMode*/)
{
	METHOD_PROLOGUE_EX(COleLinkingDoc, PersistFile)
	ASSERT_VALID(pThis);
	...
	TRY
	{
		...
		if (!pThis->OnOpenDocument(lpszFileName))
		{
		...
		pThis->SetPathName(lpszFileName, FALSE);
		AfxOleSetUserCtrl(bUserCtrl);
...
STDMETHODIMP COleLinkingDoc::XPersistFile::Save(
	LPCTSTR lpszFileName, BOOL fRemember)
{
	METHOD_PROLOGUE_EX(COleLinkingDoc, PersistFile)
	ASSERT_VALID(pThis);
	...
	TRY
	{
		...
		pThis->OnSaveDocument(lpszFileName);
		sc = S_OK;
	}
	END_TRY
	sc = pThis->EndDeferErrors(sc);
...
STDMETHODIMP COleLinkingDoc::XPersistFile::SaveCompleted(LPCTSTR lpszFileName)
{
	METHOD_PROLOGUE_EX(COleLinkingDoc, PersistFile)
	ASSERT_VALID(pThis);
...
STDMETHODIMP COleLinkingDoc::XPersistFile::GetCurFile(LPTSTR* lplpszFileName)
{
	METHOD_PROLOGUE_EX(COleLinkingDoc, PersistFile)
	ASSERT_VALID(pThis);
	...
	*lplpszFileName = NULL;
	...
	LPTSTR lpszPathName = (LPTSTR)AfxAllocTaskMem(nLen * sizeof(TCHAR));
	if (lpszPathName == NULL)
		return E_OUTOFMEMORY;

	// copy the file name and return it
	memcpy(lpszPathName, lpszResult, nLen*sizeof(TCHAR));
...
STDMETHODIMP COleLinkingDoc::XOleItemContainer::ParseDisplayName(LPBC lpbc,
	LPTSTR lpszDisplayName, ULONG* cchEaten, LPMONIKER* ppMoniker)
{
	METHOD_PROLOGUE_EX(COleLinkingDoc, OleItemContainer)
	ASSERT_VALID(pThis);
	...
	*ppMoniker = NULL;
	...
	LPCTSTR lpszSrc = lpszDisplayName;
	...
	*lpszDest = 0;
	...
	LPUNKNOWN lpUnknown;
	SCODE sc = GetObject(szItemName, BINDSPEED_INDEFINITE, lpbc,
...
STDMETHODIMP COleLinkingDoc::XOleItemContainer::GetObject(
	LPTSTR lpszItem, DWORD dwSpeedNeeded, LPBINDCTX /*pbc*/, REFIID riid,
	LPVOID* ppvObject)
{
	METHOD_PROLOGUE_EX(COleLinkingDoc, OleItemContainer)
	ASSERT_VALID(pThis);
...
STDMETHODIMP COleLinkingDoc::XOleItemContainer::GetObjectStorage(
	LPTSTR lpszItem, LPBINDCTX /*pbc*/, REFIID riid, LPVOID* ppvStorage)
{
	METHOD_PROLOGUE_EX(COleLinkingDoc, OleItemContainer)
	ASSERT_VALID(pThis);
...
STDMETHODIMP COleLinkingDoc::XOleItemContainer::IsRunning(LPTSTR lpszItem)
{
	METHOD_PROLOGUE_EX(COleLinkingDoc, OleItemContainer)
	ASSERT_VALID(pThis);
