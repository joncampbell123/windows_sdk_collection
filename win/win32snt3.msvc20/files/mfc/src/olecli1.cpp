...
void COleClientItem::Release(OLECLOSE dwCloseOption)
{
	...
	{
		DWORD dwAspect;
		VERIFY(m_lpViewObject->GetAdvise(&dwAspect, NULL, NULL) == NOERROR);
		VERIFY(m_lpViewObject->SetAdvise(dwAspect, 0, NULL) == NOERROR);
...
void COleClientItem::UpdateItemType()
{
	...
	LPOLELINK lpOleLink = WRAPINTERFACE(m_lpObject, IOleLink);
	if (lpOleLink != NULL)
	{
		lpOleLink->Release();
		m_nItemType = OT_LINK;
...
BOOL COleClientItem::FinishCreate(SCODE sc)
{
	...
	TRY
	{
		...
		m_lpViewObject = WRAPINTERFACE(m_lpObject, IViewObject2);
		if (m_lpViewObject == NULL)
	...
	OLERENDER render, CLIPFORMAT cfFormat, LPFORMATETC lpFormatEtc)
{
	...
	// attempt to create the object
	LPOLECLIENTSITE lpClientSite = GetClientSite();
	SCODE sc = ::OleCreateFromFile(clsid, lpszFileName,
	...
	OLERENDER render, CLIPFORMAT cfFormat, LPFORMATETC lpFormatEtc)
{
	...
	// attempt to create the link
	LPOLECLIENTSITE lpClientSite = GetClientSite();
	SCODE sc = ::OleCreateLinkToFile(lpszFileName,
		IID_IUnknown, render, lpFormatEtc, lpClientSite, m_lpStorage,
		(LPLP)&m_lpObject);
	BOOL bResult = FinishCreate(sc);
...
BOOL COleClientItem::CreateCloneFrom(const COleClientItem* pSrcItem)
{
	...
	LPPERSISTSTORAGE lpPersistStorage =
		WRAPINTERFACE(pSrcItem->m_lpObject, IPersistStorage);
	...
	DWORD dwAspect;
	VERIFY(pSrcItem->m_lpViewObject->GetAdvise(&dwAspect, NULL, NULL) == NOERROR);
...
void COleClientItem::GetItemStorageCompound()
{
	...
	LPSTORAGE lpStorage;
	SCODE sc = pDoc->m_lpRootStg->CreateStorage(szItemName,
...
void COleClientItem::ReadItemCompound(CArchive& ar)
{
	...
	else
	{
		...
		LPSTORAGE lpStorage;
		SCODE sc = pDoc->m_lpRootStg->OpenStorage(szItemName, NULL,
...
BOOL COleClientItem::SetIconicMetafile(HGLOBAL hMetaPict)
{
	...
	LPOLECACHE lpOleCache = WRAPINTERFACE(m_lpObject, IOleCache);
	if (lpOleCache == NULL)
	{
...
HGLOBAL COleClientItem::GetIconicMetafile()
{
	...
	LPDATAOBJECT lpDataObject = WRAPINTERFACE(m_lpObject, IDataObject);
...
void COleClientItem::AddCachedData(COleDataSource* pDataSource)
{
	...
	while (lpEnumSTATDATA->Next(1, &statData, NULL) == NOERROR)
	{
		...
		if (lpDataObject->GetData(&statData.formatetc, &stgMedium) != NOERROR)
		{
			...
			AfxFreeTaskMem(statData.formatetc.ptd);
		}
	...
	LPPOINT lpOffset, LPSIZE lpSize, LPSTGMEDIUM lpStgMedium)
{
	...
	HGLOBAL hGlobal = _AfxOleGetObjectDescriptorData(
		m_lpObject, pDoc->GetPathName(), m_nDrawAspect, pointT, &sizeT);
	--m_dwRef;
...
STDMETHODIMP COleClientItem::XOleClientSite::SaveObject()
{
	...
	LPPERSISTSTORAGE lpPersistStorage =
		WRAPINTERFACE(pThis->m_lpObject, IPersistStorage);
	ASSERT(lpPersistStorage != NULL);
	SCODE sc = NOERROR;
	...
	DWORD dwAssign, DWORD dwWhichMoniker, LPMONIKER* ppMoniker)
{
	...
	*ppMoniker = NULL;
	...
	return *ppMoniker != NULL ? NOERROR : E_FAIL;
}
	...
	LPOLECONTAINER* ppContainer)
{
	...
	*ppContainer = pDoc->GetContainer();
	...
	return *ppContainer != NULL ? NOERROR : E_NOINTERFACE;
}
