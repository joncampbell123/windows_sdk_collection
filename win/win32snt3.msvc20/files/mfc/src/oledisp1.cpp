...
#define _STACK_PTR      void*
#ifdef _DEBUG
#define _SCRATCH_SIZE   192
#else
#define _SCRATCH_SIZE   128
#endif
#endif
	...
	VARIANT* pvarResult, UINT* puArgErr)
{
	...
	case VT_CY:
		...
		break;
	case VT_BSTR:
		pvarResult->bstrVal = SysAllocString((LPTSTR)(LPCTSTR)*(CString*)pProp);
		break;
	case VT_ERROR:
		pvarResult->scode = *(SCODE*)pProp;
		break;
	case VT_BOOL:
		pvarResult->bool = (VARIANT_BOOL)(*(BOOL*)pProp != 0 ? -1 : 0);
		break;
	case VT_VARIANT:
	...
	for (const BYTE* pb = pbParams; *pb != '\0'; ++pb)
	{
		...
		case VT_ERROR:
			*(_STACK_LONG*)pStack = (_STACK_LONG)pArg->scode;
			...
			break;
		case VT_BOOL:
			*(_STACK_LONG*)pStack = (_STACK_LONG)(pArg->bool != 0);
			pStack += sizeof(_STACK_LONG);
			break;
		case VT_VARIANT:
		...
		case VT_BOOL|VT_BYREF:
			*(_STACK_PTR*)pStack = (_STACK_PTR)pArg->pbool;
			pStack += sizeof(_STACK_PTR);
			break;
	...
	VARIANT* pvarResult, DISPPARAMS* pDispParams, UINT* puArgErr)
{
	...
	if (pvarResult != NULL)
	{
		...
		case VT_ERROR:
			...
			break;
		case VT_BOOL:
			pvarResult->bool = (VARIANT_BOOL)((BOOL)dwResult != 0 ? -1 : 0);
			break;
		case VT_VARIANT:
