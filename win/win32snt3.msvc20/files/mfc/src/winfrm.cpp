...
void CFrameWnd::AddFrameWnd()
{
	...
	m_pNextFrameWnd = pThreadState->m_pFirstFrameWnd;
	pThreadState->m_pFirstFrameWnd = this;
}
...
void CFrameWnd::RemoveFrameWnd()
{
	...
	AFX_THREAD_STATE* pThreadState = AfxGetThreadState();
	CFrameWnd* pFrameWnd = pThreadState->m_pFirstFrameWnd;
	ASSERT(pFrameWnd != NULL);
	if (pFrameWnd == this)
	{
		// special case for first frame in the list -- point first at next
		pThreadState->m_pFirstFrameWnd = m_pNextFrameWnd;
	}
	else
	{
...
void CFrameWnd::NotifyFloatingWindows(DWORD dwFlags)
{
	...
	if (dwFlags & (FS_DEACTIVATE|FS_ACTIVATE))
	{
		...
		pParent->SendMessage(WM_NCACTIVATE,
			!(dwFlags & FS_DEACTIVATE) && pParent->IsWindowEnabled());
	}
	...
	CCreateContext* pContext)
{
	...
	{
		CString strAbout;
		if (!strAbout.LoadString(AFX_IDS_ABOUT))
			strAbout = _T("About ");
		strAbout += AfxGetAppName();
...
LRESULT CFrameWnd::OnActivateTopLevel(WPARAM wParam, LPARAM)
{
	UNUSED wParam;  // not used in some builds
	...
	if (m_pNotifyHook != NULL)
		m_pNotifyHook->OnActivate(wParam != WA_INACTIVE);
#endif
...
void CFrameWnd::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
	...
	pTopLevel->m_bStayActive = bStayActive;
...
{
	if (m_bStayActive && IsWindowEnabled())
		bActive = TRUE;

	// don't call base class because it will call Default()
...
BOOL CFrameWnd::OnToolTipText(UINT, NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	AFX_TOOLTIPTEXT* pTTT = (AFX_TOOLTIPTEXT*)pNMHDR;
	TCHAR szFullText[256];
	CString strTipText;
	if (pNMHDR->idFrom != 0) // will be zero on a separator
	{
