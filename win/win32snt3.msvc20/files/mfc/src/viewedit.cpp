...
int CEditView::OnCreate(LPCREATESTRUCT lpcs)
{
	...
	if (hUnicodeFont == NULL)
	{
		...
		LOGFONT logFont;
		VERIFY(::GetObject(hSystemFont, sizeof(LOGFONT), (void*)&logFont));
		hUnicodeFont = ::CreateFont(logFont.lfHeight, 0, 0, 0,
			FW_DONTCARE, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
			OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
			DEFAULT_PITCH|FF_DONTCARE, _T("Lucida Sans Unicode"));
		if (hUnicodeFont == NULL)
			TRACE0("Unable to create unicode font 'Lucida Sans Unicode'.\n");
	}
...
#ifndef _WINDLL
AFX_DATADEF AFX_EDIT_STATE _afxEditState;
#endif
