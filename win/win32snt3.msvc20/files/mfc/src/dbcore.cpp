	...
	else
	{
		...
		else
		{
			if (bForward)
			{
				...
				// If sitting before first record
				if (m_bBOF)
				{
					// Moving off BOF takes 1 record
					lRows--;
					m_lCurrentRecord = 0;
				}
			}
			else
			{
				...
				// If sitting after last record
				if (m_bEOF)
				{
					// Moving off EOF takes 1 record
					lRows--;
					if (m_bEOFSeen)
			...
			if (nRetCode != SQL_NO_DATA_FOUND)
			{
				...
				else if (m_lCurrentRecord != AFX_CURRENT_RECORD_UNDEFINED)
				{
					if (bForward)
						...
						// If on deleted record, current record already decremented
						if (!m_bDeleted)
							m_lCurrentRecord--;
				}
			}
	...
	TRY
	{
		...
		if ((m_strFilter != m_strRequeryFilter) || (m_strSort != m_strRequerySort))
		{
			...
			if (m_strRequerySQL.IsEmpty())
				return Open(m_nOpenType, NULL, m_dwRequeryOptions);
			else
				return Open(m_nOpenType, m_strRequerySQL, m_dwRequeryOptions);
		}
		else
		{
...
void CRecordset::ExecuteSetPosUpdate()
{
	...
	if (m_wRowStatus != wExpectedRowStatus)
...
BOOL CRecordset::UpdateInsertDelete()
{
	...
	{
		// Make sure m_hstmtUpdate allocated, then build and execute SQL
		PrepareUpdateHstmt();
		BuildUpdateSQL();
		ExecuteUpdateSQL();
	}
