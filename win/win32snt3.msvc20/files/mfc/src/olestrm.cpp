...
BOOL COleStreamFile::GetStatus(CFileStatus& rStatus) const
{
	...
	if (statstg.pwcsName != NULL)
	{
		...
		AfxFullPath(rStatus.m_szFullName, statstg.pwcsName);
		AfxFreeTaskMem(statstg.pwcsName);
	}
	return TRUE;
}
	...
	DWORD nOpenFlags, CFileException* pError)
{
	...
	SCODE sc = lpStorage->OpenStream(lpszStreamName, NULL, nOpenFlags, 0,
		&m_lpStream);
	...
	DWORD nOpenFlags, CFileException* pError)
{
	...
	SCODE sc = lpStorage->CreateStream(lpszStreamName, nOpenFlags, 0, 0,
		&m_lpStream);
	if (FAILED(sc) && pError != NULL)
		_AfxFillOleFileException(pError, sc);
