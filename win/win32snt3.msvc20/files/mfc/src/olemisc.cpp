...
SCODE AFXAPI _AfxOleDoConvert(LPSTORAGE lpStg, REFCLSID rClsidNew)
{
	...
	TCHAR szNew[256];
	if (_AfxOleGetUserTypeOfClass(rClsidNew, szNew, sizeof(szNew), NULL) == 0)
		szNew[0] = '\0';
	...
	if ((sc = WriteFmtUserTypeStg(lpStg, cfOld, szNew)) != NOERROR)
		goto RewriteInfo;
	...
	REFCLSID rclsid, LPTSTR lpszUserType, UINT cch, HKEY hKey)
{
	LONG     dw;
	LONG     lRet;
	LPTSTR   lpszCLSID, lpszProgID;
	BOOL     bFreeProgID = FALSE;
	...
	*lpszUserType = '\0';
	...
	if (ERROR_SUCCESS != lRet && CoIsOle1Class(rclsid))
	{
