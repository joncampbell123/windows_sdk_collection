...
void COleFrameHook::OnRecalcLayout()
{
	...
	m_lpActiveObject->ResizeBorder(&rectBorder, &m_xOleInPlaceUIWindow,
...
BOOL COleFrameHook::OnDocActivate(BOOL bActive)
{
	...
	{
		// clear border space
		pNotifyHook->m_xOleInPlaceUIWindow.SetBorderSpace(NULL);
		if (m_pActiveItem->m_pInPlaceDoc != NULL)
			m_pActiveItem->m_pInPlaceDoc->m_xOleInPlaceUIWindow.SetBorderSpace(NULL);
...
BOOL COleFrameHook::OnPreTranslateMessage(MSG* pMsg)
{
	...
	if (pMsg->message >= WM_KEYFIRST && pMsg->message <= WM_KEYLAST)
		return m_lpActiveObject->TranslateAccelerator(pMsg) == NOERROR;
...
BOOL COleFrameHook::OnUpdateFrameTitle()
{
	...
	m_pActiveItem->OnUpdateFrameTitle();
	return TRUE;
}
	...
	BOOL bParam, BOOL (COleFrameHook::*pNotifyFunc)(BOOL bParam))
{
	...
	POSITION pos = pApp->m_templateList.GetHeadPosition();
	while (pos != NULL)
	{
		pTemplate = (CDocTemplate*)pApp->m_templateList.GetNext(pos);
...
void COleClientItem::OnActivateUI()
{
	...
	HWND hWnd;
	if (lpInPlaceObject->GetWindow(&hWnd) != NOERROR)
		hWnd = NULL;
	...
	LPOLEINPLACEFRAMEINFO pFrameInfo)
{
	...
	*ppMainFrame = m_pView->GetTopLevelFrame();
	...
	ASSERT((*ppMainFrame)->IsKindOf(RUNTIME_CLASS(CFrameWnd)));
	...
	if (pDocFrame != *ppMainFrame)
	{
		*ppDocFrame = pDocFrame;
		...
		ASSERT((*ppDocFrame)->IsKindOf(RUNTIME_CLASS(CFrameWnd)));
	}
...
void COleClientItem::OnUpdateFrameTitle()
{
	ASSERT_VALID(this);
	ASSERT(m_pInPlaceFrame != NULL);
	ASSERT_VALID(m_pInPlaceFrame->m_pFrameWnd);

	// format the string into <objname> in <docname>
	CString strTitle;
	AfxFormatString2(strTitle, AFX_IDS_APP_TITLE_INPLACE,
		m_pInPlaceFrame->m_strObjName, m_pDocument->GetTitle());

	// use DefWindowProc to set the text rather than SetWindowText to
	//  avoid MDI behavior of adding - [current child window] to the text.
	::DefWindowProc(m_pInPlaceFrame->m_pFrameWnd->m_hWnd, WM_SETTEXT, 0,
		(LPARAM)(LPCTSTR)strTitle);
}
...
BEGIN_INTERFACE_MAP(COleFrameHook, CCmdTarget)
	INTERFACE_PART(COleFrameHook, IID_IOleWindow, OleInPlaceUIWindow)
	INTERFACE_PART(COleFrameHook, IID_IOleInPlaceUIWindow, OleInPlaceUIWindow)
	INTERFACE_PART(COleFrameHook, IID_IOleInPlaceFrame, OleInPlaceFrame)
END_INTERFACE_MAP()

/////////////////////////////////////////////////////////////////////////////
// COleFrameHook::XOleInPlaceUIWindow implementation

STDMETHODIMP_(ULONG) COleFrameHook::XOleInPlaceUIWindow::AddRef()
{
	METHOD_PROLOGUE_EX(COleFrameHook, OleInPlaceUIWindow)
	return pThis->ExternalAddRef();
}

STDMETHODIMP_(ULONG) COleFrameHook::XOleInPlaceUIWindow::Release()
{
	METHOD_PROLOGUE_EX(COleFrameHook, OleInPlaceUIWindow)
	return pThis->ExternalRelease();
}

STDMETHODIMP COleFrameHook::XOleInPlaceUIWindow::QueryInterface(
...
STDMETHODIMP COleFrameHook::XOleInPlaceUIWindow::GetWindow(
	HWND* lphwnd)
{
	METHOD_PROLOGUE_EX(COleFrameHook, OleInPlaceUIWindow)
	ASSERT_VALID(pThis);
	...
	*lphwnd = pThis->m_hWnd;
	return *lphwnd != NULL ? NOERROR : E_FAIL;
}

STDMETHODIMP COleFrameHook::XOleInPlaceUIWindow::ContextSensitiveHelp(
	BOOL fEnterMode)
{
	METHOD_PROLOGUE_EX(COleFrameHook, OleInPlaceUIWindow)
	ASSERT_VALID(pThis);
...
STDMETHODIMP COleFrameHook::XOleInPlaceUIWindow::GetBorder(LPRECT lpRectBorder)
{
	METHOD_PROLOGUE_EX(COleFrameHook, OleInPlaceUIWindow)
	ASSERT_VALID(pThis);
...
STDMETHODIMP COleFrameHook::XOleInPlaceUIWindow::RequestBorderSpace(
	LPCRECT lpRectWidths)
{
	METHOD_PROLOGUE_EX(COleFrameHook, OleInPlaceUIWindow)
	ASSERT_VALID(pThis);

	if (!pThis->m_pFrameWnd->NegotiateBorderSpace(
...
STDMETHODIMP COleFrameHook::XOleInPlaceUIWindow::SetBorderSpace(
	LPCRECT lpRectWidths)
{
	METHOD_PROLOGUE_EX(COleFrameHook, OleInPlaceUIWindow)
	ASSERT_VALID(pThis);
...
STDMETHODIMP COleFrameHook::XOleInPlaceUIWindow::SetActiveObject(
	LPOLEINPLACEACTIVEOBJECT lpActiveObject, LPCTSTR lpszObjName)
{
	METHOD_PROLOGUE_EX(COleFrameHook, OleInPlaceUIWindow)
	ASSERT_VALID(pThis);
...
#ifdef _AFX_NO_NESTED_DERIVATION

STDMETHODIMP_(ULONG) COleFrameHook::XOleInPlaceFrame::AddRef()
{
	METHOD_PROLOGUE(COleFrameHook, OleInPlaceFrame)
	return pThis->ExternalAddRef();
}
	...
	HWND* lphwnd)
{
	...
	return pThis->m_xOleInPlaceUIWindow.GetWindow(lphwnd);
}
	...
	BOOL fEnterMode)
{
	...
	return pThis->m_xOleInPlaceUIWindow.ContextSensitiveHelp(fEnterMode);
}

STDMETHODIMP COleFrameHook::XOleInPlaceFrame::GetBorder(
	LPRECT lpRectBorder)
{
	METHOD_PROLOGUE(COleFrameHook, OleInPlaceFrame)
	ASSERT_VALID(pThis);

	return pThis->m_xOleInPlaceUIWindow.GetBorder(lpRectBorder);
}
	...
	LPCRECT lpRectWidths)
{
	...
	return pThis->m_xOleInPlaceUIWindow.RequestBorderSpace(lpRectWidths);
}
	...
	LPCRECT lpRectWidths)
{
	...
	return pThis->m_xOleInPlaceUIWindow.SetBorderSpace(lpRectWidths);
}
	...
	LPOLEINPLACEACTIVEOBJECT lpActiveObject, LPCTSTR lpszObjName)
{
	...
	return pThis->m_xOleInPlaceUIWindow.SetActiveObject(
		lpActiveObject, lpszObjName);
}

#endif //_AFX_NO_NESTED_DERIVATION
...
STDMETHODIMP COleFrameHook::XOleInPlaceFrame::EnableModeless(BOOL fEnable)
{
	...
	TRY
	{
		if (!fEnable)
		{
			...
			if (!pThis->m_pFrameWnd->InModalState())
	...
	BOOL fEnterMode)
{
	...
	return pThis->m_pInPlaceFrame->
		m_xOleInPlaceUIWindow.ContextSensitiveHelp(fEnterMode);
}
...
STDMETHODIMP COleClientItem::XOleIPSite::CanInPlaceActivate()
{
	...
	return pThis->CanActivate() ? NOERROR : S_FALSE;
}
