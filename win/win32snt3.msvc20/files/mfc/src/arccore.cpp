...
UINT CArchive::Read(void* lpBuf, UINT nMax)
{
	...
	if (nMaxTemp != 0)
	{
		...
		UINT nRead = m_pFile->Read(lpBuf, nTemp);
		lpBuf = (BYTE*)lpBuf + nRead;
		nMaxTemp -= nRead;
		...
		if (nRead == nTemp)
		{
			...
			if (!m_bDirectBuffer)
			{
				nRead = m_pFile->Read(m_lpBufStart, max(nMaxTemp, (UINT)m_nBufSize));
...
void CArchive::FillBuffer(UINT nBytesNeeded)
{
	...
	if (!m_bDirectBuffer)
	{
		...
		if (m_lpBufCur > m_lpBufStart)
		{
			...
			UINT nActual = m_pFile->Read(m_lpBufStart+nUnused, m_nBufSize-nUnused);
			m_lpBufCur = m_lpBufStart;
