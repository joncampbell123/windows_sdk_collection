...
{
	// return buffer size required for file title
	if (lpszTitle == NULL)
		return GetFileTitle(lpszPathName, NULL, 0);

	ASSERT(AfxIsValidAddress(lpszTitle, _MAX_FNAME));
	ASSERT(AfxIsValidString(lpszPathName, FALSE));

#ifndef _MAC
	if (GetFileTitle(lpszPathName, lpszTitle, (WORD)nMax) != 0)
	{
		// when GetFileTitle fails, use cheap imitation
		LPCTSTR lpszTemp = lpszPathName;
