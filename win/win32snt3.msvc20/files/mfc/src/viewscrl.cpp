...
void CScrollView::ScrollToDevicePosition(POINT ptDev)
{
	...
	DoScrollWindow(xOrig - ptDev.x, yOrig - ptDev.y);
}

int CScrollView::GetScrollLimit(int nBar)
{
	int nMin, nMax;
...
BOOL CScrollView::OnScrollBy(CSize sizeScroll, BOOL bDoScroll)
{
	...
	if (bDoScroll)
	{
		...
		DoScrollWindow(-(x-xOrig), -(y-yOrig));
		if (x != xOrig)
...
void CScrollView::DoScrollWindow(int xAmount, int yAmount)
{
	if (IsWindowVisible())
	{
