...
//CHAPTER16MOD
//This is filled by CFigure on in-place activation
PCGizmoBar  g_pInPlaceGB=NULL;
//End CHAPTER16MOD
...
CCosmoFrame::CCosmoFrame(HINSTANCE hInst, HINSTANCE hInstPrev
    ...
    {
    UINT        i;
    char        szTemp[CCHPATHMAX];     //ParseCmdLine is ANSI
    ...
    //CHAPTER16MOD
    m_pIOleIPFrame=NULL;    //Flag for in-place or not.
    //End CHAPTER16MOD
...
CCosmoFrame::~CCosmoFrame(void)
    ...
    //Should be last Release, which frees the class factory.
...
BOOL CCosmoFrame::FInit(PFRAMEINIT pFI)
    ...
    if (m_fEmbedding)
        ...
        hr=CoRegisterClassObject(CLSID_Cosmo2Figure, m_pIClassFactory
...
/*
 * CCosmoFrame::MessageLoop
 *
 * Purpose:
 *  Override of standard message loop function in CLASSLIB to use
 *  in-place accelerators when necessary.  We only need to make
 *  sure we call OleTranslateAccelerator as appropriate.
 *
 * Parameters:
 *  None
 *
...
WPARAM CCosmoFrame::MessageLoop(void)
    ...
    while (GetMessage(&msg, NULL, 0,0 ))
        ...
        /*
         * Let the container have a crack during in-place.  Note
         * that if we were an MDI multiple-use server, which we
         * are not, then we'd have to make sure that the document
         * with the in-place object was active, and we would
         * have to make sure that we use the right IOleInPlaceFrame
         * and frame info variables.  So what's shown here is a
...
/*
 * CCosmoFrame::FMessageHook
 *
 * Purpose:
 *  Override of CFrame::FMessageHook so we can specifically trap
 *  WM_MENUSELECT messages in order to call
...
BOOL CCosmoFrame::FMessageHook(HWND hWnd, UINT iMsg
    ...
    /*
     * We have to now get the message that the StatStrip would
     * display for this menu over to the container.  We do this
     * by letting the StatStrip do its thing first on our currently
     * hidden window (beware of this technique if you are multiple
     * use and might be visible!) then we ask the StatStrip for its
     * current text which we send to the container via
     * IOleInPlaceFrame::SetStatusText.
     */

    m_pSS->MenuSelect(wParam, lParam);
    m_pSS->MessageGet(szText, sizeof(szText));
#if defined(WIN32) && !defined(UNICODE)
    OLECHAR pwcsText[128];
    mbstowcs(pwcsText, szText, 128);
    m_pIOleIPFrame->SetStatusText(pwcsText);
#else
...
BOOL CCosmoFrame::FRegisterAllClasses(void)
    ...
    //CHAPTER16MOD
    if (!FHatchWindowRegister(m_hInst))
        return FALSE;
    //End CHAPTER16MOD
...
/*
 * CCosmoFrame::ParseCommandLine
 *
 * Purpose:
 *  Allows the application to parse the command line and take action
 *  after the window has possibly been shown. For a compound
 *  document server we need to just make sure that if -Embedding
 *  is there that we take no file action.  FPreShowInit has already
...
void CCosmoFrame::ParseCommandLine(void)
    {
...
void CCosmoFrame::CheckLineSelection(UINT uID)
    ...
    if (uID!=m_uIDCurLine)
        ...
        //CHAPTER16MOD
        //Affect in-place Gizmobar as well
        if (NULL!=g_pInPlaceGB)
            g_pInPlaceGB->Check(uID, TRUE);
        //End CHAPTER16MOD
        }
...
void CCosmoFrame::UpdateEmbeddingUI(BOOL fEmbedding
    ...
    //Now let's play with the gizmobar.
