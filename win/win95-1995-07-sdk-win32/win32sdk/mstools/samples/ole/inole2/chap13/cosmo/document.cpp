    ...
    *pLRes=0;
    ...
    if (WM_LBUTTONDOWN==iMsg)
        ...
        if (NULL!=pIDataObject)
            {
            //CHAPTER13MOD
            //Add DROPEFFECT_LINK now that we source links
         ...
         * IDropTarget::Drop in IDROPTGT.CPP).  In any case,
...
UINT CCosmoDoc::ULoad(BOOL fChangeFile, LPTSTR pszFile)
    ...
    pwcsFile=pszFile;
#endif
    if (NOERROR!=StgIsStorageFile(pwcsFile))
        {
        ...
        {
        hr=StgOpenStorage(pwcsFile, NULL, STGM_DIRECT | STGM_READ
...
void CCosmoDoc::Rename(LPTSTR pszFile)
    ...
    /*
     * Get rid of the old moniker.  The OldStd function
    ...
    if (NULL!=pszFile)
        ...
        if (NULL!=pmk)
            ...
            OleStdRegisterAsRunning(m_pFigure, m_pMoniker
                , &m_dwRegROT);
...
BOOL CCosmoDoc::FRenderMedium(UINT cf, LPSTGMEDIUM pSTM)
    {
    if (NULL==pSTM)
        return FALSE;
    ...
    if (cf==m_cfEmbedSource)
        ...
        pSTM->pstg=OleStdCreateStorageOnHGlobal(NULL, TRUE
        ...
        WriteClassStg(pSTM->pstg, CLSID_Cosmo2Figure);
#if defined(WIN32) && !defined(UNICODE)
        OLECHAR pwcsTemp[256];
        mbstowcs(pwcsTemp, PSZ(IDS_CLIPBOARDFORMAT), 256);
        WriteFmtUserTypeStg(pSTM->pstg, m_cf, pwcsTemp);
#else
...
#if defined(WIN32) && !defined(UNICODE)
        OLECHAR pwcsTemp[256];
        mbstowcs(pwcsTemp, PSZ(IDS_OBJECTDESCRIPTION), 256);
        pSTM->hGlobal=OleStdGetObjectDescriptorData
            (CLSID_Cosmo2Figure, DVASPECT_CONTENT, szl, ptl
            , OLEMISC_RECOMPOSEONRESIZE, pwcsTemp
            , psz);
#else
        pSTM->hGlobal=OleStdGetObjectDescriptorData
            (CLSID_Cosmo2Figure, DVASPECT_CONTENT, szl, ptl
    ...
    if (cf==m_cfLinkSource)
        ...
        if (NULL!=m_pMoniker)
            ...
            hr=OleStdGetLinkSourceData(m_pMoniker
                , (LPCLSID)&CLSID_Cosmo2Figure, &fe, pSTM);
