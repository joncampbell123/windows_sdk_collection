        ...
        case DID_OK:
          ...
          if (gdwEnumFlags)
          {
            if (gdwEnumFlags & PRINTER_ENUM_NAME)
            {
              ...
              {
                MessageBox (hwnd,
                            (LPCTSTR) "Please enter a valid domain/server name",
                            (LPCTSTR) "", MB_OK);
...
BOOL DisplayEnumPrintersInfo (HWND hwnd)
{
  ...
  {
    ErrMsgBox ("EnumPrinters/LocalAlloc failed", ERR_MOD_NAME);
    bReturn = FALSE;
  ...
  {
    ErrMsgBox ("EnumPrinters failed", ERR_MOD_NAME);
    bReturn = FALSE;
  ...
  if (!dwPrtRet1 || !dwPrtRet2)
  {
    ErrMsgBox ("EnumPrinters returned 0 printers", "");
    bReturn = FALSE;
    goto display_prts_info_done3;
  }

  dwMaxPrt = dwPrtRet1 > dwPrtRet2 ? dwPrtRet2 : dwPrtRet1;

  SetEnumPrintersDlgFields (hwnd, dwMaxPrt, pPrtInfo1,
                            pPrtInfo2);

display_prts_info_done3:
                               ...
                               LPPRINTER_INFO_2 pPrtInfo2)
{
  ...
  for (j = 0; j < dwMaxPrt; j++)
  {
    ...
    if ((pPrtInfo2 + j)->pDevMode)
    {
      ...
      ComplexEnumPrintersLine (hwnd, gaEnumPrt[18], gaFields, MAX_FIELDS,
                               dwFields);
      ...
      if (dwFields & DM_ORIENTATION)
        ...
        ComplexEnumPrintersLine (hwnd, gaEnumPrt[19], gaOrientation,
                                 MAX_ORIENTATION,
                                 (DWORD)
                                 (pPrtInfo2+j)->pDevMode->dmOrientation);
      ...
      if (dwFields & DM_COLLATE)
        ...
        outnum (gaEnumPrt[30], NULL);

      if (dwFields & DM_FORMNAME)

        outnum (gaEnumPrt[31], NULL);
    }
    else
    {
    ...
    ComplexEnumPrintersLine (hwnd, gaEnumPrt[43], gaStatus, MAX_STATUS,
                             (pPrtInfo2 + j)->Status);
...
BOOL DisplayPrinterDriversInfo (HWND hwnd)
{
  ...
  if (!EnumPrinterDrivers ((LPTSTR) NULL, (LPTSTR) NULL, 1,
                           ...
                           &dwDrvRet))
  {
    ErrMsgBox ("EnumPrinterDrivers returned 0 drivers", ERR_MOD_NAME);
    bReturn = FALSE;
  ...
  if (!dwDrvRet)
  {
    ErrMsgBox ("EnumPrinterDrivers returned 0 drivers", "");
    bReturn = FALSE;
