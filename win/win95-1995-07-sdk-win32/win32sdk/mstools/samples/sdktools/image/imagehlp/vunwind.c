...
#define OP_INTENT(OP,INTENT) ((OP) << 2 | (INTENT))
    ...
    switch (OP_INTENT (I->Primary_Op, Intent)) {
      ...
      case OP_INTENT (B_OP, UnwindReverse):
        ...
        //
        if ((Info->FunctionEntry = RtlLookupFunctionEntry(Pc)) != NULL
...

{
    ...
    if (InstrClass == InstrRestoreCode) {
        ...
        // beyond the end of the prologue, then the control PC is
        ...
        if (ControlPc >= (FunctionEntry->PrologEndAddress & ~3)) {
  ...
  CheckForGlue:
    ...
    if ((FunctionEntry = RtlLookupFunctionEntry(Pc)) != NULL) {
        //
        ...
        if (I.Primary_Op == LWZ_OP && I.Dform_RA == GPR2) {
            ...
            if (I.Primary_Op == B_OP && I.Iform_LK) {
                ...
                FunctionEntry = RtlLookupFunctionEntry(Pc);
            }
        }
    }
