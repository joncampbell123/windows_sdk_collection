...
#define JUMP_CONT_DEEPER rConfig.rPlayer[rGameState.iPlayerTurn].iShortJumpEval
...
note: REVIEW do not prune equal qualities becuase they may be used to increase
...
inline int AlphaBeta(int t,long q, long c)
{
    pdebug(stddbg,"alpha-beta pruning t=%d q=%ld c=%ld %s(%d)\n",t,q,c,__FILE__,__LINE__);
        ...
        if (0==rConfig.rPlayer[1].iUseAlphaBetaPruning) return 0;
    #endif
    if (c > 0)
        {
        if (computer_color & t)
                         ...
                         long c, long p, int* fBreak)
{
    *fBreak = AlphaBeta(t,q,c);
    if (q == *bestquality)
        ...
        #ifndef SHIPPING_VERSION_REVIEW
        pdebug(stddbg,"!!! WARNING this needs to be removed !!! %s(%d)\n",__FILE__,__LINE__); //REVIEW
        if (0!=rConfig.rPlayer[1].iUseEqualMoveSkipping) return;
        #endif
        if (1 & rand()) return;
        pdebug(stddbg,"evaluation (equal move assumed better) t=%d type=%d num=%d q=%ld d=%d %s(%d)\n",t,type,number,q,d,__FILE__,__LINE__);
        }
    ...
    if ((RED == t) || (KING & b[start]))
        ...
        if (0 == b[red_jump_lut[start][1]] && red_jump_lut[start][1] && (next(t) & b[red_jump_lut[start][0]]))
            ...
            if (0 == red_jump_lut[start][4] || (KING & play_board[red_jump_lut[start][1]]))
                {
                q = CheckMiniMaxAlphaBeta(play_board,t,red_jump_lut[start][1],JUMP_CONT_DEEPER+d,c,p); //REVIEW: inifinite recursion?
                }
            else
                {
        ...
        if (0 == b[red_jump_lut[start][3]] && red_jump_lut[start][3] && (next(t) & b[red_jump_lut[start][2]]))
            ...
            if (0 == red_jump_lut[start][4] || (KING & play_board[red_jump_lut[start][3]]))
                {
                q = CheckMiniMaxAlphaBeta(play_board,t,red_jump_lut[start][3],JUMP_CONT_DEEPER+d,c,p); //REVIEW: inifinite recursion?
                }
            else
                {
    ...
    if ((BLACK == t) || (KING & b[start]))
        ...
        if (0 == b[black_jump_lut[start][1]] && black_jump_lut[start][1] && (next(t) & b[black_jump_lut[start][0]]))
            ...
            if (0 == black_jump_lut[start][4] || (KING & play_board[black_jump_lut[start][1]]))
                {
                q = CheckMiniMaxAlphaBeta(play_board,t,black_jump_lut[start][1],JUMP_CONT_DEEPER+d,c,p); //REVIEW: inifinite recursion?
                }
            else
                {
        ...
        if (0 == b[black_jump_lut[start][3]] && black_jump_lut[start][3] && (next(t) & b[black_jump_lut[start][2]]))
            ...
            if (0 == black_jump_lut[start][4] || (KING & play_board[black_jump_lut[start][3]]))
                {
                q = CheckMiniMaxAlphaBeta(play_board,t,black_jump_lut[start][3],JUMP_CONT_DEEPER+d,c,p); //REVIEW: inifinite recursion?
                }
            else
                {
...
long CheckMiniMaxAlphaBeta(BOARD b, int t, int j, int d, long c, long p)
{
    ...
    //$REVIEW!! this assert goes off often AssertSz(0==fBreak,"should I cry?");
    if (0==moves_made)
        {
        // REVIEW: bug? should remember move? or what value does this move have?
        // REVIEW: pruning parameters?
