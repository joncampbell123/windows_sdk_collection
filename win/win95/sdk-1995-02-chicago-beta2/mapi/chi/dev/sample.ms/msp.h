...
#ifdef VALIDATE
#define OBJ_ValidateParameters(pobj, eMethod, cbObj, _lpVtbl)               \
    {   ValidateParameters(eMethod, (LPVOID)&pobj);                         \
        if (IsBadWritePtr(pobj, cbObj) || (pobj)->lpVtbl != _lpVtbl)        \
            return(ResultFromScode(MAPI_E_INVALID_PARAMETER)); }

#define OBJ_CheckParameters(pobj, eMethod, cbObj, _lpVtbl)                  \
    {   CheckParameters(eMethod, (LPVOID)&pobj);                            \
        AssertSz(!IsBadWritePtr(pobj, cbObj) && (pobj)->lpVtbl == _lpVtbl,  \
            "Bad object pointer"); }
#else
#define OBJ_ValidateParameters(pobj, eMethod, cbObj, lpVtbl)
#define OBJ_CheckParameters(pobj, eMethod, cbObj, lpVtbl)
#endif /* VALIDATE */
...
HRESULT HrWrap_GetProps(HRESULT hr, PIMS pims, ULONG cvalInt,
    ...
    BOOL fStore, BOOL fTagsSpecified);
...
struct _MSP
{
    ...
    STANDARD_OBJECT_MEMBERS
    CRITICAL_SECTION_MEMBERS        /* Critical section (WIN32 only) */
...
struct _MSL
{
    ...
    STANDARD_OBJECT_MEMBERS
    CRITICAL_SECTION_MEMBERS        /* Critical section (WIN32) */
    LMR                 lmr;        /* Linked memory routines */
    IFDBG(BOOL          fInvalid;)  /* TRUE if invalid (DEBUG only) */
};
