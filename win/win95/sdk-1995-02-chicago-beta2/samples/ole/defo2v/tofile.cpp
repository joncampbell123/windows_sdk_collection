...
#else
        wsprintf( szBuf,    _T("' Type Library: %s, Library Version %d.%03d\r\n"),
                            BSTR_PRT(bstrName), pTlibAttr->wMajorVerNum, pTlibAttr->wMinorVerNum ) ;
#endif
        StringToFile( hf, szBuf ) ;
...
#else
        wsprintf( szBuf,    _T("' Type Info: %s, TypeInfo Version %d.%03d\r\n"),
                            BSTR_PRT(bstrName), pTA->wMajorVerNum, pTA->wMinorVerNum ) ;
#endif
        StringToFile( hfile, szBuf ) ;
    ...
    for ( iIndex = 0 ; iIndex < cFuncs ; iIndex++)
    {
        ...
        if (SUCCEEDED( hr ))
        {
            ...
            if (cNames > 0)
            {
                ...
                wcstombs(szFunc, rgbstrNames[0], wcslen(rgbstrNames[0]) + 1);
#else
            ...
            for ( int n = 0 ; n < cParams - cParamsOpt ; n++ )
            {
                if (n+1 < cNames)
                {
                    ...
                    wcstombs(lpsz, rgbstrNames[n+1], wcslen(rgbstrNames[n+1]) + 1);
#else
            ...
            if (SUCCEEDED( hr ))
            {
                ...
                if (pVarDesc->varkind == VAR_CONST)
                {
                    ...
                    wsprintf( szBuf, _T("Const %ws As %s = "),
                                        ...
                                        (LPTSTR)TYPEDESCtoString( &pVarDesc->elemdescVar.tdesc )
                                     ) ;
#else
                    ...
                    {
#if defined (WIN32) && !defined (UNICODE)
                        wcstombs(&(szBuf[strlen(szBuf)+1]), varValue.bstrVal, wcslen(varValue.bstrVal)+1);
#else
...
#if defined (WIN32) && !defined (OLE2ANSI)
                    wsprintf( szBuf, _T("Dim %ws "), (LPOLESTR)(rgbstrNames[0] ? rgbstrNames[0]: OLESTR("(name less)")) ) ;
#else
                    wsprintf( szBuf, _T("Dim %s "), (LPTSTR)(rgbstrNames[0] ? rgbstrNames[0]: _T("(name less)")) ) ;
#endif
