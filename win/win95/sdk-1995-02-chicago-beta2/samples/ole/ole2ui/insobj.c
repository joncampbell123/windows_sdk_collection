      ...
      case WM_COMMAND:
         switch (wID)
            ...
            case ID_IO_CHANGEICON:
            {
               ...
               OLECHAR   pszUniString[256], pszUniCLSID[256];
               ...
               if (lpIO->dwFlags & IOF_SELECTCREATENEW)
               {
                  ...
                  CLSIDFromString((LPOLESTR)pszUniCLSID, (LPCLSID)&(ci.clsid));
               ...
               else  // IOF_SELECTCREATEFROMFILE
               {
                  ...
                  A2W (szFileName, pszUniString, 256);
                  if (NOERROR != GetClassFile(pszUniString, (LPCLSID)&(ci.clsid)))
                  {
                    LPSTR lpszExtension;
                    int   istrlen;
                    ...
                    GetAssociatedExecutable(lpszExtension, (LPSTR)szAnsiStr);
                    A2W (szAnsiStr, ci.szIconExe, OLEUI_CCHPATHMAX);
                    ci.cchIconExe = OLESTRLEN(ci.szIconExe);
               ...
               if (0!=uRet)
               {
                  ...
                  if (lpIO->dwFlags & IOF_SELECTCREATENEW)
                    SendMessage(hList, LB_SETITEMDATA, iCurSel, (LPARAM)(UINT)ci.hMetaPict);
               }
            }
               break;
            ...
            case ID_IO_FILE:
               ...
               if (FValidateInsertFile(hDlg, FALSE, &lpIO->nErrCode))
               {
                  ...
                  wGetFileTitle((LPSTR)szTemp, szAnsiStr, 256);
                  A2W (szAnsiStr, lpIO->szFile, OLEUI_CCHPATHMAX);
                  istrlen = OLESTRLEN(lpIO->szFile);
               ...
               W2A (lpIO->szFile, szAnsiStr, 256);
               uRet=UStandardHook(lpIO, hDlg, uMsgBrowse
                  , OLEUI_CCHPATHMAX, (LPARAM)(LPSTR)szAnsiStr);
...
//                  dwOfnFlags |= OFN_SHOWHELP;
               ...
               W2A (lpIO->szFile, szAnsiStr, 256);
               if (0!=uRet && 0!=lstrcmpi(szTemp, szAnsiStr))
               {
                  SetDlgItemText(hDlg, ID_IO_FILEDISPLAY, szAnsiStr);
            ...
            case IDOK:
            {
               ...
               if (lpIO->dwFlags & IOF_SELECTCREATENEW)
               {
                  ...
                  lpszAnsiStr=PointerToNthField((LPSTR)szBuffer, 2, '\t');
                  A2W (lpszAnsiStr, lpszCLSID, 256);
               ...
               else  // IOF_SELECTCREATEFROMFILE
               {
                  ...
                  GetDlgItemText(hDlg, ID_IO_FILEDISPLAY,
                             szAnsiStr, lpOIO->cchFile);
                  A2W (szAnsiStr, lpIO->szFile, lpOIO->cchFile);
                  A2W (szAnsiStr, lpOIO->lpszFile, lpOIO->cchFile);
...
BOOL FInsertObjectInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
   ...
   if ( (lpOIO->lpszFile) && ('\0' != *lpOIO->lpszFile) )
      OLESTRCPY(lpIO->szFile, lpOIO->lpszFile);
   else
      *(lpIO->szFile) = '\0';
   ...
   if (NULL!=lpOIO->lpszCaption) {
      W2A (lpOIO->lpszCaption, szAnsiStr, 256);
   ...
   while (TRUE)
      ...
      if ((LONG)ERROR_SUCCESS==lRet)
         ...
         */
         ...
         pszID=pIMalloc->lpVtbl->Alloc(pIMalloc, OLEUI_CCHKEYMAX*sizeof(OLECHAR));
         ...
         *(pszClass+cch)=0;  // set back to rootkey
         ...
         dw=OLEUI_CCHKEYMAX;
         lRet=RegQueryValue(hKey, pszClass, szAnsiStr, &dw);
         A2W (szAnsiStr, pszID, OLEUI_CCHKEYMAX);

         if ((LONG)ERROR_SUCCESS!=lRet)
            continue;   // CLSID subkey not found

         lstrcpy(pszExec, "CLSID\\");
         W2A (pszID, szAnsiStr, 256);
         lstrcat(pszExec, szAnsiStr);
      ...
      if (NULL==pszID)
         ...
         CLSIDFromProgID(szUniStr, &clsid);
...
void UpdateClassType(HWND hDlg, LPINSERTOBJECT lpIO, BOOL fSet)
{
   ...
   CLSID clsid;
   char  szFileName[OLEUI_CCHPATHMAX];
   OLECHAR szUniFileName[OLEUI_CCHPATHMAX];
   char  szFileType[OLEUI_CCHLABELMAX];
   OLECHAR szUniFileType[OLEUI_CCHLABELMAX];
   ...
   *szFileType = '\0';
   ...
   if (fSet)
   {
     ...
     A2W (szFileName, szUniFileName, OLEUI_CCHPATHMAX);
     if (NOERROR == GetClassFile(szUniFileName, &clsid) )
     {
      OleStdGetUserTypeOfClass(&clsid, szUniFileType, OLEUI_CCHLABELMAX, NULL);
      W2A (szUniFileType, szFileType, OLEUI_CCHLABELMAX);
     }
   }
...
void UpdateClassIcon(HWND hDlg, LPINSERTOBJECT lpIO, HWND hList)
   ...
   *pszName=0;
   ...
   if (lpIO->dwFlags & IOF_SELECTCREATENEW)
   {
      ...
      *pszTemp='\0';
      A2W (pszCLSID, pszUniCLSID, 256);
      CLSIDFromString(pszUniCLSID, &lpIO->clsid);
   ...
   else
   {
      ...
      A2W (pszName, pszUniCLSID, 256);
      if (lpIO->dwFlags & IOF_CHECKLINK ||
         NOERROR != GetClassFile(pszUniCLSID, &clsid) ) {

         // get icon using filename as label (if creating a link)
         hMetaPict = OleGetIconOfFile(pszUniCLSID,
   ...
   if (lpIO->dwFlags & IOF_SELECTCREATENEW)
      SendMessage(hList, LB_SETITEMDATA, (WPARAM)iSel, (LPARAM)(UINT)hMetaPict);
