    ...
    )
{
    ...
    while (p = *pp) {
        if (!stricmp( p->ModuleName, ModuleName )) {
            return p;
            }
    ...
    p = LocalAlloc( LMEM_ZEROINIT, sizeof( *p ) );
    if (p != NULL) {
        if (Dll != NULL) {
    ...
    )
{
    ...
    *BoundForwarder = FALSE;
    ...
    strncpy( DllName, ForwarderString, cb );
    DllName[ cb ] = '\0';
    strcat( DllName, ".DLL" );
    ...
    for ( HintIndex = 0; HintIndex < Exports->NumberOfNames; HintIndex++){
        ...
        if ( NameTableName ) {
            if ( !strcmp((PCHAR)s, NameTableName) ) {
                ...
                while (p = *pp) {
                    if (!stricmp(DllName, p->ModuleName)) {
                        break;
                        }
                ...
                ForwarderString = (LPSTR)ForwardedAddress;
                *BoundForwarder = TRUE;
                break;
                }
            }
    ...
    )
{
    ...
    *NewImportsSize = 0;
    ...
    cbString = BindpEndCapturedModuleNames - BindpCapturedModuleNames;
    BindpEndCapturedModuleNames = NULL;
    *NewImportsSize = cbStruct+((cbString + sizeof(ULONG) - 1) & ~(sizeof(ULONG)-1));
    NewImports = LocalAlloc(LMEM_ZEROINIT, *NewImportsSize);
    if (NewImports != NULL) {
        ...
        while (p = *pp) {
            ...
            NewImport->OffsetModuleName = (USHORT)(cbStruct + (p->ModuleName - BindpCapturedModuleNames));
            ...
            while (p1 = *pp1) {
                ...
                NewForwarder->OffsetModuleName = (USHORT)(cbStruct + (p1->ModuleName - BindpCapturedModuleNames));
                NewForwarder += 1;
                pp1 = &p1->Next;
                }
    ...
    )
{
    ...
    if (lpMappedAddress != Dll->MappedAddress) {
        Dll->MappedAddress = lpMappedAddress;
        ...
        if (LoadedImage->FileHeader != NULL &&
            ...
            if (!Parms.fNoUpdate && ImageModified &&
                ...
                if (fSymbolsAlreadySplit) {
                    UpdateDebugInfoFile(DebugFileName, SymbolPath, DebugFilePath, LoadedImage->FileHeader);
                    }
    ...
    )
{
    ...
    if (Ordinal) {
        ...
        strcat((PCHAR) ImportName->Name, _ultoa((ULONG) OrdinalNumber, szOrdinal, 16));
        }
    else {
    ...
    if ((ULONG)FunctionAddress->u1.Function > (ULONG)ExportsBase &&
        ...
        if (NewImport != NULL) {
            FunctionAddress->u1.ForwarderString = BindpAddForwarderReference(Parms,
                                           ...
                                           NewImport,
                                           DllPath,
                                           BindpRvaToVa( Parms, (PVOID)(FunctionTableBase[OrdinalNumber]), Dll ),
                                           &BoundForwarder
                                          );
            }
