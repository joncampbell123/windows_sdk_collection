...
STDMETHODIMP CImpIPersistFile::Load(LPCOLESTR pszFile, DWORD grfMode)
    ...
    uRet=m_pObj->m_pDoc->ULoad(TRUE, (LPTSTR)pszFile);
...
STDMETHODIMP CImpIPersistFile::Save(LPCOLESTR pszFile, BOOL fRemember)
     ...
     * remember this file, as that cons USave into ignoring this
    ...
    wcstombs(szTemp, pszFile, MAX_PATH);
    uRet=m_pObj->m_pDoc->USave(0, (LPTSTR)szTemp);
#else
    uRet=m_pObj->m_pDoc->USave(0, (LPTSTR)pszFile);
#endif
...
STDMETHODIMP CImpIPersistFile::GetCurFile(LPOLESTR * ppszFile)
    ...
    *ppszFile=NULL;
    ...
    psz=(LPOLESTR)pIMalloc->Alloc(OLEUI_CCHPATHMAX*sizeof(OLECHAR));
    pIMalloc->Release();
#if defined(WIN32) && !defined(UNICODE)
    TCHAR szTemp[OLEUI_CCHPATHMAX];
    uRet=m_pObj->m_pDoc->FilenameGet(szTemp, OLEUI_CCHPATHMAX);
    mbstowcs(psz, szTemp, OLEUI_CCHPATHMAX);
#else
    uRet=m_pObj->m_pDoc->FilenameGet(psz, OLEUI_CCHPATHMAX);
#endif
    //If we have no filename, return the prompt for File Open/Save.
    if (0==uRet)
#if defined(WIN32) && !defined(UNICODE)
        mbstowcs(psz, (*m_pObj->m_pST)[IDS_EXTENSION], CCHPATHMAX);
#else
