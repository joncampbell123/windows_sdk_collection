...
///////////////////////////////////////////////////////////////////////////////
// pointers to interface object
MyExchExt * pExchExt = NULL;
MyExchExtCommands *  pExchExtCommands = NULL;
MyExchExtUserEvents *  pExchExtUserEvents = NULL;
...
//
//    Purpose
//    The entry point that calls our interface.  
//
//    Return Value
//    Pointer to Exchange Extension Object
//
//    Comments
//    This is called for each context entry.  AddRef is called here because
//    for each call to ExchEntryPoint, there will be a call to 
//    pExchExt->Release() by Exchange.
//
LPEXCHEXT CALLBACK ExchEntryPoint(void)
{
    if (pExchExt == NULL)
        pExchExt = new MyExchExt;

    ((LPEXCHEXT)pExchExt)->AddRef();

    return (LPEXCHEXT)pExchExt;
}
...
//
//    Purpose
//    Returns a pointer to an interface object that is requested by ID.  This
//    is called for each interface that is requested.  The interfaces are
//    requested everytime a new context is entered.  The IID_IExchExt* 
//    interfaces are ignored if not specified in the Exchange .INI file for 
//    this extension.
//
//    Return Value - none
//
...
STDMETHODIMP MyExchExt::QueryInterface(REFIID riid, LPVOID * ppvObj)
{
    ...
    *ppvObj = NULL;
    ...
    else if (IsEqualIID(riid, IID_IExchExt))
    {
        *ppvObj = (LPUNKNOWN) pExchExt;
    }
    else if (IsEqualIID(riid, IID_IExchExtCommands))
    {
        if (pExchExtCommands == NULL)
            pExchExtCommands =  new MyExchExtCommands;
            *ppvObj = (LPUNKNOWN) pExchExtCommands;
        
    }
    else if (IsEqualIID(riid, IID_IExchExtUserEvents))
    {
        if (pExchExtUserEvents == NULL)
            pExchExtUserEvents =  new MyExchExtUserEvents;
            *ppvObj = (LPUNKNOWN) pExchExtUserEvents;
    }
    else
...
//    number is checked here.  AddRef is called to track the number of 
//    installs.
//
//    Return Value - none
//
                            ...
                            UINT ctbe, ULONG ulFlags)
{
 ...
 if (pExchExt->GetLastExchContext() == EECONTEXT_ADVANCEDCRITERIA)
 {
  ...
  //// add our extension command
   ...
   m_cmdidFind = *pcmdidBase;

   (*pcmdidBase)++;
 }

 if (pExchExt->GetLastExchContext() == EECONTEXT_VIEWER)
 {
...
STDMETHODIMP MyExchExtCommands::DoCommand(LPEXCHEXTCALLBACK pmecb, UINT cmdid)
{
 ...
 if (cmdid == m_cmdidMain)
 {
    ...
    if (cmdid == m_cmdidMain)
        hWnd = m_hWndMain;
    if (cmdid == m_cmdidFind)
        hWnd = m_hWndFind;

    MessageBox(hWnd, szBuffer, "Sample Extension - Folder Stats", MB_OK);

    hr = S_OK;
    
 }

 if (cmdid == m_cmdidFind)     // must use a different technique in Find window
 {
...
STDMETHODIMP MyExchExtCommands::Help(LPEXCHEXTCALLBACK pmecb, UINT cmdid)
{
 ...
 if ((cmdid == m_cmdidMain) || (cmdid == m_cmdidFind))
 {
   if (cmdid == m_cmdidMain)
      hWnd = m_hWndMain;
   if (cmdid == m_cmdidFind)
   ...
   MessageBox(hWnd, "Called through IExchExtCommands::Help()\n"
                    "Contained in cmdext32.dll\n\n"
                    "Copyright (c) 1995 Microsoft Corporation.\n"
                    "All rights reserved.",
                    "About Sample Extension", MB_OK);
                                              ...
                                              LPTSTR psz, UINT cch)
{
 ...
 if ( (cmdid == m_cmdidFind) || (cmdid == m_cmdidMain) )
 {
  if (ulFlags == EECQHT_STATUS)
    lstrcpyn(psz, "EXCHEXT QueryHelpText", cch);
  
  if (ulFlags == EECQHT_TOOLTIP)
    lstrcpyn(psz, "EXCHEXT ToolTip Text", cch);
                            ...
                            ULONG ulFlags)
{
 HRESULT hr = S_FALSE;
  ...
  if (m_itbb == itbb)
  {
   ...
   ptbb->idCommand = m_cmdidMain;      // same command for Main or Find windows
   ptbb->fsState = TBSTATE_ENABLED;
   ptbb->fsStyle = TBSTYLE_BUTTON;
   ptbb->dwData = 0;
   ptbb->iString = -1;
   lstrcpyn(lpsz, "Help - Sample Command Extension Menu Item", cch);
...
STDMETHODIMP_(VOID) MyExchExtUserEvents::OnSelectionChange(LPEXCHEXTCALLBACK pmecb)
{
 ...
 cmdid = pExchExtCommands->GetCommandID();


 // Enable or Disable the command depending if the selected object is
 // a folder or a "non-folder".
 // EnableMenuItem will effect both the menu item and the corresponding 
 // toolbar button
