...
TCHAR szTextureFilter[GEN_STRING_SIZE];
TCHAR szBmp[GEN_STRING_SIZE];
TCHAR szDotBmp[GEN_STRING_SIZE];
TCHAR szClassName[] = TEXT("3DFOScreenSaverClass");
...
TCHAR achTexPathname[MAX_PATH];    // current texture pathname
        ...
        LoadString(hMainInstance, IDS_INIFILE, fname, 30))
    {
                           ...
                           &hkey) == ERROR_SUCCESS )
        {
            ...
            if ( RegQueryValueEx(hkey,
                                  ...
                                  (LPDWORD) NULL,
                                  (LPDWORD) NULL,
                                  (LPBYTE) achDefaultBitmap,
                                  (LPDWORD) &cjDefaultBitmap) == ERROR_SUCCESS
                 && (cjDefaultBitmap / sizeof(TCHAR) + 4) <= MAX_PATH )
                lstrcat(achDefaultBitmap, szDotBmp);
            else
                lstrcpy(achDefaultBitmap, TEXT("winnt.bmp"));
        ...
        if (LoadString(hMainInstance, IDS_TEXTURE, itemName, 30))
            GetPrivateProfileString(sectName, itemName, achDefaultBitmap,
...
static void saveIniSettings(HWND hDlg)
{
    ...
    if (LoadString(hMainInstance, IDS_SAVERNAME, sectName, 30) &&
        ...
        if (LoadString(hMainInstance, IDS_OPTIONS, itemName, 30)) {
            ...
            wsprintf(tmp, TEXT("%ld"), options);
            WritePrivateProfileString(sectName, itemName,
                                      tmp, fname);
        }
        if (LoadString(hMainInstance, IDS_OBJTYPE, itemName, 30)) {
            wsprintf(tmp, TEXT("%ld"), Type);
            WritePrivateProfileString(sectName, itemName,
                                      tmp, fname);
        }
        if (LoadString(hMainInstance, IDS_TESSELATION, itemName, 30)) {
...
// directory will end up with the default current directory.
    ...
    // Attempt to make it a full path by calling SearchPathW.
        ...
        if ( SearchPath(NULL, dirName, NULL, MAX_PATH,
                         achTexPathname, &psz) )
        {
            nTexFileOffset = psz - achTexPathname;
            dirName[nTexFileOffset-1] = TEXT('\0');
            lstrcpy(fileName, &dirName[nTexFileOffset]);
        }
    ...
    switch (message) {
        case WM_INITDIALOG:
            ...
            for (wTmp = 0; wTmp <= MAX_TYPE; wTmp++) {
                LoadString(hMainInstance, idsStyles[wTmp], ach, GEN_STRING_SIZE);
...
BOOL initClass(HANDLE hMainInstance)
{
    ...
    class.lpszMenuName = (LPCTSTR)NULL;
    class.lpszClassName = (LPCTSTR)szClassName;
...
BOOL initInstance(HANDLE hMainInstance, int nCmdShow, HWND hwndParent)
{
    HDC hdc;
    DWORD style;
    float sizeFact;
    ...
    hwndOpenGL = CreateWindowEx(WS_EX_TOPMOST,
                                 szClassName,
                                 szScreenSaverTitle,
                                 style,
                                 0,                  // horizontal pos
        ...
        // Do initialization of the bouncing top most window here.
            ...
            if (!bInited) {
                ...
                if ( !initClass(hMainInstance) ||
                     !initInstance(hMainInstance, SW_SHOW, hwnd) )
                {

                    if ( LoadString(hMainInstance, IDS_ERROR, gachTitle, MAX_PATH) &&
                         LoadString(hMainInstance, IDS_START_FAILED, gachFormat, MAX_PATH) )
                    {
                        MessageBox(NULL, gachFormat, gachTitle, MB_OK);
                    }
                    return FALSE;
