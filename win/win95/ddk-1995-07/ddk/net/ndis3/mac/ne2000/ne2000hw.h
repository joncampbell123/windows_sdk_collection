...
Abstract:
	...
	The hardware specifics for an NE2000 MAC driver.
...
// Supported Microchannel Adapter Id's
...
//
// the interrupt number the board is using.
//
...
//
// Offsets from AdaptP->IoPAddr of the ports used to access
...
#define CardStart(AdaptP) \
	NdisRawWritePortUchar(AdaptP->IoPAddr+NIC_XMIT_CONFIG, TCR_NO_LOOPBACK)
...
#define CardGetInterruptStatus(_AdaptP,_InterruptStatus) \
	NdisRawReadPortUchar((_AdaptP)->IoPAddr+NIC_INTR_STATUS, (_InterruptStatus))
...
#define CardBlockInterrupts(AdaptP) \
	NdisRawWritePortUchar((AdaptP)->IoPAddr+NIC_INTR_MASK, 0)
...
#define CardAcknowledgeCounterInterrupt(AdaptP) \
	NdisRawWritePortUchar((AdaptP)->IoPAddr+NIC_INTR_STATUS, ISR_COUNTER)
