/* SUSCR.C -- screen/keyboard I/O for SETUP */

/*  History
**
**  28 apr 87	plb	Finished ScrSelectList()
**  29 apr 87	plb	added colGlobal to truncate strings at screen edge.
**  30 apr 87	plb	modified displaystring() to handle inserted strings
**                 	indexed by control characters.
**                 	ScrInputPath() has enhanced error-checking, and
**                 	redisplays input string if an error is found.
**   1 may 87  	plb	Modified ScrInputPath() to handle DEL.
**   1 jun 87	plb	Modified ScrSelectList() to highlight selected
**			text. The displaystring() function now uses the
**			current attribute, rather than the existing attribut
**			stored in screen memory.
**   3 jun 87	plb	Changed LCOL to 2.
**			Moved new ContinueOrQuit() function here.
**  13 jun 87	plb	Put in MAXWINBOT def. in ScrSelectList(), leave
**			bottom 2 lines free for status display.
**			Adding error and status output routines, including
**			DisplayStatusLine() and FatalError()
**			In ScrInputPath, allow typeing ':'.
**			Added PutNumber(), PutFileName(), PutSecName(s),
**			PutDestName(s).
**			Now only use 24 lines of screen, in case VT52 is
**			installed.  But clear all 25 lines.
**  14 jun 87	plb	Combining header files.
**  15 jun 87	plb	Changed lch() function to uch().  ScrInputPath()
**			now forces the input to be upper-case string, and
**			displays it that way. PutNumber stores null string
**			if its param. is negative.
**  16 jun 87	plb	Taking measures to avoid sign extension in character
**			input (uch(), ScrInputPath(), etc.) and string output
**			(displaystring()).
**                      The function uch() now uses strupr() function.
**  15 jul 87   mp      no flashing cursor in ScrSelectChar()
**  16 jul 87   mp      no return necessary after letterkey in ScrSelectChar()
**                      usage of ESC to quit is allowed everywhere
**                      usage of INS/DEL(for -1) ScrSelectList()
**  17 jul 87   mp      add multi lines to ContinueOrQuit()
**  20 jul 87   mp      remove cursor functions
**  21 jul 87   mp      Scrolling status line for quick setup
**  31 jul 87   mp      invoking of ExitPgm() if you press ^X, ESC doesn't exit
**                      INS,DEL no special case any more
**                      displaying of "more.." and "to exit.." message
**                      ContinueOrQuit() simplyfied (^X or other key)
**   4 aug 87   mp      rename inregs,outregs to scrinr, scroutr; now global
**  11 aug 87   mp      changing scrolling algorithm to Windows behavior
**  20 aug 87   mp      allow dot and slash in path name
**/
...
/* indices to inserttext[] in SUDATA.C for status and error messages */

#define SIDISKETTE      012 - 1
#define SIFNAME		013 - 1
#define SISECNAME	014 - 1
#define SIDESTNAME	015 - 1
#define SINUMBER	016 - 1


/* static data */

char attr = 0x7;	/* global screen attribute */
...
extern CHAR smMore[];             /* text and row for "more.." message */
extern int iMoreLine;
extern textline LeaveSetupMsg[];  /* Screen after pressing ^X */
extern textline ConfirmPathMsg[]; /* Msg to confirm the entered path */
extern textline ConfirmSelectionMsg[];   /* Msg to confirm a selection */
...
/* beep(); */
			ttyout(7);	    /* unknown control character */
		    break;
...
int errorcode;
	...
	{
/* setreverse(); */
	attr = 0x70;               /* prepare for reverse video display */
...
/* PutNumber() -- convert decimal number to string in siNumber[].
**
** If the number is negative, a null string is written.
*/
PutNumber(n)
int n;
    {
	...
	itoa(n, siNumber, 10);
	if (n > 0)
	    for (pd = pheads[N_DISKETTE]; pd; pd = pd->n.p)
		if (--n == 0)
		    {
		    inserttext[SIDISKETTE] = pd->n.fname;
		    break;
		    }
	}
    else
...
unsigned char * f;
    {
    inserttext[SIFNAME] = f;
    }

/* PutSecName() -- set section name string pointer in inserttext[] array
*/
PutSecName(s)
unsigned char * s;
    {
    inserttext[SISECNAME] = s;
    }
