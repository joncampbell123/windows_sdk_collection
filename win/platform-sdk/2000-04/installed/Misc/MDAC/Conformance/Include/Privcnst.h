...
//Compare (boolean)
//64bit TODO - Remove tracing once tests start working on 64bit.
#ifdef _WIN64
	#define TESTC(exp)						{ PRVTRACE(L"OLEDB64:"); PRVTRACE(WIDESTRING(#exp)); PRVTRACE(ENDL); if(!GCOMPARE(exp, TRUE))	goto CLEANUP; }
#else
...
//64bit TODO - Remove tracing once tests start working on 64bit.
#ifdef _WIN64
	#define TESTC_(exp, hr)					{  PRVTRACE(L"OLEDB64:"); PRVTRACE(WIDESTRING(#exp)); PRVTRACE(ENDL); if(!GCHECK(exp, hr))		goto CLEANUP; }
#else
...
enum EQUERY
{
	...
	INSERT_1ROW,								//@emem Inserts 1 row of data
