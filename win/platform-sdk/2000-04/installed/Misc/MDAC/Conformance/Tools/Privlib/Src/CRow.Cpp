				...
				rgColAccess[i].pData)
			{
				switch(rgBindings[i].wType)
				{
					...
					case DBTYPE_IUNKNOWN | DBTYPE_BYREF:
					{
						...
						if(pUnkSrc)
						{
							TESTC_(hr = pUnkSrc->QueryInterface(rgBindings[i].pObject->iid, (void**)ppUnkDest),S_OK);
...
)
{
	...
	if(SUCCEEDED(hr))
	{
		//The caller may have their own testing strategies for this object.  If we either
		//read the data or move the position in the rowset by this additional testing it may
		//skew the results.  So only do automatic testing if the user doesn't request the returned 
		//object...
		if(ppIUnknown)
			goto CLEANUP;
		...
		//If its a known object type and interface we can do additional testing...
		...
		else if(rGuidType == DBGUID_ROWSET)
		{
		}
	}
	else
