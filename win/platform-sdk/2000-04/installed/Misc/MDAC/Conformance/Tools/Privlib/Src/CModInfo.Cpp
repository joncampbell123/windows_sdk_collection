...
//
//////////////////////////////////////////////////////////////////////
const static GUID IID_IPoolInfo		= { 0xe231fcd8,	0xbe3d,	0x11d2, { 0x91, 0xa2, 0x00, 0xc0, 0x4f, 0xb6, 0xee, 0x9d }};
const static GUID IID_IGetPool		= { 0x10c2eaa8,	0xbe3e, 0x11d2, { 0x91, 0xa2, 0x00, 0xc0, 0x4f, 0xb6, 0xee, 0x9d }};
const static GUID IID_IPoolMngrInfo = { 0x07e78e52, 0xbe3e, 0x11d2,	{ 0x91, 0xa2, 0x00, 0xc0, 0x4f, 0xb6, 0xee, 0x9d }};
#endif //INTERNAL_ONLY 
...
HRESULT DisplayPooling(IUnknown* pDataSource, BOOL fEnumPools)
{
	...
	static ULONG cPooled = 1;
	if(GetModInfo()->GetPoolManager() && DrawnFromPool(pDataSource))
	{
		//PoolID
		CPool cPool(pDataSource);
		pwszPoolID = cPool.GetPoolID();
		
		//Total Number of Pools
		ULONG cPools = 0;
		GetModInfo()->GetPoolManager()->EnumPools(&cPools, NULL);
		
		//Output using pooling...
		if(GetModInfo()->GetDebugMode() & DEBUGMODE_POOLING)
			TOUTPUT(L"POOLING(" << cPooled << " / " << cPools << " ): DataSource PoolID(\"" << pwszPoolID << L"\")");
		cPooled++;
	}
	else
	{
		if(fEnumPools)
		{
			TOUTPUT(L"NOT POOLED: DataSource Not Pooled");
//			TESTC(DisplayPools());
		}
	}

//CLEANUP:
	SAFE_FREE(pwszPoolID);
#endif //INTERNAL_ONLY 
...
#ifdef INTERNAL_ONLY 
	IGetPool* pIGetPool = NULL;
	HRESULT hr = S_OK;

	if(pDataSource)
	{
		//Obtain the IGetPool interface
		TEST2C_(hr = pDataSource->QueryInterface(IID_IGetPool, (void**)&pIGetPool),S_OK,E_NOINTERFACE);
		
		if(hr == S_OK)
		{
			//Obtain the Pool state...
			TESTC_(pIGetPool->GetState(&dwPoolState),S_OK);
		}
	}

CLEANUP:
	SAFE_RELEASE(pIGetPool);
#endif //INTERNAL_ONLY 
...
#ifdef INTERNAL_ONLY 
	fValue = ((GetPoolState(pDataSource) & DRAWN_FROMPOOL) == DRAWN_FROMPOOL);
#endif //INTERNAL_ONLY 
...
#ifdef INTERNAL_ONLY 
	fValue = ((GetPoolState(pDataSource) & CAN_BEPOOLED) == CAN_BEPOOLED);
#endif //INTERNAL_ONLY 
...
#ifdef INTERNAL_ONLY 
	fValue = ((GetPoolState(pDataSource) & CREATED_FROMPOOL) == CREATED_FROMPOOL);
#endif //INTERNAL_ONLY 
...
HRESULT CPool::Create(IUnknown* pObject)
{
	...
	//Is the object already a PoolInfo?
	hr = pObject->QueryInterface(IID_IPoolInfo, (void**)&m_pUnkPoolInfo);
	if(FAILED(hr))
	{
		//Maybe its a datasource?
		hr = pObject->QueryInterface(IID_IGetPool, (void**)&pIGetPool);
		if(SUCCEEDED(hr))
		{
			hr = pIGetPool->GetPool((IPoolInfo**)&m_pUnkPoolInfo);
		}
	}
...
{	
	...
	//Obtain the PoolID object...
	//E_NOTIMPL:  Pooling hooks are not available in retail drops
	TEST2C_(hr = ((IPoolInfo*)m_pUnkPoolInfo)->GetPoolID(&pwszPoolID),S_OK,E_NOTIMPL);
	SetHooks(hr != E_NOTIMPL);
...
{	
	...
	//Obtain the PoolID object...
	//E_NOTIMPL:  Pooling hooks are not available in retail drops
	TEST2C_(hr = ((IPoolInfo*)m_pUnkPoolInfo)->GetPoolTimeOut(&dwValue),S_OK,E_NOTIMPL);
	SetHooks(hr != E_NOTIMPL);
...
{	
	...
	//Obtain the PoolID object...
	//E_NOTIMPL:  Pooling hooks are not available in retail drops
	TEST2C_(hr = ((IPoolInfo*)m_pUnkPoolInfo)->GetRetryTimeOut(&dwValue),S_OK,E_NOTIMPL);
	SetHooks(hr != E_NOTIMPL);
...
{	
	...
	//Obtain the PoolID object...
	//E_NOTIMPL:  Pooling hooks are not available in retail drops
//TODO: This hook needs to be added...
//	TEST2C_(hr = ((IPoolInfo*)m_pUnkPoolInfo)->GetExpBackOff(&dwValue),S_OK,E_NOTIMPL);
	SetHooks(hr != E_NOTIMPL);
...
#ifdef INTERNAL_ONLY 
	//Create an Instance of the PoolMngrInfo, (if we haven't already)
	if(!m_pUnkMngrInfo)
	{
		TESTC_(hr = CoCreateInstance(CLSID_MSDAINITIALIZE, NULL, CLSCTX_INPROC_SERVER, IID_IPoolMngrInfo, (void**)&m_pUnkMngrInfo),S_OK); 
	}
...
#ifdef INTERNAL_ONLY 
	ULONG cPools = 0;
	IPoolInfo** rgpPoolInfo = NULL;

	//Create an Instance of the PoolMngrInfo
	TESTC_(Create(),S_OK);

	//Enumerate Pools
	TESTC_(hr = ((IPoolMngrInfo*)m_pUnkMngrInfo)->EnumPools(&cPools,  &rgpPoolInfo),S_OK);

CLEANUP:
	if(pcPools)
		*pcPools = cPools;
	if(prgpPoolInfo)
		*prgpPoolInfo = (IUnknown**)rgpPoolInfo;
	else
	{
		ReleasePools(cPools, (IUnknown**)rgpPoolInfo);
		SAFE_FREE(rgpPoolInfo);
	}
#endif //INTERNAL_ONLY 
...
{	
#ifdef INTERNAL_ONLY 
	//Loop over pools trying to find the appropiate one...
	for(ULONG iPool=0; iPool<cPools; iPool++)
	{
		//We need to walk the array and free the objects anyway.
		SAFE_RELEASE(rgpPoolInfo[iPool]);
	}
...
{	
	...
	if(!ppIPoolInfo || !pwszPoolID)
		return E_INVALIDARG;
	*ppIPoolInfo = NULL;
	WCHAR* pwszTempID = NULL;
	ULONG iPool,cPools = 0;

	//Enumerate Pools
	TESTC_(hr = EnumPools(&cPools, &rgpPoolInfo),S_OK);

	//Loop over pools trying to find the appropiate one...
	for(iPool=0; iPool<cPools; iPool++)
	{
		IPoolInfo* pIPoolInfo = (IPoolInfo*)rgpPoolInfo[iPool];
		
		//We need to walk the array and free the objects anyway.
		if(*ppIPoolInfo == NULL)
		{
			//Obtain this pool id
			TESTC_(hr = pIPoolInfo->GetPoolID(&pwszTempID),S_OK);
			ASSERT(pwszTempID);

			//See if this is the correct
			if(wcscmp(pwszTempID, pwszPoolID)==0)
			{
				*ppIPoolInfo = pIPoolInfo;
				SAFE_ADDREF(pIPoolInfo);
			}
		}
		
		//Otheriwse release this object
		SAFE_RELEASE(pIPoolInfo);
		SAFE_FREE(pwszTempID);
	}
...
{	
...
#ifdef INTERNAL_ONLY 
	ULONG iPool,cPools = 0;
	IUnknown** rgpIPoolInfo = NULL;

	//Enumerate Pools
	TESTC_(hr = EnumPools(&cPools, &rgpIPoolInfo),S_OK);
	...
	for(iPool=0; iPool<cPools; iPool++)
	{
		IPoolInfo* pIPoolInfo = (IPoolInfo*)rgpIPoolInfo[iPool];
		ASSERT(pIPoolInfo);
		...
		//Pool ID
		WCHAR* pwszPoolID = NULL;
		TESTC_(hr = pIPoolInfo->GetPoolID(&pwszPoolID),S_OK);
		
		//Elements in the pool
		pIPoolInfo->GetCount(&dwInUse, &dwIdle);

		//Display this pool
		TOUTPUT(L"     PoolID = \"" << pwszPoolID << "\"");
		TOUTPUT(L"          InUse=" << dwInUse << " Idle= " << dwIdle);
...
#ifdef INTERNAL_ONLY 
	IGetPool* pIGetPool = NULL;
	IPoolInfo* pIPoolInfo = NULL;
	DWORD dwPoolState = 0;
	DWORD dwInUseBefore = 0;
	DWORD dwIdleBefore = 0;

	//Obtain the PoolInfo object off this object
	if(SUCCEEDED((*ppObject)->QueryInterface(IID_IGetPool, (void**)&pIGetPool)))
	{
		//Obtain the PoolState for this DataSource
		ASSERT(pIGetPool);
		TESTC_(hr = pIGetPool->GetState(&dwPoolState),S_OK);
		
		//Obtain the PoolInfo
		if(dwPoolState != 0)
		{
			//Obtain the poolinfo...
			hr = pIGetPool->GetPool(&pIPoolInfo);
			if(hr==S_OK)
			{
				//Obtain the Pool Count (before)
				pIPoolInfo->GetCount(&dwInUseBefore, &dwIdleBefore);
			}
		}
	}

	//Since this is a Datasource interface, we have to release our last
	//reference before anything can go into the pool...
	SAFE_RELEASE(pIGetPool);
#endif //INTERNAL_ONLY 
...
#ifdef INTERNAL_ONLY 
	//Make sure it got added to the pool...
	if(dwPoolState && pIPoolInfo)
	{
		DWORD dwInUse = 0;
		DWORD dwIdle = 0;

		//Obtain the current pool count
		//TODO: remove when we have a callback...
//		Sleep(0);
		pIPoolInfo->GetCount(&dwInUse, &dwIdle);
/*		
		//Sleep so the sweeper thread has a chance to release the DSO 
		//TODO: remove when we have a callback...
		for(ULONG i=0; i<100 && ((dwInUse >= dwInUseBefore) || (dwIdle <= dwIdleBefore)); i++)
		{
			Sleep(i);
			pIPoolInfo->GetCount(&dwInUse, &dwIdle);
		}

		//Make sure the released pooled DSO was counted as an idle DSO...
		if((dwInUse >= dwInUseBefore) || (dwIdle <= dwIdleBefore))
		{
			if(dwIdle <= dwIdleBefore)
			{	
				TERROR(L"Released DSO is not in the pool as IDLE");
				DisplayPools();
			}
			else
			{
				TERROR(L"Released DSO is still INUSE");
				DisplayPools();
			}
		}
