...
)
{
	...
	for(DBORDINAL i=0; i<cColumns; i++)
	{
		...
		POSITION pos = pCTable->GetColList()->GetHeadPosition();
		while(pos)
		{
			CCol& rCol = pCTable->GetColList()->GetNext(pos);
			
			if (rCol.GetColNum() == rgColInfo[i].iOrdinal)
			{
				//Prefix
				OutputFile(",%ls,", rCol.GetPrefix());
				
				//Suffix
				OutputFile("%ls", rCol.GetSuffix());
			}
		}
		...
		case DBSTATUS_S_TRUNCATED:
			switch(wType)
			{
				...
				case DBTYPE_HCHAPTER:
				{
					...
					swprintf(wszBuffer, L"%l", hChapter);
					p = wszBuffer;	
					break;
				}
