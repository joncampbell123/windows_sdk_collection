...
public:
	...
	virtual HRESULT GetStatus(ULONG *pulProgress = NULL, ULONG* pulProgressMax = NULL, ULONG* pulAsynchPhase = NULL, LPOLESTR* ppwszStatusText = NULL);
	virtual HRESULT GetStatus(ULONG ulOperation, ULONG *pulProgress = NULL, ULONG* pulProgressMax = NULL, ULONG* pulAsynchPhase = NULL, LPOLESTR* ppwszStatusText = NULL);
	...
	virtual BOOL	IsAsynch()				{ return m_ulAsynch;		}

	//Helpers
	virtual ULONG	GetProgress()			{ return m_ulProgress;		}
	virtual ULONG	GetProgressMax()		{ return m_ulProgressMax;	}
	virtual ULONG	GetAsynchPhase()		{ return m_ulAsynchPhase;	}
	virtual WCHAR*	GetStatusText()			{ return m_pwszStatusText;	}
	
	virtual ULONG	GetPrevProgress()		{ return m_ulPrevProgress;		}
	virtual ULONG	GetPrevProgressMax()	{ return m_ulPrevProgressMax;	}
...
HRESULT CAsynch::GetStatus(ULONG *pulProgress, ULONG* pulProgressMax, ULONG* pulAsynchPhase, LPOLESTR* ppwszStatusText)
{
	//Delegate
	return GetStatus(DBASYNCHOP_OPEN, pulProgress, pulProgressMax, pulAsynchPhase, ppwszStatusText);
}

HRESULT CAsynch::GetStatus(ULONG ulOperation, ULONG *pulProgress, ULONG* pulProgressMax, ULONG* pulAsynchPhase, LPOLESTR* ppwszStatusText)
{
	TBEGIN
	HRESULT hr = S_OK;
	...
	if(IsAsynch())
	{
		...
		if(dwResult & OP_ABORTED)
		{
			if(dwResult & OP_COMPLETED)
			{
				...
				//Depends upon which listener veto'd the change, and depends upon the order that
				//the provider sends the notifications, on how many actually recieved the notification
				//before 1 had a chance to veto it.  This will be tested in the actual variation
				TCOMPARE_(pCListener->GetTimesNotified(DBREASON_ONPROGRESS,	DBASYNCHPHASE_CANCELED)		>= 0);
			}
			else
			{
				//2.  Abort, and operation was aborted
...
HRESULT CAsynchDSO::Initialize(DWORD dwPropVal)
{
	...
	if(dwPropVal == DBPROPVAL_ASYNCH_INITIALIZE)
		SetSettableProperty(DBPROP_INIT_ASYNCH, DBPROPSET_DBINIT, (void*)dwPropVal, DBTYPE_I4);
	else
		SetProperty(DBPROP_INIT_ASYNCH, DBPROPSET_DBINIT, (void*)dwPropVal, DBTYPE_I4);
...
HRESULT CAsynchRowset::CreateRowset(DWORD dwPropVal)
{
	...
	SetSettableProperty(DBPROP_ROWSET_ASYNCH, DBPROPSET_ROWSET, (void*)dwPropVal, DBTYPE_I4);
		...
		{	
			switch(ePhase)
			{
				case DBASYNCHPHASE_INITIALIZATION:
					...
					TESTC(pCListener->GetTimesNotified(DBREASON_ONPROGRESS, DBASYNCHPHASE_INITIALIZATION)	>= 0);
					break;
...
{ 
    ...
    OpenRowsetA.SetProperty(DBPROP_ROWSET_ASYNCH,	DBPROPSET_ROWSET, (void*)INVALID(DWORD), DBTYPE_I4, DBPROPOPTIONS_OPTIONAL, DB_NULLID); 
...
{ 
	...
	//BADVALUE (wrong value)
    OpenRowsetA.SetProperty(DBPROP_ROWSET_ASYNCH,	DBPROPSET_ROWSET, (void*)INVALID(DWORD), DBTYPE_I4, DBPROPOPTIONS_REQUIRED, DB_NULLID); 
