...
ULONG CThreads::Thread_CreateAccessor(LPVOID pv)
{
	...
	ULONG cBindings = pThis->m_cBindings;
	DBBINDING* rgBinding = pThis->m_rgBinding;
	ULONG cbRowSize = pThis->m_cRowSize;
...
ULONG CThreads::Thread_GetNextRows(LPVOID pv)
{
	...
	//Thread Stack Variables
	CRowset*	pThis     = (CRowset*)THREAD_FUNC;
	LONG		lOffset   = (LONG)THREAD_ARG1;
	LONG		cRows	  = (LONG)THREAD_ARG2;
	HRESULT		hrExpected= (HRESULT)THREAD_ARG3;
	HRESULT hr = S_OK;
	ASSERT(pThis);
...
ULONG CThreads::Thread_GetRowsByBookmark(LPVOID pv)
{
	...
	//Thread Stack Variables
	CRowset*  pThis		= (CRowset*)THREAD_FUNC;
	ULONG* pcbBookmark	= (ULONG*)THREAD_ARG1;
...
BOOL TCDataSource::Init()
{
	...
	// }}
	{
		if(CreateRowset(DBPROP_CANHOLDROWS)==S_OK)
			return TRUE;
	}
...
{		
	...
	TESTC_(hr = RowsetA.CreateRowset(), DB_E_ERRORSOCCURRED);
...
int TCSequence::Variation_5()
{
	...
	//CreateRowset
	TESTC_PROVIDER(RowsetA.CreateRowset(DBPROP_IRowsetResynch)==S_OK);
