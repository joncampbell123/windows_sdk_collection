...
BOOL DisplayNotification(WCHAR* pwszHeader, ULONG cRows, HROW* rghRows, ULONG cColumns, ULONG* rgColumns, DBREASON eReason, DBEVENTPHASE ePhase, BOOL fError = TRUE);
...
public:
	...
	COMMAND(COMMANDTYPE eCommandType, void* pvObjectId, ULONG ulCommandLevel, DBREASON eReason, DBEVENTPHASE ePhase, HRESULT hrExpected	= S_OK,	ULONG  cRows = 0, ULONG* rgRowIds = NULL, ULONG cColumns = 0, ULONG* rgColumns = NULL, DWORD dwOption = 0);
	virtual ~COMMAND();
	...
	ULONG         m_cRows;
	ULONG		  m_rgRowIds[MAX_ROW_COUNT];
...
public:
	//Constructors
	EXPECT(CImpIListener* pListener, IRowset* pIRowset, ULONG cRows, HROW* rghRows, ULONG cColumns, ULONG rgColumns[], DBREASON eReason, DBEVENTPHASE ePhase);
	...
	BOOL IsEqual(CImpIListener* pListener, IRowset* pIRowset, ULONG cRows, HROW* rghRows, ULONG cColumns, ULONG rgColumns[], DBREASON eReason, DBEVENTPHASE ePhase);
	BOOL IsCompleted() { return m_bCompleted; };

	BOOL VerifyEqual(DBEVENTPHASE ePhase);
	BOOL VerifyEqual(CImpIListener* pListener, IRowset* pIRowset, ULONG cRows, HROW* rghRows, ULONG cColumns, ULONG rgColumns[], DBREASON eReason, DBEVENTPHASE ePhase);
...
public:
	...
	ULONG		GetGranularity();
	ULONG		GetSupportedPhases();
...
//	HRESULT		RefreshVisibleData(COMMAND* pCommand);
	...
	HRESULT		GetBookmark(HROW hRow, ULONG* pulBookmark);

	//Interface methods
	ROWINFO*	GetRowInfo(ULONG ulRowId);
	BOOL		IsBufferedMode();

	//Rowset reference
	ULONG		m_cRowsetRef;
	BOOL		m_fTableAltered;
	ULONG		m_ulNextFetchPos;
...
protected:
	...
	ULONG		m_ulSupportedGranularity;
	ULONG		m_ulSupportedPhases;

	BOOL		m_bReentrantEvents;
	ULONG		m_rgCancelableEvents[DBREASON_ALL];
...
public:
	...
	EXPECT* FindExpectation(CImpIListener* pListener, IRowset* pIRowset, ULONG cRows, HROW* rghRows, ULONG cColumns, ULONG rgColumns[], DBREASON eReason, DBEVENTPHASE ePhase, BOOL fCantDeny);
	EXPECT* PopExpectation(CImpIListener* pListener, IRowset* pIRowset, ULONG cRows, HROW* rghRows, ULONG cColumns, ULONG rgColumns[], DBREASON eReason, DBEVENTPHASE ePhase);
