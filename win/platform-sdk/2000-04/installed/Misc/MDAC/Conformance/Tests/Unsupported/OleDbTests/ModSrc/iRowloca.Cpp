...
LONG			g_lRowLast = 0;					// Keeps track of rows on table
...
class TCIRowsetLocate : public CTestCases
{
	...
	protected:
		...
		BOOL	VerifyCursorPosition(
			ULONG	cRow,			//the cursor potision expected
			CTable	*pCTable,		//pointer to the CTable
...
)
{
	...
	dwAddr=(DWORD)m_pData;

	//get the length of the bookmark
	*pcbBookmark= *((ULONG *)(dwAddr+m_rgBinding[0].obLength));
	...
	*ppBookmark=(BYTE *)PROVIDER_ALLOC(*pcbBookmark);
	...
	memcpy(*ppBookmark, (void *)(dwAddr+m_rgBinding[0].obValue), *pcbBookmark);
...
CLEANUP:
	if(pHRow)
	{
		CHECK(m_pIRowset->ReleaseRows(1,pHRow,NULL,&cCount,NULL),S_OK);
		PROVIDER_FREE(pHRow);
	}
...
{
	ULONG	cBytes;
	DWORD	dwAddr1=(DWORD)pBookmark1;
	DWORD	dwAddr2=(DWORD)pBookmark2;
	...
	switch(g_dwBookmarkType)
	{
		...
		case DBTYPE_BYTES:
				...
				{
					if(*(unsigned char*)(dwAddr1+(cBytes-1)) > *(unsigned char *)(dwAddr2+(cBytes-1)))
					{
						*pDBCompare=DBCOMPARE_GT;
						break;
					}
				...
				return TRUE;
		//can not compare bookmark of 8 bytes literally
		case DBTYPE_I8:
		case DBTYPE_UI8:
...
private:
	...
	ULONG			m_rgcbBookmarks[1];
	BYTE			*m_rgpBookmarks[1];
	DBBOOKMARK		m_DBBookmark;
	DWORD			m_rgHashedValues[1];
...
int BOOKMARKS::Variation_8()
{
	...
	rgcbBookmarks = (ULONG *)PROVIDER_ALLOC(sizeof(ULONG)*g_lRowLast);
...
int BOOKMARKS::Variation_9()
{
	...
	if(!GetBookmark(ULONG(g_lRowLast/3),&rgcbBookmarks[0],&rgpBookmarks[0]))
		return TEST_FAIL;
...
{
	ULONG			cbFirstBookmark;
	ULONG			cbSecondBookmark;
...
{ 
	...
	TEST2C_(m_hr = m_pIRowsetLocate->GetRowsAt(NULL,NULL,cbBookmark,pBookmark,
					0,LONG_MAX/2+1,&cRowsObtained,&pHRow),DB_S_ENDOFROWSET,E_OUTOFMEMORY);
...
{
	ULONG		cbFirstBookmark=0, cbSecondBookmark=0;
	BYTE		*pFirstBookmark=NULL, *pSecondBookmark=NULL;
	ULONG		cFirstRowsObtained=0, cSecondRowsObtained=0;
...
int Consistency::Variation_1()
{
	...
	ULONG			cRowsObtained1=0;
	HROW			hRow1[3];
	HROW			*pHRow1=hRow1;

	BYTE			*rgpBookmarks2[3]={NULL,NULL,NULL};
	ULONG			rgcbBookmarks2[3];
	ULONG			cRowsObtained2=0;
	HROW			hRow2[3];
	HROW			*pHRow2=hRow2;
	BOOL			fTestPass=FALSE;
	ULONG			cRefCount;
...
{ 
	...
	TEST2C_(m_hr = m_pIRowsetLocate->GetRowsAt(NULL,NULL,cbBookmark,pBookmark,
					0,LONG_MAX,&cRowsObtained,&pHRow),DB_S_ENDOFROWSET,E_OUTOFMEMORY);
...
{ 
	...
	TEST2C_(m_hr = m_pIRowsetLocate->GetRowsAt(NULL,NULL,cbBookmark,pBookmark,
					LONG_MAX,1,&cRowsObtained,&pHRow),DB_S_ENDOFROWSET,E_OUTOFMEMORY);
...
int keysetCursor::Variation_3()
{
	...
	ULONG		cbSecondBookmark;
	BYTE		*pSecondBookmark=NULL;
	ULONG		cFirstRowsObtained;
	ULONG		cSecondRowsObtained;
	HROW		*pFirstHRow=NULL;
	HROW		*pSecondHRow=NULL;
	BOOL		fTestPass=FALSE;
...
int DeleteRows::Variation_3()
{
	...
	ULONG					cRowsObtained=0;
	IRowsetChange			*pIRowsetChange=NULL;
	ULONG					cbBookmark=0, cbDeleteBookmark=0;
...
int RemoveDeleted::Variation_4()
{
	...
	BOOL					fTestPass=FALSE;
	ULONG					ulRowEarly=0, ulRowAfter=0;
...
int BookmarkSkipped::Variation_3()
{
	DBPROPID			rgguidProperty[3];
	HROW			*pHRow=NULL;
	ULONG			cRowsObtained=0, cbBookmark=0;
	IRowsetChange	*pIRowsetChange=NULL;
	BYTE			*pBookmark=NULL;
	BOOL			fTestPass=FALSE;
...
{
	ULONG			rgcbBookmarks[3];
	ULONG			rgcRowCount[3]={1, g_lRowLast, 3};
	ULONG			cRowCount=0;
	HROW			rgHRow[3]={NULL,NULL,NULL};
	BYTE			*rgpBookmarks[3]={NULL, NULL, NULL};
	HROW			*pHRow=NULL;
...
{ 
	...
	ULONG		cbBmk = STD_BOOKMARKLENGTH;
