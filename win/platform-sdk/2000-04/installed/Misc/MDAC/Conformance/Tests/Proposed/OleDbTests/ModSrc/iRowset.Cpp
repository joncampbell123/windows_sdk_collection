...
public:
	...
	virtual BOOL	VerifyNextFetchPosition
						(
								LONG	iRow			//the NextFetchPosition
						);	

	//@mfunc: verify RestartPosition
	virtual BOOL	VerifyRestartPosition
						(
								HCHAPTER hChapter = DB_INVALID_HCHAPTER,
...
(
	LONG	iRow		//the NextFetchPosition expected
)				
{
	TBEGIN
...
//
//////////////////////////////////////////////////////////////////////////////
BOOL TCIRowset::VerifyGetNextRows(LONG lOffset, LONG cRows, ULONG iRowStart, ECOLUMNORDER eOrder, ULONG cRowsExpected, DBMEMOWNERENUM eMemoryOwner)
{
	TBEGIN
	...
	{
		rghRows = PROVIDER_ALLOC_((ULONG)abs(cRows), HROW);
	...
	if((ULONG)abs(cRows) == cRowsExpected || cRowsExpected == ULONG_MAX)
	{
		//Verify results
		if(hr == DB_S_ROWLIMITEXCEEDED && (ULONG)abs(cRows)>1)
		{
		...
		{	
			TESTC_(hr,S_OK);
			TESTC(cRowsObtained==(ULONG)abs(cRows));
			TESTC(VerifyRowHandles(cRowsObtained, rghRows, iRowStart, eOrder));
	...
	else
	{
		...
		else
		{
			...
			if(hr == E_OUTOFMEMORY)
			{
				...
				TESTC((ULONG)abs(cRows) >  1000 || (ULONG)abs(lOffset) >  1000);
			}
		}
	}
...
BOOL TCIRowset::VerifyAllRows()
{
	...
	ULONG ulRowCount = pTable()->CountRowsOnTable();
...
BEG_TEST_CASE(TCIRowset_MaxOpenRows, TCIRowset, L"test max open rows")
	...
	TEST_VARIATION(2, 		L"Set the maxopen rows to be 5.")
	TEST_VARIATION(3, 		L"MaxOpenRows is not unlimited.  Test the limit")
	TEST_VARIATION(4, 		L"DBPROP_MAXROWS - Get Value")
	TEST_VARIATION(5, 		L"DBPROP_MAXROWS - Set Value")
END_TEST_CASE()
#undef THE_CLASS
// }} TCW_TESTCASE_END
// }} TCW_TEST_CASE_MAP_END
...
int TCIRowset_RetrieveData::Variation_1()
{
	...
	TESTC_PROVIDER(CreateRowset(SELECT_ORDERBYNUMERIC)==S_OK);
...
int TCIRowset_RetrieveData::Variation_2()
{
	...
	TESTC_PROVIDER(CreateRowset(SELECT_ORDERBYNUMERIC, IID_IRowset, DBACCESSOR_ROWDATA,
...
int TCIRowset_RetrieveData::Variation_5()
{
	...
	//open rowset with desired properties
	//bind all columns, forward binding, all columns are ORed with _BYREF
	TESTC_PROVIDER(CreateRowset(SELECT_ORDERBYNUMERIC, IID_IRowset, DBACCESSOR_ROWDATA | DBACCESSOR_OPTIMIZED,
...
int TCIRowset_Rowsets::Variation_4()
{
	...
	TESTC(VerifyGetNextRows(0, LONG_MAX, FIRST_ROW, FORWARD, m_ulTableRows, DBMEMOWNER_PROVIDEROWNED));
	
	//Loop through large values...
	//Some providers like to hard code to work arround LONG_MAX, LONG_MIN
	//Work arround this!
	for(i=m_ulTableRows+1; i<LONG_MAX && (i+i>i); i+=i)
	{
		//Restart Position
...
int TCIRowset_Rowsets::Variation_10()
{
	...
	//Open a new rowset
	TESTC_PROVIDER(RowsetA.CreateRowset(SELECT_ORDERBYNUMERIC)==S_OK);
...
int TCIRowset_Properties::Variation_1()
{
	...
	//open rowset with desired properties
	TESTC_PROVIDER(CreateRowset(SELECT_ORDERBYNUMERIC, IID_IRowsetLocate)==S_OK);
...
int TCIRowset_Properties::Variation_2()
{
	...
	//open rowset with desired properties
	TESTC_PROVIDER(CreateRowset(SELECT_ORDERBYNUMERIC, IID_IRowsetScroll)==S_OK);
...
BOOL TCIRowset_Parameters::Init()
{
	...
	{
		//open rowset with desired properties
		if(CreateRowset(SELECT_ORDERBYNUMERIC)==S_OK)
			return TRUE;
	}
...
int TCIRowset_PendingChange::Variation_1()
{
	...
	TESTC_PROVIDER(RowsetA.CreateRowset(SELECT_ORDERBYNUMERIC, IID_IRowsetUpdate, NULL, 
...
int TCIRowset_MaxOpenRows::Variation_1()
{
	...
	//Create rowset
	TESTC_PROVIDER(CreateRowset(SELECT_ORDERBYNUMERIC)==S_OK);
	TESTC_PROVIDER(GetProperty(DBPROP_CANHOLDROWS, DBPROPSET_ROWSET));
...
int TCIRowset_MaxOpenRows::Variation_5()
{
	...
	//open a rowset with DBPROP_CANHOLDROWS
	SetProperty(DBPROP_CANHOLDROWS);
	TESTC_PROVIDER(SetSettableProperty(DBPROP_MAXROWS, DBPROPSET_ROWSET, (void*)ulMaxRows, DBTYPE_I4));
...
BOOL TCIRowset_ScrollBackwards_ForwardOnly::Init()
{
	if(TCIRowset::Init())
	{
		...
		//Create rowset
		TEST_PROVIDER(CreateRowset(SELECT_ORDERBYNUMERIC)==S_OK);
		return TRUE;
	}
...
int TCIRowset_ScrollBackwards_ForwardOnly::Variation_2()
{
	...
	//Some providers like to hard code to work arround LONG_MAX, LONG_MIN
	//Work arround this!
	for(i=m_ulTableRows + 10; i<LONG_MAX && (i+i>i); i+=i)
	{
		hr = GetNextRows(i, 1, &hRow);
		//2.0 spec indicates DB_S_ENDOFROWSET for all outofbounds cases
		//We no longer have DB_E_BADSTARTPOSITION for 2.x providers
