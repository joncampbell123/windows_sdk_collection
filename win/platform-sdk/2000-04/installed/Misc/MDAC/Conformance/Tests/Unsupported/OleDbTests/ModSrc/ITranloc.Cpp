...
{	
	...
	if (m_pChgUpdRowset1)
	{
		(CTxnRowset *)m_pChgUpdRowset1->Terminate();
...
IRowset		*pIRowset2	= NULL;
	...
	//Make sure the Txn Info is correct
	VerifyTxnInfo(m_pChgRowset1->m_pITxnLocal, m_fIsoLevel);
...
{
	//TODO:  Make this loop thru all supported DBPROP_SUPPORTEDTXNISOLEVELS	
	//For now just do it for one level which we know is supported
	if (CommitRetain(m_fIsoLevel))
		return TEST_PASS;
	else
		return TEST_FAIL;
}
...
{															  
	//TODO:  Make this loop thru all supported DBPROP_SUPPORTEDTXNISOLEVELS	
	//For now just do it for one level which we know is supported
	if (CommitNonRetain(m_fIsoLevel))
		return TEST_PASS;
	else
		return TEST_FAIL;
...
BOOL TCITxnReturnVals::Init()
{
	...
	{		
		...
		if (CHECK(m_pChgRowset1->m_pITxnLocal->GetOptionsObject(&m_pITxnOptions),
			S_OK))
			return TRUE;		
	}
	return FALSE;
...
{	
	...
	if (CHECK(m_pITxnLocal->GetOptionsObject(&pITxnOptions), S_OK))
	{
		//Make sure we can also get the options
		if (CHECK(pITxnOptions->GetOptions(&TxnOptions), S_OK))
			fResults = TRUE;
...
int CSequence::Variation_2()
{
	...
	if (CHECK(m_pITxnLocal->StartTransaction(m_fIsoLevel,
		0, NULL, NULL), S_OK))		
	{
		if (CHECK(m_pITxnLocal->GetOptionsObject(&m_pITxnOptions), S_OK))
		{
