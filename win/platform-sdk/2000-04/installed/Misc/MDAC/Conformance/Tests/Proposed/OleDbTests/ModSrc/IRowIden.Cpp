...
LONG				*g_rgTableColOrds;				//Array of col ordinals for table.			
ULONG				g_cRowsetCols;					//Count of columns in rowset.
ULONG				g_ulRowCount;
...
DBSTATUS GetStatus(void *pData, DBBINDING *pBinding)
{
	...
	return(*(DBSTATUS *)(dwAddr+(pBinding->obStatus)));
}
...
class TCIRowsetIdentity : public CRowsetObject
{
	...
	protected:
		...
		BOOL	GetNonKeyAndUpdatable(ULONG	*pcbCol,
										ULONG	**prgColNum,
										IMalloc	*pIMalloc);
...
BOOL TCIRowsetIdentity::Init()
{
	...
	// if this test is allowed to use commands and IRowsetChange (checked here from init string, not properties)
	if( GetModInfo()->GetFileName() && g_fIRowsetChange )
	{
		//delete all rows in the table.
		if(!CHECK(g_pCTable->DeleteRows(ALLROWS),S_OK))
			return FALSE;
...
(	
	...
	ULONG				cColsToBind,			//the count of columns to bind
...
)
{
	...
	switch(eAccessorLocation)
	{
		...
		case ON_ROWSET_ACCESSOR:  
				...
				eColsByRef,NULL,&cCnt,NULL,dbTypeModifier,cColsToBind,(LONG *)rgColsToBind),S_OK))
					goto END;
			break;
		default:
			return FALSE;
...
)
{
	...
	CHECK(m_pIRowset->ReleaseRows(1,pHRow,NULL,NULL,&cCount),S_OK);
...
)
{
	...
	DBTYPE_EMPTY, 1, (LONG *)&ulColToBind),S_OK))
		goto END;
...
BOOL	TCIRowsetIdentity::GetNonKeyAndUpdatable(
				ULONG	*pcbCol,			//[out] the count of the rgColNum
				ULONG	**prgColNum,		//[out] the col ordinals array
				IMalloc	*pIMalloc			)
{
	ULONG	cColsCount;
	...
	//initialization
	*pcbCol=0;
	*prgColNum=(ULONG *)pIMalloc->Alloc(sizeof(ULONG) * m_cRowsetCols);
