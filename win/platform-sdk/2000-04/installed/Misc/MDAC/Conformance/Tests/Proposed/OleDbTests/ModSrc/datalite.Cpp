...
static ULONG GetDataLength(void *pMakeData, DBTYPE wColType, ULONG cbBytesLen)
{
	ULONG cbLength;
...
static void FindVariantTypes(IUnknown *pIUnknown, CTable *pTable)
{
	...
	UDWORD cBinding = 0, cRowSize = 0, cCount = 0, i = 0;
...
CLEANUP:
	pIRowset->ReleaseRows(1,pHRow,NULL,&cCount,NULL);
...
//
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
class TCDATALITE : public CTestCases
{
	private:

	protected:

		//@cmember: interface pointer for IOpenRowset
...
{
	if (!VerifyInterface(m_pThisTestModule->m_pIUnknown2, IID_IOpenRowset, SESSION_INTERFACE, (IUnknown **)&m_pIOpenRowset))
			return FALSE;
...
//--------------------------------------------------------------------
HRESULT TCDATALITE::CreateAccessor(DBTYPE wType, eCoerceType eCrcType, ULONG ulRowIndex, ULONG ulColIndex)
{
	CCol			TempCol;
	HRESULT			hr = S_OK;
	...
	*(ULONG *)(m_pExpectedData+offsetof(DATA, ulLength)) = cbDstLength;
...
int TCDATALITE::GetDataCoerce(DBTYPE wType, eCoerceType eCrcType)
{
	...
	ULONG		cCols = g_pCTable->CountColumnsOnTable();
	ULONG		cRows = g_pCTable->GetRowsOnCTable();
	ULONG		ulColIndex = 0;
	ULONG		ulRowIndex = 0;
	...
	for ( ulRowIndex = 1; ulRowIndex <= cRows; ulRowIndex++ )
	{
		...
		{
			TEST2C_(m_hr = RowObject.CreateRowObject(m_pIRowset, prghRows[0]), S_OK, E_NOINTERFACE);
