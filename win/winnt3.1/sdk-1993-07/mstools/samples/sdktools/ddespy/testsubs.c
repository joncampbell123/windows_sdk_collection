...
#define OFF2P(psw, off)  ((char *)psw + off)
...
BOOL InitTestSubs()
{
    ...
    cls.lpszClassName = RefString(IDS_STRINGCLASS);
...
HANDLE hInst)
{
    UnregisterClass(RefString(IDS_STRINGCLASS), hInst);
}
...
BOOL StrWndCreate(HWND hwnd, INT cchLine, INT cLine)
{
    ...
    if ((hpsw = LocalAlloc(LMEM_MOVEABLE, sizeof(STRWND) + cchLine * cLine)) == NULL)
        return(FALSE);
...
VOID PaintStrWnd( HWND hwnd, LPPAINTSTRUCT pps)
{
    ...
    do {
        ...
        if (y - cyChar <= rc.bottom) {
            ...
            rcOut.right = 1000;
            rcOut.top = y - cyChar;
            DrawText(pps->hdc, (LPSTR)OFF2P(psw, off), -1, (LPRECT)&rcOut,
