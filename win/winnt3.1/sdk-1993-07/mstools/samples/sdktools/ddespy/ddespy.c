...
INT         fhOutput = 0;
OFSTRUCT    ofsOpen;
CHAR        OpenName[MAX_FNAME + 1];
CHAR        TBuf[BUFFER_SIZE];
CHAR        TBuf2[BUFFER_SIZE];
CHAR        szNULL[] = "";
PSTR        apszResources[IDS_LAST + 1];
PFNCALLBACK pfnDdeCallback = NULL;
HWND        hwndTrack[IT_COUNT] = { 0 };
PSTR        TrackTitle[IT_COUNT];
BOOL        fBlockMsg[WM_DDE_LAST - WM_DDE_FIRST + 1] = { 0 };
BOOL        fBlockCb[15] = { 0 };
PSTR        TrackHeading[IT_COUNT];
struct {                           /* profile data */
...
BOOL LoadResourceStrings()
{
    ...
    strcpy(TBuf, RefString(IDS_DEFAULT_OUTPUT_FNAME));
...
LONG  CALLBACK MainWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        if (pro.fOutput[IO_FILE])
            pro.fOutput[IO_FILE] = fhOutput =
                    DoDialog(MAKEINTRESOURCE(IDD_OPEN), (DLGPROC)OpenDlg, 0, TRUE, hWnd, hInst);
        break;
                            ...
                            UINT dwData2)
{
    ...
    if (pData = DdeAccessData(hData, (LPDWORD)&cb)) {
        ...
        case MF_HSZ_INFO:
            if (pro.fTrack[IT_HSZS]) {
                ...
                case MH_DELETE:
                    wsprintf(TBuf, fInt ? "0x%lx\t*\t%s(int)" : "0x%lx\t*\t%s",
                            ((MONHSZSTRUCT FAR *)pData)->hsz, 
                            (LPSTR)((MONHSZSTRUCT FAR *)pData)->str);
                    i = GetMCLBColValue(TBuf, hwndTrack[IT_HSZS], 2);
                    if (i > 1) {
                        wsprintf(TBuf2, fInt ? "0x%lx\t%d\t%s(int)" : "0x%lx\t%d\t%s",
                                ((MONHSZSTRUCT FAR *)pData)->hsz,
                                 i - 1,
                                 (LPSTR)((MONHSZSTRUCT FAR *)pData)->str);
            ...
            if (pro.fTerse) {
                ...
                wsprintf(TBuf, "Task:0x%x, Time:%ld, String Handle %s: %lx(%s)",
                        ((MONHSZSTRUCT FAR *)pData)->hTask,
                        ((MONHSZSTRUCT FAR *)pData)->dwTime,
                        (LPSTR)szAction,
                        ((MONHSZSTRUCT FAR *)pData)->hsz,
        ...
        case MF_POSTMSGS:
            ...
            if (pro.fTerse) {
                wsprintf(TBuf, RefString(IDS_FMT_TRS_MSG1),
                        ...
                        (LPSTR)DdeMsg2String(((MONMSGSTRUCT FAR *)pData)->wMsg));
            } else {
        ...
        case MF_CALLBACKS:
            ...
            wsprintf(TBuf,
                    ...
                    (LPSTR)Type2String(((MONCBSTRUCT FAR *)pData)->wType));
            wsprintf(DumpFormat(((MONCBSTRUCT FAR *)pData)->wFmt, &TBuf[strlen(TBuf)]),
                    pro.fTerse ? RefString(IDS_FMT_TRS_CB2) : RefString(IDS_FMT_CB2),
                    (UINT)((MONCBSTRUCT FAR *)pData)->hConv,
                    ((MONCBSTRUCT FAR *)pData)->hsz1,
                    (LPSTR)(psz1 = GetHszName(((MONCBSTRUCT FAR *)pData)->hsz1)),
                    ((MONCBSTRUCT FAR *)pData)->hsz2,
                    (LPSTR)(psz2 = GetHszName(((MONCBSTRUCT FAR *)pData)->hsz2)),
        ...
        case MF_ERRORS:
            wsprintf(TBuf, pro.fTerse ? RefString(IDS_FMT_TRS_ER1) : RefString(IDS_FMT_ER1),
                    ...
                    (LPSTR)Error2String(((MONERRSTRUCT FAR *)pData)->wLastError));
            break;
        ...
        case MF_LINKS:
            ...
            if (!GetClipboardFormatName(((MONLINKSTRUCT FAR *)pData)->wFmt, TBuf2, BUFFER_SIZE))
                strcpy(TBuf2, pdf(((MONLINKSTRUCT FAR *)pData)->wFmt));
            if (!strcmp(RefString(IDS_HUH), TBuf2)) {
                wsprintf(TBuf2, "%d", ((MONLINKSTRUCT FAR *)pData)->wFmt);
            }

            wsprintf(TBuf, "%s\t%s\t%s\t%s\t%s\t%lx\t%lx",
                    (LPSTR)psz1, (LPSTR)psz2, (LPSTR)psz3,
                    (LPSTR)TBuf2,
        ...
        case MF_CONV:
            ...
            wsprintf(TBuf, "%s\t%s\t%lx\t%lx",
                    (LPSTR)psz1, (LPSTR)psz2, 
        ...
        default:
            strcpy(TBuf, RefString(IDS_UNKNOWN_CALLBACK));
        }
        DdeUnaccessData(hData);
        OutputString(TBuf);
    }
    ...
    case XTYP_UNREGISTER:
        ...
        wsprintf(TBuf, "%s\t%s", (LPSTR)psz1, (LPSTR)psz2);
...
PSTR DisectMsgLP(UINT msg, MONMSGSTRUCT *pmms,  PSTR pszBuf)
{
    static LONG m2t[] = {
...
PSTR DisectWord( UINT type,
                 UINT data,
                 DDEML_MSG_HOOK_DATA *pdmhd,
                PSTR pstr)
{
    UINT wT;
    CHAR szData[32]; // for truncating strings w/o disturbing the hDdeData

    *pstr = '\0';   // in case we do nothing.

    if (type & T_ATOM) {
        wT = GlobalGetAtomName((ATOM)data, (LPSTR)pstr, 25);
        if (wT || data == 0) {
            if (type & T_APP) {
                strcpy(pstr, RefString(IDS_APPIS));
                pstr += strlen(pstr);
            }

            if (type & T_TOPIC) {
                strcpy(pstr, RefString(IDS_TOPICIS));
                pstr += strlen(pstr);
            }

            if (type & T_ITEM) {
                strcpy(pstr, RefString(IDS_ITEMIS));
                pstr += strlen(pstr);
            }
        }
        if (wT) {
            wsprintf(pstr, "0x%x(\"", data);
            pstr += strlen(pstr);
            GlobalGetAtomName((ATOM)data, (LPSTR)pstr, 25);
            pstr += wT;
            if (wT == 25) {
                *pstr++ = '.';
                *pstr++ = '.';
            ...
            *pstr = '\0';
            ...
            wsprintf(pstr, "Bad Atom (0x%x)", data);
            pstr += strlen(pstr);
        }
    }

    if (type & T_OR) {
        strcpy(pstr, RefString(IDS_OR));
        pstr += strlen(pstr);
    }
    ...
    if (type & T_OPTIONHANDLE) {
        if (pdmhd->cbData >= 4) {
            ...
            if (LOWORD(pdmhd->Data[0]) & DDE_FACKREQ) {
                strcpy(pstr, RefString(IDS_FACKREQ));
                pstr += strlen(pstr);
            }
            if (LOWORD(pdmhd->Data[0]) & DDE_FDEFERUPD) {
                strcpy(pstr, RefString(IDS_DEFERUPD));
                pstr += strlen(pstr);
            }
            *pstr++ = ')';
            *pstr++ = ' ';
    ...
    if (type & T_STATUS) {
        ...
        pstr += strlen(pstr);
        if (data & DDE_FACK) {
            strcpy(pstr, RefString(IDS_FACK));
            pstr += strlen(pstr);
        }
        if (data & DDE_FBUSY) {
            strcpy(pstr, RefString(IDS_FBUSY));
            pstr += strlen(pstr);
        }
        *pstr++ = ')';
        *pstr = '\0';
    ...
    if (type & T_STRINGHANDLE && pdmhd->cbData) {
        memcpy(szData, pdmhd->Data, min(32, pdmhd->cbData));
        szData[31] = '\0';
        wsprintf(pstr, pro.fTerse ? 
                    RefString(IDS_FMT_TRS_EXEC1) : RefString(IDS_FMT_EXEC1), szData);
        pstr += strlen(pstr);
        *pstr = '\0';
    }
    ...
    if (type & T_DATAHANDLE && pdmhd->cbData) {
        ...
        if (LOWORD(pdmhd->Data[0]) & DDE_FRELEASE) {
            strcpy(pstr, RefString(IDS_FRELEASE));
            pstr += strlen(pstr);
        }
        ...
        *pstr++ = ' ';
        ...
        strcpy(pstr, pro.fTerse ? RefString(IDS_FMT_TRS_DATAIS1) : RefString(IDS_FMT_DATAIS1));
        pstr += strlen(pstr);
...
PSTR DumpFormat(UINT fmt, PSTR pstr)
{
    UINT cb;

    wsprintf(pstr, "fmt=0x%x(\"", (WORD)fmt);
    pstr += strlen(pstr);
...
PSTR DumpData(LPBYTE pData, UINT cb, CHAR *szBuf, UINT fmt)
{
    register INT i;
    PSTR psz = szBuf;
    ...
    while (cb) {
        if (fmt == CF_TEXT) {
            ...
            *szBuf++ = '\"';
            ...
            i = 0;
            while (cb && (i < 16)) {
                wsprintf(&szBuf[i * 3 + 1], "%02x ", pData[0]);
                wsprintf(&szBuf[17 * 3 + i + 1], "%c", MPRT(pData[0]));
                pData++;
                cb--;
                i++;
            }
...
PSTR Error2String(UINT error)
{
    static CHAR szErr[23];

    if (error == 0) {
        strcpy(szErr, RefString(IDS_ZERO));
    } else if (error > DMLERR_LAST || error < DMLERR_FIRST) {
        strcpy(szErr, RefString(IDS_HUH));
    } else {
        strcpy(szErr, apszResources[IDS_ERRST0 + error - DMLERR_FIRST]);
    }
    return(szErr);
}
...
PSTR DdeMsg2String(UINT msg)
{
    static CHAR szBadMsg[10];

    if (msg < WM_DDE_FIRST || msg > WM_DDE_LAST) {
        return((PSTR)itoa(msg, szBadMsg, 10));
    } else {
...
VOID OutputString(PSTR pstr)
{
    if (pro.fOutput[IO_FILE] & fhOutput) {
        _lwrite(fhOutput, pstr, strlen(pstr));
        _lwrite(fhOutput, RefString(IDS_CRLF), 2);
        flushall();
    }
    if (pro.fOutput[IO_DEBUG]) {
        OutputDebugString((LPSTR)pstr);
...
BOOL CALLBACK OpenDlg(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) {
        case WM_INITDIALOG:
            SetDlgItemText(hDlg, IDC_EDIT, (LPSTR)OpenName);
...
BOOL GetProfileBoolean(LPSTR pszKey, BOOL fDefault)
{
    ...
    return(stricmp(RefString(IDS_NO), TBuf));
}



VOID SetProfileBoolean(LPSTR pszKey, BOOL fSet)
{
                ...
                HANDLE hInst)
{
    ...
    wRet = DialogBoxParam(hInst, (LPCSTR)lpTemplateName, hwndParent,
            lpDlgProc, param);
