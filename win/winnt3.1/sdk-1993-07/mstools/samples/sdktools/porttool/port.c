...
BOOL WINAPI InitPortData (HANDLE);
...
BOOL WINAPI GetIniFile (HANDLE, char *);
...
DebugBreak ();
    ...
    case DLL_PROCESS_ATTACH:
        if (!InitPortData (hModule))
        {
        /* display error loading DLL data and exit process */
        MessageBox (NULL, "Error loading port information, check file PORT.INI",
                "PORT.DLL", MB_OK);
        return FALSE;
        }
        break;
    ...
    char    *lpszFile)
{
    ...
    *lpPath = 0;
    ...
    LoadString (hDLL, IDS_INIFILE, lpszFileName, MAX_PATH);
