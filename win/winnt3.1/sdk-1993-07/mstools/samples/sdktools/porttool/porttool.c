    ...
    int    nCmdShow;
{
    ...
    GetCmdLine (lpCL, lpszCmdLine, &bBkgnd);
    ...
    LONG    lParam)
{
    ...
    case WM_CREATE:
        ...
        hWndEdit = CreateWindow ("edit",
                     " ",
                     WS_CHILD | WS_VISIBLE | DS_LOCALEDIT |
        ...
        if ((nResult = LoadFile (hWnd, lpcs->lpCreateParams)) == TRUE )
            {
    ...
    LONG    lParam)
{
    ...
    case IDM_FILEOPEN:
        ...
        {
        if ((nResult = LoadFile (hWnd, lpszFilePath)) != TRUE )
            /* notify user of error */
            ErrorNotify (hWnd, nResult);
        else
            {
    ...
    case IDM_FILESAVE:
        ...
        if (!lParam &&
        SendMessage (hWndEdit, EM_GETMODIFY, 0, 0))
        return FALSE;
...
{
    DWORD   dwStyle;
    HANDLE  hOldData, hNewData;
    char    *lpOldData, *lpNewData;
    ...
    SendMessage (hWndOld, EM_FMTLINES, !(dwStyle & ES_AUTOHSCROLL), 0);
    ...
    /* set line formatting */
    SendMessage (hWndNew, EM_FMTLINES, !(dwStyle & ES_AUTOHSCROLL), 0);
    ...
    int     nErrorString)
{
    ...
    if (!LoadString ((HANDLE)GetModuleHandle (NULL),
             ...
             sizeof (lpszErrStr)))
    {
    strcpy (lpszErrStr, "Unknown error => ");
    strcat (lpszErrStr, itoa (nErrorString, szNum, 10));
    }

    MessageBox (hWnd, lpszErrStr, lpszAppName, MB_OK | MB_ICONSTOP);
}
