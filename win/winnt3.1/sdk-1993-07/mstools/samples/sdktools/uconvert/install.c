...
int GetTableFileNames (HWND hwnd)
{
    ...
    p = szFilter;
    lstrcpy (buffer,TEXT("Conversion Table (*.nls)"));
    lstrcpy (p,buffer);
    p += lstrlen (buffer) +1;
    ...
    OpenFileName.lpstrTitle        = TEXT("Locate c_*.nls files from distribution media.");
...
LONG rValue;
    ...
    if (myScanf (szName, &cp) != 1) {
      MessageBox (NULL, TEXT("Incorrect file type.\n Need c_*.nls."),
                        szMBERROR, MB_ICONSTOP | MB_OK);
      return FALSE;
    }
    ...
    if (nChar >= MAX_PATH) {
      MessageBox (NULL, TEXT("File name overflow."),szMBERROR, MB_ICONSTOP | MB_OK);
      return FALSE;
    }
    ...
    if (rValue == ERROR_ACCESS_DENIED) {
      MessageBox (NULL, TEXT("Log on as Administrator to complete this action."),
               TEXT("Access Denied, Insufficient Privilege") , MB_ICONSTOP | MB_OK);
      return FALSE;
    }
    if (rValue != ERROR_SUCCESS) {
      MessageBox (NULL, TEXT("RegOpenKeyEx() failed."),szMBERROR, MB_ICONSTOP | MB_OK);
      return FALSE;
    }
    ...
    if (!CopyFile (szPathAndName, szTargetFile, TRUE)) {
      ...
      if (GetLastError() == ERROR_FILE_EXISTS) {
        if (MessageBox (NULL, TEXT("File already exists.\n Replace existing?"),
                        TEXT("Warning"), MB_ICONEXCLAMATION | MB_YESNO) == IDNO) {
          goto close_and_exit;
        } else {
          if (!CopyFile (szPathAndName, szTargetFile, FALSE)) {
            MessageBox (NULL, TEXT("File copy failed again."),
                          szMBERROR, MB_ICONSTOP | MB_OK);
            goto close_and_exit;
          }
        }
       ...
       */
        ...
        MessageBox (NULL, TEXT("File copy failed."),
                      szMBERROR, MB_ICONSTOP | MB_OK);
        goto close_and_exit;
    ...
    if (myScanf (szName, &cp) == 1) {
      ...
      MessageBox (NULL, szName, TEXT("Parsing file title failed."), MB_ICONSTOP | MB_OK);
