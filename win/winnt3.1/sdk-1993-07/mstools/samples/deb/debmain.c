...
static LPCTSTR lpszSourceFileName = TEXT(__FILE__);
  ...
  INT nCmdShow )
{
  ...
  //-- Load the "A Windows API Failed" resource string
  {
    ...
    if( !LoadString( Global.hInstance, IDS_API_FAILED_MSG,
          Global.szApiFailedMsg, sizeof(Global.szApiFailedMsg) ) ) {
      ErrorMessageBox( TEXT( "First LoadString()" ),
        szApiFailedMsg, lpszSourceFileName, __LINE__ );
      lstrcpy( Global.szApiFailedMsg, szApiFailedMsg );
    }
  }
  ...
  if( !LoadString( Global.hInstance, IDS_APPNAME,  Global.szAppName,
    ...
    ErrorMessageBox( TEXT("LoadString()"),
      Global.szApiFailedMsg, lpszSourceFileName, __LINE__ );
  ...
  WndClass.hIcon         = NULL;
  WndClass.hCursor       = LoadCursor( Global.hInstance, (LPTSTR) IDC_ARROW );
  ...
  Global.hIconCurrent = LoadIcon( Global.hInstance, lpszIconName );
  ...
  switch( uMsg ) {
    ...
    case WM_PAINT:
      if( IsIconic( hWnd ) ) {
        ...
        DrawIcon( ps.hdc, 2, 2, Global.hIconCurrent );
    ...
    //-- update the icon when dragged
    case WM_QUERYDRAGICON:
      return( (LRESULT) (UINT) Global.hIconCurrent );
    ...
    case WM_SIZE:
      ...
      switch( wParam ) {
        ...
        case SIZE_RESTORED: {
          ...
          if( Global.idIconTimer ) {
            KillTimer( Global.hWndMain, Global.idIconTimer );
            Global.idIconTimer = 0;
          }
          return( FALSE );
        }
        ...
        case SIZE_MINIMIZED:
          ...
          Profile.xPos       = Global.xPosOld;
          Profile.yPos       = Global.yPosOld;
          Global.idIconTimer = SetTimer( Global.hWndMain, 1,
                                 TIMEOUT_ANIMATED_ICON, TimerProc );
          return( FALSE );
...
AboutDlgProc( HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
  switch( uMsg ) {
    ...
    case WM_INITDIALOG:
      Global.hDlgAbout = hDlg;
      Global.fAboutBox = TRUE;
      Global.idIconTimer = SetTimer( hDlg, 1, TIMEOUT_ANIMATED_ICON,
        TimerProc );  // for the animated icon
      return( TRUE );
    ...
    case WM_DESTROY:
      KillTimer( hDlg, Global.idIconTimer );
      Global.idIconTimer = 0;
      return( TRUE );
  }
...
TimerProc( HWND hWnd, UINT uMsg, UINT idEvent, DWORD dwTime )
{
 ...
 if( Global.fAboutBox ) {
   ...
   hWndIcon = CreateIconWindow( Global.hDlgAbout, lpszIconNames[uIconNumber] );
   if( hWndIcon != NULL ) {
     DestroyWindow( hWndIconOld );
   }
 }

 if( IsIconic( Global.hWndMain ) ) {
   InvalidateRect( Global.hWndMain, NULL, TRUE );
   Global.hIconCurrent = LoadIcon( Global.hInstance, lpszIconNames[uIconNumber] );
 }
