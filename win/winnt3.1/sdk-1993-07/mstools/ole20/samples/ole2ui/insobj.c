            ...
            switch (wID)
                ...
                case ID_IO_FILE:
                    ...
                    if (FValidateInsertFile(hDlg, FALSE, &lpIO->nErrCode))
                    {
                        ...
                        GetFileTitle((LPSTR)szTemp, lpIO->szFile, OLEUI_CCHPATHMAX);
...
// dont show the help button: easiest fix for #4567, since there is no hook proc
...
UINT UFillClassList(HWND hList, UINT cIDEx, LPCLSID lpIDEx, BOOL fVerify)
    ...
    pszExec=(LPSTR)pIMalloc->lpVtbl->Alloc(pIMalloc, OLEUI_CCHKEYMAX*4);
    ...
    while (TRUE)
        ...
        if ((LONG)ERROR_SUCCESS==lRet) 
            ...
            */
            ...
            if (fVerify) 
                hFile=OpenFile(pszKey, &of, OF_EXIST);
            ...
            *(pszClass+cch)=0;  // set back to rootkey
            ...
            if (fVerify)
                {
                if (HFILE_ERROR==OpenFile(pszKey, &of, OF_EXIST))
                    continue;
                }
...
BOOL FToggleObjectSource(HWND hDlg, LPINSERTOBJECT lpIO, DWORD dwOption)
     ...
     *     so it updates the icon from the object type.
     *
...
BOOL FValidateInsertFile(HWND hDlg, BOOL fTellUser, UINT FAR* lpnErrCode)
    ...
    *lpnErrCode = 0;
    ...
    hFile=OpenFile(szFile, &of, OF_EXIST);
