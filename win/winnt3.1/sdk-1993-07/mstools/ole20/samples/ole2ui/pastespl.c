...
BOOL CALLBACK EXPORT PasteSpecialDialogProc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (iMsg)
    {
        ...
        case WM_DRAWCLIPBOARD:
        {
            ...
            SetProp(hDlg, NEXTCBVIEWER, HWND_BROADCAST);
            ChangeClipboardChain(hDlg, hwndNextViewer);
            wParam = OLEUI_PSERR_CLIPBOARDCHANGED;
            goto LCancel;
        }
            ...
            switch (wID)
            {
                ...
                case IDCANCEL:
                  ...
                  LCancel:
                    ...
                    SendDlgItemMessage(hDlg, ID_PS_ICONDISPLAY, IBXM_IMAGEFREE, 0, 0L);
                    SendMessage(hDlg, uMsgEndDialog, wParam, 0L);
                    break;
...
BOOL FPasteSpecialInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
{
            ...
            &medium))
    {
        ...
        if (NULL == lpPS->hMetaPictLSD)
        {
            ...
            nWidth = (IconRect.right * 3) / 2;   // width is 1.5 times width of iconbox
    ...
    // If one of Paste or PasteLink is disabled, select the other one regardless of what
    //    the input flags say
...
void SetPasteSpecialHelpResults(HWND hDlg, LPPASTESPECIAL lpPS)
{
    ...
    LPSTR       szFullUserTypeName = (lpPS->fLink) ? lpPS->szFullUserTypeNameLSD : lpPS->szFullUserTypeNameOD;
    LPSTR       szInsert;
...
BOOL FAddPasteListItem(
        HWND hList, BOOL fInsertFirst, int nPasteEntriesIndex, LPPASTESPECIAL lpPS,
        LPMALLOC pIMalloc, LPSTR lpszBuf)
{
    LPOLEUIPASTESPECIAL lpOPS = lpPS->lpOPS;
    LPPASTELISTITEMDATA lpItemData;
    int                 nIndex;
...
BOOL FFillPasteList(HWND hDlg, LPPASTESPECIAL lpPS)
{
    ...
    for (i = 0; i < lpOPS->cPasteEntries; i++)
    {
        ...
        if (lpOPS->arrPasteEntries[i].dwScratchSpace) {
            ...
            if (!FAddPasteListItem(hList,fInsertFirst,i,lpPS,pIMalloc,lpszBuf))
                goto error;
            nItems++;
        }
    }
