...
#ifdef STANDALONE
// these three values should be the same as in ole2ui.h
...
#ifdef STANDALONE
    GetModuleFileName(s_hInst, (LPSTR)szIconFile, OLEUI_CCHPATHMAX);
    IconIndex = ICONINDEX;
    hDefIcon = LoadIcon(s_hInst, (LPSTR)szVanillaDocIcon);
#else
...
HICON FAR PASCAL HIconAndSourceFromClass(REFCLSID rclsid, LPSTR pszSource, UINT FAR *puIcon)
    ...
    hIcon=ExtractIcon(s_hInst, pszSource, IconIndex);
...
STDAPI_(HGLOBAL) OleUIMetafilePictFromIconAndLabel(HICON hIcon, LPSTR pszLabel
    ...
    if (NULL==hIcon)  // null label is valid
        return NULL;
