...
#endif
#pragma message ("INCLUDING DEBUG.H from " __FILE__ DBGSTATE)
#endif  /* RC_INVOKED */



STDAPI_(void) DumpObject( IUnknown FAR * pUnk );
STDAPI_(BOOL) IsObjectValid( IUnknown FAR * pUnk );
STDAPI_(void) DumpAllObjects( void );
STDAPI_(BOOL) ValidateAllObjects( BOOL fSuspicious );
STDAPI_(void) DumpClassName( IUnknown FAR * pUnk );


#ifdef _DEBUG

BOOL WINAPI InstallHooks(void);
BOOL WINAPI UnInstallHooks(void);
...
#define ASSERTDATA  static char  _szAssertFile[]= __FILE__;
#else
#define ASSERTDATA  static char _based(_segname("_CODE")) _szAssertFile[]= __FILE__;
#endif
#endif
#undef Assert
...
STDAPI_(IDebugStream FAR *) MakeDebugStream(short margin=70, short tabsize=4);
