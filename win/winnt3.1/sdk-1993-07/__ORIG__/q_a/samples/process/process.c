    ...
    *  Fill in the header list box.
    ...
    case WM_INITDIALOG: {
      ...
      SendDlgItemMessage (hwnd, DID_HEADER,  LB_ADDSTRING,   0, (LONG)
            "hProcess \thTread \tdwProcessID \tdwThreadID \tImage file");
...
VOID doCreate (HWND hwnd)
{
    ...
    if (ret == TRUE){
      ...
      ret = GetLastError ();
      sprintf (buffer, "0x%lx", (int)ret);
      MessageBox (hwnd, buffer, "GetLastError...", MB_ICONSTOP | MB_OK);
    }
...
VOID doTerminate(HWND hwnd)
{
    ...
    if (sel == LB_ERR) {
      MessageBox (hwnd, "No listbox item is selected.",
                  "Incorrect use", MB_ICONSTOP | MB_OK);
      return;
    }
