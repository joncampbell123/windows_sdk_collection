...
static LOGFONT lf_FontVar = {
	...
	case WM_MEASUREITEM:
            ((MEASUREITEMSTRUCT FAR *)lParam)->itemHeight = yChar;
            break;
	...
	case WM_PAINT:
	    ...
	    BeginPaint(hWnd, &ps);
	    SetRect(&rc, 0, 0, GetSystemMetrics (SM_CXSCREEN), yChar);
	...
	case WM_SIZE:
		...
		MoveWindow(GetDlgItem (hWnd, IDC_LISTBOX),
			   ...
			   yChar,
			   LOWORD(lParam),
			   HIWORD(lParam)-(yChar + yStatus - yBorder),
			   TRUE);
...
NOT:
		...
		case IDM_REMOVEVIEWWND:
		    ...
		    if (GetMenuItemCount (hViewMenu) == 5)
			RemoveMenu (hViewMenu, 4, MF_BYPOSITION);
		    }
		    break;
    ...
    DRAWITEMSTRUCT *lpItem)
{
	...
	/* change TextColor for new entries to */
