...
int    WINAPI HandleMipsBreakPoint(DBGPROCESS *, DWORD);
		...
		if ((WaitForDebugEvent (&de, (DWORD)100)))
			...
			switch (de.dwDebugEventCode)
				...
				switch (de.u.Exception.ExceptionRecord.ExceptionCode)
...
#ifdef _MIPS_				/* mips needs special handling here to reset breakpoint */
					HandleMipsBreakPoint(lpDbgProcess, de.dwThreadId);
					...
					if (!bHooked)
					    {
					    if (InterruptThread_HookProcess (lpDbgProcess, &ProcessState))
						SetStatusText (lpDbgProcess->hWnd,
							       IDS_PROCESSINIT,
							       RGB (0, 0xff, 0));
					    else
						ReportError (IDS_HOOKPROCESS);
					    ...
					    bHooked = TRUE;
						 ...
						 !bUnHooked)
					    {
					    ResetInterruptedThread (lpDbgProcess, &ProcessState);
					    SetStatusText (lpDbgProcess->hWnd,
							   IDS_STATUSREADY,
							   0);
    ...
    LOAD_DLL_DEBUG_INFO    *lpdbgDll)
{
    ...
    /* if imagename is valid, copy locally */
    if ((lpdbgDll->lpImageName != NULL) &&
	(*(char **)(lpdbgDll->lpImageName)) != NULL)
	{
	/* if unicode, convert to ascii */
	if (lpdbgDll->fUnicode)
	    {
	    if (!ReadProcessMemory (lppr->hProcess,
				    *(char **)(lpdbgDll->lpImageName),
				    szImageName,
				    MAX_PATH*2,
				    NULL))
		ReportError (IDS_ERRREADPROCESSMEMORY);
...
#ifdef _MIPS_
/* 
 * MIPS must increment the FIR on a breakpoint 
 * in order to fetch the next instruction
 */

int WINAPI HandleMipsBreakPoint(
    DBGPROCESS		      *lppr,
    DWORD		      dwThreadID
    )
{
