...
void OnComputerChanged (HDLG hDlg)
   ...
   if (pLocalSystem && pLocalPerfData)
      ...
      DialogEnable (hDlg, IDD_ADDLINEADD, TRUE) ;
      }
   ...
   switch (LOWORD (wParam))
      ...
      case IDD_ADDLINECOMPUTER:
         if (HIWORD (wParam) == EN_UPDATE)
            {
            ComputerChange = TRUE ;
            DialogEnable (hDlg, IDD_ADDLINEADD, FALSE) ;
            }
...
#if 0
      case WM_DLGKILLFOCUS:
      case WM_KILLFOCUS:
         if (EditModified (hWndComputer))
            {
            SetHourglassCursor() ;
            OnComputerChanged (hDlg) ;
            SetArrowCursor() ;
            }
         EditSetModified (hWndComputer, FALSE) ;
         break ;
#endif
