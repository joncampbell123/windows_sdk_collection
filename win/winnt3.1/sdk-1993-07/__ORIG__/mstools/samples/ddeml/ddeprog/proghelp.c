...
BOOL ConnectToProgman()
{
    ...
    if (!pti->idInst) {
        if (DdeInitialize(&(pti->idInst), ProgHelpDdeCallback,
    ...
    if (pti->hConv == 0) {
        hszProgman = DdeCreateStringHandle(pti->idInst, "Progman", 0);
