...
#ifdef SIZE_T_MAX
		ASSERT((long)nNewSize * sizeof(BYTE) <= SIZE_T_MAX);    // no overflow
#endif
...
#endif
		BYTE* pNewData = (BYTE*) new BYTE[m_nSize * sizeof(BYTE)];
		// copy new data from old
		memcpy(pNewData, m_pData, m_nSize * sizeof(BYTE));
...
void CByteArray::InsertAt(int nStartIndex, CByteArray* pNewArray)
{
	...
	ASSERT(pNewArray->IsKindOf(RUNTIME_CLASS(CByteArray)));
...
#define MAKESTRING(x) #x
	dc << "a " MAKESTRING(CByteArray) " with " << m_nSize << " elements";
#undef MAKESTRING
	if (dc.GetDepth() > 0)
	{
		dc << "\n";
