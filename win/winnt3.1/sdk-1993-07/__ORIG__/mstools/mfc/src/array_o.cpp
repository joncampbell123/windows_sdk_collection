...
#ifdef SIZE_T_MAX
		ASSERT((long)nNewSize * sizeof(CObject*) <= SIZE_T_MAX);    // no overflow
#endif
...
#ifdef SIZE_T_MAX
		ASSERT((long)nNewMax * sizeof(CObject*) <= SIZE_T_MAX); // no overflow
#endif
...
#ifdef SIZE_T_MAX
		ASSERT((long)m_nSize * sizeof(CObject*) <= SIZE_T_MAX); // no overflow
#endif
		CObject** pNewData = (CObject**) new BYTE[m_nSize * sizeof(CObject*)];
		// copy new data from old
		memcpy(pNewData, m_pData, m_nSize * sizeof(CObject*));
...
void CObArray::InsertAt(int nStartIndex, CObArray* pNewArray)
{
	...
	ASSERT(pNewArray->IsKindOf(RUNTIME_CLASS(CObArray)));
...
#define MAKESTRING(x) #x
	dc << "a " MAKESTRING(CObArray) " with " << m_nSize << " elements";
#undef MAKESTRING
	if (dc.GetDepth() > 0)
	{
		dc << "\n";
