...
BOOL OleApp_InitInstance(LPOLEAPP lpOleApp, HINSTANCE hInst, int nCmdShow)
{
    ...
    *****************************************************************/
...
#if defined( USE_DRAGDROP )
    // time delay (in msec) before scroll
    ...
    // Border threshold to start drag scroll
    ...
    /* OLE2NOTE: even though OLE provides default cursors for
    **    drag/drop, we need to have a copy of the dragmove cursor so
    **    that we can give immediate feedback on cursor down of a
    **    pending drag operation. we do not call DoDragDrop immediately
    **    on button down; the user must move the mouse a minimum
    **    distance to start the drag. we will show this cursor
    **    immediately, however, to show the user there is a drag
    **    pending. 
    */
 ...
 *     app to shut down.
 */
BOOL OleApp_CloseAllDocsAndExitCommand(LPOLEAPP lpOleApp)
{
    LPOUTLINEAPP lpOutlineApp = (LPOUTLINEAPP)lpOleApp;
...
void OleApp_ShowWindow(LPOLEAPP lpOleApp, BOOL fGiveUserCtrl)
{
    ...
    */
    if (lpOleApp->m_lpMsgFilter != NULL)
        OleStdMsgFilter_EnableBusyDialog(lpOleApp->m_lpMsgFilter, TRUE);
...
void OleApp_DocUnlockApp(LPOLEAPP lpOleApp, LPOUTLINEDOC lpOutlineDoc)
{
    ...
    */
    OleDbgAssertSz (lpOleApp->m_cDoc > 0, "DocUnlockApp called with cDoc == 0");
...
**    coming OLE method calls from an external process (via the LRPC
**    interprocess commnunication mechanism), then it should call
...
)
{
    ...
    HRESULT hrErr;
    BOOL bMsgFilterInstalled = (lpOleApp->m_lpMsgFilter != NULL);
...
void OleApp_OpenCommand(LPOLEAPP lpOleApp)
{
    ...
    if (! OutlineDoc_CheckSaveChanges(lpOutlineDoc, OLECLOSE_PROMPTSAVE))
    ...
    OleDbgAssertSz(
            lpOutlineApp->m_lpDoc==NULL,"Closed doc NOT properly destroyed");
...
)
{
    ...
    */
    ...
    lpOleApp->m_arrPasteEntries[0].lpstrResultText = "as Outline Data";
    ...
    lpOleApp->m_arrPasteEntries[1].lpstrResultText = "as text";
...
BOOL ServerApp_InitVtbls (LPSERVERAPP lpServerApp)
{
    ...
    g_PseudoObj_DataObjectVtbl.DAdvise       = PseudoObj_DataObj_Advise;
    g_PseudoObj_DataObjectVtbl.DUnadvise     = PseudoObj_DataObj_Unadvise;
...
)
{
    ...
    */
    ...
    lpContainerApp->m_arrSingleObjGetFmts[4].tymed    = TYMED_HGLOBAL;
    lpContainerApp->m_arrSingleObjGetFmts[4].lindex   = -1;

    lpContainerApp->m_arrSingleObjGetFmts[5].cfFormat = CF_TEXT;
    ...
    lpContainerApp->m_arrSingleObjGetFmts[6].cfFormat = 
                                                    lpOleApp->m_cfLinkSource;
    lpContainerApp->m_arrSingleObjGetFmts[6].ptd      = NULL;
    lpContainerApp->m_arrSingleObjGetFmts[6].dwAspect = DVASPECT_CONTENT;
    lpContainerApp->m_arrSingleObjGetFmts[6].tymed    = TYMED_ISTREAM;
    lpContainerApp->m_arrSingleObjGetFmts[6].lindex   = -1;

    lpContainerApp->m_arrSingleObjGetFmts[7].cfFormat = 
                                              lpOleApp->m_cfLinkSrcDescriptor;
    lpContainerApp->m_arrSingleObjGetFmts[7].ptd      = NULL;
    lpContainerApp->m_arrSingleObjGetFmts[7].dwAspect = DVASPECT_CONTENT;
    lpContainerApp->m_arrSingleObjGetFmts[7].tymed    = TYMED_HGLOBAL;
    lpContainerApp->m_arrSingleObjGetFmts[7].lindex   = -1;

    lpContainerApp->m_nSingleObjGetFmts = 8;
    ...
    */
    ...
    lpOleApp->m_arrPasteEntries[1].lpstrResultText = "as Outline Data";
    lpOleApp->m_arrPasteEntries[1].dwFlags         = OLEUIPASTE_PASTEONLY;
    ...
    lpOleApp->m_arrPasteEntries[3].lpstrResultText = "as text";
    ...
    lpOleApp->m_arrPasteEntries[4].lpstrResultText = "as a static picture";
    ...
    lpOleApp->m_arrPasteEntries[5].lpstrResultText = "as a static picture";
    ...
    lpOleApp->m_arrPasteEntries[6].lpstrResultText = "as a static picture";
