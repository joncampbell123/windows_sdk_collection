...
void TextLine_CalcExtents(LPTEXTLINE lpTextLine, HDC hDC)
{
	...
	// Allow size for boundary space
	lpLine->m_nHeightInHimetric += 2 * 
			XformHeightInPixelsToHimetric(hDC, LINE_BOUNDARY_WIDTH);
	lpLine->m_nWidthInHimetric +=  2 *
			XformWidthInPixelsToHimetric(hDC, LINE_BOUNDARY_WIDTH);
 ...
 */
void TextLine_Draw(LPTEXTLINE lpTextLine, HDC hDC, LPRECT lpRect)
{
	RECT rc;
...
#if defined( WONT_WORK_IN_METAFILE )
	SaveDC(hDC);
	IntersectClipRect(
			hDC,
			lpRect->left,
	...
	TabbedTextOut(
            hDC, 
            lpRect->left+((LPLINE)lpTextLine)->m_nTabWidthInHimetric + 
                XformWidthInPixelsToHimetric(NULL, 1), 
            lpRect->top + XformHeightInPixelsToHimetric(NULL, 1),
            lpTextLine->m_szText, 
            lpTextLine->m_nLength, 
			0,
			NULL,
 ...
 */
BOOL TextLine_SaveToStg(LPTEXTLINE lpTextLine, UINT uFormat, LPSTORAGE lpSrcStg, LPSTORAGE lpDestStg, LPSTREAM lpLLStm, BOOL fRemember)
{
    HRESULT hrErr;
    ULONG nWritten;
    ...
    if (! OleDbgVerifySz(hrErr == NOERROR, "Error writing TextLine data")) 
        return FALSE;
...
LPLINE TextLine_LoadFromStg(LPSTORAGE lpSrcStg, LPSTREAM lpLLStm, LPOUTLINEDOC lpDestDoc)
{
    ...
    if (! OleDbgVerifySz(hrErr == NOERROR, "Error reading TextLine data")) 
        return NULL;
