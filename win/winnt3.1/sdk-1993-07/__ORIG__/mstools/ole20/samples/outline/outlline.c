...
)
{
    ...
    if(itemAction == ODA_DRAWENTIRE) {
        ...
        Line_Draw(lpLine, hDC, &rcLogical);
        ...
        // If item selected, do additional drawing  -- invert rectangle
        if (itemState & ODS_SELECTED) 
            Line_DrawSelHilight(lpLine, hDC, lprcPix, itemAction, itemState);
        else 
            lpLine->m_fSelected = FALSE;
    ...
    * and draws item in selected or unselected state.
    */
...
void Line_Draw(LPLINE lpLine, HDC hDC, LPRECT lpRect)
{
    switch (lpLine->m_lineType) {
        case TEXTLINETYPE:
            TextLine_Draw((LPTEXTLINE)lpLine, hDC, lpRect);
            break;

#if defined( OLE_CNTR )
        case CONTAINERLINETYPE:
            ContainerLine_Draw((LPCONTAINERLINE)lpLine, hDC, lpRect);
            break;
#endif
...
void Line_DrawFocusRect(LPLINE lpLine, HDC hDC, LPRECT lpRect, UINT itemAction, UINT itemState)
{
    ...
    //  if (lpdis->itemID >= 0 && lpdis->itemID < nNumLine) 
    if(lpLine) {
        if (itemState & ODS_FOCUS) {
            /* Draw focus rect */
...
BOOL Line_SaveToStg(LPLINE lpLine, UINT uFormat, LPSTORAGE lpSrcStg, LPSTORAGE lpDestStg, LPSTREAM lpLLStm, BOOL fRemember)
{
    ...
    if (! OleDbgVerifySz(hrErr == NOERROR, 
                            "Could not write Line header to LineList stream"))
        return FALSE;
        
    switch (lpLine->m_lineType) {
        case TEXTLINETYPE:
            return TextLine_SaveToStg(
                    (LPTEXTLINE)lpLine, 
                    uFormat, 
                    lpSrcStg, 
                    lpDestStg, 
...
#if defined( OLE_CNTR )
        case CONTAINERLINETYPE:
            return ContainerLine_SaveToStg(
                    (LPCONTAINERLINE)lpLine, 
                    uFormat, 
                    lpSrcStg, 
                    lpDestStg, 
...
LPLINE Line_LoadFromStg(LPSTORAGE lpSrcStg, LPSTREAM lpLLStm, LPOUTLINEDOC lpDestDoc)
{
    ...
    if (! OleDbgVerifySz(hrErr == NOERROR, 
                        "Could not read Line header from LineList stream")) 
        return NULL;
