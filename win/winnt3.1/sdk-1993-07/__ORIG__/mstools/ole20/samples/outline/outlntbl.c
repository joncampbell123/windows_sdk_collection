...
BOOL OutlineNameTable_SaveSelToStg(LPOUTLINENAMETABLE lpOutlineNameTable, LPLINERANGE lplrSel, UINT uFormat, LPSTORAGE lpDestStg)
{
    ...
    if (! OleDbgVerifySz(hrErr == NOERROR, 
                                    "Could not create NameTable stream")) {
    ...
    */
    ...
    if (! OleDbgVerifySz(hrErr == NOERROR,
                                "Could not write header to NameTable stream"))
        goto error;
    ...
    if (! OleDbgVerifySz(hrErr == NOERROR, "Error writing NameTable header")) 
        goto error;
...
BOOL OutlineNameTable_LoadFromStg(LPOUTLINENAMETABLE lpOutlineNameTable, LPSTORAGE lpSrcStg)
{
    ...
    if (! OleDbgVerifySz(hrErr == NOERROR, "Could not open NameTable stream"))
        goto error;

    hrErr = lpNTStm->lpVtbl->Read(lpNTStm,&nCount,sizeof(nCount),&nRead);

    if (! OleDbgVerifySz(hrErr == NOERROR, 
                            "Could not read header from NameTable stream")) 
        goto error;
