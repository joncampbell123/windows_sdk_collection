        ...
        case WM_COMMAND:
            switch (wID)
            {
                ...
                case IDCV_ACTIVATEAS:
                {
                    ...
                    fState = (lpCV->dwFlags & CF_SELECTACTIVATEAS) ? SW_HIDE : SW_SHOW;

                    StandardShowDlgItem(hDlg, IDCV_DISPLAYASICON, fState);
                    StandardShowDlgItem(hDlg, IDCV_CHANGEICON, fState);

                    // Only display the icon if convert is selected AND display
                    // as icon is checked.
...
BOOL FConvertInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
    ...
    if (!(lpCV->dwFlags & CF_SHOWHELPBUTTON))
        StandardShowDlgItem(hDlg, IDCV_HELP, SW_HIDE);      

    //Fill the Object Type listbox with entries from the reg DB.
    FillClassList(lpOCV->clsid, 
                  lpCV->hListVisible, 
                  lpCV->hListInvisible, 
    ...
    while ((LONG)ERROR_SUCCESS==lRet)
    {
             ...
             && (FormatIncluded((LPSTR)szFormat, wFormat)) )
        {
            ...
            {  
                ...
                SendMessage(hList, LB_ADDSTRING, 0, (DWORD)(LPSTR)szHRClassName);

                // add to convert list too.
                SendMessage(hListInvisible, LB_ADDSTRING, 0, (DWORD)(LPSTR)szHRClassName);
...
void UpdateCVClassIcon(HWND hDlg, LPCONVERT lpCV, HWND hList)
    ...
    if ((cch = OleStdGetAuxUserType(&clsid, AUXUSERTYPE_SHORTNAME, 
       ...
       if ((cch = OleStdGetUserTypeOfClass(&clsid, (LPSTR)szLabel, 
           ...
           LoadString(ghInst,IDS_DEFICONLABEL,(LPSTR)szLabel,sizeof(szLabel));
    ...
    uWrapIndex = OleStdIconLabelTextOut(NULL, hFont, 0, 0, 0, &LabelRect, (LPSTR)szLabel, cch, NULL);

    if (0 == uWrapIndex)
    {
       SendMessage(hLabel1, WM_SETTEXT, 0, (LPARAM)(LPSTR)szLabel);
       SendDlgItemMessage(hDlg, IDCV_ICONLABEL2, WM_SETTEXT, 0, (LPARAM)(LPSTR)"");
    }
    else
    {
       ...
       szLabel[uWrapIndex] = '\0';

       SendMessage(hLabel1, WM_SETTEXT, 0, (LPARAM)(LPSTR)szLabel);

       szLabel[uWrapIndex] = chKeep;
       lpszSecondLine = szLabel + uWrapIndex;
