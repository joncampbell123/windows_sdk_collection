        ...
        case WM_COMMAND:
            switch (wID)
                ...
                case ID_BROWSE:
                {
                    ...
                    lstrcpy(szTemp, lpCI->szFile);   //Copy for reference
...
//                       dwOfnFlags |= OFN_SHOWHELP;
                    ...
                    if (0==uRet)
                        uRet=(BOOL)Browse(hDlg, lpCI->szFile, NULL, OLEUI_CCHPATHMAX, IDS_ICONFILTERS, dwOfnFlags);
...
BOOL FChangeIconInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
    ...
    //7.  Call the hook with lCustData in lParam
...
UINT UFillIconList(HWND hDlg, UINT idList, LPSTR pszFile)
    ...
    if (HFILE_ERROR!=OpenFile(pszFile, &of, OF_EXIST))
        {
