...
//
char FAR* CRemPoly::m_rgszMethods[] = {
    "draw",
    "dump",
    "reset",
    "addpoint",
...
CRemPoly::Create(CLSID clsid, CRemPoly FAR* FAR* pprempoly)
{
    ...
    if(hresult != NOERROR){
      ...
      IfWin(MessageBox(NULL, "Unable to create polygon object", NULL, MB_OK));
      goto LFreeCRemPoly;
    }
    ...
    if(hresult != NOERROR){
      ...
      IfWin(MessageBox(NULL, "Unable to QueryInterface to IDispatch", NULL, MB_OK));
      goto LReleaseUnk;
    }
	...
	IfWin(MessageBox(NULL, "Unrecognized member name", NULL, MB_OK));
	goto LReleaseUnk;
      }
    }
