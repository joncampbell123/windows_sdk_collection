...
This command will spawn a command shell(cmd) from which you can exit back to the TPCTL
prompt. This feature is useful for interactive testing where you want to bounce back and
forth between the Tool TPCTL and the command window
...
This command gives you the ability to query/set keys/values within the registry.
...
The tpctl tool by default uses a mode which ensures that while running thru a script,
the log file is updated on each command log. Since this bypasses the file cache and direct
disk writes slow things down, this is useful during the preliminary stages of development
since all information prior to a crash is important. However, caching can be enabled by the
switch /w when invoking the tool. This improves the speed of testing by at least a factor
of 2-3.

/C

During script testing, should an error occur, the tool stops, unloads any other scripts
and closes all instances. In short testing activity is suspended. Once again this is useful
during the preliminary stages of development. However, should you decide the tester to continue
on error, it can be done by using the /C switch when invoking the tool

/?

This is a simple online help function which describes the usage of TPCTL.


More help is available on-line by typing in TPCTL /?
