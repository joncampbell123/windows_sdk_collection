...
INT             InstIndex = 0;
    ...
    if ((pCounter1 = FindCounter (pProcessObject, PX_PROCESS_ID)) &&
        ...
        while (InstIndex < pAddressObject->NumInstances)
            {
...
INT             InstIndex = 0;
    ...
    if (pImageObj)
        {
        if (pImageInst = FirstInstance (pImageObj))
            while (InstIndex < pImageObj->NumInstances)
                {
...
PPERF_INSTANCE  pInst;
    ...
    pInst = FindInstanceN (pImageObject, dwIndex);
...
INT             i = 0;
    ...
    if ((pCounter1 = FindCounter (pProcessObject, PX_PROCESS_ID)) &&
        ...
        while (i < pAddressObject->NumInstances)
            {
            pProcessID2 = (DWORD *) CounterData (pAddressInstance, pCounter2);
...
INT             i = 0;
    ...
    if (pCounter = FindCounter (pThreadDetailsObject, PX_THREAD_PC))
        ...
        while (i < pThreadDetailsObject->NumInstances)
            {
...
INT             err;
    ...
    if (pObject)
        {
        pCounterCPU    = FindCounter (pObject, PX_PROCESS_CPU);
        ...
        while (InstanceIndex < pObject->NumInstances)
            {
...
PPERF_INSTANCE  pInstance;
    ...
    pInstance = FindInstanceN (pObject, ProcessIndex);
...
INT             err;
    ...
    if (pObject)
        {
        pCounterCPU    = FindCounter (pObject, PX_THREAD_CPU);
        ...
        while (InstanceIndex < pObject->NumInstances)
            ...
            if (ParentIndex == pInstance->ParentObjectInstance)
                ...
                ListIndex = SendMessage (hThreadList, LB_ADDSTRING, 0, (DWORD)szListText);
...
BOOL            bPrioCounter;
    ...
    pInstance = FindInstanceN (pThreadObj, ThreadIndex);
