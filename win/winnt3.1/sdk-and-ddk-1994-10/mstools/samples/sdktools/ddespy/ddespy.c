...
{
    UINT wT;
    TCHAR szData[32]; // for truncating strings w/o disturbing the hDdeData
...
VOID OutputString(LPTSTR pstr)
{
    if (pro.fOutput[IO_FILE] & fhOutput) {
        _lwrite(fhOutput, (LPCSTR) pstr, lstrlen(pstr));
        _lwrite(fhOutput, (LPCSTR) RefString(IDS_CRLF), 2);
        flushall();
    }
