...
int    WINAPI HandleRiscBreakPoint(DBGPROCESS *, DWORD);
...
#ifndef _X86_				/* Risc needs special handling here to reset breakpoint */
					HandleRiscBreakPoint(lpDbgProcess, de.dwThreadId);
...
#ifndef _X86_
/* 
 * MIPS/ALPHA must increment the FIR on a breakpoint 
 * in order to fetch the next instruction
 */

int WINAPI HandleRiscBreakPoint(
    DBGPROCESS		      *lppr,
    DWORD		      dwThreadID
    )
{
