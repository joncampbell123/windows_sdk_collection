...
BOOL /* static */ LogComputerChanged (HDLG hDlg)
   ...
   DialogEnable (hDlg, IDD_ADDLOGADD, TRUE) ;
...
void /* static */ OnInitAddLogDialog (HDLG hDlg)
   ...
   if (PlayingBackLog())
      ...
      DialogEnable (hDlg, IDD_ADDLOGELLIPSES, TRUE) ;
      }
   else
      {
      pPerfData = MemoryAllocate (STARTING_SYSINFO_SIZE) ;
      DialogSetString (hDlg, IDD_ADDLOGCOMPUTER, LocalComputerName) ;
      DialogEnable (hDlg, IDD_ADDLOGELLIPSES, FALSE) ;
      }
...
//            case IDD_OK:
            ...
            case IDD_ADDLOGCOMPUTER:
               if (HIWORD (wParam) == EN_UPDATE)
                  {
                  ComputerChange = TRUE ;
                  DialogEnable (hDlg, IDD_ADDLOGADD, FALSE) ;
                  }
