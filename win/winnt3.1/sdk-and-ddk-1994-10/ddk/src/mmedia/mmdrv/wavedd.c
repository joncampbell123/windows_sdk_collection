                         ...
                         DWORD dwParam2)
{
    ...
    if (dwParam2 & WAVE_FORMAT_QUERY) {
        ...
        // Check the format
        //
                Format = (LPPCMWAVEFORMAT)((LPWAVEOPENDESC)dwParam1)->lpFormat;
        Result = DeviceIoControl(
                        hDev,
                        IOCTL_WAVE_QUERY_FORMAT,
                        (PVOID)Format,
                        sizeof(PCMWAVEFORMAT) +
                                                (Format->wf.wFormatTag == WAVE_FORMAT_PCM ? 0 :
                                                                       *(LPWORD)(Format + 1)),
                                                     // Input buffer size
                        NULL,                        // Output buffer
    ...
    Result = DeviceIoControl(
                 ...
                 IOCTL_WAVE_SET_FORMAT,
                        (PVOID)((LPWAVEOPENDESC)dwParam1)->lpFormat,
/* BUGBUG NOT GENERAL */sizeof(PCMWAVEFORMAT),       // Input buffer size
                        NULL,                        // Output buffer
                        0,                           // Output buffer size
                        &BytesReturned,
                        NULL);
