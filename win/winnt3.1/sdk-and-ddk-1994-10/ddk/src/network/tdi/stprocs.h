...
//
// These routines are used to maintain counters that are
// locked using the interlocked routines.
//

#define INCREMENT_COUNTER_LOCKED(_DeviceContext,_Field) \
    ExInterlockedAddUlong( \
        &((_DeviceContext)->_Field), \
        1, \
        &((_DeviceContext)->StatisticsInterlock))

#define INCREMENT_COUNTER(_DeviceContext,_Field) \
    ++(_DeviceContext)->_Field

#define DECREMENT_COUNTER_LOCKED(_DeviceContext,_Field) \
    ExInterlockedAddUlong( \
        &((_DeviceContext)->_Field), \
        (ULONG)-1, \
        &((_DeviceContext)->StatisticsInterlock))
...
#define ADD_TO_LARGE_INTEGER(_LargeInteger,_Ulong,_Interlock) \
{ \
    LARGE_INTEGER TmpLarge; \
    TmpLarge.HighPart = 0; \
    TmpLarge.LowPart = (ULONG)(_Ulong); \
