    ...
    for (;;) {
        ...
        case NdisRequestClose:
            ...
            if (Status == NDIS_STATUS_SUCCESS) {
                ...
                // is done with us, we reload the CAM there to get
                // queue processing going again.
                //
...
Routine Description:
    ...
    This routine starts a CAM reload (even though nothing may
    have changed) so that queue processing is restarted in the
    completion handler).
...

{
    ...
    SonicStartCamReload(PSONIC_ADAPTER_FROM_BINDING_HANDLE(MacBindingHandle));
