...

{
    ...
    if (pRequestedMode->MonitorType) {
        ...
        if (pRequestedMode->hres == 640) {
            ...
            if (pRequestedMode->Frequency == 72) { // 72 hz refresh

                // this high refresh setup only takes effect in 640x480
    ...
    //
    // thhis code fixes a bug for color TFT panels only
...
#ifdef INT10_MODE_SET
{
    ...
    //
    // if 6410 rev. AB, then fix CR13 in hires mode
    //

    if ( (HwDeviceExtension->ChipType == CL6420) &&      
         (HwDeviceExtension->ChipRevision >= 0x7e) &&
         (HwDeviceExtension->DisplayType == crt) && 
         (pRequestedMode->hres == 1024) ) {

      VideoPortWritePortUchar(HwDeviceExtension->IOAddress +
            CRTC_ADDRESS_PORT_COLOR, 0x13);
      VideoPortWritePortUchar(HwDeviceExtension->IOAddress +
...

{
    ...
    for (i = 0; i < NumVideoModes; i++) {
        ...
        if (ModesVGA[i].ValidMode) {
            ...
            videoModes->XMillimeter = 330;        // temporary hardcoded constant
...

{
    ...
    ModeInformation->XMillimeter = 330;        // temporary hardcoded constant
...
--*/
{
    ...
    case CL6410:
        ...
        max = 1;       // 6410 max memory is 256k
        break;

    case CL6420:

        max = 4;       // 6410 max memory is 1meg.
        break;

    case CL542x:

        max = 8;       // 6410 max memory is 2meg.
        break;
    ...
    for (i = 0; i < max; i++) {
        ...
        SetCirrusBanking(HwDeviceExtension,i);     // setup next 256k bank
        VideoPortZeroMemory(HwDeviceExtension->VideoMemoryAddress, 0xFFFF);

    }

    SetCirrusBanking(HwDeviceExtension,0);     // reset the banks
