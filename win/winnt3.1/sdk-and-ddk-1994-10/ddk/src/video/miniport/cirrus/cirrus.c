...
Abstract:
    ...
    This is the miniport driver for the Cirrus Logic 6410/6420/542x VGA's.
...
//
    //
    // Determine whether an CL6410/6420/542x is present.
    //
...
Routine Description:
    ...
    This routine returns TRUE if an CL6410, 6420 or 542x is present. It 
    assumes that it's already been established that a VGA is present.  
    ...
    542x: Enable extended registers by writing 0x12 to the extensions
          enable register, and reading back 0x12.  Then read from the 
          ID register and make sure it specifies a 5420, 5422, 5424,
          or 5426.  Finally, disable the extensions and make sure the
...
Return Value:
    ...
    TRUE if an CL6410/6420/542x is present, FALSE if not.
...
//
// we don't support 6340 in this driver, so force it not to be installed
//
...
//          retvalue = FALSE;          // already FALSE
   ...
   if (!retvalue)          // Did not detect a 64x0, see if it's a 542x
      {
      //
    ...
    //
    // Determines whic modes are valid.
    //
...
--*/
{
    ...
    if (HwDeviceExtension->ChipType == CL6420) {
        ...
        //
        // Read the POST scratch pad reg SR0A to determine amount of Video
        // memory
        //
        ...
        switch (PostScratchPad & 0x18) {  // It's in bits 3 and 4
