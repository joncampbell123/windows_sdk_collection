...
;-----------------------------------------------------------------------;
        ...
        or      eax,eax
        jnz     draw_color_pointer      ;Color pointer?
...
clip_on_bottom_eh?:
        ...
        add     ecx, eax
        sub     ecx, cyScreen
        jle     finish_y_clipping       ;EAX = amount to clip if positive
...
clip_on_rhs:
        ...
        add     ecx,eax
        sub     ecx,lNextScan
        jle     x_clipping_done         ;EAX = amount to clip if positive
        sub     al,cl                   ;Compute new height
        jle     clear_visible_bit       ;Clipped away, nothing visible
...
; See if we'll wrap in X.  If so, split the operation into two parts
        ...
        add     bl, dl
        sub     bl, SAVE_BUFFER_WIDTH   ;BL = is overhang
        jle     and_from_save_do_last_pass ;No overhang
...
and_from_save_width_0:
        ...
        .ERRNZ  and_from_save_width_0-and_from_save_width_1-5
        .ERRNZ  and_from_save_width_1-and_from_save_width_2-5
        .ERRNZ  and_from_save_width_2-and_from_save_width_3-5
        .ERRNZ  and_from_save_width_3-and_from_save_width_4-5
        .ERRNZ  and_from_save_width_4-and_from_save_width_5-5
...
@@:
        ...
        jmp     copy_save_next_scan     ;do the next block of scans
...
copy_save_next_scan_wrap_loop:
        ...
        add     edi,eax
        dec     dh
        jnz     copy_save_next_scan_wrap_loop
...
copy_screen_next_scan_loop:
        ...
        add     esi,eax
        dec     dh
        jnz     copy_screen_next_scan_loop
...
@@:
        ...
        jmp     copy_screen_next_scan   ;do the next block of scans
...
copy_screen_next_scan_wrap_loop:
        ...
        add     esi,eax
        dec     dh
        jnz     copy_screen_next_scan_wrap_loop
...
@@:
        ...
        pop     eax                     ;restore source next scan
        sub     ecx,ecx                 ;prepare for loading CL in loop
        jmp     copy_screen_next_scan_wrap_loop ;do the next block of scans
...
flip_next_scan:
        ...
        xchg    [edi][00h],eax          ;Swap
        mov     [esi][00h],eax          ;Save

        mov     eax,[esi][04h]          ;Load
        xchg    [edi][04h],eax          ;Swap
        mov     [esi][04h],eax          ;Save

        mov     eax,[esi][08h]          ;Load
        xchg    [edi][08h],eax          ;Swap
        mov     [esi][08h],eax          ;Save

        mov     eax,[esi][0Ch]          ;Load
        xchg    [edi][0Ch],eax          ;Swap
        mov     [esi][0Ch],eax          ;Save
