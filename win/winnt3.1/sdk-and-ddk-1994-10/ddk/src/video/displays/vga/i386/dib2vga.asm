...
;-----------------------------------------------------------------------;
;
; Note: The general-purpose buffer attached to the PDEV that we used for
; temp storage (pvBankBufferPlane0) must be at least 1K+4 bytes wide to
; support
;
...
;-----------------------------------------------------------------------;
; The following tables *must* start on 256-byte boundaries, so that a
; table look-up may be accomplished by loading BL.
;-----------------------------------------------------------------------;

;-----------------------------------------------------------------------;
; Table used to multiplex bits from form accumulated from DIB4 to VGA
; planar byte format for plane 0. Translation is from 64207531 source
...
;@@@
        db      256-20h dup (?)
;@@@
;DIB4_to_VGA_plane0_table label byte
x=0
        rept    256
        db ((x and 10h) shl 3)+((x and 01h) shl 6)+\
            (x and 20h)       +((x and 02h) shl 3)+\
           ((x and 40h) shr 3)+ (x and 04h)       +\
           ((x and 80h) shr 6)+((x and 08h) shr 3)
x=x+1
        endm
;-----------------------------------------------------------------------;
; Table used to multiplex bits from form accumulated from DIB4 to VGA
; planar byte format for plane 0. Translation is from 46025713 source
; DIB format (where 0 is leftmost, 7 is rightmost) to 76543210 VGA
; format (where 7 is leftmost, 0 is rightmost).

;DIB4_to_VGA_plane1_table label byte
x=0
        rept    256
        db ((x and 20h) shl 2)+((x and 02h) shl 5)+\
           ((x and 10h) shl 1)+((x and 01h) shl 4)+\
           ((x and 80h) shr 4)+((x and 08h) shr 1)+\
           ((x and 40h) shr 5)+((x and 04h) shr 2)
x=x+1
        endm
;-----------------------------------------------------------------------;
; Table used to multiplex bits from form accumulated from DIB4 to VGA
; planar byte format for plane 0. Translation is from 20643175 source
; DIB format (where 0 is leftmost, 7 is rightmost) to 76543210 VGA
; format (where 7 is leftmost, 0 is rightmost).

;DIB4_to_VGA_plane2_table label byte
x=0
        rept    256
        db ((x and 40h) shl 1)+((x and 04h) shl 4)+\
           ((x and 80h) shr 2)+((x and 08h) shl 1)+\
           ((x and 10h) shr 1)+((x and 01h) shl 2)+\
           ((x and 20h) shr 4)+((x and 02h) shr 1)
x=x+1
        endm
;-----------------------------------------------------------------------;
; Table used to multiplex bits from form accumulated from DIB4 to VGA
; planar byte format for plane 0. Translation is from 02461357 source
; DIB format (where 0 is leftmost, 7 is rightmost) to 76543210 VGA
; format (where 7 is leftmost, 0 is rightmost).

;DIB4_to_VGA_plane3_table label byte
x=0
        rept    256
        db  (x and 80h)       +((x and 08h) shl 3)+\
           ((x and 40h) shr 1)+((x and 04h) shl 2)+\
           ((x and 20h) shr 2)+((x and 02h) shl 1)+\
           ((x and 10h) shr 3)+ (x and 01h)
x=x+1
        endm
endif
;-----------------------------------------------------------------------;
...
;-----------------------------------------------------------------------;
        ...
        mov     eax,[ebx].ptl_y
        mul     [esi].dsurf_lNextScan   ;offset in bitmap of top src rect scan
        mov     edx,[ebx].ptl_x
...
check_whole_bytes:
        ...
        mov     eax,[esi].xLeft         ;check for leading whole byte
...
@@:
        ...
        mov     eax,[esi].xRight        ;check for trailing whole byte
        and     eax,1111b               ;intra-word address
...
;-----------------------------------------------------------------------;
        ...
        jz      short set_initial_banking ;if it's 0, we have no alignment to
...
;-----------------------------------------------------------------------;
        ...
        mov     ch,byte ptr [esi].xRight ;CH = lsb of dest right edge
...
@@:
        ...
        mov     ecx,[edi].dsurf_ppdev ;point to pdev for 386 flag
        and     eax,eax         ;shift left or right?
        js      short @F        ;shift right
                                ;shift left

        mov     edx,offset align_burst_lshift_486 ;shifting left, assume 486
        cmp     [ecx].PDEV_is386,0      ;486?
        jz      short set_shift_vec     ;yes
        mov     edx,offset align_burst_lshift_386 ;386
        jmp     short set_shift_vec
...
@@:                             ;shifting right
        ...
        and     eax,7           ;for left shift, shift value is already
                                ...
                                ; right shifts)
        mov     edx,offset align_burst_rshift_386 ;shifting right, assume 386
        cmp     [ecx].PDEV_is386,0      ;386?
        jnz     short set_shift_vec     ;yes
...
set_shift_vec:
        ...
        mov     eax,[esi].xRight
        sub     eax,[esi].xLeft ;pixel width of rectangle to copy
        ...
        mov     eax,[edi].dsurf_pvBankBufferPlane0 ;this is the general-purpose
...
;-----------------------------------------------------------------------;
; Loops to align this burst's scans for 386/486, left or right shift.
;
...
align_burst_lshift_loop:
       ...
       if Is486
        M_BSWAP edx             ;make it big endian
       else
        M_BSWAP_386 edx
       endif
...
align_lshift_loop:
        ...
        add     esi,4           ;point to next source dword
       if Is486
        M_BSWAP eax             ;make it big endian
       else
        M_BSWAP_386 eax
       endif
...
align_rshift_loop:
       ...
       if Is486
        ...
        shld    edx,eax,cl      ;shift to generate a 32-bit shifted value
       if Is486
        M_BSWAP edx             ;make it little endian
       else
        M_BSWAP_386 edx
       endif
        mov     [edi],edx       ;store it
...
;-----------------------------------------------------------------------;
        ...
        align   4
align_burst_rshift_386:
        ALIGN_BURST_RSHIFT 0
        jmp     set_alignment_source

        align   4
align_burst_lshift_386:
        ALIGN_BURST_LSHIFT 0
        jmp     set_alignment_source
...
public align_burst_rshift_386
public align_burst_lshift_386
