...
// BUGBUG all routines should get the scan width out of the surface
ULONG ulNextScan_global;
...
BOOL SetUpBanking(PDEVSURF pdsurf, PPDEV ppdev)
{
    ...
    pdsurf->lNextScan = BankInfo->BitmapWidthInBytes;
         ...
         // BUGBUG this should not be static
         ulNextScan_global = BankInfo->BitmapWidthInBytes;
    ...
    pPtrWork = (LPBYTE) pdsurf->ulBitmapSize - POINTER_WORK_AREA_SIZE;
    pPtrSave = pPtrWork - POINTER_SAVE_AREA_SIZE;
    ...
    if (BankInfo->BankingType == VideoNotBanked) {
        ...
        if (BankInfo->BankingType == VideoBanked2RW) {
            ...
            // Not a 2RW bank adapter, just use standard table for 2-window
            // cases

            pdsurf->pbiBankInfo2RW = pdsurf->pbiBankInfo;
            pdsurf->ulBankInfo2RWLength = pdsurf->ulBankInfoLength;
