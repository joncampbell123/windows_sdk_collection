...
opaq_sl_row_loop:
        ...
        inc     ecx             ;adjust for word calc
        shr     ecx,1           ;we'll do a word per loop
                                ;guaranteed to have at least one word to do, or
                                ; we wouldn't be here
        jnc     short opaq_sl_enter_odd
opaq_sl_byte_loop:
...
opaq_draw_edge_entry:
        ...
        and     eax,11b                 ;# of fractional quadscans to copy
        mov     edx,ulScreenDelta       ;width of a screen scan in addresses
        mov     ebx,dword ptr OpaqFgEdgeTable[eax*4]
        push    ebx                     ;entry point into unrolled loop
                                        ;***stack frame unavailable***
        mov     eax,ulNumScans          ;height of text
        add     eax,3
        shr     eax,2                   ;# of whole & partial quadscans to copy
        mov     ebx,pGlyphFlipTable     ;point to the look-up table we'll use
                                        ; to flip the glyph bits into the form
                                        ; required by planar mode
        mov     ebp,ulBufDelta          ;width of a source scan in bytes
        retn                            ;branch into the unrolled loop

        align   4
OpaqFgEdgeTable label   dword
        dd      opaq_edge_entry_4
        dd      opaq_edge_entry_1
        dd      opaq_edge_entry_2
        dd      opaq_edge_entry_3
...
;-----------------------------------------------------------------------;
        ...
        mov     eax,dword ptr s8wao_byte_table[eax*4] ;entry point into
...
public opaq_edge_entry_4
public opaq_edge_entry_3
public opaq_edge_entry_2
public opaq_edge_entry_1
