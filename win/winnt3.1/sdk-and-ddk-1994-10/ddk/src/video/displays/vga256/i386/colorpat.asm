...
left_8xN:
        ...
        SET_UP_UNROLL_VARS esi,ecx,esi,pfnVenetianEdgeEntry,LOOP_UNROLL_SHIFT
        mov     pfnUnroll,ecx
...
; ecx = unrolled loop count
; edx = venetian delta
; esi = unrolled loop count
; edi = screen

        call    pfnUnroll               ;repeat this quadpixel of the pattern
...
;-----------------------------------------------------------------------;
; Unrolled routine to do a venetian edge.
;
; Input:
;
;       ecx = count of unrolled loops
;       edx = venetian delta
;       edi = destination address
;
;-----------------------------------------------------------------------;

        UNROLL_LOOP_ENTRY_TABLE pfnVenetianEdgeEntry,DVE,LOOP_UNROLL_COUNT

DRAW_8xN_EDGE macro ENTRY_LABEL,ENTRY_INDEX
&ENTRY_LABEL&ENTRY_INDEX&:
        ...
        align   4
        public  draw_8xN_edge_loop
draw_8xN_edge_loop:
        UNROLL_LOOP     DRAW_8xN_EDGE,DVE,LOOP_UNROLL_COUNT
        dec     ecx
        jnz     draw_8xN_edge_loop
        PLAIN_RET
...
middle_8xN:
        ...
        SET_UP_UNROLL_VARS esi,ecx,esi,pfnVenetianMiddleEntry,LOOP_UNROLL_SHIFT
        mov     pfnUnroll,ecx
...
;-----------------------------------------------------------------------;
; Unrolled routine to write the middle of an n-color 8xN pattern.
;
; Input:
;
...
;    ecx = unrolled loop count
;    edx = venetian delta
;    esi = unrolled loop count (same as ecx)
;    edi = destination address
;    pfnUnroll = unroll start
;
; Output:
;
;    ebx = 0
...
;-----------------------------------------------------------------------;
        ...
        UNROLL_LOOP_ENTRY_TABLE pfnVenetianMiddleEntry,H8M,LOOP_UNROLL_COUNT

DRAW_HORIZONTAL_8xN_MIDDLE macro ENTRY_LABEL,ENTRY_INDEX
&ENTRY_LABEL&ENTRY_INDEX&:
        mov     [edi],al                ;write the quadpixel
...
draw_horizontal_8xN_middle_loop:
        UNROLL_LOOP     DRAW_HORIZONTAL_8xN_MIDDLE,H8M,LOOP_UNROLL_COUNT
        dec     ecx
        jnz     draw_horizontal_8xN_middle_loop
        add     edi,edx                 ;move to next scan

        dec     ebx                     ;that's one more scan done
        jz      short draw_horizontal_8xN_middle_done
        mov     ecx,esi                 ;reload our unroll count
        jmp     pfnUnroll

        align   4
draw_horizontal_8xN_middle_done:
        PLAIN_RET
...
middle_00_4xN:
        ...
        SET_UP_UNROLL_VARS esi,ecx,esi,pfnMiddle00Entry,LOOP_UNROLL_SHIFT
        mov     pfnUnroll,ecx
        mov     cUnrolledLoops,esi
...
;-----------------------------------------------------------------------;
; Unrolled routine to do a venetian middle.
;
; Input:
;
;       ecx      = count of unrolled loops
;       edx      = venetian delta
;       edi      = destination address
;       cwMiddle = number of words on each scan
;
;-----------------------------------------------------------------------;

        UNROLL_LOOP_ENTRY_TABLE pfnMiddle00Entry,M00,LOOP_UNROLL_COUNT

DRAW_4xN_MIDDLE_00 macro ENTRY_LABEL,ENTRY_INDEX
&ENTRY_LABEL&ENTRY_INDEX&:
        mov     ecx,cwMiddle
        rep     stosw
        add     edi,edx
        ...
        align   4
        public  draw_4xN_middle_00_loop
draw_4xN_middle_00_loop:
        UNROLL_LOOP     DRAW_4xN_MIDDLE_00,M00,LOOP_UNROLL_COUNT
        dec     esi
        jnz     draw_4xN_middle_00_loop
        PLAIN_RET
...
middle_01_4xN:
        ...
        SET_UP_UNROLL_VARS esi,ecx,esi,pfnMiddle01Entry,LOOP_UNROLL_SHIFT
        mov     pfnUnroll,ecx
        mov     cUnrolledLoops,esi
...
;-----------------------------------------------------------------------;
        ...
        UNROLL_LOOP_ENTRY_TABLE pfnMiddle01Entry,M01,LOOP_UNROLL_COUNT

DRAW_4xN_MIDDLE_01 macro ENTRY_LABEL,ENTRY_INDEX
&ENTRY_LABEL&ENTRY_INDEX&:
        mov     ecx,cwMiddle
        rep     stosw
        mov     [edi],al
        ...
        align   4
        public  draw_4xN_middle_01_loop
draw_4xN_middle_01_loop:
        UNROLL_LOOP     DRAW_4xN_MIDDLE_01,M01,LOOP_UNROLL_COUNT
        dec     esi
        jnz     draw_4xN_middle_01_loop
        PLAIN_RET
...
middle_10_4xN:
        ...
        SET_UP_UNROLL_VARS esi,ecx,esi,pfnMiddle10Entry,LOOP_UNROLL_SHIFT
        mov     pfnUnroll,ecx
        mov     cUnrolledLoops,esi
...
;-----------------------------------------------------------------------;
        ...
        UNROLL_LOOP_ENTRY_TABLE pfnMiddle10Entry,M10,LOOP_UNROLL_COUNT

DRAW_4xN_MIDDLE_10 macro ENTRY_LABEL,ENTRY_INDEX
&ENTRY_LABEL&ENTRY_INDEX&:
        mov     [edi-1],al
        mov     ecx,cwMiddle
        rep     stosw
        ...
        align   4
        public  draw_4xN_middle_10_loop
draw_4xN_middle_10_loop:
        UNROLL_LOOP     DRAW_4xN_MIDDLE_10,M10,LOOP_UNROLL_COUNT
        dec     esi
        jnz     draw_4xN_middle_10_loop
        PLAIN_RET
...
middle_11_4xN:
        ...
        SET_UP_UNROLL_VARS esi,ecx,esi,pfnMiddle11Entry,LOOP_UNROLL_SHIFT
        mov     pfnUnroll,ecx
        mov     cUnrolledLoops,esi
...
;-----------------------------------------------------------------------;
        ...
        UNROLL_LOOP_ENTRY_TABLE pfnMiddle11Entry,M11,LOOP_UNROLL_COUNT

DRAW_4xN_MIDDLE_11 macro ENTRY_LABEL,ENTRY_INDEX
&ENTRY_LABEL&ENTRY_INDEX&:
        mov     [edi-1],al
        mov     ecx,cwMiddle
        rep     stosw
        ...
        align   4
        public  draw_4xN_middle_11_loop
draw_4xN_middle_11_loop:
        UNROLL_LOOP     DRAW_4xN_MIDDLE_11,M11,LOOP_UNROLL_COUNT
        dec     esi
        jnz     draw_4xN_middle_11_loop
        PLAIN_RET
