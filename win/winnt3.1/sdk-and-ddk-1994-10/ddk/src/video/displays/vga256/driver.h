...
#define BRUSH_MAX_CACHE_SIZE    330     // Maximum brush cache size (chose 330
                                        //  so that BRUSHCACHEENTRY array
                                        //  would fit entirely in 1 page)
...
typedef struct _PDEV
{
    ...
    DDAOBJ*     pdda;                   // DDA drawing object for trapezoids
...
// NOTE: This stuff must come at the end of the structure, so that we don't
//       have to declare an array size in the ASM version:
...
typedef struct _BBENUMTRAP
{
    ULONG     c;
    TRAPEZOID atrap[BB_RECT_LIMIT];
} BBENUMTRAP;

#define MAX_DDA_RECTS   50  // most rects we want enumerated by DDAOBJ at once
