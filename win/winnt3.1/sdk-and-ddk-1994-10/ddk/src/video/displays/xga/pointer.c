...
/*
        These globals must be global.
        Need to look into putting them into the pdev.
*/


LONG        gxHot, gyHot,
        gcxPointer, gcyPointer,
        gxLast, gyLast ;
...
VOID DrvMovePointer(SURFOBJ *pso,LONG x,LONG y,RECTL *prcl)
{
        ...
        if (gPointerFlags & COLOR_POINTER)
...
INT     XgaIndexReg ;
        ...
        gcxPointer = psoMask->sizlBitmap.cx ;
        gcyPointer = psoMask->sizlBitmap.cy / 2;
        ...
        else
        {
            ...
            if (   (gPointerFlags & COLOR_POINTER)
                && (gPointerFlags & VALID_SAVE_BUFFER)
               )
            {
