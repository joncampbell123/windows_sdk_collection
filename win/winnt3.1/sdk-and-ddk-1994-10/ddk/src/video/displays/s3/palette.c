...
BOOL bInitDefaultPalette(PPDEV ppdev)
{
    if (ppdev->ulBitCount == 8)
    {
        ...
        if ((ppdev->pPal) == NULL) {
            DISPDBG((0, "S3.DLL!bInitDefaultPalette -  LocalAlloc failed \n"));
    ...
    {
        RIP("S3.DLL!bInitDefaultPalette - ppdev->ulBitCount is invalid\n");
        return (FALSE);
    }
...
BOOL bInit256ColorPalette(PPDEV ppdev)
{
    ...
    // Copy Colors in.
    memcpy(pScreenClut->LookupTable, ppdev->pPal, sizeof(ULONG) * 256);
    ...
    if (!DeviceIoControl(ppdev->hDriver,
                         ...
                         NULL))
    {
        DISPDBG((0, "DISP bInit256ColorPalette failed DeviceIoControl\n"));
        return(FALSE);
    }
    ...
    ULONG  cColors)
{
    ...
    {
        DISPDBG((0, "DISP DrvSetPalette failed PALOBJ_cGetColors\n"));
        return(FALSE);
    }
    ...
    if (!DeviceIoControl(((PPDEV)(dhpdev))->hDriver,
                          ...
                          NULL))
    {
        DISPDBG((0, "DISP DrvSetPalette failed DeviceIoControl\n"));
        return(FALSE);
    }
