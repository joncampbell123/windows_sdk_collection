...

{
    ...
    DriverObject->MajorFunction[IRP_MJ_CREATE] = ScsiPrinterOpen;
    ...
    do {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            DebugPrint((1,
                        "ScsiPrinterInitialize: Could not initalize unicode string %s\n",
                        deviceNameString));
...

{
    ...
    if (currentIrpStack->Parameters.Write.Length > maximumTransferLength) {
        ...
        DebugPrint((2,"ScsiPrinterWrite: Request greater than maximum\n"));
        DebugPrint((2,"ScsiPrinterWrite: Maximum is %lx\n",
                    maximumTransferLength));
        DebugPrint((2,"ScsiPrinterWrite: Byte count is %lx\n",
    ...
    if (transferPages >
        ...
        DebugPrint((1,"ScsiPrinterWrite: Request greater than maximum\n"));
        DebugPrint((1,"ScsiPrinterWrite: Maximum pages is %lx\n",
                    deviceExtension->PortCapabilities->MaximumPhysicalPages));
        DebugPrint((1,"ScsiPrinterWrite: Number of pages is %lx\n",
                    transferPages));
        ...
        maximumTransferLength = (transferPages - 1) * PAGE_SIZE;
