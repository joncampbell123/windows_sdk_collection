...
NTSTATUS
CdAudioHitatchiSendPauseCommand(
    IN PDEVICE_OBJECT DeviceObject
    );

NTSTATUS
CdAudioHitatchiDeviceControl(
    IN PDEVICE_OBJECT DeviceObject,
    IN PIRP Irp
    );
...
{
    return CdAudioInitialize(DriverObject);
}
    ...
    for (disks = 0; disks < configurationInformation->CdRomCount; disks++) {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            if ((inquiryDataPtr[8] =='H') &&
                ...
                //
                // It's a Hitatchi drive...is it one we want to
                // handle...?
                //
                ...
                if ( ((inquiryDataPtr[20]=='1') &&
                     ...
                     ((inquiryDataPtr[20]=='3') &&
                      ...
                      (inquiryDataPtr[23]=='0'))
                        ...
                        //
                        // Yes, we want to handle this HITATCHI drive...
                        //

                        deviceExtension->Active = CDAUDIO_HITATCHI;
                        inquiryDataPtr[25] = (UCHAR)0;
                        CdDump(( 3,
                            "CdAudioInitialize: %s is a HITATCHI %s drive.\n",
...

{
    ...
    switch( deviceExtension->Active ) {
        ...
        case CDAUDIO_HITATCHI:
            status = CdAudioHitatchiDeviceControl( DeviceObject, Irp );
            break;
...
retry:
    ...
    if (SRB_STATUS(Srb->SrbStatus) != SRB_STATUS_SUCCESS) {
        ...
        retry = CdAudioNECInterpretSenseInfo(
            DeviceObject,
            Srb,
            IRP_MJ_SCSI,
            0,
...
NECRestart:
    ...
    case IOCTL_CDROM_GET_LAST_SESSION:
        ...
        if (currentIrpStack->Parameters.Read.Length <
            FIELD_OFFSET(CDROM_TOC, TrackData[1])) {
        ...
        cdaudioDataOut->Length[0]  = bytesTransfered >> 8;
        cdaudioDataOut->Length[1]  = (bytesTransfered & 0xFF);
...
PioneerRestart:
    ...
    case IOCTL_CDROM_READ_TOC:
        ...
        for( i=0;
            ...
            RtlZeroMemory( cdb, MAXIMUM_CDB_SIZE );
            cdb->PNR_READ_TOC.OperationCode     = PIONEER_READ_TOC_CODE;
            cdb->PNR_READ_TOC.TrackNumber       = DEC_TO_BCD((i+cdaudioDataOut->FirstTrack)); // track
            cdb->PNR_READ_TOC.AssignedLength[1] = PIONEER_TRANSFER_SIZE;
...
Routine Description:
    ...
    This routine sends a PAUSE cdb to the Hitatchi drive.  The Hitatchi
...

{
    ...
    srb.CdbLength    = 12;
    srb.TimeOutValue = AUDIO_TIMEOUT;
    cdb->PAUSE_AUDIO.OperationCode = HITATCHI_PAUSE_AUDIO_CODE;
    status = ScsiClassSendSrbSynchronous( deviceExtension->DeviceObject,
                                 &srb,
                                 PausePos,
                                 3,
...
Routine Description:
    ...
    This routine is called by CdAudioDeviceControl to handle
    audio IOCTLs sent to Hitatchi cdrom drives.
...
HitatchiRestart:
    ...
    case IOCTL_CDROM_READ_TOC:
        CdDump(( 3,
                     "CdAudioHitatchiDeviceControl: IOCTL_CDROM_READ_TOC received.\n"
                     ));
        ...
        cdb->READ_DISC_INFO.OperationCode       = HITATCHI_READ_TOC_CODE;
        ...
        if (!NT_SUCCESS(status) && (status!=STATUS_DATA_OVERRUN)) {
            ...
            CdDump(( 1,
                "CdAudioHitatchiDeviceControl: READ_TOC error (0x%08lX)\n",
                 status ));


            if (SRB_STATUS(srb.SrbStatus)!=SRB_STATUS_DATA_OVERRUN) {

                CdDump(( 1, "CdAudioHitatchiDeviceControl: SRB ERROR (0x%lX)\n",
                             srb.SrbStatus ));
                ExFreePool( Toc );
                goto SetStatusAndReturn;
            }
    ...
    case IOCTL_CDROM_STOP_AUDIO:
        ...
        CdAudioHitatchiSendPauseCommand( DeviceObject );
    ...
    case IOCTL_CDROM_PLAY_AUDIO_MSF:
        ...
        CdDump(( 3,
            "CdAudioHitatchiDeviceControl: IOCTL_CDROM_PLAY_AUDIO_MSF received.\n"
            ));
        ...
        srb.CdbLength                 = 12;
        srb.TimeOutValue              = AUDIO_TIMEOUT;
        cdb->PLAY_AUDIO.OperationCode = HITATCHI_PLAY_AUDIO_MSF_CODE;
        ...
        if (NT_SUCCESS(status)) {
            ...
            CdDump(( 3,
                "CdAudioHitatchiDeviceControl: PLAY failed (0x%08lX)\n",
                status ));
        }
    ...
    case IOCTL_CDROM_SEEK_AUDIO_MSF:
        ...
        CdDump(( 3,
            "CdAudioHitatchiDeviceControl: IOCTL_CDROM_SEEK_AUDIO_MSF received.\n"
             ));
        ...
        if (NT_SUCCESS(status)) {
            ...
            CdDump(( 3,
                "CdAudioHitatchiDeviceControl: SEEK failed (0x%08lX)\n",
                status ));
        }
    ...
    case IOCTL_CDROM_PAUSE_AUDIO:
        ...
        CdDump(( 3,
            "CdAudioHitatchiDeviceControl: IOCTL_CDROM_PAUSE_AUDIO received.\n"
            ));
    ...
    case IOCTL_CDROM_RESUME_AUDIO:
        ...
        CdDump(( 3,
            "CdAudioHitatchiDeviceControl: IOCTL_CDROM_RESUME_AUDIO received.\n"
            ));
    ...
    case IOCTL_CDROM_READ_Q_CHANNEL:
        ...
        CdDump(( 5,
            "CdAudioHitatchiDeviceControl: IOCTL_CDROM_READ_Q_CHANNEL received.\n"
            ));

        if (SubQPtr==NULL) {

            CdDump(( 1,
                "CdAudioHitatchiDeviceControl: READ_Q_CHANNEL, SubQPtr==NULL!\n"
                ));
        ...
        if ( ((PCDROM_SUB_Q_DATA_FORMAT)userPtr)->Format!=
            ...
            CdDump(( 1,
                "CdAudioHitatchiDeviceControl: READ_Q_CHANNEL, illegal Format (%d)\n",
                ((PCDROM_SUB_Q_DATA_FORMAT)userPtr)->Format
                ));
        ...
        srb.CdbLength            = 12;
        srb.TimeOutValue         = AUDIO_TIMEOUT;
        cdb->AUDIO_STATUS.OperationCode = HITATCHI_READ_SUB_Q_CHANNEL_CODE;
        status = ScsiClassSendSrbSynchronous( deviceExtension->DeviceObject,
                                     &srb,
                                     SubQPtr,
                                     sizeof(SUB_Q_CHANNEL_DATA),
        ...
        if ((NT_SUCCESS(status)) || (status==STATUS_DATA_OVERRUN)) {
            ...
            RtlZeroMemory( userPtr, sizeof(SUB_Q_CURRENT_POSITION) );
            CdDump(( 1,
                "CdAudioHitatchiDeviceControl: READ_Q_CHANNEL failed (0x%08lX)\n",
                status
                ));
    ...
    case IOCTL_CDROM_EJECT_MEDIA:
        ...
        CdDump(( 3,
            "CdAudioHitatchiDeviceControl: IOCTL_CDROM_EJECT_MEDIA received.\n"
            ));
        ...
        srb.CdbLength            = 12;
        srb.TimeOutValue         = AUDIO_TIMEOUT;
        cdb->EJECT.OperationCode = HITATCHI_EJECT_CODE;
    ...
    case IOCTL_CDROM_SET_VOLUME:
        ...
        CdDump(( 3, "CdAudioHitatchieviceControl: Not Supported yet.\n" ));
        status = STATUS_INVALID_DEVICE_REQUEST;
        break;
    ...
    default:
        ...
        CdDump((5,"CdAudioHitatchiDeviceControl: Unsupported device IOCTL\n"));
        return CdAudioSendToNextDriver( DeviceObject, Irp );
        break;
...

{
    ...
    case IOCTL_CDROM_SEEK_AUDIO_MSF:
        ...
        cdb->SEEK.LogicalBlockAddress[0] = (destblock >> 24) & 0xFF;
        cdb->SEEK.LogicalBlockAddress[1] = (destblock >> 16) & 0xFF;
        cdb->SEEK.LogicalBlockAddress[2] = (destblock >>  8) & 0xFF;
        cdb->SEEK.LogicalBlockAddress[3] = destblock & 0xFF;
        status = ScsiClassSendSrbSynchronous( deviceExtension->DeviceObject,
                                     &srb,
                                     NULL,
                                     0,
...

{
    ...
    case IOCTL_CDROM_CHECK_VERIFY:
        ...
        //
        // This normally uses the Test_Unit_Ready (TUR) command.  The
        // CDS-435/431 respond to the TUR command with a status byte
        // of 0x08 to indicate that an audio play is in progress.  That
        // looks like SCSISTAT_BUSY, so it ends up getting retried.
        // There are a couple of solutions:
        //  a) install a class error handler that watches for
        //     Status == STATUS_DEVICE_NOT_READY
        //     and/or Srb->ScsiStatus = SCSISTAT_BUSY
        //     and converts it to Status = STATUS_SUCCESS,
        //     Srb->ScsiStatus = SCSISTAT_GOOD
        //     This is considerably more general than really necessary,
        //     since only the TUR command behaves this way.
        //  b) use another command, like ReadSubQ to get the drive's status
        //     (we don't need to transfer any data, i.e. allocation length
        //     = 0)
        //
