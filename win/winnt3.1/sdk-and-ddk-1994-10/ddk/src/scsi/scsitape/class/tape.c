...
#pragma alloc_text(init, FindScsiTapes)
#pragma alloc_text(init, CreateTapeDeviceObject)
#pragma alloc_text(init, UpdateTapeInformationInRegistry)
#endif
...

{
     ...
     //
     // Initialize fixed block size of 512 bytes.
     //
...

{
    ...
    DebugPrint((3,"ScsiTapeRead: Enter routine\n"));
    ...
    if (DeviceObject->Flags & DO_VERIFY_VOLUME &&
        ...
        DebugPrint((3,"ScsiTapeRead: Volume verfication needed\n"));
...

{
    ...
    case IOCTL_TAPE_SET_MEDIA_PARAMS:
    {
        ...
        //
        // Make sure the block size is a power of 2.
        //

        WHICH_BIT(tapeSetMediaParams->BlockSize, sectorShift);

        if (tapeSetMediaParams->BlockSize != (ULONG)(1 << sectorShift)) {
        ...
        if (NT_SUCCESS(status)) {
            ...
            deviceExtension->SectorShift = (UCHAR)sectorShift;
