...
//        DriverObject->DriverUnload = AtDiskUnloadDriver;
...

{
    ...
    PDEVICE_OBJECT nextPartitionObject;      // ptr for walking chain
    ...
    if ( !NT_SUCCESS( ntStatus ) ) {
        ...
        AtDump(
            ATERRORS,
            ("ATDISK: Couldn't create the arc name string\n")
            );
    ...
    diskExtension->StartingOffset = RtlConvertLongToLargeInteger( 0 );

    diskExtension->PartitionLength = RtlExtendedIntegerMultiply(
    ...
    if ( !NT_SUCCESS( ntStatus ) ) {
        ...
        partitionPointer = &diskExtension->PartitionObjects;
        ...
        for ( partitionNumber = 0;
            ...
            //
            // Create a symbolic link from the partition name to the
            // corresponding ARC name, to be used if we're booting off
            // the partition.  This will fail if it's not system
            // initialization time; that's fine.  The ARC name looks
            // something like \ArcName\multi(0)disk(0)rdisk(0)partition(1)
            //

            sprintf(
                arcNameBuffer,
                "%sdisk(%d)rdisk(%d)partition(%d)",
                ConfigData->ArcNamePrefix,
                ControllerNum,
                DiskNum,
                partitionNumber + 1 );
            ...
            if ( !NT_SUCCESS( ntStatus ) ) {
                ...
                AtDump(
                    ATERRORS,
                    ("ATDISK: Couldn't create the arc names for the"
                     " partition 0 partition objects\n")
                    );
                RtlFreeUnicodeString( &ntUnicodeString );
                goto AtInitializeDiskExit;
            }
...
AtInitializeDiskExit:
    ...
    if ( !NT_SUCCESS( ntStatus ) ) {
        ...
        //
        // We don't have to worry about the ARC symbolic links; we
        // already closed our handle to them, and the system will
        // delete them when boot is complete.
        //
        ...
        if ( diskExtension != NULL ) {
            ...
            nextPartitionObject = diskExtension->PartitionObjects;

            while ( nextPartitionObject != NULL ) {

                partitionObject = nextPartitionObject;
                partitionExtension =
                    ( PPARTITION_EXTENSION )( partitionObject->DeviceExtension );
                nextPartitionObject = partitionExtension->NextPartitionObject;
    ...
    switch ( irpSp->Parameters.DeviceIoControl.IoControlCode ) {
        ...
        case IOCTL_DISK_GET_PARTITION_INFO: {
            ...
            if ( irpSp->Parameters.DeviceIoControl.OutputBufferLength <
                ...
                outputBuffer->PartitionType = partitionExtension->PartitionType;

                outputBuffer->BootIndicator = partitionExtension->BootIndicator;
        ...
        case IOCTL_DISK_SET_PARTITION_INFO: {
            ...
            if ( irpSp->Parameters.DeviceIoControl.InputBufferLength <
                ...
                if ( IoSetPartitionInformation(
                    diskExtension->DeviceObject,
                    diskExtension->BytesPerSector,
                    partitionExtension->PartitionNumber,
...

{
    ...
    //
    // If the power failed, exit quietly since the notification routine
    // will fix the hardware and restart the packet.
    //

    if ( controllerExtension->PowerFailed ) {
...
Routine Description:
    ...
    BUGBUG this routine only deletes one controller, but deletes all device
    objects.  We need to figure out how unload will work.
...
{
    PCONTROLLER_EXTENSION controllerExtension;
    PDEVICE_OBJECT deviceObject, oldDeviceObject;
    PDISK_EXTENSION diskExtension;

    deviceObject = DriverObject->DeviceObject;
    diskExtension = deviceObject->DeviceExtension;
    controllerExtension = diskExtension->Partition0->ControllerExtension;

    //
    // First, get rid of the controller objects and associated items in
    // the controller extension.
    // The timer is stored in the device object of the first disk out of
    // necessity, but is a per-controller object.
    //

    IoStopTimer( controllerExtension->Disk1->DeviceObject );
    IoDisconnectInterrupt( controllerExtension->InterruptObject );
    IoDeleteController( controllerExtension->ControllerObject );

    //
    // Now follow the I/O system's chain of device objects.  This will
    // include device objects (associated with partition 0) and "partition"
    // objects (associated with partition N, for N != 0).  Deleting the
    // objects will automatically delete the extensions as well, regardless
    // of their type.
    //

    do {

        diskExtension = ( PDISK_EXTENSION )( deviceObject + 1 );

        //
        // If this extension is partition 0, then it has the handle to the
        // device directory.  Close the handle to get rid of the directory
        // (which was made to be temporary).
        //

        if ( diskExtension->Partition0 == diskExtension ) {

            ZwClose( diskExtension->DirectoryHandle );
        }

        oldDeviceObject = deviceObject;
        deviceObject = oldDeviceObject->NextDevice;
...
Arguments:
    ...
    DiskNumber - Which disk on the controller (0 or 1).
...

{
    ...
    PDRIVE_DATA driveData = &ControllerData->Disk[DiskNumber];
