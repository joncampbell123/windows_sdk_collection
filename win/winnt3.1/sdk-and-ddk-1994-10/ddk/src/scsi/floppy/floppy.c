...

    Data structures:
        ...
        fields:
            ...
            LastDisketteExtension - this value is used with
            CreatedArtificialDrive, below, to decide when to return an
            error.  If the system only has one drive, it uses this field
            to determine whether the device object describing the drive
            has changed, so it knows when to return an error so the user
            will be prompted to insert the other diskette.
            ...
            CreatedArtificialDrive - if there's only one drive in the
            system, the initialization code will create a *two* device
            objects to describe it, and set this to TRUE.  Whenever a
            drive is accessed, if this is TRUE and the device object
            being used is different, an error is returned so the user
            will be prompted to insert the other diskette.
        ...
        for the controller that it's attached to.  Fields in the structure:
            ...
            ArtificialVerifyRequired - if there's only one drive in the
            system, the user is prompted to switch drives by returning
            STATUS_NO_MEDIA_IN_DEVICE.  At that time, this value is set
            to TRUE, so that the next time STATUS_VERIFY_REQUIRED can be
            returned, to tell the file system to clean out it's caches.
...

{
    ...
    PVOID threadObject;
    CONFIG_CONTROLLER_DATA *tempData;
    NTSTATUS ntStatus;
    NTSTATUS ntStatus2;
    ULONG ioBufferSize;
    ULONG maximumDriveTrackSize;
    ...
    controllerData->ControllerConfigurable = TRUE;
    ...
    if ( NT_SUCCESS( ntStatus ) ) {
        ...
        //
        // Calculate the size of the largest track that we could possibly
        // deal with on this controller.
        //

        ioBufferSize = 0;
        tempData = &ConfigData->Controller[ControllerNumber];

        for ( driveNumber = 0;
              driveNumber < tempData->NumberOfDrives;
              driveNumber++ ) {

            //
            // Get the size out of the BIOS information store in the
            // diskette configuration information
            //

            maximumDriveTrackSize =
                tempData->BiosDriveMediaConstants[driveNumber].BytesPerSector *
                tempData->BiosDriveMediaConstants[driveNumber].SectorsPerTrack;

            if ( maximumDriveTrackSize > ioBufferSize ) {

                ioBufferSize = maximumDriveTrackSize;
            }
        }

        //
        // If we don't have enough map registers to handle the largest
        // possible track size, allocate a contiguous buffer to hold it.
        //

        if ( ( tempData->NumberOfMapRegisters * PAGE_SIZE ) < ioBufferSize ) {

            controllerData->IoBuffer = MmAllocateContiguousMemory(
                ioBufferSize,
        ...
        //
        // We need a single page to do FORMATs.  If we already allocated a
        // buffer, we'll use that.  If not, let's allocate a single page.
        // Note that we'd have to do this anyway if there's not enough map
        // registers.
        //

        if ( controllerData->IoBuffer == NULL ) {

            ioBufferSize = PAGE_SIZE;

            controllerData->IoBuffer = ExAllocatePool(
                NonPagedPool,
                ioBufferSize );
        }

        //
        // If we can't even get a page to use as a buffer, exit.
        //
        ...
        if ( controllerData->IoBuffer == NULL ) {
            ...
            //
            // We have a buffer.  Get and lock down an MDL for it now.
            //

            controllerData->IoBufferMdl = IoAllocateMdl(
            ...
            if ( controllerData->IoBufferMdl == NULL ) {
                ...
                FloppyDump(
                    FLOPSHOW,
                    ("Floppy: Buffer size = %x\n", ioBufferSize)
                    );

                controllerData->IoBufferSize = ioBufferSize;
    ...
    if ( !NT_SUCCESS( ntStatus ) ) {
        ...
        if ( controllerData->IoBuffer != NULL ) {
            ...
            if ( controllerData->IoBufferSize > PAGE_SIZE ) {
...
//
// Support for the ghost drive is being removed.  If it
// is needed later, then we can just "un" ifdef this
// code.
//

#ifdef FLOPPY_ARTIFICIAL

    } else {

        //
        // Controller and drive initialization were successful.  If
        // there's only 1 drive, set up an artificial second drive to
        // point to the same thing.
        //

        if ( ( ConfigData->NumberOfControllers == 1 ) &&
            ( controllerData->NumberOfDrives == 1 ) ) {

            //
            // Discard status, since we want to return success even
            // if this artificial creation fails.
            //

            ntStatus2 = FlInitializeDrive(
                ConfigData,
                controllerData,
                ControllerNumber,
                1,
            ...
            if ( NT_SUCCESS( ntStatus2 ) ) {
                ...
                controllerData->CreatedArtificialDrive = TRUE;
            }
        }
#endif
    }
...

{
    ...
    if ( ControllerData->PerpendicularDrives != 0 ) {
        ...
        if ( NT_SUCCESS( ntStatus ) ) {
            ntStatus = FlSendByte( COMMND_PERPENDICULAR_MODE_OW |
    ...
    if ( NT_SUCCESS( ntStatus ) ) {
        ...
        if ( NT_SUCCESS( ntStatus ) ) {
            ...
            if ( !NT_SUCCESS( ntStatus ) ) {
                ...
                //
                // In the event that the system only has one drive, we
                // want this variable to initially point to the extension
                // of the *first* device object that describes it, so the
                // user's initial access of the drive doesn't result in
                // a prompt to insert media.
                //

                if ( ( DisketteNum == 0 ) && ( DisketteUnit == 0 ) ) {

                    ControllerData->LastDisketteExtension = disketteExtension;
                }
    ...
    switch( irpSp->Parameters.DeviceIoControl.IoControlCode ) {
        ...
        case IOCTL_DISK_GET_DRIVE_GEOMETRY:
            ...
            ExInterlockedInsertTailList(
                &disketteExtension->ControllerData->ListEntry,
                &Irp->Tail.Overlay.ListEntry,
                &disketteExtension->ControllerData->ListSpinLock );

            KeReleaseSemaphore(
                &disketteExtension->ControllerData->RequestSemaphore,
                (KPRIORITY) 0,
                1,
                FALSE );
...

{
    ...
    if ( ( controllerData->CreatedArtificialDrive ) &&
        ( controllerData->LastDisketteExtension != DisketteExtension ) ) {

        //
        // There's an artificial drive, and the user has switched, so
        // pretend there's no media so the user will be prompted to
        // insert the other diskette.
        //

        controllerData->LastDisketteExtension = DisketteExtension;
        DisketteExtension->ArtificialVerifyRequired = TRUE;

        FloppyDump(
            FLOPSHOW,
            ("Floppy: start drive - artificial drive needs verification\n")
            );
        return STATUS_NO_MEDIA_IN_DEVICE;
    }

    //
    // There is no override of the artifically induced verify.
    //

    if ( DisketteExtension->ArtificialVerifyRequired == TRUE ) {
        ...
        DisketteExtension->ArtificialVerifyRequired = FALSE;
        FloppyDump(
            FLOPSHOW,
            ("Floppy: start drive - artificial verify required\n")
            );
                ...
                switch ( irpSp->MajorFunction ) {
                    ...
                    case IRP_MJ_WRITE: {
                        ...
                        if (( disketteExtension->DeviceObject->Flags &
                            ...
                            ntStatus = FlReadWrite( disketteExtension, irp );
                            ...
                            IoFreeAdapterChannel( controllerData->AdapterObject );
                            ...
                            switch (
                                ...
                                case IOCTL_DISK_FORMAT_TRACKS: {
                                    ...
                                    if ( NT_SUCCESS( ntStatus ) ||
                                        ...
                                        //
                                        // Wait for the adapter to be allocated.
                                        // No timeout; we trust the system to do
                                        // it properly - so KeWaitForSingleObject
...

{
    ...
    FloppyDump(
        FLOPIRPPATH,
        ("Floppy: Started read/write for irp %x extension %x\n",
    ...
    //
    // If the drive is not already running, wait for it to start up.  If
    // we're writing, this will check to see if the diskette is protected.
    //

    FloppyDump(
        FLOPIRPPATH,
        ("Floppy: Waiting for drive to start for irp %x extension %x\n",
         Irp,DisketteExtension)
        );
    ntStatus = FlStartDrive( DisketteExtension,
                             writeOperation,
                             TRUE,
    ...
    if ( !NT_SUCCESS( ntStatus ) ) {
        ...
        FloppyDump(
            FLOPIRPPATH,
            ("Floppy: Couldnt start drive for irp %x extension %x\n",
             Irp,DisketteExtension)
            );
        return ntStatus;
    }
    ...
    FloppyDump(
        FLOPIRPPATH,
        ("Floppy: Drive started for irp %x extension %x\n",
         Irp,DisketteExtension)
        );
    ...
    if ( DisketteExtension->MediaType == Unknown ) {
        ...
        FloppyDump(
            FLOPWARN,
            ("Floppy: read/write: media type not determined, failing\n")
            );

        FloppyDump(
            FLOPIRPPATH,
            ("Floppy: don't know media irp %x extension %x\n",
             Irp,DisketteExtension)
            );
        return STATUS_UNRECOGNIZED_MEDIA;
    ...
    //
    // Must get this pointer AFTER FlStartDrive, since DriveMediaType may
    // have been newly determined there.
    //
    ...
    //
    // Transfer portions of the request until the request is done or failed
    //

    while ( ( bytesTransferredSoFar < irpSp->Parameters.Read.Length ) &&
        !( fatalError ) ) {
        ...
        startingSectorOfTransfer =
            ( ( irpSp->Parameters.Read.ByteOffset ).LowPart +
                bytesTransferredSoFar ) >> byteToSectorShift;

        transferCylinder = (UCHAR)( startingSectorOfTransfer /
            ( driveMediaConstants->SectorsPerTrack *
            driveMediaConstants->NumberOfHeads ) );

        transferSector = (UCHAR)( ( startingSectorOfTransfer %
            driveMediaConstants->SectorsPerTrack ) + 1 );

        transferHead = (UCHAR)( ( startingSectorOfTransfer /
            driveMediaConstants->SectorsPerTrack ) %
            driveMediaConstants->NumberOfHeads );

        totalBytesOfTransfer = irpSp->Parameters.Read.Length -
        ...
        if ( ( totalBytesOfTransfer >> byteToSectorShift ) > (ULONG)
            ( driveMediaConstants->SectorsPerTrack - transferSector + 1 ) ) {

            totalBytesOfTransfer = ( driveMediaConstants->SectorsPerTrack -
                transferSector + 1 ) << byteToSectorShift;
        }

        //
        // If there aren't enough map registers to handle the whole
        // transfer, then make the transfer size equal to the size of the
        // contiguous buffer.
        //
        ...
        if ( numberOfPagesInTransfer <=
            (ULONG) controllerData->NumberOfMapRegisters ) {

            needToUseBuffer = FALSE;

        } else {

            FloppyDump(
                FLOPSHOW,
                ("Floppy: Not enough map registers, using own buffer\n")
                );
            ...
            FloppyUsedBuffer++;
#endif
            if ( totalBytesOfTransfer > controllerData->IoBufferSize ) {

                totalBytesOfTransfer = controllerData->IoBufferSize;
            }
        }
        ...
        do {
            ...
            if ( ( !controllerData->ControllerConfigurable ) ||
                ...
                if ( NT_SUCCESS( ntStatus ) ) {
                    ...
                    if ( ( !( controllerData->FifoBuffer[0] &
                            STREG0_SEEK_COMPLETE ) ||
                        ( controllerData->FifoBuffer[1] != (UCHAR)
                            ( transferCylinder <<
                            driveMediaConstants->CylinderShift ) ) ) ) {
                        ...
                        if ( !NT_SUCCESS( ntStatus ) ) {
                            ...
                            FloppyDump(
                                FLOPWARN,
                                ("Floppy: READID after seek returned %x\n",
                                 ntStatus)
                                );
                        }
                        ...
                        if ( NT_SUCCESS( ntStatus ) ) {
                            ...
                            FloppyDump(
                                FLOPSHOW,
                                ("Floppy: ReadID says we're on cylinder %x\n",
                                 controllerData->FifoBuffer[3])
                                );

                            if ( ( controllerData->FifoBuffer[0] != (UCHAR)
                                    ( ( transferHead << 2 ) |
                                    DisketteExtension->DeviceUnit ) ) ||
                                ( controllerData->FifoBuffer[1] != 0 ) ||
                                ( controllerData->FifoBuffer[2] != 0 ) ||
                                ...
                                FloppyDump(
                                    FLOPWARN,
                                    ("Floppy: READID returned bad registers\n"
                                     "------  StatusRegister0  = %x\n"
                                     "------  StatusRegister1  = %x\n"
                                     "------  StatusRegister2  = %x\n"
                                     "------  StatusRegister3  = %x\n"
            ...
            if ( NT_SUCCESS( ntStatus ) ) {
                ...
                if ( !writeOperation ) {
                    ...
                    ntStatus = FlIssueCommand(
                        COMMND_READ_DATA + COMMND_MFM,
                        DisketteExtension );

                } else {

                    ntStatus = FlIssueCommand(
                        COMMND_WRITE_DATA + COMMND_MFM,
                        DisketteExtension );
                }
        ...
        if ( NT_SUCCESS( ntStatus ) ) {
            ...
            //
            // If this was a read of the very first sector, check the
            // media descriptor byte to verify that we have the right
            // media type.
            //

            if ( ( startingSectorOfTransfer == 0 ) && ( !writeOperation ) ) {

                mediaDescriptor = *( userBuffer + MEDIA_DESCRIPTOR_OFFSET );
                FloppyDump(
                    FLOPSHOW,
                ...
                switch ( mediaDescriptor ) {
                    ...
                    case MEDIA_DESCRIPTOR_720K_OR_1220K: {
                        ...
                        if ( DisketteExtension->DriveType == DRIVE_TYPE_1200 ) {

                            FloppyDump(
                                FLOPSHOW,
                                ("FLOPPY: 720K or 1220K ambiguity - going for "
                                 "1.2\n")
                                );
                    ...
                    case MEDIA_DESCRIPTOR_1440K_OR_2880K: {
                        ...
                        if (DisketteExtension->DriveType == DRIVE_TYPE_1440) {
                            ...
                            FloppyDump(
                                FLOPSHOW,
                                ("FLOPPY: 1440/2880KB ambiguity - going for "
                                 "1440KB\n")
                                );
                            ...
                            //
                            // If the diskette media type is already 1.44
                            // then leave it that way.  The reasoning
                            // here is that if it was a 2.88 then we
                            ...
                            if (DisketteExtension->MediaType !=
                                ...
                                FloppyDump(
                                    FLOPSHOW,
                                    ("FLOPPY: 1440/2880KB ambiguity - guess"
                                     " for 2880KB\n")
                                    );
                                ...
                                FloppyDump(
                                    FLOPSHOW,
                                    ("FLOPPY: 1440/2880KB ambiguity - deduce"
                                     " 1440KB\n")
                                    );
                    ...
                    default: {
                        ...
                        FloppyDump(
                            FLOPDBGP,
                            ("Floppy: unknown media descriptor %x\n",
                              mediaDescriptor)
                            );
                    }
                ...
                if ( ( mediaType != 0 ) &&
		    ( mediaType != DisketteExtension->MediaType )  ) {

                    FloppyDump(
                        FLOPSHOW,
                        ("FLOPPY: media type doesn't agree with previous\n"
                         "------  readid determination\n")
                        );


                    driveMediaType = DriveMediaLimits[
                        DisketteExtension->DriveType].HighestDriveMediaType;

                    while ( ( DriveMediaConstants[driveMediaType].MediaType !=
                            mediaType ) &&
                        ( driveMediaType > DriveMediaLimits[DisketteExtension->
                            DriveType].LowestDriveMediaType ) ) {

                        driveMediaType--;
                    }
		    // williamh - June 11 1993
		    // make sure the data we read from the media are consistent
		    // with the new media ID we are going to use.
		    // if they do not match with the new media type,
		    // we have a bogus media and should leave it unchanged.
		    // Actually, the right thing to do here is to derive
		    // a correct media type from what we read(sector 0).
		    // I simply have no time for this.

                    if (DriveMediaConstants[driveMediaType].MediaType ==
			mediaType &&
			*(userBuffer+MEDIA_DESCRIPTOR_SECTORS_PER_TRACK) ==
			 DriveMediaConstants[driveMediaType].SectorsPerTrack &&
			*(userBuffer + MEDIA_DESCRIPTOR_NUM_OF_HEADS) ==
			 DriveMediaConstants[driveMediaType].NumberOfHeads
		       ){

                        //
                        // We only want to reset if the media types
                        // actually match up.  If they don't match
                        // up, the maximum size is as good a guess.
                        // as any.
                        //

                        FloppyDump(
                            FLOPWARN,
                            ("Floppy: New media type after lookup %x\n", mediaType)
                            );

                        DisketteExtension->MediaType = mediaType;
                      ...
                      &DriveMediaConstants[DisketteExtension->DriveMediaType];
                        ...
                        //
                        // Since the I/O returned without failure, the gap,
                        // density and wait times are correct.  Only the
                        // sectors, cylinders, and heads need to change.
                        //

                        if ((mediaDescriptor == MEDIA_DESCRIPTOR_720K_OR_1220K)
                             ||
                            (mediaDescriptor == MEDIA_DESCRIPTOR_1440K_OR_2880K)
                            ) {

                            //
                            // The media indicator for 1.22 MB and 720KB are the
                            // same, but it is possible to have a 1.44 floppy
                            // formatted as a 1.22, so use the sectors per track
                        ...
                        FloppyDump(
                            FLOPDBGP,
                            ("Floppy: Bogus media descriptor for size\n"
                             "------  Guessing that max is ok\n")
                            );
            ...
            bytesTransferredSoFar += totalBytesOfTransfer;
        }

    }                        // while more transfers to be made for this request
    ...
    Irp->IoStatus.Information = bytesTransferredSoFar;
...

{
    ...
    do {
        ...
        for ( whichSector = 1;
            whichSector <=
                (UCHAR)( driveMediaConstants->SectorsPerTrack );
            whichSector++ ) {

            controllerData->IoBuffer[( whichSector - 1 ) * 4] =
                currentTrack / driveMediaConstants->NumberOfHeads;
            controllerData->IoBuffer[( whichSector - 1 ) * 4 + 1] =
                currentTrack % driveMediaConstants->NumberOfHeads;
            controllerData->IoBuffer[( whichSector - 1 ) * 4 + 2] =
                whichSector;
            controllerData->IoBuffer[( whichSector - 1 ) * 4 + 3] =
                driveMediaConstants->SectorLengthCode;
