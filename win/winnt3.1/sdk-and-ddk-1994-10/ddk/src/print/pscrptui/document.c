...
extern VOID vDoColorAdjUI(LPSTR, COLORADJUSTMENT *, BOOL, BOOL);
...
//!!! we will at some point.  -kentse.
    ...
    if (fMode & DM_PROMPT)
    {
        ...
        pwRealDeviceName = (PWSTR)((PSTR)DocDetails.pntpd +
                                   DocDetails.pntpd->lowszPrinterName);

        WideCharToMultiByte(CP_ACP,
                            0,
                            (LPWSTR)pwRealDeviceName,
                            (int)(wcslen(pwRealDeviceName) + 1),
                            DP_AnsiDeviceName,
                            sizeof(DP_AnsiDeviceName),
                            NULL,
                            NULL);
        ...
        case WM_COMMAND:
            ...
            switch(LOWORD(wParam))
            {
                ...
                case IDD_HALFTONE_PUSH_BUTTON:
                    ...
                    vDoColorAdjUI(DP_AnsiDeviceName, &pDocDetails->coloradj,
