...
extern BOOL CreatePFMFromAFM(PWSTR, PWSTR);
    ...
    switch (message)
    {
        case WM_INITDIALOG:
            ...
            SendDlgItemMessage (hwnd, IDD_NEW_FONT_DIR_EDIT_BOX, EM_LIMITTEXT,
                                MAX_PATH, 0);
            LoadString(hModule, (IDS_DEFAULT_FONT_DIR + STRING_BASE),
            ...
            *pwstrPath = '\0';
            ...
            LoadString(hModule, (IDS_ALL_PFM_FILES + STRING_BASE),
            ...
            if ((hFile == (HANDLE)-1) || (cInstalledFonts == 0))
            {
                ...
                LoadString(hModule, (IDS_NO_INSTALLED + STRING_BASE),
                           wstringbuf, (sizeof(wstringbuf) / 2));
            ...
            switch (LOWORD(wParam))
            {
                case IDD_OPEN_BUTTON:
                    ...
                    *pwstrPath = '\0';
                    ...
                    LoadString(hModule, (IDS_ALL_PFB_FILES + STRING_BASE),
                               wstringbuf, (sizeof(wstringbuf) / 2));
                ...
                case IDD_ADD_BUTTON:
                    ...
                    *pwstrPath = '\0';
                    ...
                    // now that we have the source directory, concat "*.AFM"
                    // to it.
                    ...
                    // files, it is time to create the .PFM file from the
                    // .AFM and .PFA files.

                    // first find the .AFM files.
                    ...
                    *pwstrPath = (WCHAR)'\0';
                    ...
                    LoadString(hModule, (IDS_ALL_AFM_FILES + STRING_BASE),
                               wstringbuf, (sizeof(wstringbuf) / 2));

                    wcsncat(wcbuf, wstringbuf, (sizeof(wcbuf) / 2) - cwBuf);

                    if (!LocateFile(hwnd, wcbuf, IDS_AFM_NOT_FOUND,
                                    &FileFindData, &hFile))
                    {
                        SetCursor(hCursor);
                        return(FALSE);
                    ...
                    // reset values to deal with .AFM files.

                    cToAdd = cSave;
                    pEntries = pSave;

                    // indicate we have not found all the .AFM files.

                    bAllAFMs = FALSE;

                    // allocate a bit array to keep track of which
                    // .AFM files have been found.

                    pjbitarray = (BYTE *)LocalAlloc(LPTR, (cToAdd + 7) / 8);

                    if (!pjbitarray)
                    {
                        RIP("PSCRPTUI!IDD_ADD_BUTTON: LocalAlloc for pjbitarray failed.\n");
                        SetCursor(hCursor);
                        return(FALSE);
                    }
                    ...
                    memset((PVOID)pjbitarray, 0, (cToAdd + 7) / 8);

                    // create a new .PFM file from each .AFM file and delete
                    ...
                    while(!bAllAFMs)
                    {
                        ...
                        for (i = 0; i < cToAdd; i++)
                        {
                            ...
                            if ((BYTE)pjbitarray[i >> 3] & (BYTE)(1 << (i & 0x07)))
                            {
                                pEntries++;
                                continue;
                            }
                            ...
                            // pointer and create a pointer to .AFM filename.

                            pwstrAFMFile = (PWSTR)SendDlgItemMessage(hwnd,
                                                         IDD_NEW_FONT_LIST_BOX,
                                                         LB_GETITEMDATA,
                                                         index, 0);

                            if (pwstrAFMFile == (PWSTR)LB_ERR)
                            {
                                RIP("FontInstDlgProc: LB_GETITEMDATA failed.");
                                LocalFree((LOCALHANDLE)pSave);
                                return(FALSE);
                            ...
                            pwstrSave = pwstrAFMFile;

                            // pwstrAFMFile is actually pointing to the .PFB
                            // filename. so overwrite the .PFB with .AFM.

                            while (*pwstrAFMFile)
                                pwstrAFMFile++;

                            // backup and overwrite PFB with AFM.

                            pwstrAFMFile -= 3;
                            *pwstrAFMFile++ = (WCHAR)'A';
                            *pwstrAFMFile++ = (WCHAR)'F';
                            *pwstrAFMFile = (WCHAR)'M';

                            // reset pointer to beginning of string.

                            pwstrAFMFile = pwstrSave;

                            // build the fully qualified pathname for the .PFM
                            // file to create.
                            ...
                            // pwstrAFMFile now points to the filename itself, so
                            // concat it with the directory.

                            cwBuf = wcslen(wcbuf);

                            wcsncat(wcbuf, pwstrAFMFile, (sizeof(wcbuf) / 2) - cwBuf);
                            pwstrAFMFile = pwstrSave;

                            // now change the .AFM to .PFM.
                            ...
                            // back up to the 'A', and overwrite it with 'P'.

                            pwstrPFMFile -= 3;
                            *pwstrPFMFile = (WCHAR)'P';
                            ...
                            // create a .PFM file from the .AFM file.  if
                            // this fails, it is most like due to the fact
                            // that this .AFM file is on another disk.  so,
                            // just go to the next one for now.

                            if (!CreatePFMFromAFM(pwstrAFMFile, pwstrPFMFile))
                            {
                                pEntries++;
                                continue;
                            }

                            // we have now created both the and .PFM file
                            ...
                            LocalFree((LOCALHANDLE)pwstrAFMFile);

                            // allocate memory to store away the .PFM filename.

                            if (!(pwstrPFMFile = (PWSTR)LocalAlloc(LPTR, wcslen(wcbuf) + 1)))
                            {
                            ...
                            (BYTE)pjbitarray[i >> 3] |= (BYTE)(1 << (i & 0x07));
                        ...
                        // see if we have found all the .AFM files.
                        ...
                        {
                            LoadString(hModule,
                                       (IDS_MORE_AFMS_NEEDED + STRING_BASE),
                                       wstringbuf, (sizeof(wstringbuf) / 2));
                    ...
                    LocalFree((LOCALHANDLE)pSave);
                    LocalFree((LOCALHANDLE)pjbitarray);
...
//--------------------------------------------------------------------------
// BOOL InsertInstalledFont(hwnd, pwstrPFMFile)
// HWND    hwnd;
// PWSTR   pwstrPFMFile;
//
...
PWSTR   pwstrPFMFile;
{
    ...
    // open the given .PFM file.

    if (!(pntfm = (PNTFM)MapFile(pwstrPFMFile)))
    {
    ...
    pSoftFont = (CHAR *)pntfm + pntfm->loSoftFont;
    ...
    *pBuffer = '\0';
    ...
    // create local copy of file name, since the copy pwstrPFMFile will probably
    // no longer exist when we need to access it.

    if (!(pwstrFileName = (PWSTR)LocalAlloc(LPTR, (wcslen(pwstrPFMFile) + 1) * 2)))
    {
    ...
    wcsncpy(pwstrFileName, pwstrPFMFile, wcslen(pwstrPFMFile) + 1);
...
//!!!  or what???  -kentse.
    LoadStringA(hModule, IDS_FULLNAME + STRING_BASE, buf, sizeof(buf));
...
//!!!  or what???  -kentse.
    ...
    LoadStringA(hModule, (IDS_FONTNAME + STRING_BASE), buf, sizeof(buf));
    ...
    while (TRUE)
    {
        *phFile = FindFirstFile(pwstrSearchFile, pFileFindData);
        ...
        if (*phFile == (HANDLE)-1)
        {
            ...
            {
                LoadString(hModule,
                           (IDS_PATH_NOT_FOUND + STRING_BASE),
                           wstringbuf, (sizeof(wstringbuf) / 2));
