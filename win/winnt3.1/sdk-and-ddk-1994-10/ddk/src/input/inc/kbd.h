...
*    CAPLOK    - The CAPS-LOCK/SHIFT-LOCK key affects this key like SHIFT
*
...
/***************************************************************************\
*
* VK_F    - associate a Virtual Key with a bitmask
*
* Uses:
*   Special handle of Virtual Keys - fake different VK sequences
*
* Vk      - A virtual key that is to produce key event simulation.
* fKEProc - A bitmask identifying the Key Event simulator procedure(s) to use.
*           The postion of each set bit is an index into a table of procedure
*           addresses.  Using a bitmask here allows one key to have more than
*           one Key Event simulator procedure.
*
\***************************************************************************/
typedef struct {
    BYTE Vk;
