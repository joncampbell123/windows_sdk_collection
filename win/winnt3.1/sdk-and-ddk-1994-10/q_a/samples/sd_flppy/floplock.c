...
service_main(DWORD dwArgc, LPTSTR *lpszArgv)
{
    ...
    *
...

    {
      ...
      if(!LookupAccountName((LPSTR)NULL, /* local name */
                            ...
                            &dwDomainName,
                            psnuType))
      { StopSimpleService("LookupAccountName");
      }

      if (*psnuType != SidTypeAlias)
      { StopSimpleService("LookupAccountName returned the wrong SID type");
      }
    }

    /************************************************************************\
    *
    * Get SID of Power Users, which is not defined on Windows NT Advanced
    *   Server machines, so if we fail to get the SID, simply record that
    *   fact, and don't try to use the Power Users' SID later
    *
    \************************************************************************/

    {
      ...
      bGotPowerUsersSid =
          LookupAccountName((LPSTR)NULL, /* local name */
                            "Power Users",
                            psidPowerUsers,
                            &dwSID,
                            lpszDomain,
    ...
    *
    ...
    if     (UNLOCK_AT_SERVICE_STOP && bGotPowerUsersSid)
    { if (!AddAccessAllowedAce(pNewDACL,
                               ACL_REVISION2,
                               FILE_ALL_ACCESS,
                               psidPowerUsers))
