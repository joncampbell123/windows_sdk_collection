...
/*
 * defines used for compile time reminder messages
 */
#define QUOTE(x) #x
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            else if( current_dialog == 0 )
            {
                ...
                SetInfoText(hDlg, szBuffer);
                /*
                 * DSETUP_REQUIRESD3D flag is set if the application must have
                 * D3D installed to run.  This may not be the case on some versions
                 * Windows NT.  If this flag is not set you will still get the
                 * error message DSETUPERR_NTWITHNO3D returned from DirectXSetup
                 * but the user will not get a dialog box.
                 * If you set this flag DirectXSetup will inform the end-user
                 * that Direct3D is not present on their Windows NT system.
                 */
                #ifdef NODIRECT3D
                    #pragma message( REMIND( "This code calls DirectXSetup WITHOUT the" ))
                    #pragma message( REMIND( "DSETUP_REQUIRESD3D flag set." ))
                    #pragma message( REMIND( "You should set this flag if your app" ))
                    #pragma message( REMIND( "requires Direct3D to run." ))
                    #pragma message( REMIND( "You can do this by removing the -DNODIRECT3D" ))
                    #pragma message( REMIND( "flag from the compiler command line" ))

                    result = DirectXSetup( hDlg, NULL, DSETUP_DIRECTX );
                    /*
                     * if you don't need Direct3D you must have the following code in place:
                     */
                     if( result == DSETUPERR_NTWITHNO3D )
                     {
                         result = 0;
                     }
                #else
                    #pragma message( REMIND( "This code calls DirectXSetup with the" ))
                    #pragma message( REMIND( "DSETUP_REQUIRESD3D flag set." ))
                    #pragma message( REMIND( "You should remove this flag if you are" ))
                    #pragma message( REMIND( "not using Direct3D." ))
                    #pragma message( REMIND( "You can do this by adding -DNODIRECT3D to" ))
                    #pragma message( REMIND( "the compile flags" ))

                    result = DirectXSetup( hDlg, NULL, DSETUP_REQUIRESD3D | DSETUP_DIRECTX );
                #endif
                SetInfoText(hDlg, "");
