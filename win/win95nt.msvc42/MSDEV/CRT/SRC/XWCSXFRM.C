        ...
        )
{
        ...
        if (handle == _CLOCALEHANDLE) {
            ...
            if (_n2 <= _n1)
                /* what is wmemcpy?  The following used to be that. */
                memcpy(_string1, _string2, _n2 * sizeof (wchar_t));
                return _n2;
        }
#endif  /* _MT */
        ...
        if (0 == (size = __crtLCMapStringW(handle, LCMAP_SORTKEY,
            _string2, _n2, (wchar_t *)bbuffer, _n1, coll_codepage)))
        {
                /* buffer not big enough, get size required. */

            if (0 == (size = __crtLCMapStringW(handle, LCMAP_SORTKEY,
                _string2, _n2, NULL, 0, coll_codepage)))
