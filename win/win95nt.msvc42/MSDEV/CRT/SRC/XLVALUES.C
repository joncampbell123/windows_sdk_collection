...
#define NBITS	64
 ...
 #endif
		/* static data */
const _Dconst _LDenorm = {INIT3(0, 0, 1)};
const _Dconst _LEps = {INIT(_LBIAS - NBITS - 1, 0x8000)};
 #if _LONG_DOUBLE_HAS_HIDDEN_BIT
const _Dconst _LInf = {INIT(_LMAX, 0)};
const _Dconst _LNan = {INIT(_LSIGN | _LMAX, 0x8000)};
const _Dconst _LSnan = {INIT(_LSIGN | _LMAX, 0)};
 #else
const _Dconst _LInf = {INIT(_LMAX, 0x8000)};
const _Dconst _LNan = {INIT(_LSIGN | _LMAX, 0xc000)};
const _Dconst _LSnan = {INIT(_LSIGN | _LMAX, 0x8000)};
 #endif
const _Dconst _LRteps = {INIT(_LBIAS - NBITS / 2, 0x8000)};
const long double _LXbig = (NBITS + 1) * 347L / 1000;
...
_DYNAMIC_INIT_CONST(_LRteps);
 ...
 #endif
		/* static data */
const _Dconst _LDenorm = {INIT2(0, 1)};
const _Dconst _LEps = {INIT((_DBIAS - NBITS - 1) << _DOFF)};
const _Dconst _LInf = {INIT(_DMAX << _DOFF)};
const _Dconst _LNan = {INIT(_DSIGN | (_DMAX << _DOFF)
	| (1 << _DOFF - 1))};
const _Dconst _LRteps = {INIT((_DBIAS - NBITS / 2) << _DOFF)};
const _Dconst _LSnan = {INIT(_DSIGN | (_DMAX << _DOFF))};
const long double _LXbig = (NBITS + 1) * 347L / 1000;
