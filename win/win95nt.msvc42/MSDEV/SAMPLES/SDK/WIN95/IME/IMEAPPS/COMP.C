...
void MakePaintString(HWND hWnd, LPSTR lpStr,DWORD dwStrLen,LPDWORD lpCls,DWORD dwClsLen,LPSTR lpPaintStr)
{
    LPSTR lpPaintStart = lpPaintStr;
    ...
    if (dwStrLen)
    {
        if (dwClsLen)
        {
            ...
            while (*(lpCls+1) && *lpCls < dwStrLen)
            {
                ...
                LPSTR lpT = lpStr + *lpCls;

                memcpy(lpPaintStr,lpT,dwTextLen);
...
LRESULT HandleComposition(HWND hWnd,WPARAM wParam,LPARAM lParam)
{
    ...
    if (lParam & GCS_RESULTREADSTR)
    {
        ...
        szResultReadStr[dwResultReadStrLen] = '\0';
