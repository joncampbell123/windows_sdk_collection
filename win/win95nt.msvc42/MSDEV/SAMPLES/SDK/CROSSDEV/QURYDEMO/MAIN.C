/*
	...
	MDIChildDlgProc() - processes messages for dummy child dialog box in MDI child window
...
UINT		uStatusText;				// Status text state
...
char		szAppName[MAXBUFLEN];		// Appname.
...
int PASCAL WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	LoadString(hInstance, IDS_APPLICATION, szAppName, sizeof(szAppName));
	...
	wc.hIcon = hMainIcon = LoadIcon(hAppInstance, MAKEINTRESOURCE(APPICON));
	wc.hCursor = LoadCursor((HINSTANCE)NULL, IDC_ARROW);
	wc.hbrBackground = (HBRUSH)(COLOR_APPWORKSPACE+1);
	wc.lpszMenuName =  MAKEINTRESOURCE(QURYDEMOMENU);
	...
	wc.hIcon = hMDIChildIcon = LoadIcon(hAppInstance, MAKEINTRESOURCE(QUERYWINDOWICON));
	...
	hAccel = LoadAccelerators(hAppInstance, MAKEINTRESOURCE(APPACCELERATOR));


	#ifdef _MAC
    // Initialize Apple Menu About...
	{
		HMENU	hmenuSys;
		CHAR 	szAbout[80];
		...
		ModifyMenu(hmenuSys, 0, MF_BYPOSITION, IDM_ABOUT, szAbout);
    }
	#endif
...
long CALLBACK MainWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static int	iStatusY;       //remember Statusbar height for resize
		...
		// Create child windows
                //1. combobox to display connections - DSN, HDBC
                //2. combobox to display HSTMT on current HDBC
		...
		hWndToolbar = CreateWindow(ODBCTOOLCLASS, NULL, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS,
			0, 0, 0, 0, hWnd, (HMENU)IDW_TOOLBAR, hAppInstance, NULL);

		hWndStatusbar = CreateWindow(ODBCSTATUSCLASS, NULL, WS_CHILD | WS_VISIBLE |
		...
		#else
		iToolY = BTTNHEIGHT + TOOLBARMARGINY;
		#endif
	...
	//	Ctl3dColorChange();
	...
	#ifdef _MAC
		if (lParam ==0 && LOWORD(wParam) == IDM_ABOUT)
        {
		...
		for (nPos = 0; nPos < nItems; nPos++)
			...
			EnableMenuItem((HMENU)wParam, iMenuId, MF_BYCOMMAND|
				((SendMessage(hWndCrsrList, CB_GETCOUNT, 0, 0) > 0)?MF_ENABLED:MF_GRAYED));
			break;
			...
			EnableMenuItem((HMENU)wParam, iMenuId, MF_BYCOMMAND|
				(GetWindow(hWndMDIClient, GW_CHILD)?MF_ENABLED:MF_GRAYED));
			break;
		...
		case WM_MENUSELECT: // update status bar to reflect menu selection
		{
			...
			else if (uStatusText && (fMenuFlag & MF_POPUP))
			{
				hMenu =  GET_WM_MENUSELECT_HMENU(wParam, lParam);
				idMenu = GetMenuItemID(hMenu, 0);
				switch (idMenu)
				{
					case IDM_POPUPLOGIN:
					case IDM_POPUPQUERY:
					...
					default:
						if (GetMenuString(hMenu, SC_NEXTWINDOW, szMenuName, MAXBUFLEN, MF_BYCOMMAND)>0)
							uStatusText = IDM_POPUPMDISYS;
						else
							uStatusText = 0;
				}
		...
		{
		FARPROC	lpDlgProc;      //Procedure address for dialogbox

			case IDM_CONNECT: // bring up connect dialog & do connect processing

			lpDlgProc = MakeProcInstance(ConnectDlgProc, hAppInstance);
			DialogBox(hAppInstance, MAKEINTRESOURCE(CONNECTDIALOG), hWnd,lpDlgProc);
			...
			lpDlgProc = MakeProcInstance(DisconnectDlgProc, hAppInstance);
			DialogBox(hAppInstance, MAKEINTRESOURCE(DISCONNECTDIALOG), hWnd,lpDlgProc);
			FreeProcInstance(lpDlgProc);
			break;
			...
			WinHelp(hWnd, SAMPLESHELPFILE, HELP_CONTEXT, HLP_QURYDEMO);
			break;
            
			case IDM_ABOUT: // bringup About dialog

			lpDlgProc = MakeProcInstance(AboutDlgProc, hAppInstance);
			DialogBox(hAppInstance, MAKEINTRESOURCE(ABOUTDIALOG), hWnd, lpDlgProc);
			FreeProcInstance(lpDlgProc);
			break;
		...
		return (CloseDatabases() ? DefFrameProc(hWnd, hWndMDIClient, message, wParam, lParam) : FALSE);
...
long CALLBACK MDIChildProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	HWND	hWndTemp;	//temporary window handle
		...
		lpDlgProc = MakeProcInstance(MDIChildDlgProc, hAppInstance);
		hWndTemp = CreateDialog(hAppInstance, MAKEINTRESOURCE(MDICHILDDIALOG), hWndActiveChild, lpDlgProc);
		...
		MoveWindow(GetDlgItem(hWndTemp, IDLIST_RSLT), -1, size.cy+3, wWidth + 2, wHeight - (size.cy+2), TRUE);
		break;
                }
...
long CALLBACK ToolbarProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static int	nLastButtonDown = 0;		//Which btn was down before?
		...
		hbmpNewQuery = LoadBitmap(hAppInstance, MAKEINTRESOURCE(BMP_NEWQUERY));
		hbmpRunQuery = LoadBitmap(hAppInstance, MAKEINTRESOURCE(BMP_RUNQUERY));
		...
		if (PtInRect(&stNewQuery, stMousePosition))
		{
			...
			uStatusText = nLastButtonDown = IDM_NEW;
			SetCapture(hWnd);
		...
		else if (PtInRect(&stRunQuery, stMousePosition))
		{
			...
			uStatusText = nLastButtonDown = IDM_QUERY;
			SetCapture(hWnd);
...
long CALLBACK StatusbarProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		...
		case WM_CREATE:
        {
			...
			if (hDC = GetDC(hWnd))
			{
				char szText[MAXBUFLEN];
				
				LoadString(hAppInstance, IDS_STATUSNEW, szText, sizeof(szText));
				GetTextExtentPoint(hDC, szText, lstrlen(szText), &sizeText);
				
				LoadString(hAppInstance, IDS_TIMETEXT, szText, sizeof(szText));
				GetTextExtentPoint(hDC, szText, lstrlen(szText), &sizeTime);

				LoadString(hAppInstance, IDS_DATETEXT, szText, sizeof(szText));
				GetTextExtentPoint(hDC, szText, lstrlen(szText), &sizeDate);
		...
		case WM_PAINT:
        {
			...
			// value of uStatusText global flag
			...
			switch (uStatusText)
			{
				case IDM_POPUPAPPSYS:
					LoadString(hAppInstance, IDS_STATUSPOPUPAPPSYS, szText, sizeof(szText));
					break;
			
				case IDM_POPUPMDISYS:
					LoadString(hAppInstance, IDS_STATUSPOPUPMDISYS, szText, sizeof(szText));
					break;
			
				case SC_RESTORE:
					LoadString(hAppInstance, IDS_STATUSRESTORE, szText, sizeof(szText));
					break;
				...
				case SC_SIZE:
					LoadString(hAppInstance, IDS_STATUSSIZE, szText, sizeof(szText));
					break;
			
				case SC_MINIMIZE:
					LoadString(hAppInstance, IDS_STATUSMINIMIZE, szText, sizeof(szText));
					break;
			
				case SC_MAXIMIZE:
					LoadString(hAppInstance, IDS_STATUSMAXIMIZE, szText, sizeof(szText));
					break;
			
				case SC_CLOSE:
					LoadString(hAppInstance, IDS_STATUSCLOSE, szText, sizeof(szText));
					break;
			
				case SC_NEXTWINDOW:
					LoadString(hAppInstance, IDS_STATUSNEXTWINDOW, szText, sizeof(szText));
					break;
			
				case SC_PREVWINDOW:
					LoadString(hAppInstance, IDS_STATUSPREVWINDOW, szText, sizeof(szText));
					break;
			
				case SC_TASKLIST:
					LoadString(hAppInstance, IDS_STATUSTASKLIST, szText, sizeof(szText));
					break;
			
				case IDM_POPUPLOGIN:
					LoadString(hAppInstance, IDS_STATUSPOPUPLOGIN, szText, sizeof(szText));
					break;
			
				case IDM_CONNECT:
					LoadString(hAppInstance, IDS_STATUSCONNECT, szText, sizeof(szText));
					break;
			
				case IDM_DRIVERCONNECT:
					LoadString(hAppInstance, IDS_STATUSDRIVERCONNECT, szText, sizeof(szText));
					break;
			
				case IDM_DISCONNECT:
					LoadString(hAppInstance, IDS_STATUSDISCONNECT, szText, sizeof(szText));
					break;
			
				case IDM_EXIT:
					LoadString(hAppInstance, IDS_STATUSEXIT, szText, sizeof(szText));
					break;
			
				case IDM_POPUPQUERY:
					LoadString(hAppInstance, IDS_STATUSPOPUPQUERY, szText, sizeof(szText));
					break;
			
        		case IDM_QUERY:
					LoadString(hAppInstance, IDS_STATUSQUERY, szText, sizeof(szText));
	        		break;
				...
				case IDM_POPUPWINDOW:
					LoadString(hAppInstance, IDS_STATUSPOPUPWINDOW, szText, sizeof(szText));
					break;
			
				case IDM_TILE:
					LoadString(hAppInstance, IDS_STATUSTILE, szText, sizeof(szText));
					break;
			
				case IDM_CASCADE:
					LoadString(hAppInstance, IDS_STATUSCASCADE, szText, sizeof(szText));
					break;
			
				case IDM_ICONS:
					LoadString(hAppInstance, IDS_STATUSICONS, szText, sizeof(szText));
					break;
			
				case IDM_CLOSEALL:
					LoadString(hAppInstance, IDS_STATUSCLOSEALL, szText, sizeof(szText));
					break;
			
				case IDM_POPUPHELP:
					LoadString(hAppInstance, IDS_STATUSPOPUPHELP, szText, sizeof(szText));
					break;
			
				case IDM_APPHELP:
					LoadString(hAppInstance, IDS_STATUSAPPHELP, szText, sizeof(szText));
					break;
			
				case IDM_ABOUT:
					LoadString(hAppInstance, IDS_STATUSABOUT, szText, sizeof(szText));
					break;
			
				default:
					if (uStatusText >= IDM_MDICHILD)
					{
						char szFormat[MAXBUFLEN];
						LoadString(hAppInstance, IDS_STATUSMDICHILD, szFormat, sizeof(szFormat));
						wsprintf(szText, szFormat, uStatusText-IDM_MDICHILD+1);
					}
					else	
						LoadString(hAppInstance, IDS_STATUSDEFAULT, szText, sizeof(szText));
					break;
				}
...
/*
	...
	COMMENTS: Callback dialog box procedure for disconnect dialog.
		  provides a list of available HDBCs and a list of HSTMTs
		  for currently selected HDBC. Allows closure of all HDBCs
