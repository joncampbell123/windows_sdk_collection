...
VOID SelectFile(register HWND hwnd)
{
    register PSTR pch;
#if !defined(_MAC)    
    PSTR          pch2;
...
/****************************************************************************
 *                                                                          *
 *  FUNCTION   : GetFileName (LPSTR lpPath, LPSTR lpName)                   *
 *                                                                          *
...
BOOL GetFileName(LPSTR lpPath, LPSTR lpName)
{
    ...
    DialogBoxParam (hInst, IDD_FILEOPEN, hwndFrame, FileOpenDlgProc, (LONG)lpPath);
	lstrcpy(lpName, lpPath);
#else
    OPENFILENAME ofn;
#if defined(_MAC)
    ...
    if (!GetOpenFileName ((LPOPENFILENAME)&ofn)){
        ...
        *lpName = 0;
		if (CommDlgExtendedError() != 0)
			MessageBox(hwndFrame, " FileOpen FAILed!", "Multipad", MB_OK);
        return FALSE;
    }

#if defined(_MAC)
    lstrcpy(lpName, szFileTitle);
#else
    lstrcpy(lpName, szFileName);
#endif
    memcpy(lpPath, szFileName, MAXFILENAME);
