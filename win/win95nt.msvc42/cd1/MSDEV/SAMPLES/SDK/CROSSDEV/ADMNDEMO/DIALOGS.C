...
BOOL CALLBACK	CatalogWndProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam);
...
void DoList(CHILDINFO FAR * ci, int idMenuSelection)
{
	...
	// Display the dialog
	dlgproc = MakeProcInstance(CatalogWndProc, cati.hInst);
	if(-1 == DialogBoxParam(cati.hInst, 
			MAKEINTRESOURCE(Dialogs[dex].iDialogName),
			ci->hwnd, 
			dlgproc, (LPARAM)(CATALOGINFO FAR *)lpcati))
