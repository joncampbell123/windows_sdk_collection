...
//*---------------------------------------------------------------------------------
BOOL FindTerminator(LPSTR str, LPSTR term, LPSTR * nxtstr);
BOOL NotInQuote(LPSTR str, LPSTR tar);
BOOL ValidSQLStmt(LPSTR str);
BOOL CALLBACK ExecuteFileWndProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam);
void InternalDestroyResultsWindow(CHILDINFO FAR * ci, RESULTSSET FAR * rs);
void DestroyResultsWindow(CHILDINFO FAR * ci, lpRESULTSINFO lpri);
...

{
	...
	if(!szExeFile) {
		dlgproc = MakeProcInstance(ExecuteFileWndProc, ef.hInst);
		if(-1 == DialogBoxParam(ci->hInst, 
				MAKEINTRESOURCE(IDD_EXECUTE_FILE),
				hwnd, 
				dlgproc, (LPARAM)(EXECUTEFILE FAR *)&ef))
	...
	if((hf = _lopen(ef.szFile, OF_READ)) != HFILE_ERROR) {
		...
		szMessageBox(GetActiveWindow(),
						MB_ICONEXCLAMATION,
						szOPENFILE,
						GetidsString(idsOpenFileFailed, szBuff, sizeof(szBuff)),
						(LPSTR)ef.szFile);
		}
...
void DoCommitRollback(CHILDINFO FAR * ci, int type)
{
	...
	switch(type) {
		case IDM_COMMIT:
			retcode = SQLTransact(ci->henv, ci->hdbc, SQL_COMMIT);
			break;
			
		case IDM_ROLLBACK:
			retcode = SQLTransact(ci->henv, ci->hdbc, SQL_ROLLBACK);
			break;
		}
...
//*---------------------------------------------------------------------------------
BOOL CreateResultsWindow(CHILDINFO FAR * ci, lpRESULTSSET rs)
{
...
//|	Nothing.
//*---------------------------------------------------------------------------------
void DestroyResultsWindow(lpCHILDINFO lpci, lpRESULTSINFO lpri)
{
	//
...
BOOL SetMetaDataFromSql(HWND hwndOut, HSTMT hstmt, RESULTSSET FAR * rs, int cbCols)
{
	...
	for(dex=0;  dex<cbCols;  dex++) {
		...
		if(retcode != SQL_SUCCESS) 
			PrintErrorsHwnd(hwndOut, NULL, NULL, hstmt);
		else 
...
//*------------------------------------------------------------------------
SDWORD FetchAllRecordsToResults
(
	HWND			hwndOut,
	HSTMT			hstmt,
	RESULTSSET FAR*	rs,
