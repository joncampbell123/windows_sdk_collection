...
void pascal TrackScroll (theScroll, partCode)
...
Integer	mods;
{
	...
	if ((thePart = TestControl (helpScroll, thePt)) == inThumb)
	{
		...
		DoScroll ((Integer)(GetCtlValue (helpScroll) - helpLine));
	}
	else if (thePart != 0)
	{
		SetCRefCon (helpScroll, (Longint) thePart);
		(void) TrackControl (helpScroll, thePt, NewControlActionProc((ControlActionUPP)TrackScroll));
	}
	return 0;
}
