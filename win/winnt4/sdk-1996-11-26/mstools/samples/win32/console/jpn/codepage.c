...
void demoOutputCodePage(HANDLE hConOut)
{
  ...
  myPuts(hConOut, "警告: コンソールの中で TrueType フォントを使用していない場合は、\n"
         "コード ページ情報が正しく表示されない可能性があります。システム メニューの\n"
         "[フォントの指定]を選択して、利用可能なリストの中から TrueType フォントを\n"
         "選択してください。");
...
{
  myPuts(hConOut, "SetConsoleCP と GetConsoleCP のデモ");
  myPuts(hConOut, "未だインプリメントされていません。");
  myPuts(hConOut, "\n\n続けるには Enter を押してください...");
  myGetchar();
  return;
}
