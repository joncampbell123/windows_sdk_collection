...
PCHAR conAPIs[] = {
  "AllocConsole                    現在の ﾌﾟﾛｾｽ の為に ｺﾝｿｰﾙ を作成します",
  "CreateConsoleScreenBuffer       新しい ｽｸﾘｰﾝ ﾊﾞｯﾌｧ の ﾊﾝﾄﾞﾙ を返します",
  "FillConsoleOutputAttribute      ｽｸﾘｰﾝ ﾊﾞｯﾌｧ に ｱﾄﾘﾋﾞｭｰﾄ を書き込みます",
  "FillConsoleOutputCharacter      ｽｸﾘｰﾝ ﾊﾞｯﾌｧ に文字を書き込みます。",
  "FlushConsoleInputBuffer         ｺﾝｿｰﾙ ｲﾝﾌﾟｯﾄ ﾊﾞｯﾌｧ を ｸﾘｱ します",
  "FreeConsole                     現在の ｺﾝｿｰﾙ を解放します",
  "GenerateConsoleCtrlEvent        指定の ﾌﾟﾛｾｽ に ｼｸﾞﾅﾙ を送ります",
  "GetConsoleCP                    入力 ｺｰﾄﾞ ﾍﾟｰｼﾞの識別 ｺｰﾄﾞを返します",
  "GetConsoleOutputCP              出力 ｺｰﾄﾞ ﾍﾟｰｼﾞの識別 ｺｰﾄﾞを返します",
  "GetConsoleCursorInfo            ｶｰｿﾙ の ｻｲｽﾞと可視性を返します",
  "GetConsoleMode                  ｺﾝｿｰﾙ の入力／出力 ﾓｰﾄﾞを返します",
  "GetConsoleScreenBufferInfo      ｽｸﾘｰﾝ ﾊﾞｯﾌｧ の情報を返します",
  "GetConsoleTitle                 ｺﾝｿｰﾙ ｳｨﾝﾄﾞｳ の ﾀｲﾄﾙ を返します",
  "GetLargestConsoleWindowSize     可能な ｳｨﾝﾄﾞｳ ｻｲｽﾞの最大値を返します",
  "GetNumberOfConsoleInputEvents   ｺﾝｿｰﾙ　ｷｭｰ ｲﾍﾞﾝﾄ の数を得ます",
  "GetNumberOfConsoleMouseButtons  ﾏｳｽ ﾎﾞﾀﾝ の数を得ます",
  "PeekConsoleInput                ｺﾝｿｰﾙ ｲﾝﾌﾟｯﾄ ﾃﾞｰﾀ を ﾌﾟﾚﾋﾞｭｰ します",
  "ReadConsoleInput                ｺﾝｿｰﾙ ｲﾝﾌﾟｯﾄ ﾃﾞｰﾀ を読み出します",
  "ReadConsoleOutput               ｽｸﾘｰﾝ ﾊﾞｯﾌｧ ﾃﾞｰﾀ を読み出します",
  "ReadConsoleOutputAttribute      ｺﾝｿｰﾙ ｱﾄﾘﾋﾞｭｰﾄ ｽﾄﾘﾝｸﾞを読み出します",
  "ReadConsoleOutputCharacter      ｽｸﾘｰﾝ ﾊﾞｯﾌｧ ｽﾄﾘﾝｸﾞを読み出します",
  "ScrollConsoleScreenBuffer       ｽｸﾘｰﾝ ﾊﾞｯﾌｧ 内の ﾃﾞｰﾀ を ｽｸﾛｰﾙ します",
  "SetConsoleActiveScreenBuffer    表示される ｽｸﾘｰﾝ ﾊﾞｯﾌｧ を変更します",
  "SetConsoleCP                    入力 ｺｰﾄﾞ ﾍﾟｰｼﾞを設定します",
  "SetConsoleOutputCP              出力 ｺｰﾄﾞ ﾍﾟｰｼﾞを設定します",
  "SetConsoleCursorInfo            ｶｰｿﾙ の ｻｲｽﾞと可視性を設定します",
  "SetConsoleCursorPosition        ｶｰｿﾙ の位置を設定します",
  "SetConsoleMode                  ｺﾝｿｰﾙ の入力／出力 ﾓｰﾄﾞを設定します",
  "SetConsoleScreenBufferSize      ｽｸﾘｰﾝ ﾊﾞｯﾌｧ ｻｲｽﾞを変更します",
  "SetConsoleTextAttribute         ｽｸﾘｰﾝ ﾃｷｽﾄ の ｱﾄﾘﾋﾞｭｰﾄを 設定します",
  "SetConsoleTitle                 ｺﾝｿｰﾙ ｳｨﾝﾄﾞｳ の ﾀｲﾄﾙ を設定します",
  "SetConsoleWindowInfo            ｺﾝｿｰﾙ ｳｨﾝﾄﾞｳ の ｻｲｽﾞを設定します",
  "SetConsoleCtrlHandler           ｺﾝｿｰﾙ の ctrl-c ﾊﾝﾄﾞﾗ を設定します",
  "WriteConsoleInput               ｺﾝｿｰﾙ ｲﾝﾌﾟｯﾄ ﾊﾞｯﾌｧ に書き出します",
  "WriteConsoleOutput              ｽｸﾘｰﾝ ﾊﾞｯﾌｧ に書き出します",
  "WriteConsoleOutputAttribute     ｱﾄﾘﾋﾞｭｰﾄ ｽﾄﾘﾝｸﾞをｺﾝｿｰﾙ に書き出します",
  "WriteConsoleOutputCharacter     ｷｬﾗｸﾀ ｽﾄﾘﾝｸﾞをｺﾝｿｰﾙ に書き出します"
};
...
void perr(PCHAR szFileName, int line, PCHAR szApiName, DWORD dwError)
{
  ...
  sprintf(szTemp, "ｴﾗｰｺｰﾄﾞ:%d, API:%s, ｿｰｽﾌｧｲﾙ:%s, 行:%d\n",
        dwError, szApiName, szFileName, line);
  ...
  cMsgLen = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM |
      ...
      NULL);
  if (!cMsgLen)
    sprintf(szTemp + strlen(szTemp), "ｴﾗｰﾒｯｾｰｼﾞ 取得失敗！ \n"
        "ｴﾗｰｺｰﾄﾞ:%d, API:%s, ｿｰｽﾌｧｲﾙ:%s, 行:%d\n",
        GetLastError(), "FormatMessage",__FILE__, __LINE__);
  else
    strcat(szTemp, msgBuf);
  strcat(szTemp, "\n\n処理を続行しますか？");
  MessageBeep(MB_ICONEXCLAMATION);
  iButtonPressed = MessageBox(NULL, szTemp, "コンソール API エラー",
...
void showConAPIs(HANDLE hConsole)
{
  ...
  myPuts(hConsole, "\nAPI 名をクリックするとその API のデモを行ないます。\n"
                   "プログラムを終了するには ESC を押してください。\n");
  return;
}
...
int main(void)
{
  ...
  bSuccess = SetConsoleTitle("Win32 コンソール API デモ");
  PERR(bSuccess, "SetConsoleTitle");
    ...
    switch (inputBuffer.EventType)
      ...
      case KEY_EVENT:
        if (inputBuffer.Event.KeyEvent.bKeyDown)
          ...
          sprintf(bOutBuf, "キー: 仮想=%d アスキー=%c",
