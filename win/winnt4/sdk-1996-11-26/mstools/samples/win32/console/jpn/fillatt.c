...
void demoFillAtt(HANDLE hConOut)
{
  ...
  BOOL bSuccess;
  PCHAR szForeg = "> ｽｸﾘｰﾝ ﾃｷｽﾄ"; /* foreground 'button' and text */
  PCHAR szBackg = "> ｽｸﾘｰﾝ ﾊﾞｯｸｸﾞﾗｳﾝﾄﾞ"; /* background 'button' */
  PCHAR szOk = "> 了解"; /* OK 'button' */
  ...
  PCHAR szSampText = "コンソールはキーボードとマウスの入力バッファ、および"
                     "１つ以上のスクリーン バッファとから成っています。"
                     "\"CONIN$\" は入力バッファを参照します。Stdin は"
                     "\"CONIN$\" へのハンドルです。\"CONOUT$\" はスクリーン"
                     "バッファを参照します。";
  PCHAR p;
  ...
  for (i = 0; i < SAMPTEXTY; i++)
    ...
    nLen = min(strlen(p), SAMPTEXTX);
    for (j = 0; j < nLen; j++) {
        if (IsDBCSLeadByte(*(p+j))) {
            j++;
        }
    }
    if (j > nLen) {
        //DBCS charcter is splited.
        nLen--;
    }

    bSuccess = WriteConsoleOutputCharacter(hConOut, p,
        nLen, wCurPos, &dwCharsWritten);
    PERR(bSuccess, "WriteConsoleOutputCharacter");
