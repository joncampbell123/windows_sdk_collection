...
void demoGetTitle(HANDLE hConOut)
{
  ...
  myPuts(hConOut, "GetConsoleTitle でコンソールのタイトルを得ましょう。");
  /* get the console title */
  dwBytesRead = GetConsoleTitle(szTitleBuf, sizeof(szTitleBuf));
  PERR(dwBytesRead, "GetConsoleTitle");
  sprintf(szTemp, "コンソールのタイトルは: %s", szTitleBuf);
  myPuts(hConOut, szTemp);
  myPuts(hConOut, "\nここで SetConsoleTitle で新しいコンソールのタイトルを設定しましょう。");
  myPuts(hConOut, "\n新しいコンソールのタイトルを入力してください:");
