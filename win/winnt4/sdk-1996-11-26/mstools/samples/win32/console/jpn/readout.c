...
/********************************************************************
* FUNCTION: adjustCoordinate(HANDLE hConOut, PCOORD coordStart,     *
*                           PCOORD coordCur)                        *
*                                                                   *
* PURPOSE: adjust coordinate, if negative/over                      *
*                                                                   *
* INPUT: the output handle of the console to make the changes to,   *
*        and two sets of the pointer to COORD structures defining   *
*        the rectangle to invert                                    *
********************************************************************/

void adjustCoordinate(HANDLE hConOut, PCOORD coordStart, PCOORD coordCur)
{
  SHORT dwSizeX = getConX(hConOut);
  SHORT dwSizeY = getConY(hConOut);

  if (coordStart->X < 0) coordStart->X = 0;
  else if (coordStart->X >= dwSizeX) coordStart->X = dwSizeX - 1;
  if (coordStart->Y < 0) coordStart->Y = 0;
  else if (coordStart->Y > dwSizeY) coordStart->Y = dwSizeY;
  if (coordCur->X < 0) coordCur->X = 0;
  else if (coordCur->X >= dwSizeX) coordCur->X = dwSizeX - 1;
  if (coordCur->Y < 0) coordCur->Y = 0;
  else if (coordCur->Y > dwSizeY) coordCur->Y = dwSizeY;
...
void invertRectangle(HANDLE hConOut, COORD coordStart, COORD coordCur)
{
  ...
  adjustCoordinate(hConOut, &coordStart, &coordCur);
  ...
  while (coordStart.Y <= coordCur.Y)
    {
  adjustCoordinate(hConOut, &coordStart, &coordCur);
    dwBoxWidth = right - coordStart.X + 1;
...
void demoReadConOut(HANDLE hConOut)
{
  ...
  myPuts(hConOut, "マウスの左ボタンで、簡単なカット アンド ペーストを実現してみ\n"
                  "ましょう。クリックとドラッグでスクリーンの中のコピーする領域\n"
                  "を選択してください。ドラッグによって選択されつつある領域は、\n"
                  "スクリーン属性が読み込まれ、反転され、書き込まれることによっ\n"
                  "て示されます。マウスのボタンを放すと、選択された領域は読み込\n"
                  "まれ、その内容は現在のカーソル位置に表示されます。\n"
                  "戻るには ESC を押してください。\n\n");
      ...
      case MOUSE_EVENT:
        ...
        if (inputBuffer.Event.MouseEvent.dwEventFlags != MOUSE_MOVED)
          ...
          if(inputBuffer.Event.MouseEvent.dwButtonState ==
            ...
            /* adjust coordinate, if negative/over */
            adjustCoordinate(hConOut, &coordStart, &coordEnd);
        ...
        if (inputBuffer.Event.MouseEvent.dwEventFlags == MOUSE_MOVED &&
          ...
          {
          SHORT dwSizeX = getConX(hConOut);
          if (inputBuffer.Event.MouseEvent.dwMousePosition.X >= dwSizeX)
              inputBuffer.Event.MouseEvent.dwMousePosition.X = dwSizeX - 1;
