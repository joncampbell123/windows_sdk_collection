...
void demoWriteIn(HANDLE hConOut)
{
  ...
  myPuts(hConOut, "\n簡単なマクロ レコード／プレイバック機能を実現しましょう。\n"
                  "Alt+R でレコードされ、Alt+P でプレイバックされます。レコ\n"
                  "ード モードでいくつかのキーストロークかマウスのクリックを\n"
                  "行なってください。Alt+R でレコード モードが終了し、Alt+P\n"
                  "でキーストロークがプレイバックされます。WriteConsoleInput\n"
                  "API を使ってレコードされたイベントを入力バッファに挿入し\n"
                  "ています。いつでも ESC で戻れます。");
    ...
    switch (inputBuffer.EventType)
      ...
      case KEY_EVENT:
        ...
        if (!bRecording)
          ...
          if (*szDBCS) {
            *(szDBCS+1) = c;
            sprintf(bOutBuf, "キー: 仮想=%d アスキー=%s",
              inputBuffer.Event.KeyEvent.wVirtualKeyCode, szDBCS);
          ...
          if (*szDBCS) {
            ...
            *szDBCS = c;
DBCSWriteEvent:
            bSuccess = WriteFile(hConOut, &c, sizeof(c), &dwBytesWritten, NULL);
            PERR(bSuccess, "WriteFile");
            break;	// break if DBCS
          }
          /* if it's printable, and no control keys are down, show it */
          if (_ismbbprint(c) && !(inputBuffer.Event.KeyEvent.dwControlKeyState & CONTROL_KEY))
            {
          ...
          if (c == 'r' && (inputBuffer.Event.KeyEvent.dwControlKeyState &
            ...
            putStatusLine(hConOut, "レコーディング中...");
            }
