...
BOOL RunListBoxItem(LPCINFO lpCInfo, int cmd)
{
	...
	if (!IsEncrypted(szFileName) && cmd == ENCRYPT_DECRYPT) {
		...
		// not a directory
			...
			// encrypt the file
			if	(!EncryptFile(ghwndDrv, szFileName)) {
				ErrorMsg(TEXT("RunListBoxItem: EncryptFile failed."));
				return FALSE;
			}
		}
		...
		if (CFilerIsDirectory(szFileName)) {
			...
			// not a directory
			if (!DecryptFile(ghwndDrv, szFileName)) {
	 			ErrorMsg(TEXT("RunListBoxItem: DecryptFile failed."));
	 			return FALSE;
			}
		}
		UpdateFileLB(ghwndDrv);
