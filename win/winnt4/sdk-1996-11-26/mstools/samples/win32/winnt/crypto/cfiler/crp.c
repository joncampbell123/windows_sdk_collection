...
/************************************************************************\
* Exit1 and Exit2 close handles, etc. for EncryptFile,
* DecryptFile, SignFile, and VerifyFile.
\************************************************************************/
...
BOOL EncryptFile(HWND hWnd, LPTSTR szFileName) {
	...
	if (!szFileName) {
		MessageBox(hWnd, TEXT("EncryptFile: szFileName is NULL."), NULL, MB_OK);
		return FALSE;
	}
	...
	if (!hfnKey) {
		MessageBox(hWnd, TEXT("EncryptFile: GetfnKey failed."), NULL, MB_OK);
		return FALSE;
	}
	...
	if (!AlgidEncrypt) {
		if (RCRYPT_FAILED(CryptCreateHash(hMe, CALG_MD4, 0, 0, &hHash))) {
			MessageBox(hWnd, TEXT("EncryptFile: CryptCreateHash failed."), NULL, MB_OK);
			return FALSE;
		}

		if (RCRYPT_FAILED(CryptHashData(hHash, (LPBYTE)sPass, lstrlen(sPass), 0))) {
			MessageBox(hWnd, TEXT("EncryptFile: CryptHashData failed."), NULL, MB_OK);
			return FALSE;
		}

		if (RCRYPT_FAILED(CryptDeriveKey(hMe, CALG_RC2, hHash, CRYPT_EXPORTABLE, &hFileKey))) {
			MessageBox(hWnd, TEXT("EncryptFile: CryptDeriveKey failed."), NULL, MB_OK);
			return FALSE;
		}
	...
	// Generate a key to encrypt this file only (not derived from password).
		...
		MessageBox(hWnd, TEXT("EncryptFile: CryptGenKey failed."), NULL, MB_OK);
		return FALSE;
	} 
	...
	if (RCRYPT_FAILED(CryptExportKey(hFileKey, 
					  ...
					  pbFileKeyBlob, 
					  &dwFileBlobLen))) {
		MessageBox(hWnd, TEXT("EncryptFile: CryptExportKey failed."), NULL, MB_OK);
		return FALSE;
	}
	...
	if (hFileRead == INVALID_HANDLE_VALUE) {
		MessageBox(hWnd, TEXT("EncryptFile: CreateFile failed."), NULL, MB_OK);
		return FALSE;
	}
	...
	if (!hwndDlg) {
		Exit2(hWnd, TEXT("EncryptFile: CreateDialog failed."), hwndDlg, hFileRead, hFileWrite);
		DeleteFile(szFileName2);
		return FALSE;
	}
	
	if (RCRYPT_FAILED(CryptEncrypt(hfnKey, 0, TRUE, 0, (LPBYTE)szEncryptedFileName, &dwDataLen, dwBufLen))) {
		Exit2(hWnd, TEXT("EncryptFile: CryptEncrypt failed."), hwndDlg, hFileRead, hFileWrite);
		DeleteFile(szFileName2);
		return FALSE;
	}	
	...
	if (RCRYPT_FAILED(WriteFile(hFileWrite, 
				      ...
				      &dwNumBytesWritten, 
				      0))) {
		Exit2(hWnd, TEXT("EncryptFile: WriteFile failed."), hwndDlg, hFileRead, hFileWrite);
		DeleteFile(szFileName2);
		return FALSE;
	}
	...
	for ( ; ; ) {
		...
		if (!ReadFile(hFileRead, pEncryptBuf, BUF_SIZE, &dwNumBytesRead, 0)) {
			Exit2(hWnd, 
					TEXT("EncryptFile: ReadFile failed."), 
					hwndDlg, 
					hFileRead, 
					hFileWrite);
			DeleteFile(szFileName2);
		...
		if (RCRYPT_FAILED(CryptEncrypt(hFileKey, 0, FALSE, 0, pEncryptBuf, &dwEncryptLen, BUF_SIZE))) {
			Exit2(hWnd, 
					TEXT("EncryptFile: CryptEncrypt failed."), 
					hwndDlg, 
					hFileRead, 
					hFileWrite);
			DeleteFile(szFileName2);
		...
		if (!WriteFile(hFileWrite, pEncryptBuf, dwEncryptLen, &dwNumBytesWritten, 0)) {
			Exit2(hWnd, 
					TEXT("EncryptFile: WriteFile failed."), 
					hwndDlg, 
					hFileRead, 
					hFileWrite);
			DeleteFile(szFileName2);
...
//	BYTE					pHashData[BUF_SIZE];
	...
	if (!szFileName) {
		MessageBox(hWnd, TEXT("DecryptFile: szFileName is NULL."), NULL, MB_OK);
		return FALSE;
	}
	...
	if (!ReadFile(hFileRead, &dwDecryptBlobLen, sizeof(dwDecryptBlobLen), &dwNumBytesRead, 0)) {
		MessageBox(hWnd, TEXT("DecryptFile: ReadFile failed."), NULL, MB_OK);
		CloseHandle(hFileRead);
		return FALSE;
	}
	...
	if (dwDecryptBlobLen && !CryptImportKey(hMe, 
											...
											0, 
											&hFileKey)) {
		MessageBox(hWnd, TEXT("DecryptFile: CryptImportKey failed."), NULL, MB_OK);
		CloseHandle(hFileRead);
		return FALSE;
	}
	...
	if (!dwDecryptBlobLen) {
		...
		if (!GetPass(hWnd)) {
			MessageBox(hWnd, TEXT("DecryptFile: GetPass failed."), NULL, MB_OK);
			CloseHandle(hFileRead);
			return FALSE;
		}
		...
		if (RCRYPT_FAILED(CryptCreateHash(hMe, CALG_MD4, 0, 0, &hHash))) {
			MessageBox(hWnd, TEXT("DecryptFile: CryptCreateHash failed."), NULL, MB_OK);
			CloseHandle(hFileRead);
			return FALSE;
		}
		...
		if (RCRYPT_FAILED(CryptDeriveKey(hMe, CALG_RC2, hHash, CRYPT_EXPORTABLE, &hFileKey))) {
			MessageBox(hWnd, TEXT("DecryptFile: CryptDeriveKey failed."), NULL, MB_OK);
			CloseHandle(hFileRead);
			return FALSE;
		}
	...
	if (hFileWrite == INVALID_HANDLE_VALUE) {
		if (GetLastError() == ERROR_FILENAME_EXCED_RANGE) {
			...
			MessageBox(hWnd, TEXT("DecryptFile: CreateFile failed."), NULL, MB_OK);
			CloseHandle(hFileRead);
			return FALSE;
		}
	}
	...
	if (!hwndDlg) {
		Exit2(hWnd, TEXT("DecryptFile: CreateDialog failed."), hwndDlg, hFileRead, hFileWrite);
		DeleteFile(szFileName2);
		return FALSE;
	}
	...
	for ( ; ; ) {
		if (!ReadFile(hFileRead, pDecryptBuf, BUF_SIZE, &dwNumBytesRead, 0)) {
			Exit2(hWnd, 
					TEXT("DecryptFile: ReadFile failed."), 
					hwndDlg, 
					hFileRead, 
					hFileWrite);
			DeleteFile(szFileName2);
		...
		if (RCRYPT_FAILED(CryptDecrypt(hFileKey, 0, FALSE, 0, pDecryptBuf, &dwDecryptDataLen))) {
			Exit2(hWnd, 
					TEXT("DecryptFile: CryptDecrypt failed."), 
					hwndDlg, 
					hFileRead, 
					hFileWrite);
			DeleteFile(szFileName2);
		...
		if (!WriteFile(hFileWrite, pDecryptBuf, dwDecryptDataLen, &dwNumBytesWritten, 0)) {
			Exit2(hWnd, 
					TEXT("DecryptFile: WriteFile failed."), 
					hwndDlg, 
					hFileRead, 
					hFileWrite);
			DeleteFile(szFileName2);
	...
	if (!CryptDestroyKey(hFileKey)) {
		Exit2(hWnd, 
				TEXT("DecryptFile: CryptDestroyKey failed."), 
				hwndDlg, 
				hFileRead, 
				hFileWrite);
		DeleteFile(szFileName2);
	...
	if (!DeleteFile(szFileName)) {
		MessageBox(hWnd, TEXT("DecryptFile: DeleteFile failed."), NULL, MB_OK);
		DeleteFile(szFileName2);
	}
	...
	do {
		...
		if (!IsEncrypted(szFileName)
		   ...
		   && lstrcmp(FindFileData.cFileName, TEXT("dirinfo"))) {	
			...
			else if (!EncryptFile(hWnd, szFileName)) {
				MessageBox(hWnd, TEXT("EncryptDir: EncryptFile failed."), NULL, MB_OK);
				return FALSE;
			}
		}
	...
	do {
		...
		if (IsEncrypted(szFileName)
		   ...
		   && lstrcmp(FindFileData.cFileName, TEXT(".."))) {	
			...
			else if (!DecryptFile(hWnd, szFileName)) {
				MessageBox(hWnd, TEXT("DecryptDir: DecryptFile failed."), NULL, MB_OK);
				return FALSE;
			}
		}
