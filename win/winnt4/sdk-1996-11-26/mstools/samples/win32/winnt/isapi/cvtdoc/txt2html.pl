...
# It is available at:
#   http://cuiwww.unige.ch/ftp/PUBLIC/oscar/scripts/html.pl
#
...
sub deal_with_options
{
	...
	{
	    print '$Header: /users/hilco/seth/projects/txt2html/txt2html.pl,v 1.10 1994/12/28 20:10:25 seth Exp seth $ ';
	    print "\n";
	    exit;
	}
...
sub continuelist
{
    ...
    $line =~ s/^\s*(\d+|[a-zA-Z]).\s*/$list_indent<LI> /    if $list[$listnum-1] == $OL;
    $line_action |= $LIST;
}
