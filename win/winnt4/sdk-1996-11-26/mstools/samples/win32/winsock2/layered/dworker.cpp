...
INT
DWORKERTHREAD::UnregisterSocket(
    PDSOCKET Socket
    )
/*++
Routine Description:
...
Arguments:
    ...
    Socket - A pointer to a DSOCKET object for the socket that is
             unregistering. 
...
--*/
{
    ...
    while (!m_exit_thread){
        ...
        if ((ReturnCode >= WAIT_OBJECT_0 + 1) &&
            ...
            Socket = (PDSOCKET)m_socket_array[Index];
            if (Socket){
...
Routine Description:
    ...
    Adds a DSOCKET object and its associated event the set of events the worker
    thread is interested in.

Arguments:

    Socket - A pointer to a DSOCKET object.
...
--*/
{
    ...
    for (Index =0;
         ...
         Index++ ){
        if (m_socket_array[Index] == (DWORD) Socket){
            Found = TRUE;
    ...
    if (!Found && (m_event_count < MAXIMUM_WAIT_OBJECTS)){
        ...
        m_socket_array[m_event_count] = (DWORD)Socket;
...
INT
DWORKERTHREAD::StoreOverlappedParams(
...
Arguments:
    ...
    Provider - A pointer to a DPROVIDER object that is receive the overlapped
               request. 
...
INT
DWORKERTHREAD::RetrieveOverlappedParams(
