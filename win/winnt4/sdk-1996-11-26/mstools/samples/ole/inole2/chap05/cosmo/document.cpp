...
UINT CCosmoDoc::ULoad(BOOL fChangeFile, LPTSTR pszFile)
    ...
    poleFile = new OLECHAR[strlen(pszFile)+1];
    if (!poleFile)
        {
        return DOCERR_COULDNOTOPEN;
        }
    mbstowcs(poleFile, pszFile, strlen(pszFile)+1);
#else
    poleFile = pszFile;
#endif
    ...
    if (NOERROR!=StgIsStorageFile(poleFile))
        {
        hr=StgCreateDocfile(poleFile,STGM_TRANSACTED | STGM_READWRITE
        ...
        {
        hr=StgOpenStorage(poleFile, NULL, STGM_DIRECT | STGM_READ
...
UINT CCosmoDoc::USave(UINT uType, LPTSTR pszFile)
    ...
    if (lVer==MAKELONG(0, 1))
        ...
        {
#if defined(WIN32) && !defined(UNICODE)
        unsigned uTemp = strlen(pszFile);
        LPOLESTR lpoleTemp = new OLECHAR[uTemp];
        if (lpoleTemp)
        {
            mbstowcs(lpoleTemp, pszFile, uTemp);
        }
        hr=StgCreateDocfile(lpoleTemp, STGM_DIRECT | STGM_READWRITE
...
#if defined(WIN32) && !defined(UNICODE)
        uTemp = strlen(PSZ(IDS_CLIPBOARDFORMAT))+1;
        lpoleTemp = new OLECHAR[uTemp];
        if (lpoleTemp)
        {
            mbstowcs(lpoleTemp, PSZ(IDS_CLIPBOARDFORMAT), uTemp);
