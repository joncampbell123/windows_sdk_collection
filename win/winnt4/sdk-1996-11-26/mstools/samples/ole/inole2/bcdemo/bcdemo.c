...
TCHAR *     rgpsz[6]={TEXT("Up"), TEXT("Mouse Down")
                     , TEXT("Disabled"), TEXT("Down")
                     , TEXT("Down Disabled"), TEXT("Indeterminate")
                     };


//More strings we'll use as labels.
TCHAR szStandard[]=TEXT("Standard Buttons");
TCHAR szApp[]=TEXT("App-defined Buttons");
TCHAR szMsg[]=TEXT("Down, Down Disabled, and  Indeterminate states are not\
 valid  on standard command buttons.");
...
LONG WINAPI BCDemoWndProc(HWND hWnd, UINT iMsg, WPARAM wParam
    , LPARAM lParam)
    {
    PAPPVARS    pAV;
    WORD        wID;
    ...
    switch (iMsg)
        ...
        case WM_PAINT:
            ...
            //Draw a horizontal separator line between text & buttons
            MoveToEx(ps.hdc, 0, pAV->tdd.cyButton-4, NULL);
            LineTo(ps.hdc, rc.right-rc.left, pAV->tdd.cyButton-4);

            /*
             * Draw a vertical separator line between standard and
             * app buttons.
             */
            x-=2;
            ...
            /*
             * Final label--show a message stating that the three
             * attribute-only states are not available for standard
             * command buttons.  We use the x & y we calculated for
             * lines to position the text.
             */
            SetRect(&rc, 0, y-((5+pAV->tdd.cyButton)*3), x, y);
            DrawText(ps.hdc, szMsg, lstrlen(szMsg), &rc
            ...
            for (i=0; i<6; i++)
                ...
                for (j=TOOLIMAGE_MIN; j<=TOOLIMAGE_MAX; j++)
                    {
                    /*
                     * Draw a standard button (NULL in hBmp).  If
                     * we fail, like we're attempting to draw an
                     * unsupported state on a command button,
                     * nothing appears.
                     */
                ...
                //Now draw two app-defined bitmaps in the same row.
                for (j=0; j<=1; j++)
                    {
                    /*
                     * Draw an application-defined button using
                     * indices 0 and 1 (a computer and a phone).
                     * Adding TOOLIMAGE_MAX+1 to j places these
                     * buttons at the ends of the existing rows of
                     * standard buttons.  These are 4 pixels larger
                     * in each direction over the standard size.
                     */
                    UIToolButtonDrawTDD(ps.hdc, (j+TOOLIMAGE_MAX+1)
                        *(5+pAV->tdd.cxButton), y
                        , pAV->tdd.cxButton+4 , pAV->tdd.cyButton+4
                        , hBmp , pAV->tdd.cxImage, pAV->tdd.cyImage
                        , j , rguState[i], &pAV->tdd);
                    }

                /*
                 * Label the row with the state it shows.
                 *
                 * TOOLIMAGE_MAX+3 is the number of buttons.  +5 on
                 * width provides some spacing.  +4 on position
                 * spaces the text from the buttons a little.
                 */
        ...
        case WM_COMMAND:
            ...
            if (wID >= IDC_NEWUICURSORMIN
                && wID <= IDC_NEWUICURSORMAX)
                {
                /*
                 * Retrieve a new cursor and set it as the class
                 * cursor of our main window.
                 */
