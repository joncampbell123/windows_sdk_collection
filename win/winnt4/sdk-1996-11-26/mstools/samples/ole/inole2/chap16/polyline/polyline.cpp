/*
 * POLYLINE.CPP
 * Polyline Component Object Chapter 16
 *
...
CPolyline::CPolyline(LPUNKNOWN pUnkOuter, PFNDESTROYED pfnDestroy
    ...
    m_pIRunnableObject=NULL;
    m_hDlg=NULL;

    m_pIExternalConnection=NULL;
...
CPolyline::~CPolyline(void)
    ...
    if (NULL!=m_pIRunnableObject)
        delete m_pIRunnableObject;

    //Anything we might have registered in IRunnableObject::Run
    if (0L!=m_dwRegROT)
        RevokeAsRunning(&m_dwRegROT);

    if (NULL!=m_pIExternalConnection)
        delete m_pIExternalConnection;
...
BOOL CPolyline::FInit(void)
    ...
    m_pIRunnableObject=new CImpIRunnableObject(this, pIUnknown);

    if (NULL==m_pIRunnableObject)
        return FALSE;

    m_pIExternalConnection=new CImpIExternalConnection(this
        , pIUnknown);
    ...
    //CHAPTER16MOD
    m_pIOleIPObject=new CImpIOleInPlaceObject(this, pIUnknown);
    ...
    hr=CreateDataCache(pIUnknown, CLSID_Polyline11
...
STDMETHODIMP CPolyline::QueryInterface(REFIID riid, PPVOID ppv)
    ...
    *ppv=NULL;
    ...
    if (IID_IRunnableObject==riid)
        *ppv=m_pIRunnableObject;

    if (IID_IExternalConnection==riid)
        *ppv=m_pIExternalConnection;
...
/*
 * DataGetMem/SetMem eliminated:  no need with IDataObject.
...
HRESULT CPolyline::InPlaceActivate(LPOLECLIENTSITE pActiveSite
    ...
    if (!m_pHW->FInit(hWnd, ID_HATCHWINDOW, NULL))
        {
        InPlaceDeactivate();
        return ResultFromScode(E_OUTOFMEMORY);
        }
    ...
    /*
     * 3, 4, 5.  Do Shared menu and in-place tools, UI sorts of
     * things including SetActiveObject calls.
     */
    hr=NOERROR;
...
HRESULT CPolyline::UIActivate(void)
    {
    //1.  Negotiate border space.  Polyline doesn't need any.
    if (NULL!=m_pIOleIPFrame)
    ...
    /*
     * 2.  Create the shared menu.  We don't have any, so tell
    ...
    //4.  Set the active object
    if (NULL!=m_pIOleIPFrame)
        {
#if defined(WIN32) && !defined(UNICODE)
        OLECHAR pwcsTemp[256];
        mbstowcs(pwcsTemp, PSZ(IDS_USERTYPE), 256);
...
void CPolyline::UIDeactivate(void)
    ...
    //We don't have any shared menu to clean up.
