    ...
    switch (iMsg)
        ...
        case WM_COMMAND:
            if (ID_HATCHWINDOW==wID
                ...
                {
                ppl->m_pIOleObject->DoVerb(OLEIVERB_OPEN, NULL
...
BOOL APIENTRY PolyDlgProc(HWND hDlg, UINT iMsg
    ...
    UINT            cx, cy;
    //CHAPTER16MOD
    //hWnd and pt variables moved to CalcPolyRectInDialog.
    //End CHAPTER16MOD
    ...
    switch (iMsg)
        ...
        case WM_INITDIALOG:
            ...
            ppl->m_pIPolyline->Init(hDlg, &rc, WS_CHILD | WS_VISIBLE
                , ID_POLYLINE);
            ...
            switch (uID)
                ...
                case ID_COLORBACK:
                    if (NULL!=ppl)
                        ...
                        cc.hwndOwner=hDlg;
                        cc.Flags=CC_RGBINIT;
                        ppl->m_pIPolyline->ColorGet(uTemp
                            , &cc.rgbResult);

                        if (ChooseColor(&cc))
                            {
                            //rgColor is just some COLORREF pointer
                            ppl->m_pIPolyline->ColorSet(uTemp
