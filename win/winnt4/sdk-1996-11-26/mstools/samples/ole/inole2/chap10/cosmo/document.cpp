...
CCosmoDoc::~CCosmoDoc(void)
     ...
     * Release that reference count, we'll free the object and
     * start shutdown.  If the object was destroyed before we
     * are destroyed, it will NULL out m_pFigure for us (see
     * CFigure::Release in FIGURE.CPP)
     */
...
BOOL CCosmoDoc::FRenderMedium(UINT cf, LPSTGMEDIUM pSTM)
    ...
    if (cf==m_cfObjectDescriptor)
        ...
        SETPOINTL(ptl, 0, 0);
#if defined(WIN32) && !defined(UNICODE)
        OLECHAR pwszTemp[256];
        mbstowcs(pwszTemp, PSZ(IDS_OBJECTDESCRIPTION), 256);
        pSTM->hGlobal=OleStdGetObjectDescriptorData
            (CLSID_Cosmo2Figure, DVASPECT_CONTENT, szl, ptl
            , OLEMISC_RECOMPOSEONRESIZE, pwszTemp
            , NULL);
#else
        pSTM->hGlobal=OleStdGetObjectDescriptorData
            (CLSID_Cosmo2Figure, DVASPECT_CONTENT, szl, ptl
