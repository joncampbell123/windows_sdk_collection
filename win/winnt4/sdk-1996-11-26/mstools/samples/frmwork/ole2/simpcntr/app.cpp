...
BOOL CSimpleApp::fInitApplication(HINSTANCE hInstance, int nCmdShow)
{
	...
	LoadString(hInstance, IDS_DESCRIPTION, lpTitle, sizeof(lpTitle));
...
long CSimpleApp::lCreateDoc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (NULL == m_lpDoc)
    {
		TCHAR szMsg[MAX_PATH], szTitle[40];
		LoadString(m_hInst, IDS_APPNAME, szTitle, sizeof(szTitle));
        LoadString(m_hInst, IDS_UNABLE_CREATE, szMsg, sizeof(szMsg));
