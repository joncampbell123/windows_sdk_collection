...
STDMETHODIMP CSimpSvrObj::SetHostNames(LPCOLESTR szContainerApp, LPCOLESTR szContainerObj)
{
    ...
    TCHAR szTitle[MAX_PATH], szStr[MAX_PATH];

    szTitle[0] = TEXT('\0');
    if (NULL != szContainerObj)
    {
#ifdef UNICODE
        lstrcpy(szTitle, szContainerObj);       // Both strings are unicode
#else
        WideCharToMultiByte(CP_ACP,             // Convert szContainerObj to ANSI
                            ...
                            NULL);
#endif
        LoadString(m_pDoc->GetApp()->GethInst(), IDS_PREFIX, szStr, sizeof(szStr));
        lstrcat(szTitle, szStr);
    }
