...
BOOL CSimpSvrObj::DoInPlaceActivate(LONG lVerb)
{
    ...
    if (!m_fUIActive)
    {
        ...
        LoadString(m_pDoc->GetApp()->GethInst(), IDS_APPNAME, szObjName, sizeof(szObjName));
#ifdef UNICODE
        // set the active object on the frame
        m_pFrame->SetActiveObject((LPOLEINPLACEACTIVEOBJECT)this, szObjName);
#else
       int cch = lstrlen(szObjName) + 1;
       LPWSTR pwObjName = new WCHAR[cch];
       MultiByteToWideChar(CP_ACP, MB_ERR_INVALID_CHARS, szObjName, -1, pwObjName, cch);
        // set the active object on the frame
        m_pFrame->SetActiveObject((LPOLEINPLACEACTIVEOBJECT)this, pwObjName);
#endif // UNICODE
    }
...
void CSimpSvrObj::AssembleMenus()
{
    ...
    if (m_pFrame->InsertMenus(m_hmenuShared, &menugroupwidths) == NOERROR)
    {
        ...
        TCHAR szMenuItem[80];
        LoadString(m_pDoc->GetApp()->GethInst(),
                    IDS_COLOR_MENUITEM, szMenuItem, sizeof(szMenuItem));
