    ...
    (HINSTANCE this_inst, HINSTANCE prev_inst, LPSTR cmdline, int cmdshow)
{
    ...
    while (!done) {
        ...
        if (!active_window->bMinimized && !bQuit)       {
            if (idle) Idle();
            if (!Render()) {
...
static void animationCallback(LPDIRECT3DRMFRAME obj, void* arg, D3DVALUE delta)
{
...
LONG FAR PASCAL WindowProc(HWND win, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_COMMAND:
        ...
        case MENU_MODEL_MONO:
            SetModel(win, info, D3DCOLOR_MONO);
            break;

        case MENU_MODEL_RGB:
            SetModel(win, info, D3DCOLOR_RGB);
            break;
