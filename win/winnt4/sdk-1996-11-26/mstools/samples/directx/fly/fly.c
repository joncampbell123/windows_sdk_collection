...
static void cleanupObjects(LPDIRECT3DRMOBJECT obj, void* arg)
{
...
void 
moveCamera(LPDIRECT3DRMFRAME camera, void *arg, D3DVALUE delta)
{
    D3DVECTOR dir, up;
...
generic_error:
    Msg("A failure has occured while building the scene.\n");
ret_with_error:
    RELEASE(lights);
    RELEASE(plane_builder);
    RELEASE(mesh_builder);
