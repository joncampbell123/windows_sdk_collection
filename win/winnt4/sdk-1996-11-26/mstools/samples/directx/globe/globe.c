...
static void cleanupPath(LPDIRECT3DRMOBJECT obj, void* arg)
{
    anim->lpVtbl->Release(anim);
}

void 
moveCamera(LPDIRECT3DRMFRAME frame, void *arg, D3DVALUE delta)
{
...
run_world_scene(LPDIRECT3DRMFRAME scene, LPDIRECT3DRMFRAME camera)
{
    ...
    if (rval != D3DRM_OK) {
        Msg("Failed to load sphere3_builder.\n%s", D3DRMErrorToString(rval));
        goto ret_with_error;
    }
    ...
    return TRUE;
generic_error:
    Msg("An error occured while building the scene.\n");
ret_with_error:
    RELEASE(light1);
    RELEASE(world_frame);
    RELEASE(l1);
