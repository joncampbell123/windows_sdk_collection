                     ...
                     )
{
    ...
    *option = 0;
    ...
    if ( (argc < 2) || (argv[1][0] == '/') )
    {
        ...
        if (argc >= 2)
        {
            if ( (0 == stricmp(&argv[1][1], "deBug")) ||
                 (0 == stricmp(&argv[1][1], "b"))  )
            {
    ...
    for (LONG k = 2; k < argc ; k++ )
    {
        if (argv[k][0] == '/')
        {
            ...
            if ( (0 == stricmp(&argv[k][1], "Tree")) ||
                 (0 == stricmp(&argv[k][1], "t")) )
            {
            ...
            if ( (0 == stricmp(&argv[k][1], "Continue")) ||
                 (0 == stricmp(&argv[k][1], "c")) )
            {
            ...
            if ( (0 == stricmp(&argv[k][1], "Edit")) ||
                 (0 == stricmp(&argv[k][1], "E")) )
            {
...
#if DBG
            if ( (0 == stricmp(&argv[k][1], "deBug")) ||
                 (0 == stricmp(&argv[k][1], "b"))  )
            {
                if (*debug)
                    return(ERROR_BAD_ARGUMENTS);
...
#endif
            if ( (0 == stricmp(&argv[k][1], "Deny")) ||
                 (0 == stricmp(&argv[k][1], "D")) )
            {
                am = ARG_MODE_INDEX_DENY;
                *option |= OPTION_DENY;
            } else if ( (0 == stricmp(&argv[k][1], "Revoke")) ||
                        (0 == stricmp(&argv[k][1], "R")) )
            {
                am = ARG_MODE_INDEX_REVOKE;
                *option |= OPTION_REVOKE;
            } else if ( (0 == stricmp(&argv[k][1], "Grant")) ||
                        (0 == stricmp(&argv[k][1], "G")) )
            {
                am = ARG_MODE_INDEX_GRANT;
                *option |= OPTION_GRANT;
            } else if ( (0 == stricmp(&argv[k][1], "rePlace")) ||
                        (0 == stricmp(&argv[k][1], "P")) )
            {
...
ULONG DisplayAces(CHAR *filename, ULONG option)
{
    ...
    if (NO_ERROR == (ret = cfe.Init(filename, &pwfilename, &fdir)))
    {
                  ...
                  (option & OPTION_CONTINUE_ON_ERROR) ) )
        {
            ...
            wprintf(TEXT("%s"),pwfilename);
#else
            fprintf(stdout, "%ws",pwfilename);
#endif
                ...
                if (NO_ERROR == (ret = cds.Init()))
                {
                    ...
                    if (NO_ERROR == ret)
                        for (retace = cds.GetNextAce(&paceh); retace >= 0; )
                        {
                            ...
                            if (retace >= 0)
#ifdef DBCS // character counts != byte counts
                                printf("%*s",
    ...
    for (ULONG j = 0, k = 1;j < MAX_OPTIONS ; k <<= 1, j++ )
    {
        ...
        if (option & k)
        {
                      ...
                      q < aend[j] ; q++ )
            {
...
#ifdef KKBUGFIX
                VERBOSER((stderr, "OPTION = %d, USER = %S, ACCESS = %lx\n",
                       option,
                       user,
                       access))
#else
                ...
                {
#ifdef KKBUGFIX
                    ERRORS((stderr, "SetAccess for %S:%lx failed, %d\n",
                           user,
                           access,
                           ret))
#else
    ...
    if (NO_ERROR == (ret = cfe.Init(filename, &pwfilename, &fdir)))
    {
                  ...
                  (option & OPTION_CONTINUE_ON_ERROR) ) )
        {
            ...
            if (NO_ERROR == (ret = cfs.Init()))
            {
                ...
                {
#ifdef KKBUGFIX
                    ERRORS((stderr, "SetFS on %S failed %ld\n",pwfilename, ret))
#else
...
#ifdef KKBUGFIX
        ERRORS((stderr, "file enumeration failed to initialize %S, %ld\n",pwfilename, ret))
#else
...
void printfsid(SID *psid, ULONG *outputoffset)
{
    ...
    if (NO_ERROR == ( ret = ca.GetAccountDomain(&domain) ) )
    {
        ...
        {
#if defined(UNICODE) && defined(DBCS)
            wprintf(TEXT(" %s\\"), domain);
            *outputoffset += 2 + WideCharToMultiByte(CP_ACP, 0,
                                               domain, -1,
                                               NULL, 0,
                                               NULL, NULL);
#else
...
#if defined(UNICODE) && defined(DBCS)
            wprintf(TEXT("%s:"), user);
            *outputoffset += 1 + WideCharToMultiByte(CP_ACP, 0,
                                               user, -1,
                                               NULL, 0,
                                               NULL, NULL);
#else
            fprintf(stdout, "%ws:",user);
