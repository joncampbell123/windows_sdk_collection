echo off
@rem
@rem  Sets the 16-bit debugging environment to use debug binaries & symbols
@rem
@rem
...
echo setdebug will now kill all 16 bit applications
pause
%windir%\sct\debug16\kill -f ntvdm
del %windir%\system32\wow32.dll
if errorlevel 2 goto error3

xcopy /V %windir%\sct\debug16\wow32.dll %windir%\system32
...
xcopy /V %windir%\sct\debug16\wow32.dbg %windir%\symbols\dll
  if errorlevel 4 goto lowmemory
  if errorlevel 2 goto abort
...
@REM NOTE: You should use the retail symbols for most 16-bit binaries
xcopy /V %windir%\sct\retail16\*.sym %windir%\system32
  if errorlevel 4 goto lowmemory
  if errorlevel 2 goto abort
...
xcopy /V %windir%\sct\debug16\wowexec.* %windir%\system32
  if errorlevel 4 goto lowmemory
  if errorlevel 2 goto abort
...
echo.
echo.
echo.   16-bit FULL DEBUG debugging components are now in place.
echo.
echo.
goto exit
