...
main( int __argc, char** __argv )
{
  ...
  if( GetVersionEx(&os) )
  {
    if ( os.dwPlatformId != VER_PLATFORM_WIN32_NT)
    {
        ...
        {
            WIN95 = TRUE;   
            printf("\n\nChanges to the text attibutes are not persistant in Windows 95.");
        }
        else
        {
            MessageBoxA( NULL,
  ...
  if( __argc < 2 || WIN95 ) {
    Syntax( hConsole );
      ...
      \**********************************************************************/
      if( BackColor == MAXCOLORS || ForeColor == MAXCOLORS || BackColor == ForeColor) {
        printf("\n\nError: %s", BackColor == ForeColor ? 
            "Foreground and background color cannot be the same." : 
...
* RETURNS     : none.
*
* COMMENTS    : Displays program banner.
*
\************************************************************************/
void PrintBanner()
{
  printf("Console Window Text Color Setter, Version 1.10");
