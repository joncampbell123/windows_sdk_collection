                    ...
                    LPSTR lpCmdLine,  int nCmdShow)
{
  ...
  if ((ghLib = LoadLibrary (TEXT("the_dll.dll"))) == NULL)
  {
    MessageBox (NULL, TEXT( "WinMain(): LoadLibrary() failed"),
                TEXT( "Err! - RESDLL"), MB_OK | MB_ICONEXCLAMATION);
    return 0;
  }
  ...
  if (!hPrevInstance)
  {
    ...
    wc.hIcon         = LoadIcon (ghLib, TEXT("dllicon"));
    wc.hCursor       = LoadCursor (ghLib, TEXT("dllcursor"));
    wc.hbrBackground = GetStockObject (WHITE_BRUSH);
    wc.lpszMenuName  = NULL;
    wc.lpszClassName = TEXT( "RESDLL");

    if (!RegisterClass (&wc))
    {
      MessageBox (NULL, TEXT( "WinMain(): RegisterClass() failed"),
                  TEXT( "Err! - MAIN"), MB_OK | MB_ICONEXCLAMATION);
      return(FALSE);
    }
  }

  if (!(hwnd = CreateWindow (TEXT("RESDLL"), TEXT("RESDLL Sample Application"),
                             WS_OVERLAPPEDWINDOW,
                              ...
                              LPARAM lParam)
{
  ...
  switch (message)
  {
    case WM_CREATE:
      ...
      hbm = LoadBitmap (ghLib, TEXT( "dllbitmap"));
      break;
