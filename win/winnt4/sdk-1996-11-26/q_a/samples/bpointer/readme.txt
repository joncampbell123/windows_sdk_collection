...
a pointer value that is valid in one process's context may not be valid
...
readdata.exe : A console process that allows you to view the shared data;
               it de-references pointers as it encounters them.
...
application using them because the pointers need to be resolved at runtime;
