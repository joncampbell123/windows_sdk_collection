...
VOID main(VOID)
{
  ...
  DWORD	    dwVersion;
//yg Modified to check for windows 95

  // check if running on Windows NT or Windows 95, if not, display notice and terminate

  dwVersion = GetVersion();
  if ((dwVersion & 0x80000000) && (LOBYTE(LOWORD(dwVersion))< 4))
  {
     MessageBox( NULL,
       "This sample application can only be run on Windows NT or Windows 95.\n"
  ...
  printf("%08X %s\n", dwDriveMask, lpszDriveStringBuffer);
