...
{                                                   \
    WRITE_PORT_USHORT((p), (x));                    \
    CP_MEMORY_BARRIER();                            \
    WRITE_PORT_USHORT((p), (x) >> 16);              \
}
...
#define IO_GP_BUSY(ppdev)   (IO_GP_STAT(ppdev) & HARDWARE_BUSY)
    ...
    #define IO_FIFO_BUSY(ppdev, level)                                  \
        ((( level == 8 ) && (!( IO_GP_STAT(ppdev) & GP_ALL_EMPTY))) ||  \
    ...
    //                            about them:
        ...
        #define INPW(pjIoBase, p)       READ_PORT_USHORT((p))
        #define INP(pjIoBase, p)        READ_PORT_UCHAR((p))
    ...
    #define WRITE_FIFO_W(p, v)              WRITE_REGISTER_USHORT((p), (USHORT) (v))
    #define WRITE_FIFO_D(p, v)              WRITE_REGISTER_ULONG((p), (ULONG) (v))
...
{                                                                           \
    ...
    while (mcd-- != 0)                                                      \
    {                                                                       \
        WRITE_REGISTER_ULONG(MM(pjMmBase, mapdMmXfer[mcd & XFER_MASK]), *mpdSrc++); \
    }                                                                       \
    if ((c) & 1)                                                            \
    {                                                                       \
...
{                                                                           \
    ...
    while (mcd-- != 0)                                                      \
    {                                                                       \
        WRITE_REGISTER_ULONG(MM(pjMmBase, mapdMmXfer[mcd & XFER_MASK]),     \
                         *mpdSrc++);                                        \
    }                                                                       \
    if ((c) & 1)                                                            \
    {                                                                       \
