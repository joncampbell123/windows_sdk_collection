...
#define GLYPHSET            260 // Glyphset resource type
#endif // DBCS
...
#define   MAX_BASE_FONT   128
#define   MAX_SET_FONT    128

typedef struct {
	WORD    GlyphID[256];        // index to glyphs in PS Font
	WORD    RecentUseList[256];  // Keep the use order list.
} DLBASEFONT,  *PDLBASEFONT;
...
typedef struct
{
	...
	ULONG   iFace;
#endif
	WORD       cBaseFont;   // number of base font in this DLFont
	WORD       iLastLink;   // keep the last node of the link list
	DLBASEFONT *BaseFonts[MAX_BASE_FONT];
#else   // !DBCS
...
#if defined(DBCS) // FE font
	BOOL        bDBCSGDIFont;   // current draw font is DBCS GDI downloadable font
    PDEVFONT    pDevFont;
	BYTE        iBaseFont;      // Currently active base font
	char        szSetFont[MAX_SET_FONT];  //command string to choose font
#endif // DBCS
} CGS;
typedef CGS *PCGS;
...
#define  IsDBCSCharSet(j)  \
	(((j) == CHINESEBIG5_CHARSET)  || \
     ((j) == GB2312_CHARSET ) || \
	 ((j) == SHIFTJIS_CHARSET) || \
	 ((j) == HANGEUL_CHARSET) )

#define ISDBCSCHARSET(pifi) \
	IsDBCSCharSet(((IFIMETRICS *)(pifi))->jWinCharSet)

#define  ISDBCSCODEPAGE(w) \
	((w)==932 || (w)==936 || (w)==949 || (w)==950)

#define DBCS_VERT_SYMBOL  L'@'
