...
#endif // !UNICODE
WINSHELLAPI TCHAR **  APIENTRY CommandLineToArgvW(TCHAR* lpCmdLine, int*pNumArgs);
...
WINSHELLAPI UINT WINAPI ExtractIconEx(LPCSTR lpszFile, int nIconIndex, HICON FAR *phiconLarge, HICON FAR *phiconSmall, UINT nIcons);
WINSHELLAPI UINT WINAPI ExtractIcons(LPCSTR szFileName, int nIconIndex, int cxIcon, int cyIcon, HICON FAR *phicon, UINT nIcons, UINT flags);
...
WINSHELLAPI void WINAPI WinExecError(HWND hwnd, int error, LPCSTR lpstrFileName, LPCSTR lpstrTitle);
...
WINSHELLAPI BOOL WINAPI SHGetNewLinkInfo(LPCSTR pszLinkTo, LPCSTR pszDir, LPSTR pszName,
