...
{
#if defined(WIN32) && !defined(CHICAGO)
    LPSTR           llpcStr[2];
...
#ifdef DBCS
                if (pszEnd > pszBeg &&
                    (!FGLeadBytep(SzGPrev(pszBeg,pszEnd))&& *(pszEnd-1)== '\r'))
#else
...
Routine Description:
    ...
    This routine returns if the service specified is running interactively (not invoked
...
SCODE BASED_DEBUG IMAPITable_QuerySortOrder_Scodes[] =
{
    MAPI_E_NOT_ENOUGH_MEMORY,
    S_OK
};
...
SCODE BASED_DEBUG IMAPISession_GetLastError_Scodes[] = {S_OK};
...
#ifdef CHICAGO
#define dwStackLimit    0x00400000      /*  4MB for Chicago */
#else
#define dwStackLimit    0x00010000      /*  64KB for NT */
#endif

void GetCallStack(DWORD *pdwCaller, int cSkip, int cFind)
{
    DWORD * pdwStack;
...
void *VMAllocEx(ULONG cb, ULONG cbCluster)
{
    ULONG   cbAlloc;
    void *  pvR;
    void *  pvC;
...
void VMFreeEx(void *pv, ULONG cbCluster)
{
...
void * VMRealloc(void *pv, ULONG cb)
{
    return VMReallocEx(pv, cb, 1);
}

void * VMReallocEx(void *pv, ULONG cb, ULONG cbCluster)
{
    void *  pvNew = 0;
    ULONG   cbCopy;
...
ULONG VMGetSizeEx(void *pv, ULONG cbCluster)
{
...
ULONG VMGetSizeEx(void *pv, ULONG ulCluster)
{
...
BOOL DBGMEM_ValidatePblk(PDBGMEM pdbgmem, PBLK pblk, char ** pszReason)
{
    ...
    if (pdbgmem->cbExtra) {
        ULONG * pul = (ULONG *)(pblk->pblktail + 1);
...
#if defined(WIN32) && defined(_X86_)
void DBGMEM_LeakHook(FARPROC pfn)
{
...
void * EXPORTDBG __cdecl DBGMEM_EncapsulateFn(void * pvmalloc, char *pszSubsys, BOOL fCheckOften)
{
    ...
    strncpy(pdbgmem->szSubsys, pszSubsys, sizeof(pdbgmem->szSubsys));
