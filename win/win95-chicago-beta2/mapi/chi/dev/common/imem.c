...
FindInstData(LPINSTDATA *lplpInstPrev)
{
    ...
    while (lpInst)
    {
        if (lpInst->dwInstKey == *(DWORD *)CurrentA5)
            break;
        lpInstPrev = lpInst;
        lpInst = lpInst->next;
    }
    *lplpInstPrev = lpInstPrev;
    return(lpInst);
}
...
DisposeInstData(LPINSTDATA lpInstPrev, LPINSTDATA lpInst)
{
    ...
    if (lpInst->lpvInst[kInstMAPIX] == lpInst->lpvInst[kInstMAPIU])
    {
        ...
        if (lpInst == lpInstHead)
            lpInstHead = lpInst->next;
        else
    ...
    if (pv)
    {
        ...
        else
        {
            ...
            if (lpInstPrev)
                ...
                lpInstHead = lpInst;
            lpInst->dwInstKey = *(DWORD *)CurrentA5;
