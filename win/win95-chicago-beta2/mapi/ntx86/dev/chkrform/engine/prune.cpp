...
REVIEW: we may wish to use iterative deepening here and base things on time
-------------------------------------------------------------------------- */
                    ...
                    int prune_depth, int prune_size, int max_depth)
{
    ...
    AssertSz(prune_size <= SQRS_MAX, "death besige me. prune size is way to big");
    ...
    if (moves_made > 1)
        ...
        if (0!=rConfig.rPlayer[1].iUseMoveShuffling)
        #endif
...
continue_jump:
    ...
    if (best_move_type > TYPE_BLACK2)
        ...
        switch (best_move_type)
            {
            case TYPE_RED_JUMP0   :
                j = piece_order[0].m = red_jump_lut[best_move_number][1]  ;
            ...
            case TYPE_RED_JUMP2   :
                j = piece_order[0].m = red_jump_lut[best_move_number][3]  ;
            ...
            case TYPE_BLACK_JUMP0 :
                j = piece_order[0].m = black_jump_lut[best_move_number][1];
            ...
            case TYPE_BLACK_JUMP2 :
                j = piece_order[0].m = black_jump_lut[best_move_number][3];
            ...
            default:
                AssertSz(0, "a descriptive sentence about people");
                break;
            }
...
no_jump:
    ...
    //BUGfix make sure everything is kinged (in case we jumped to this point)
    if (RED & b[1]) b[1] |= KING;
    if (RED & b[2]) b[2] |= KING;
    if (RED & b[3]) b[3] |= KING;
    if (RED & b[4]) b[4] |= KING;
