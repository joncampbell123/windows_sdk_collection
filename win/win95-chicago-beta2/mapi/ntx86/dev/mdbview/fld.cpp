...
BEGIN_MESSAGE_MAP(CFolderDlg, CDialog)
    ...
    ON_LBN_DBLCLK(  IDC_PROPS,          OnFldProp)
    ON_LBN_SETFOCUS(IDC_FLDCURRENT,     OnCurrentFolder)
    ON_LBN_DBLCLK(  IDC_FLDCURRENT,     OnSetSearchDestFlds)
    ON_LBN_SETFOCUS(IDC_FLDCHILD,       OnChildFolder)
    ON_LBN_DBLCLK(  IDC_FLDCHILD,       OnOpenChildFolder)
    ON_COMMAND(     IDC_FLDOPENFLD,     OnOpenChildFolder)
    ON_LBN_SETFOCUS(IDC_FLDMESSAGES,    OnMessage)
    ON_LBN_SETFOCUS(IDC_MAPILOG,        OnMapiLog)
    ON_LBN_DBLCLK(  IDC_FLDMESSAGES,    OnOpenMessage)
    ON_COMMAND(     IDC_FLDOPENMSG,     OnOpenMessage)
    ON_COMMAND(     IDC_FLDCREATE,      OnFldCreate)
    ON_COMMAND(     IDC_FLDCOPYFOLDER,  OnCopyFolder)
    ON_COMMAND(     IDC_FLDSETREC,      OnSetReceiveFolder)
    ON_COMMAND(     IDC_FLDDELETE,      OnFldDelete)
    ON_COMMAND(     IDC_MSGCREATE,      OnMsgCreate)
    ON_COMMAND(     IDC_MSGABORT,       OnAbortSubmit)
    ON_COMMAND(     IDC_MSGCOPY,        OnCopyMessages)
    ON_COMMAND(     IDC_MSGDELETE,      OnMsgDelete)
    ...
    ON_COMMAND(     IDC_FLDCONTTABLE,   OnContTable)
    ON_COMMAND(     IDC_FLDHEIRTABLE,   OnHeirTable)
    ON_COMMAND(     IDC_FLDPROP,        OnFldProp)
    ON_COMMAND(     IDC_MDBPROP,        OnMDBProp)
    ON_COMMAND(     IDC_FLD_GETSEARCH,  OnGetSearchCriteria)
    ON_COMMAND(     IDC_FLD_SETSEARCH,  OnSetSearchCriteria)
    ON_COMMAND(     IDC_COPYTODEST,     OnSetCopyToDest)
    ON_COMMAND(     IDC_FLDSETMSGSTATUS,OnSetMsgStatus)
    ON_COMMAND(     IDC_FLDGETMSGSTATUS,OnGetMsgStatus)
    ON_COMMAND(     IDC_FLDEMPTYFOLDER, OnEmptyFolder)
    ON_COMMAND(     IDC_MAPILOGCLEAR,   OnClearMapiLog)
    ON_LBN_DBLCLK(  IDC_FLD_ASS,        OnOpenAssociated)
    ON_COMMAND(     IDC_FLD_ASS,        OnOpenAssociated)
    ON_LBN_SETFOCUS(IDC_FLD_ASS,        OnAssociated)
    ON_COMMAND(     IDC_FLDNOTIFFLD,    OnNotifFld)
    ON_COMMAND(     IDC_FLDNOTIFHEIR,   OnNotifHeir)
    ON_COMMAND(     IDC_FLDNOTIFASS,    OnNotifAss)
    ON_COMMAND(     IDC_FLDNOTIFCONT,   OnNotifCont)
...
BOOL CFolderDlg::InitFldDialog()
{
    ...
    dwReturn = SendDlgItemMessage(IDC_FLDTYPE,LB_ADDSTRING,0,
                    (LPARAM) GetString("FolderType",lpspvaFldType[0].Value.l,NULL) );
...
Setup:
    ...
    if(!hlibTBLVU)
    {
        GetDlgItem(IDC_FLDCONTTABLE     )->EnableWindow(0);
        GetDlgItem(IDC_FLDHEIRTABLE     )->EnableWindow(0);
    }
    else
    {
        GetDlgItem(IDC_FLDCONTTABLE     )->EnableWindow(1);
        GetDlgItem(IDC_FLDHEIRTABLE     )->EnableWindow(1);
    }

    //Disable table Viewer button if DLL is not present.
    if(!hlibPROPVU)
    {
        GetDlgItem(IDC_FLDPROP          )->EnableWindow(0);
        GetDlgItem(IDC_MDBPROP          )->EnableWindow(0);
    }
    else
    {
        GetDlgItem(IDC_FLDPROP          )->EnableWindow(1);
        GetDlgItem(IDC_MDBPROP          )->EnableWindow(1);
    }


    GetDlgItem(IDC_FLDCREATE            )->EnableWindow(TRUE);
    GetDlgItem(IDC_FLDDELETE            )->EnableWindow(TRUE);
    GetDlgItem(IDC_FLDSETREC            )->EnableWindow(TRUE);
    GetDlgItem(IDC_FLDOPENFLD           )->EnableWindow(TRUE);
    GetDlgItem(IDC_FLDEMPTYFOLDER       )->EnableWindow(TRUE);


    GetDlgItem(IDC_MSGCREATE            )->EnableWindow(TRUE);
    GetDlgItem(IDC_MSGABORT             )->EnableWindow(TRUE);
    GetDlgItem(IDC_MSGCOPY              )->EnableWindow(TRUE);
    GetDlgItem(IDC_MSGDELETE            )->EnableWindow(TRUE);
    GetDlgItem(IDC_FLDOPENMSG           )->EnableWindow(TRUE);
    GetDlgItem(IDC_FLDSETMSGSTATUS      )->EnableWindow(TRUE);
    GetDlgItem(IDC_FLDGETMSGSTATUS      )->EnableWindow(TRUE);
...
void CFolderDlg::OnAbortSubmit()
{
    ...
    if( AbortSubmitDlg.DoModal() == IDOK )
    {
        ...
        //$ FUTURE, should split up cbEntryID and lpEntryid and allow mofidication and then
...
void CFolderDlg::OnProperty()
{
    ...
    dwIndex = SendDlgItemMessage(IDC_FLDMESSAGES,LB_SETSEL,(WPARAM)FALSE ,(LPARAM) -1);
    dwIndex = SendDlgItemMessage(IDC_FLD_ASS,    LB_SETSEL,(WPARAM)FALSE ,(LPARAM) -1);
    dwIndex = SendDlgItemMessage(IDC_MAPILOG,    LB_SETCURSEL,(WPARAM) -1 ,0 );
}
...
void CFolderDlg::OnCurrentFolder()
{
    DWORD   dwIndex;
