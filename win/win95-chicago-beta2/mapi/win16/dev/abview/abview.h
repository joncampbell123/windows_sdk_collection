...
#define PR_AB_SERVER            PROP_TAG( PT_STRING8,           0x6600)
...
BEGIN_MESSAGE_MAP( CMainWindow, CFrameWnd )
    ...
    ON_COMMAND( IDM_ADMIN,          OnAdminServices )
...
/*
 ...
 -
 *  Purpose:
 *      Defines the controls for the CAdminDlg Dialog
 *
 *
 */

class CAdminDlg : public CModalDialog
{
public:
     CAdminDlg() 
        : CModalDialog( IDD_ADMIN )
    {
        DoModal();
    }
    ...
    void OnOK();
    void OnCancel();
    void HackServicesIni(char*, char*, char*, CString);
    DECLARE_MESSAGE_MAP();
};

BEGIN_MESSAGE_MAP(CAdminDlg, CModalDialog)
END_MESSAGE_MAP()
