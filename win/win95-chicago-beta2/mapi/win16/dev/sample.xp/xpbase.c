...
SERVICEWIZARDENTRY WizardEntry;
...
ScCheckLogonProps(LPXPDLG lpXPDialog, BOOL fUIAllowed)
{
    ...
    /* Do a little loop-back test to validate PR_SAMPLE_EMAIL_ADDRESS
       ...
       The WFW and Chicago Redirector can't talk to itself, so we only
...
WizardEntry (HINSTANCE hInstance,
    LPSTR lpszRsrcName,
    FARPROC FAR * lpfnDlgProc,
    LPMAPIPROP lpMapiProp)
{
    /*  Should probably mutex access here */

    lstrcpyA (lpszRsrcName, "SampleTransportWizard");
    *lpfnDlgProc = (FARPROC)WizardWndProc;
