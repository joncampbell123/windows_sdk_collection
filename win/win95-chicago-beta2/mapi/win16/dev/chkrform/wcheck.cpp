/****************************************************************************
    ...
    PURPOSE: Convert a small game into a MAPI FORM

$REVIEW: this module needs to be cleaned up .. I am aware of this -kylel

****************************************************************************/

//---------------------------------------------------------------------------
// Other $$REVIEWS: consider removing all uses of SHOW_DEBUG, etc, since
// they were never written.
//
// Do we work in Black/White mode. The function CheckCreate can be removed
// if we don't want to bother. (it sets the brushes based on the mode).
//
// Localize?
//$REVIEW: make this configurable rConfig.iMustJump = (int)SendDlgItemMessage(hDlg, IDCB_MUST_JUMP, BM_GETCHECK, 0, 0L);
//---------------------------------------------------------------------------
...
#ifndef WIN32                   // Windows 3.x uses a FARPROC
#define DLGPROC FARPROC         // for dialogs
#endif

unsigned int _charmax;  // This is because \mbudev\limits.h has 
                        // been hacked by dgreen $REVIEW
#ifdef WIN32
...
static  POINT       pntStart;           // $$REVIEW: Initialize this!
                                        // For buttonup/down processing.

char szAppName[] = APP_NAME;            // The name of this application

#ifdef DEBUG
char szTitle[]   = "Checkers Test Form (debug)";
#else
#ifdef TEST
char szTitle[]   = "Checkers Test Form (test)";
#else
char szTitle[]   = "Checkers Test Form";
#endif
#endif
...
HBRUSH  hbrBlack;           // Review what of this is needed.
HBRUSH  hbrWhite;           // Review what of this is needed.
HBRUSH  hbrRed;             // Review what of this is needed.
HBRUSH  hbrBckgrnd;         // Review what of this is needed.
HBRUSH  hbrBlue;            // Review what of this is needed.
HBRUSH  hbrHuman;           // Review what of this is needed.
HBRUSH  hbrComputer;        // Review what of this is needed.
HBRUSH  hbrDkgrey;          // Review what of this is needed.
HPEN    hpenDkgrey;         // Review what of this is needed.
...
#define SQR_XSIZE   rConfig.iSquareSize     // $$REVIEW: ASPECT ratio?
#define SQR_YSIZE   rConfig.iSquareSize     // $$REVIEW: ASPECT ratio?
...
BOARD CMoveList::GetNextBoard(BOARD b, int *piPlayerTurn)
{
    ...
    else
    {
        AssertSz((BLACK == m_prCurrentSearchBoard->iPlayerTurn) ||
                 (RED == m_prCurrentSearchBoard->iPlayerTurn), 
                 "Kurt Cobain just died. 04/07/94");
...
BOOL CMoveList::NewMove(BOARD b, int iPlayerTurn)
{
    ...
    {
        //$$REVIEW
        MessageBox(NULL, "Out of memory. Replace this.", "OOM", MB_ICONEXCLAMATION | MB_OK);
        return FALSE;
    }
    else
    {
...
BOOL CMoveList::PurgeBoards()
{
    ...
    AssertSz(m_fClassInitialized, "MoveList class not initialized in PurgeBoards()");
...
long _inline GetDlgItemLong(HWND hDlg, WORD id);
int _inline GetDlgItemInt(HWND hDlg, WORD id);
void _inline SetDlgItemInt(HWND hDlg, WORD id, int iValue);
void _inline SetDlgItemLong(HWND hDlg, WORD id, long lValue);
BOOL ValidateNumeric(char *pszText);
LONG _inline GetPrivateProfileLong(LPCSTR lpszSection, LPCSTR lpszEntry, long lDefault, LPCSTR lpszFileName);
...
long _inline GetDlgItemLong(HWND hDlg, WORD id)
{
    ...
    // $REVIEW: Verify that length is < sizeof(szTemp);
...
void _inline
SetDlgItemInt(HWND hDlg, WORD id, int iValue)
{
    char szTemp[20];     // Can't imagine a number longer than this.
...
LONG _inline GetPrivateProfileLong(LPCSTR lpszSection, LPCSTR lpszEntry, long lDefault, LPCSTR lpszFileName)
{
    ...
    // $$REVIEW: I do this "~~" to verify that we're not getting the default,
    // so I can set the default myself.
    ...
    int nCmdShow)
{
    ...
    // NOTE: Initial idiot asserts
    
    AssertSz(((RED == PLAYER1) && (BLACK == PLAYER2)) || 
             ((BLACK == PLAYER1) && (RED == PLAYER2)), "Players are schizoid");

    // NOTE: Check for previous instances. Initialize shared stuff.
    // Exit if this fails.
    ...
    // I am a form.  Hear me roar!
    #ifndef NO_FORM
    if (NOERROR != HrStartOleAndRegisterClassFactory())
        {
        AssertSz(0,"HrStartOleAndRegisterClassFactory() failed");
        return FALSE;
        }
    #endif

    #ifndef NO_FORM
    if (lpCmdLine && lpCmdLine[0] == '/' && lpCmdLine[1] == 'f')
    {
        #ifdef DEBUG
...
VOID _inline ReadPlayerInfo(int iPlayerNum, char *pszPlayerName)
{
    AssertSz((iPlayerNum == BLACK) || (iPlayerNum == RED), "Invalid Player Num");
    AssertSz(pszPlayerName, "Playername can't be NULL");

    // Default for PLAYER1 is HUMAN
    // Default for PLAYER2 is COMPUTER
    ...
    if (PLAYER1 == iPlayerNum)
        rConfig.rPlayer[iPlayerNum].iPlayerType = GetPrivateProfileInt (pszPlayerName, "PlayerType", HUMAN_PLAYER, INI_NAME);
    else
        rConfig.rPlayer[iPlayerNum].iPlayerType = GetPrivateProfileInt (pszPlayerName, "PlayerType", COMPUTER_PLAYER , INI_NAME);
    #endif
    ...
    rConfig.rPlayer[iPlayerNum].iBookMoveDepth = GetPrivateProfileInt (pszPlayerName, "BookMoveDepth", 3, INI_NAME);
    ...
    rConfig.rPlayer[iPlayerNum].lAlphaPruningOriginalDepth = GetPrivateProfileLong (pszPlayerName, "AlphaPruningOriginalDepth", 1, INI_NAME);
    rConfig.rPlayer[iPlayerNum].lAlphaPruningWidth = GetPrivateProfileLong (pszPlayerName, "AlphaPruningWidth", 35, INI_NAME);
    rConfig.rPlayer[iPlayerNum].iShortJumpEval = GetPrivateProfileInt (pszPlayerName, "ShortJumpEval", 1, INI_NAME);

    // Get computer weighting options.

    rConfig.rPlayer[iPlayerNum].rWeights.lKingWeight = GetPrivateProfileLong (pszPlayerName, "KingWeight", 50, INI_NAME);
    rConfig.rPlayer[iPlayerNum].rWeights.lPieceWeight = GetPrivateProfileLong (pszPlayerName, "PieceWeight", 350, INI_NAME);          
    rConfig.rPlayer[iPlayerNum].rWeights.lBlankWeight = GetPrivateProfileLong (pszPlayerName, "BlankWeight", 1, INI_NAME);
    rConfig.rPlayer[iPlayerNum].rWeights.lMoveTheoryWeight = GetPrivateProfileLong (pszPlayerName, "MoveTheoryWeight", 20, INI_NAME);
    rConfig.rPlayer[iPlayerNum].rWeights.lCenterPosWeight = GetPrivateProfileLong (pszPlayerName, "CenterPositionWeight", 2, INI_NAME);
    rConfig.rPlayer[iPlayerNum].rWeights.lDoubleCornerWeight = GetPrivateProfileLong (pszPlayerName, "DoubleCornerWeight", 1, INI_NAME);
    rConfig.rPlayer[iPlayerNum].rWeights.lHoldingKingRowWeight = GetPrivateProfileLong (pszPlayerName, "HoldingKingRowWeight", 1, INI_NAME);
    rConfig.rPlayer[iPlayerNum].rWeights.lEdgesWeight = GetPrivateProfileLong (pszPlayerName, "EdgesWeight", -1, INI_NAME);
    rConfig.rPlayer[iPlayerNum].rWeights.lFormationWeight = GetPrivateProfileLong (pszPlayerName, "FormationWeight", 1, INI_NAME);
    rConfig.rPlayer[iPlayerNum].lFormationCutoff = GetPrivateProfileLong (pszPlayerName, "FormationCutoff", 5, INI_NAME);
}
...
VOID    ReadINI ()
{
    ...
    rConfig.iShowCommandWindow= GetPrivateProfileInt ("Config", "ShowCommandWindow", 1, INI_NAME);
    rConfig.iShowDebug = GetPrivateProfileInt ("Config", "ShowDebug", 1, INI_NAME);
    rConfig.iShowMoves = GetPrivateProfileInt ("Config", "ShowMoves", 1, INI_NAME);
    rConfig.iShowTimer = GetPrivateProfileInt ("Config", "ShowTimer", 1, INI_NAME);
    rConfig.iShowOpenInfo = GetPrivateProfileInt ("Config", "ShowOpenInfo", 1, INI_NAME);
    #ifndef NO_FORM
    rConfig.iBoardFlipped = 1;
    #else
    rConfig.iBoardFlipped = GetPrivateProfileInt ("Config", "BoardFlipped", 1, INI_NAME);
    #endif
...
VOID _inline WritePlayerInfo(int iPlayerNum, char *pszPlayerName)
{
    AssertSz((iPlayerNum == RED) || (iPlayerNum == BLACK), "Invalid Player Num");
    AssertSz(pszPlayerName, "Playername can't be NULL");

    // Player specific config items
    // $$REVIEW: WritePrivateProfileLong where needed!!
    ...
    WritePrivateProfileInt (pszPlayerName, "BookMoveDepth", rConfig.rPlayer[iPlayerNum].iBookMoveDepth, INI_NAME);
    WritePrivateProfileInt (pszPlayerName, "UseGoodMoveSkipping", rConfig.rPlayer[iPlayerNum].lUseGoodMoveSkipping, INI_NAME);
    ...
    WritePrivateProfileInt (pszPlayerName, "AlphaPruningOriginalDepth", rConfig.rPlayer[iPlayerNum].lAlphaPruningOriginalDepth, INI_NAME);
    WritePrivateProfileInt (pszPlayerName, "AlphaPruningWidth", rConfig.rPlayer[iPlayerNum].lAlphaPruningWidth, INI_NAME);
    WritePrivateProfileInt (pszPlayerName, "ShortJumpEval", rConfig.rPlayer[iPlayerNum].iShortJumpEval, INI_NAME);

    // Player specific weighting config items

    WritePrivateProfileInt (pszPlayerName, "KingWeight", rConfig.rPlayer[iPlayerNum].rWeights.lKingWeight, INI_NAME);
    WritePrivateProfileInt (pszPlayerName, "PieceWeight", rConfig.rPlayer[iPlayerNum].rWeights.lPieceWeight, INI_NAME);
    WritePrivateProfileInt (pszPlayerName, "BlankWeight", rConfig.rPlayer[iPlayerNum].rWeights.lBlankWeight, INI_NAME);
    WritePrivateProfileInt (pszPlayerName, "MoveTheoryWeight", rConfig.rPlayer[iPlayerNum].rWeights.lMoveTheoryWeight, INI_NAME);
    WritePrivateProfileInt (pszPlayerName, "CenterPositionWeight", rConfig.rPlayer[iPlayerNum].rWeights.lCenterPosWeight, INI_NAME);
    WritePrivateProfileInt (pszPlayerName, "DoubleCornerWeight", rConfig.rPlayer[iPlayerNum].rWeights.lDoubleCornerWeight, INI_NAME);
    WritePrivateProfileInt (pszPlayerName, "HoldingKingRowWeight", rConfig.rPlayer[iPlayerNum].rWeights.lHoldingKingRowWeight, INI_NAME);
    WritePrivateProfileInt (pszPlayerName, "EdgesWeight", rConfig.rPlayer[iPlayerNum].rWeights.lEdgesWeight, INI_NAME);
    WritePrivateProfileInt (pszPlayerName, "FormationWeight", rConfig.rPlayer[iPlayerNum].rWeights.lFormationWeight, INI_NAME);
    WritePrivateProfileInt (pszPlayerName, "FormationCutoff", rConfig.rPlayer[iPlayerNum].lFormationCutoff, INI_NAME);
...
VOID    WriteINI ()
{
    ...
    WritePrivateProfileInt ("Config", "ShowCommandWindow", rConfig.iShowCommandWindow, INI_NAME);
    WritePrivateProfileInt ("Config", "ShowDebug", rConfig.iShowDebug, INI_NAME);
    WritePrivateProfileInt ("Config", "ShowMoves", rConfig.iShowMoves, INI_NAME);
    WritePrivateProfileInt ("Config", "ShowTimer", rConfig.iShowTimer, INI_NAME);
    WritePrivateProfileInt ("Config", "ShowOpenInfo", rConfig.iShowOpenInfo, INI_NAME);
    WritePrivateProfileInt ("Config", "BoardFlipped", rConfig.iBoardFlipped, INI_NAME);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    // $$REVIEW: This conditional compilation sucks.

    #ifdef WIN32
        hbrDkgrey     = CreateSolidBrush(RGB(0x55,0x55,0x55));
    #else
        //$BUG: looks like hdc == NULL to me
    ...
    hWnd = CreateWindow(
        ...
        GetPrivateProfileInt ("Window", "Left",  13  /* REVIEW:CW_USEDEFAULT */, INI_NAME),
...

{
    ...
    HBITMAP hOldbm      = NULL;     // Store old Bitmap handle ($$REVIEW: ??)
    ...
    for (i=1; i <= 32; i++)
    {
        ...
        AssertSz(iLocation > 0, "Bee-Gees are a bit dated, methinks");
    ...
    {
        // $$BUG: Replace "*10" with location of piece rack.
...
void _inline DisplayGameDrawn(HWND hWnd, int iPlayerTurn)
{
    ...
    AssertSz((rGameState.iPlayerTurn == BLACK) || 
             (rGameState.iPlayerTurn == RED), "Voice mail bites");
...

{
    ...
    // $$REVIEW: Do I still need this? What the hell am I doing?
...
BOOL fValidSetupPiece(BOARD b, int ix, int iy)
{
    ...
    if (iEngine != -1)
    {
        ...
        // $$REVIEW:
//      // $$FIXED BUG 27? - Allow blank piece to be dragged in setup.
//      if ((iPiece & RED) || (iPiece & BLACK))
...
BOOL fValidPlayerPiece(BOARD b, int ix, int iy)
{
    ...
    AssertSz(b, "Where's the frikkin board?");
...
{
    // $$REVIEW: this hMenu deal may be from the old WndProc stuff.
    // It works now, but should probably skip this line.
...
BOOL fHandle_WM_CREATE(HWND hWnd, CREATESTRUCT FAR* lpCreateStruct)
{
    ...
    rGameState.fComputerBusy = FALSE;       // Is the computer thinking? $$REVIEW
...
// $$REVIEW: we may want this. (don't do anything based on keystroke if we're
// minimized (since the keystroke was probably not intended for the app).
VOID vHandle_WM_KEYDOWN(HWND hWnd, UINT vk, BOOL fDown, int cRepeat, UINT flags)
{
//  if (IsIconic(hWnd))
//  {
//      return FALSE;
...
//
// Notes ($$REVIEW): I should probably figure out what the hell I'm doing
//      here, since this code was snagged from elsewhere.
//
//-------------------------------------------------------------------------

VOID vHandle_WM_ACTIVATE(HWND hWnd, UINT state, HWND hwndActDeact, BOOL fMinimized)
{                                         
    if (!GetSystemMetrics(SM_MOUSEPRESENT))
    {
        ...
        // $$REVIEW: This should already be correctly repacked for WIN32

//              if (hwndActDeact != hWnd)
//              {
//                  POINT pntCur;
//
//                  pntCur.x = MousePos.x;
//                  pntCur.y = MousePos.y;
//                  ClientToScreen(hWnd, (LPPOINT)&pntCur);
//                  SetCursorPos(pntCur.x, pntCur.y);
//                  ShowCursor(state);
//              }
        }
        else
...
VOID vHandle_WM_MOUSEMOVE(HWND hWnd, int x, int y, UINT keyFlags)
{
    ...
    // $$REVIEW: The SetCapture below causes button presses to get "lost", 
    // and lots of beeping to the user. Weird. Find out why this is.
...
{
    PAINTSTRUCT ps;    // REVIEW: What can I initialize this to.
...
VOID vHandle_WM_COMMAND(HWND hWnd, int wmId, HWND hwndCtl, UINT wmEvent)
{
    ...
    switch (wmId)
    {
        ...
        case IDM_FILENEW:
        {
            ...
            {
                case IDYES:
                    MessageBox(hWnd, "Save Board code NYI, I think", "Save Board NYI", MB_ICONEXCLAMATION | MB_OK);
                    SaveAsBoard(b);
        ...
        case IDM_SETUP_BOARD:
            ...
            //$$REVIEW (JEFF): Added rGameState.fGameOver fixes here, depending on 
            //who's turn it is when we exit.
            break;
        ...
        case IDM_BACK:
//          if (!rGameState.fPaused)
            {
                // Change menu to say "Continue"
                rGameState.fPaused = TRUE;
                rGameState.fGameOver = FALSE;

                // $$REVIEW: (JEFF) Add game over fixes.
...

{
    ...
    switch (message)
    {
        ...
        HANDLE_MSG(hWnd, WM_KEYDOWN, vHandle_WM_KEYDOWN);
        //###$REVIEW!HANDLE_MSG(hWnd, WM_ACTIVATE, vHandle_WM_ACTIVATE); need to eat the mouse click that sets focus
        ...
        case WM_CLOSE:
            ...
            if (pfrm)
            {
                TraceTag(tagForm,"wcheck pfrm->Forget");
                pfrm->Forget();
                TraceTag(tagForm,"wcheck pfrm->Release");
                pfrm->Release();
            }
            #endif
...
long WINAPI ComputerMove()
{
    ...
    AssertSz(((BLACK == rGameState.iPlayerTurn) || 
        ...
        PlayBestMove(b, rGameState.iPlayerTurn, 0, 0,
            ...
            (int)rConfig.rPlayer[rGameState.iPlayerTurn].iMaxRecursionDepth   /* max depth */ //REVIEW:!!! wrong type
            );
    ...
    switch (iGameState)
    {
        ...
        case GAME_PLAYABLE:
            ...
            break;         
        default:
            AssertSz(FALSE, "Kurt Cobain is STILL dead");              
    }
...
void inline SetupRButtonDown(HWND hWnd, UINT keyFlags, int x, int y)
{
    ...
    if (fValidSetupPiece(b, iSX, iSY))
    {
        ...
        AssertSz((iSX >= 0) && ((iSX <= 7) || (iSX == 10)), "iSX out of range on ButtonDown");
        AssertSz((iSY >= 0) && (iSY <= 7), "iSY out of range on ButtonDown");
        // $$REVIEW: Why did I comment this out?
//      if ((iSX != 10))
            vClearMovingPiece(hDC, iSX, iSY);
        rGameState.fMoveInProgress = TRUE;
    }
...
void inline SetupLButtonUp(HWND hWnd, WPARAM uParam, int x, int y)
{
    ...
    {
        int     iEngineStart=0; // 3-29-1994 12:09:00 bug fix uninitialized vars REVIEW
        int     iEngineEnd=0;   // 3-29-1994 12:09:00 bug fix uninitialized vars REVIEW
        int     iSX = 0;
        int     iSY = 0;
        int     iEX = 0;
        int     iEY = 0;
                                        ...
                                        LPARAM lParam)
{
...

    {
        case WM_INITDIALOG:
            ...
            SendDlgItemMessage(hDlg, IDCB_SHORT_JUMP_EVAL, BM_SETCHECK, prPlayer->iShortJumpEval, 0L);

            // Set Weights Edit Controls

            SetDlgItemLong(hDlg, IDEC_KING, prPlayer->rWeights.lKingWeight);
            SetDlgItemLong(hDlg, IDEC_CENTER_POSITION, prPlayer->rWeights.lCenterPosWeight);
            SetDlgItemLong(hDlg, IDEC_DOUBLE_CORNERS, prPlayer->rWeights.lDoubleCornerWeight);
            SetDlgItemLong(hDlg, IDEC_KING_ROW, prPlayer->rWeights.lHoldingKingRowWeight);
            SetDlgItemLong(hDlg, IDEC_MOVE_THEORY, prPlayer->rWeights.lMoveTheoryWeight);
            SetDlgItemLong(hDlg, IDEC_PIECE, prPlayer->rWeights.lPieceWeight);
            SetDlgItemLong(hDlg, IDEC_BLANK, prPlayer->rWeights.lBlankWeight);
            SetDlgItemLong(hDlg, IDEC_EDGES, prPlayer->rWeights.lEdgesWeight);
            SetDlgItemLong(hDlg, IDEC_FORMATION, prPlayer->rWeights.lFormationWeight);
            SetDlgItemLong(hDlg, IDEC_FORMATION_CUTOFF, prPlayer->lFormationCutoff);
            ...
            SetDlgItemInt(hDlg, IDEC_BOOK_MOVE_DEPTH, prPlayer->iBookMoveDepth);
            return TRUE;
        ...
        case WM_COMMAND:
            switch (wParam)
            {
                case IDOK:
                    ...
                    prPlayer->iShortJumpEval = (int)SendDlgItemMessage(hDlg, IDCB_SHORT_JUMP_EVAL, BM_GETCHECK, 0, 0L);
                    ...
                    prPlayer->iBookMoveDepth             = GetDlgItemInt(hDlg, IDEC_BOOK_MOVE_DEPTH);

                    prPlayer->rWeights.lKingWeight       = GetDlgItemLong(hDlg, IDEC_KING);
                    prPlayer->rWeights.lCenterPosWeight  = GetDlgItemLong(hDlg, IDEC_CENTER_POSITION);
                    prPlayer->rWeights.lDoubleCornerWeight  = GetDlgItemLong(hDlg, IDEC_DOUBLE_CORNERS);
                    prPlayer->rWeights.lHoldingKingRowWeight  = GetDlgItemLong(hDlg, IDEC_KING_ROW);
                    prPlayer->rWeights.lMoveTheoryWeight  = GetDlgItemLong(hDlg, IDEC_MOVE_THEORY);
                    prPlayer->rWeights.lPieceWeight  = GetDlgItemLong(hDlg, IDEC_PIECE);
                    prPlayer->rWeights.lBlankWeight  = GetDlgItemLong(hDlg, IDEC_BLANK);
                    prPlayer->rWeights.lEdgesWeight  = GetDlgItemLong(hDlg, IDEC_EDGES);
                    prPlayer->rWeights.lFormationWeight  = GetDlgItemLong(hDlg, IDEC_FORMATION);
                    prPlayer->lFormationCutoff  = GetDlgItemLong(hDlg, IDEC_FORMATION_CUTOFF);
...
//            SendDlgItemMessage(hDlg, IDCB_SHOW_TIMER, BM_SETCHECK, rConfig.iShowTimer, 0L);
            return TRUE;
