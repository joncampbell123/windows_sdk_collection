...
void IDispDlg::DoGetInfo()
{
    ...
    for (UINT n = 1 ; n <= uiTypeInfoCount ; n++)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            {
                UINT ui ;
                wsprintf( szTemp, "%s (%u)", (LPTSTR)rgbstrNames[0], n ) ;
                ui = ComboBox_AddString( m_hwndTypeInfo, szTemp ) ;
...
void IDispDlg::OnSelChangeFunctions()
{
    ...
    if (pFuncDesc != NULL)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            wsprintf( szTemp, _T("Name = %s"), (bstrName == NULL ? _T("n/a") : bstrName ) ) ;
            ...
            wsprintf( szTemp, _T("Help File = %s"), (bstrHelpFile == NULL ? _T("n/a") : bstrHelpFile) ) ;
            ListBox_AddString( m_hwndInfo, szTemp ) ;
