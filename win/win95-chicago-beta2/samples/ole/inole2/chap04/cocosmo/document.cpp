...
BOOL CCosmoDoc::FInit(PDOCUMENTINIT pDI)
    ...
    hr=CoCreateInstance(CLSID_Polyline4, NULL, CLSCTX_INPROC_SERVER
        , IID_IPolyline4, (PPVOID)&m_pPL);
...
UINT CCosmoDoc::USave(UINT uType, LPTSTR pszFile)
     ...
     * so all the code in Chapter 2 Cosmo that dealt with 1.0 and
...
STDMETHODIMP CPolylineAdviseSink::QueryInterface(REFIID riid
    ...
    *ppv=NULL;
    ...
    if (IID_IUnknown==riid || IID_IPolylineAdviseSink4==riid)
        *ppv=this;
