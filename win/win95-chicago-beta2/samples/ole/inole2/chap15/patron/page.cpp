...
BOOL CPage::FOpen(LPSTORAGE pIStorage)
    ...
    mbstowcs(pwcsFile, szTemp, MAX_PATH);
#else
    OLECHAR * pwcsFile = szTemp;
#endif
    hr=pIStorage->OpenStorage(pwcsFile, NULL, dwMode, NULL, 0
        , &m_pIStorage);

    if (FAILED(hr))
        {
        hr=pIStorage->CreateStorage(pwcsFile, dwMode, 0, 0
            , &m_pIStorage);
        fCreated=TRUE;
        }
    ...
    m_pIOleItemContainer=new CImpIOleItemContainer(this, this
        , FALSE);
    ...
    if (!OleUIUpdateLinks(pIUILinks, hWndDoc, pwcsTemp, cLinks))
#else
...
BOOL CPage::TenantCreate(TENANTTYPE tType, LPVOID pv
    ...
    if (NULL!=m_pmkFile)
        ...
        wcstombs(pszTemp, pszFile, MAX_PATH);
        pTenant->NotifyOfRename(pszTemp, m_pmkFile, pmkPage);
#else
...
/*
 * CPage::FConvertObject
 *
 * Purpose:
 *  Invokes and handles the results of the Change Type dialog
 *
 * Parameters:
...
BOOL CPage::FConvertObject(HWND hWndFrame, BOOL fNoServer)
    {
    HRESULT         hr;
    OLEUICONVERT    ct;
    TENANTTYPE      tType;
    ...
    m_pTenantCur->ObjectClassFormatAndIcon(&ct.clsid, &ct.wFormat
        , &szTemp, &ct.hMetaPict);
    ct.lpszUserType = (LPOLESTR)OleStdMalloc(strlen(szTemp) + 1);
    mbstowcs(ct.lpszUserType, szTemp, strlen(szTemp) + 1);
    OleStdFree(szTemp);
#else
    m_pTenantCur->ObjectClassFormatAndIcon(&ct.clsid, &ct.wFormat
        , &ct.lpszUserType, &ct.hMetaPict);
#endif
    ...
    if (OLEUI_OK==uRet)
        ...
        if ((DVASPECT_ICON==ct.dvAspect && ct.fObjectsIconChanged)
            ...
            m_pPG->m_fDirty=m_pTenantCur->FSwitchOrUpdateAspect(hMem
                , FALSE);
            }
        ...
        if ((CF_SELECTCONVERTTO & ct.dwFlags)
            ...
            if (SUCCEEDED(hr))
                ...
                //Reload and update.
                m_pTenantCur->FLoad(m_pIStorage, &ti);
        ...
        if (CF_SELECTACTIVATEAS & ct.dwFlags)
             ...
             *  1.  Add the TreatAs entry in the regDB and call
             *      CoTreatAsClass which is all done in
            ...
            hr=OleStdDoTreatAsClass(ct.lpszUserType, ct.clsid
                , ct.clsidNew);
...
void CPage::SwitchActiveTenant(PCTenant pTenant)
    ...
    if (NULL!=m_pTenantCur)
        ...
        for (i=0; i < m_cTenants; i++)
            ...
            if (FTenantGet(i, &pTenList, FALSE))
                {
