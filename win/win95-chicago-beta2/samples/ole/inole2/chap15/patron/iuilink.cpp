 ...
 * interface necessary to use the Links Dialog in OLE2UI.  This
...
CIOleUILinkContainer::~CIOleUILinkContainer(void)
    {
    if (NULL!=m_pDelIUILinks)
        m_pDelIUILinks->Release();
...
BOOL CIOleUILinkContainer::FInit(void)
    {
    HRESULT     hr;
...
STDMETHODIMP_(DWORD) CIOleUILinkContainer::GetNextLink(DWORD dwLink)
    ...
    for ( ; m_iTenant < m_pPage->m_cTenants; m_iTenant++)
        {
        if (m_pPage->FTenantGet(m_iTenant, &pTenant, FALSE))
            {
 ...
 *  pszName         LPOLESTR to the displayable name of the source.
...
STDMETHODIMP CIOleUILinkContainer::SetLinkSource(DWORD dwLink
    , LPOLESTR pszName, ULONG cchName, ULONG *pchEaten
    , BOOL fValidate)
    {
 ...
 *  ppszName        LPOLESTR * in which to return the new source
 *                  name
 *  pcchName        ULONG * in which to return the length of
 *                  pszName
 *  ppszFullLink    LPOLESTR * in which to return the full name of
 *                  the class of linked object.
 *  ppszShortLink   LPOLESTR * in which to return the short name of
 *                  the class of linked object.
...
STDMETHODIMP CIOleUILinkContainer::GetLinkSource(DWORD dwLink
    , LPOLESTR *ppszName, ULONG *pcchName, LPOLESTR *ppszFullLink
    , LPOLESTR *ppszShortLink, BOOL *pfSourceAvail, BOOL * pfSelected)
    {
    HRESULT         hr;
    PCTenant        pTenant=(PCTenant)dwLink;
