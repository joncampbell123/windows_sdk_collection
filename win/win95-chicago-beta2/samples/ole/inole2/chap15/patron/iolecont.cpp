...
STDMETHODIMP CImpIOleItemContainer::GetObject(LPOLESTR pszItem
    ...
    *ppv=NULL;
    ...
    if (m_fDoc)
        ...
        TCHAR szItem[256];
        wcstombs(szItem, pszItem, 256);
        if (FTenantFromName(szItem, &pTenant))
#else
 ...
 *
 * Parameters:
 *  pszItem         LPTSTR of the tenant to locate.
...
BOOL CImpIOleItemContainer::FTenantFromName(LPTSTR pszItem
    , PCTenant *ppTenant)
    {
    DWORD       dw;
    char        szTemp[40];     //ANSI for atol
     ...
     * The string conversion macro is in book1632.h.
     */
