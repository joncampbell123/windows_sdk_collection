      ...
      case WM_DRAWITEM:
      {
         ...
         if ((ODA_DRAWENTIRE | ODA_SELECT) & lpDIS->itemAction) {
            ...
            if (lpLI->lpszDisplayName) {
               W2A (lpLI->lpszDisplayName, sz, OLEUI_CCHPATHMAX);
               lpsz = ChopText(
                     lpDIS->hwndItem,
                     nColPos[1] - nColPos[0]
            ...
            if (lpLI->lpszShortLinkType) {
               ...
               rcClip.bottom = lpDIS->rcItem.bottom;
               W2A (lpLI->lpszShortLinkType, szAnsiStr, 256);
               ExtTextOut(
                     lpDIS->hDC,
                     rcClip.left,
                     rcClip.top,
            ...
            if (lpLI->lpszAMX) {
               ...
               rcClip.bottom = lpDIS->rcItem.bottom;
               W2A (lpLI->lpszAMX, szAnsiStr, 256);
               ExtTextOut(
                     lpDIS->hDC,
                     rcClip.left,
                     rcClip.top,
...
//change
      ...
      case WM_COMPAREITEM:
      {
         ...
         return OLESTRCMP(lpLI1->lpszDisplayName,lpLI2->lpszDisplayName);
      }
   ...
   if (uMsg == uMsgFileOKString) {
      ...
      */
      ...
      //    MAKELPARAM(GetDlgItem(hDlg, edt1), EN_KILLFOCUS));
                SendCommand (hDlg, edt1, EN_KILLFOCUS, GetDlgItem(hDlg, edt1));
      ...
      A2W (lpCshData->szEdit, szUniStr, 256);
      ...
      if (lpCshData->bItemNameStored && lpCshData->bFileNameStored) {
         ...
         W2A (lpLI->lpszDisplayName, szAnsiStr, 256);
         DiffPrefix(
               szAnsiStr,
...
//                  lpCshData->nFileLength + 1);
            W2A(lpLI->lpszDisplayName, (LPSTR)lpCshData->szFileName,lpCshData->nFileLength + 1);
            W2A(lpLI->lpszDisplayName, (LPSTR)lpCshData->szEdit, sizeof(lpCshData->szEdit));
         } else {
            lpCshData->szFileName[0] = '\0';
            lpCshData->szEdit[0] = '\0';
         }
         if (lpLI->lpszItemName)
            W2A (lpLI->lpszDisplayName, (LPSTR)lpCshData->szItemName, sizeof(lpCshData->szItemName));
         else
      ...
      case WM_COMMAND:
         ...
         if (wID == edt1) {
            HWND hEdit = (HWND)hWndMsg;

            switch (wCode) {
               case EN_SETFOCUS:
               // SendMessage(hEdit, EM_SETSEL, 0,
               //       MAKELPARAM(0, lpCshData->nFileLength));
                  SendSetSel (hEdit, 0, lpCshData->nFileLength);
                  return 1;
...
BOOL Container_ChangeSource(HWND hDlg, LPEDITLINKS lpEL)
{
   ...
   if (!lpEL->fClose) {
      SetWindowText(GetDlgItem(hDlg, IDCANCEL), lpEL->szClose);
      lpEL->fClose = TRUE;
   }
   ...
   for (i = cSelItems-1; i >=0; i--) {
      ...
      uRet = UStandardHook(lpEL, hDlg, uMsgBrowse,
            OLEUI_CCHPATHMAX, (LONG)lpLI->lpszDisplayName);
      ...
      if (!uRet) {
         ...
         */
         W2A (lpLI->lpszDisplayName, szAnsiStr, 256);
         uRet = (UINT)ChangeSource(hDlg, szAnsiStr,
...
int AddLinkLBItem(HWND hListBox, LPOLEUILINKCONTAINER lpOleUILinkCntr, LPLINKINFO lpLI, BOOL fGetSelected)
{
   ...
   hErr = lpOleUILinkCntr->lpVtbl->GetLinkSource(
         lpOleUILinkCntr,
         lpLI->dwLink,
         (LPOLESTR FAR*)&lpLI->lpszDisplayName,
         (ULONG FAR*)&lpLI->clenFileName,
         (LPOLESTR FAR*)&lpLI->lpszFullLinkType,
         (LPOLESTR FAR*)&lpLI->lpszShortLinkType,
   ...
   if (lpLI->fSourceAvailable) {
      if (dwUpdateOpt == OLEUPDATE_ALWAYS) {
         lpLI->fIsAuto = TRUE;
         LoadString(ghInst, (UINT)IDS_LINK_AUTO, szAnsiStr, 256);
         A2W (szAnsiStr, lpLI->lpszAMX, LINKTYPELEN+1);
      }
      else {
         lpLI->fIsAuto = FALSE;
         LoadString(ghInst, (UINT)IDS_LINK_MANUAL, szAnsiStr, 256);
         A2W (szAnsiStr, lpLI->lpszAMX, LINKTYPELEN+1);
      }
   }
   else {
         LoadString(ghInst, (UINT)IDS_LINK_UNKNOWN, szAnsiStr, 256);
         A2W (szAnsiStr, lpLI->lpszAMX, LINKTYPELEN+1);
   }
...
VOID BreakString(LPLINKINFO lpLI)
{
   ...
   if (!lpLI->clenFileName ||
      (OLESTRLEN(lpLI->lpszDisplayName)==(int)lpLI->clenFileName)) {

      lpLI->lpszItemName = NULL;
   }
   else {
      lpLI->lpszItemName = ((LPOLESTR)lpLI->lpszDisplayName) + lpLI->clenFileName;
   }

   // search from last character of filename
   lpsz = ((LPOLESTR)lpLI->lpszDisplayName) + OLESTRLEN(lpLI->lpszDisplayName);
   while (lpsz > lpLI->lpszDisplayName) {
      ((LPOLESTR)lpsz)--;
      if ((((char)*lpsz) == '\\') || (((char)*lpsz) == '/') || (((char)*lpsz) == ':'))
         break;
   }
   ...
   if (lpsz == lpLI->lpszDisplayName)
      ...
      lpLI->lpszShortFileName = ++((LPOLESTR)lpsz);
}
...
VOID InitControls(HWND hDlg, LPEDITLINKS lpEL)
{
   ...
   int FAR*    rgIndex;
   LPLINKINFO  lpLI;
   LPOLESTR    lpszType = NULL;
   LPOLESTR    lpszSource = NULL;
   int         cAuto = 0;
   int         cManual = 0;
   BOOL        bSameType = TRUE;
   ...
   for (i = 0; i < cSelItems; i++) {
      ...
      if (lpszSource && lpLI->lpszDisplayName) {
         if (bSameSource && OLESTRCMP(lpszSource, lpLI->lpszDisplayName)) {
            bSameSource = FALSE;
         }
      }
      else
      ...
      if (lpszType && lpLI->lpszFullLinkType) {
         if (bSameType && OLESTRCMP(lpszType, lpLI->lpszFullLinkType)) {
            bSameType = FALSE;
         }
      }
      else
   ...
   */
   if (!bSameSource || !lpszSource)
      lpszSource = szNULL;
   W2A (lpszSource, (LPSTR)sz, OLEUI_CCHPATHMAX);
   ...
   */
   if (!bSameType || !lpszType)
      lpszType = szNULL;
   W2A (lpszType, szAnsiStr, 256);
   SetDlgItemText(hDlg, ID_EL_LINKTYPE, szAnsiStr);
...
VOID ChangeAllLinks(HWND hListBox, LPOLEUILINKCONTAINER lpOleUILinkCntr, LPSTR lpszFrom, LPSTR lpszTo)
{
   ...
   for (nIndex=0; nIndex<cItems; nIndex++) {
      ...
      **    link.
      */
      if (!*(lpLI->lpszDisplayName + cFrom*sizeof(OLECHAR)) ||
         (*(lpLI->lpszDisplayName + cFrom*sizeof(OLECHAR)) == '\\') ||
         (*(lpLI->lpszDisplayName + cFrom*sizeof(OLECHAR)) == '!')) {

         W2A(lpLI->lpszDisplayName, (LPSTR)szTmp, cFrom + 1);
         if (!lstrcmp((LPSTR)szTmp, lpszFrom)) {
            ...
            W2A (lpLI->lpszDisplayName+cFrom*sizeof(OLECHAR), szAnsiStr, 256);
            lstrcat((LPSTR)szTmp, szAnsiStr);
            nFileLength = lstrlen((LPSTR)szTmp) -
               (lpLI->lpszItemName ? OLESTRLEN(lpLI->lpszItemName) : 0);
...
int LoadLinkLB(HWND hListBox, LPOLEUILINKCONTAINER lpOleUILinkCntr)
{
   ...
   while ((dwLink = lpOleUILinkCntr->lpVtbl->GetNextLink(lpOleUILinkCntr,
      ...
      lpLI->fIsSelected = FALSE;
      lpLI->fDontFree = FALSE;
      lpLI->lpszAMX = (LPOLESTR)OleStdMalloc((LINKTYPELEN+1)*sizeof(OLECHAR));
