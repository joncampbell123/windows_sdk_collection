...
BOOL FBusyInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
   ...
   {
      W2A (lpBZ->lpOBZ->lpszCaption, szAnsiStr, 256);
...
void BuildBusyDialogString(HWND hDlg, DWORD dwFlags, int iControl, LPOLESTR lpTaskName, LPOLESTR lpWindowName)
{
   LPSTR       pszT, psz1, psz2, psz3;
   UINT        cch;
   LPSTR       pszDot, pszSlash;
   UINT        uiStringNum;
   char        lpAnsiTaskName[256], lpAnsiWindowName[256];
   ...
   W2A (lpTaskName, lpAnsiTaskName, 256);
   W2A (lpWindowName, lpAnsiWindowName, 256);
   ...
   if (_fstrlen(lpAnsiWindowName) > 80)
     lpAnsiWindowName[80] = '\0';
   ...
   wsprintf(psz3, psz1, (LPSTR)psz2, (LPSTR)lpAnsiWindowName, (LPSTR)psz2);
...
BOOL GetTaskInfo(HWND hWnd, HTASK htask, LPOLESTR FAR* lplpszTaskName, LPOLESTR FAR*lplpszWindowName, HWND FAR*lphWnd)
{
   BOOL        fRet = FALSE;
#if !defined( WIN32 )
   TASKENTRY   te;
...
#endif
   HWND        hwndNext;
   LPOLESTR    lpszTN = NULL;
   LPOLESTR    lpszWN = NULL;
   char        lpszAnsiWN[256];
   HWND        hwndFind = NULL;
...
#else
            (GetWindowThreadProcessId(hwndNext,NULL) == (DWORD) htask) &&
#endif
            (IsWindowVisible(hwndNext)) &&
            !GetWindow(hwndNext, GW_OWNER))
            {
            // We found our window!  Alloc space for new strings
            if ((lpszTN = OleStdMalloc(OLEUI_CCHPATHMAX*sizeof(OLECHAR))) == NULL)
               return TRUE;  // continue task window enumeration

            if ((lpszWN = OleStdMalloc(OLEUI_CCHPATHMAX*sizeof(OLECHAR))) == NULL)
               return TRUE;  // continue task window enumeration

            // We found the window we were looking for, copy info to
            // local vars
            W2A (lpszWN, lpszAnsiWN, 256);
            GetWindowText(hwndNext, lpszAnsiWN, OLEUI_CCHPATHMAX);
#if !defined( WIN32 )
