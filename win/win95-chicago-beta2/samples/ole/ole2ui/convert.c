      ...
      case WM_COMMAND:
         switch (wID)
         {
            ...
            case IDOK:
            {
               ...
               lpszAnsiCLSID = PointerToNthField(szBuffer, 2, '\t');

               A2W (lpszAnsiCLSID, lpszCLSID, 256);
               ...
               if (DVASPECT_ICON == lpCV->dvAspect)
               {
                  ...
                  OLECHAR szUniLabel[OLEUI_CCHLABELMAX];
                  int   Index;
                  ...
                  A2W (szLabel, szUniLabel, OLEUI_CCHLABELMAX);
            ...
            case IDCV_CHANGEICON:
            {
               ...
               _fmemset((LPOLEUICHANGEICON)&ci, 0, sizeof(OLEUICHANGEICON));
               ...
               ci.cbStruct =sizeof(OLEUICHANGEICON);
               ci.hWndOwner=hDlg;
               ...
               A2W (pszCLSID, pszUniCLSID, 256);
               CLSIDFromString(pszUniCLSID, (LPCLSID)&(ci.clsid));
               ...
               if (0!=uRet)
               {
                  ...
                  OleUIMetafilePictExtractLabel(ci.hMetaPict, szUniLabel, OLEUI_CCHLABELMAX, &dwWrapIndex);
                  W2A (szUniLabel, szLabel, OLEUI_CCHLABELMAX);
...
BOOL FConvertInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
   ...
   lpCV->lpszConvertDefault = (LPOLESTR)pIMalloc->lpVtbl->Alloc(pIMalloc, OLEUI_CCHLABELMAX*sizeof(OLECHAR));
   lpCV->lpszActivateDefault = (LPOLESTR)pIMalloc->lpVtbl->Alloc(pIMalloc, OLEUI_CCHLABELMAX*sizeof(OLECHAR));
   lpCV->lpszIconSource = (LPOLESTR)pIMalloc->lpVtbl->Alloc(pIMalloc, OLEUI_CCHPATHMAX*sizeof(OLECHAR));

   lpCV->lpszConvertDefault[0]=0;
   lpCV->lpszActivateDefault[0]=0;
   lpCV->lpszIconSource[0]=0;
   ...
   W2A (lpCV->lpszCurrentObject, szAnsiStr, 256);
   SetDlgItemText(hDlg, IDCV_OBJECTTYPE, szAnsiStr);
       ...
       && (IsValidClassID(lpCV->lpOCV->clsidConvertDefault)) )
   {
      ...
      if (0 == dw)
         OLESTRCPY ((LPOLESTR)lpCV->lpszConvertDefault, (LPOLESTR)lpCV->lpszCurrentObject);
   }
   else
      OLESTRCPY((LPOLESTR)lpCV->lpszConvertDefault, (LPOLESTR)lpCV->lpszCurrentObject);
   ...
   else
   {
      ...
      OLECHAR szUniValue[OLEUI_CCHKEYMAX];

      StringFromCLSID(&(lpCV->lpOCV->clsid), &lpszCLSID);
      W2A (lpszCLSID, lpszAnsiCLSID, 256);
      lstrcpy(szKey, lpszAnsiCLSID);
      lstrcat(szKey, "\\TreatAs");

      dw = OLEUI_CCHKEYMAX;
      lRet = RegQueryValue(hKey, szKey, szValue, (LPDWORD)&dw);
      ...
      {
        A2W (szValue, szUniValue, OLEUI_CCHKEYMAX);
        CLSIDFromString(szUniValue, &clsid);
...
CheckInputFlag:
       ...
       && (IsValidClassID(lpCV->lpOCV->clsidActivateDefault)) )
   {
      ...
      if (0 == dw)
         OLESTRCPY((LPOLESTR)lpCV->lpszActivateDefault, (LPOLESTR)lpCV->lpszCurrentObject);
   }
   else
      OLESTRCPY((LPOLESTR)(lpCV->lpszActivateDefault), (LPOLESTR)lpCV->lpszCurrentObject);
...
SelectStringInListbox:
   ...
   if (lpCV->dwFlags & CF_SELECTCONVERTTO) {
      W2A (lpCV->lpszConvertDefault, szAnsiStr, 256);
      lRet = SendMessage(lpCV->hListVisible, LB_SELECTSTRING, (WPARAM)-1, (LPARAM)szAnsiStr);
   }

   else {
      W2A (lpCV->lpszActivateDefault, szAnsiStr, 256);
      lRet = SendMessage(lpCV->hListVisible, LB_SELECTSTRING, (WPARAM)-1, (LPARAM)szAnsiStr);
   }
   ...
   if (DVASPECT_ICON == lpCV->dvAspect )
   {
     ...
     if ((HGLOBAL)NULL != lpOCV->hMetaPict)
     {
       ...
       char     szAnsiLabel[OLEUI_CCHLABELMAX];
       HICON hIcon;
       DWORD dwWrapIndex;
       ...
       if (0 != OleUIMetafilePictExtractLabel(lpOCV->hMetaPict, szLabel, OLEUI_CCHLABELMAX, &dwWrapIndex))
       {
          W2A (szLabel, szAnsiLabel, OLEUI_CCHLABELMAX);
          if (0 == dwWrapIndex)  // no second line
          {
            SendDlgItemMessage(hDlg, IDCV_ICONLABEL1, WM_SETTEXT, 0, (LPARAM)szAnsiLabel);
            SendDlgItemMessage(hDlg, IDCV_ICONLABEL2, WM_SETTEXT, 0, (LPARAM)"");
          }
          else
          {
            ...
            lpszSecondLine = szAnsiLabel + dwWrapIndex;
            SendDlgItemMessage(hDlg, IDCV_ICONLABEL2,
                           WM_SETTEXT, 0, (LPARAM)lpszSecondLine);

            *lpszSecondLine = '\0';
            SendDlgItemMessage(hDlg, IDCV_ICONLABEL1,
                           WM_SETTEXT, 0, (LPARAM)szAnsiLabel);
          }
   ...
   if (lpCV->lpOCV->lpszCaption && !IsBadReadPtr(lpCV->lpOCV->lpszCaption, 1)
      ...
      W2A (lpCV->lpOCV->lpszCaption, szAnsiStr, 256);
      SetWindowText(hDlg, szAnsiStr);
   }
 ...
 *  hListInvisible  HWND to invisible listbox that stores "activate as" list.
...
UINT FillClassList(
      ...
      HWND hList,
      HWND hListInvisible,
      LPOLESTR FAR *lplpszCurrentClass,
      ...
      LPCLSID lpClsidExclude)
{
   ...
   OLECHAR     szUniClass[OLEUI_CCHKEYMAX];
   ...
   LPOLESTR    lpszCLSID;
   char        lpszAnsiCLSID[256];
   char        lplpszAnsiCurrentClass[OLEUI_CCHKEYMAX];
   ...
   if (NULL == *lplpszCurrentClass)
      ...
      *lplpszCurrentClass = (LPOLESTR)pIMalloc->lpVtbl->Alloc(pIMalloc, OLEUI_CCHKEYMAX*sizeof(OLECHAR));
       pIMalloc->lpVtbl->Release(pIMalloc);

      lRet = OleStdGetUserTypeOfClass((REFCLSID)&clsid,
                              *lplpszCurrentClass, OLEUI_CCHLABELMAX, NULL);

      if (0 ==lRet)
         {
         int n = LoadString(ghInst, (UINT)IDS_PSUNKNOWNTYPE, lplpszAnsiCurrentClass,
               OLEUI_CCHKEYMAX);
         A2W (lplpszAnsiCurrentClass, *lplpszCurrentClass, OLEUI_CCHKEYMAX);
         if (!n)
            {
            RegCloseKey(hKey);
            return (UINT)-1;
   ...
   W2A (*lplpszCurrentClass, lplpszAnsiCurrentClass, OLEUI_CCHKEYMAX);

   // Get the class name of the original class.
   StringFromCLSID((REFCLSID)&clsid, &lpszCLSID);
   ...
   while ((LONG)ERROR_SUCCESS==lRet)
      ...
      A2W (szClass, szUniClass, OLEUI_CCHKEYMAX);
      CLSIDFromString(szUniClass, (LPCLSID)&clsidForList);
      ...
      lstrcat(szFormatKey, "\\Conversion\\Readwritable\\Main");

      dw=OLEUI_CCHKEYMAX;

      lRet=RegQueryValue(hKey, szFormatKey, szFormat, (LONG FAR *)&dw);

      if ( ((LONG)ERROR_SUCCESS==lRet)
          && (FormatIncluded(szFormat, wFormat)) )
         {
         ...
         dw=OLEUI_CCHKEYMAX;
         lRet=RegQueryValue(hKey, szClass, szHRClassName, (LPDWORD)&dw);

         if ((LONG)ERROR_SUCCESS==lRet)
            {
            lstrcat(szHRClassName, "\t");
      ...
      W2A (lpszCLSID, lpszAnsiCLSID, 256);
      if ( (!fIsLinkedObject)
            || (lstrcmp(lpszAnsiCLSID, szClass) == 0) )
         {
         //Check for a \Conversion\Readable\Main entry
         lstrcpy(szFormatKey, szClass);
         lstrcat(szFormatKey, "\\Conversion\\Readable\\Main");
         ...
         lRet=RegQueryValue(hKey, szFormatKey, szFormat, (LPDWORD)&dw);
...
Next:
   ...
   lstrcpy(szHRClassName, lplpszAnsiCurrentClass);
   lstrcat(szHRClassName, "\t");
   ...
   if (LB_ERR == lRet)
      {
      lstrcat(szHRClassName, lpszAnsiCLSID);
...
)
{
   ...
   while ((LONG)ERROR_SUCCESS==lRet)
   {
      if (lstrcmp(lpszAnsiCLSID, szClass) == 0)
      ...
      if ( (LONG)ERROR_SUCCESS != lRet)
      {
        ...
        lstrcpy(szFormatKey, szClass);
        lstrcat(szFormatKey, "\\DataFormats\\DefaultFile");
...
BOOL FormatIncluded(LPSTR szStringToSearch, WORD wFormat)
{
   ...
   lpToken = _fstrtok(szStringToSearch, seps);
...
void UpdateCVClassIcon(HWND hDlg, LPCONVERT lpCV, HWND hList)
   ...
   A2W (pszCLSID, pszUniCLSID, 256);
   CLSIDFromString(pszUniCLSID, (LPCLSID)&clsid);

   W2A (lpCV->lpszIconSource, szAnsiIconSource, 256);
   hIcon = HIconAndSourceFromClass(&clsid, szAnsiIconSource, &(lpCV->IconIndex));

   if (NULL == hIcon)  // Use Vanilla Document
      {
      lstrcpy(szAnsiIconSource, szOLE2DLL);
      A2W (szAnsiIconSource, lpCV->lpszIconSource, 256);
   ...
   if (lpCV->lpOCV->lpszDefLabel)
      ...
      W2A (lpCV->lpOCV->lpszDefLabel, szAnsiStr, 256);
      lpszLabel = ChopText(hLabel1, ((LabelRect.right-LabelRect.left)*3)/2, szAnsiStr);
      W2A(lpCV->lpOCV->lpszDefLabel, szLabel, OLEUI_CCHLABELMAX);
      }
   else
      {
      szUniLabel[0]=0;
      if ((cch = OleStdGetAuxUserType(&clsid, AUXUSERTYPE_SHORTNAME,
            (LPOLESTR)szUniLabel, OLEUI_CCHLABELMAX, NULL)) == 0)
         {
         // If we can't get the AuxUserType2, then try the long name
         if ((cch = OleStdGetUserTypeOfClass(&clsid, (LPOLESTR)szUniLabel,
               OLESTRLEN(szUniLabel)+1, NULL)) == 0)
            {
            //Last resort; use "Document" as label
            LoadStringW(ghInst,(UINT)IDS_DEFICONLABEL,szUniLabel,OLEUI_CCHLABELMAX);
            }
         }

      W2A (szUniLabel, szLabel, 256);
      }

   hFont = (HFONT)SendMessage(hLabel1, WM_GETFONT, 0, 0L);

   // Figure out where to split the label
   A2W (lpszLabel, szUniStr, 256);
   cch = lstrlen(lpszLabel);
   uWrapIndex = OleStdIconLabelTextOut(NULL, hFont, 0, 0, 0, &LabelRect, szUniStr, cch, NULL);
