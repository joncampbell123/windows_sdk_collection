...
BOOL FPasteSpecialInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
{
   ...
   if (NULL != lpOPS->lpszCaption) {
      W2A (lpOPS->lpszCaption, szAnsiStr, 256);
      SetWindowText(hDlg, szAnsiStr);
   }

   // Load 'Unknown Source' and 'Unknown Type' strings
   n = LoadString(ghInst, (UINT)IDS_PSUNKNOWNTYPE, szAnsiStr, PS_UNKNOWNSTRLEN);
   A2W (szAnsiStr, lpPS->szUnknownType, PS_UNKNOWNSTRLEN);
   if (n) {
      n = LoadString(ghInst, (UINT)IDS_PSUNKNOWNSRC, szAnsiStr, PS_UNKNOWNSTRLEN);
      A2W (szAnsiStr, lpPS->szUnknownSource, PS_UNKNOWNSTRLEN);
   }
   if (!n)
   {
   ...
   if (lpPS->hObjDesc = OleStdFillObjectDescriptorFromData(lpOPS->lpSrcDataObj, &medium, &cfFormat))
   {
      ...
      if (lpOD->dwFullUserTypeName)
         lpPS->szFullUserTypeNameOD = (LPOLESTR)
                                  ((LPSTR)lpOD + lpOD->dwFullUserTypeName) ;
         else lpPS->szFullUserTypeNameOD = lpPS->szUnknownType;

      if (lpOD->dwSrcOfCopy)
      {
         lpPS->szSourceOfDataOD = (LPOLESTR)((LPSTR)lpOD+lpOD->dwSrcOfCopy);
         ...
         if (cfFormat == cfFileName) {
            W2A (lpPS->szSourceOfDataOD, szAnsiStr, 256);
            lpszAnsiStr = ChopText(GetDlgItem(hDlg, ID_PS_SOURCETEXT), 0, szAnsiStr);
            A2W (lpszAnsiStr, lpPS->szSourceOfDataOD, 256);
         }
      }
      ...
      if (0==OleStdGetAuxUserType(&lpPS->clsidOD, 3, lpPS->szAppName, OLEUI_CCHKEYMAX, NULL))
      {
          ...
          if (0==LoadString(ghInst, (UINT)IDS_PSUNKNOWNAPP, szAnsiStr, PS_UNKNOWNSTRLEN))
          {
             A2W (szAnsiStr, lpPS->szAppName, PS_UNKNOWNSTRLEN);
             PostMessage(hDlg, uMsgEndDialog, OLEUI_ERR_LOADSTRING, 0L);
             return FALSE;
          }
      }
      ...
      {
         lpPS->hMetaPictOD = OleGetIconOfClass(&lpPS->clsidOD, NULL,
         ...
         &medium))
   {
      ...
      if (lpLSD->dwFullUserTypeName)
         lpPS->szFullUserTypeNameLSD =
                   (LPOLESTR) ((LPSTR)lpLSD + lpLSD->dwFullUserTypeName);
      else lpPS->szFullUserTypeNameLSD = lpPS->szUnknownType;

      if (lpLSD->dwSrcOfCopy)
         lpPS->szSourceOfDataLSD = (LPOLESTR)((LPSTR)lpLSD+lpLSD->dwSrcOfCopy);
      ...
      if (NULL == lpPS->hMetaPictLSD)
      {
         ...
         W2A(lpPS->szSourceOfDataLSD, szLabel, sizeof(szLabel));
         szLabel[sizeof(szLabel)-1] = '\0';

         lpszLabel = ChopText(hIconWnd, nWidth, (LPSTR)szLabel);

         A2W (lpszLabel, szUniLabel, 256);
         lpPS->hMetaPictLSD = OleGetIconOfClass(&lpPS->clsidLSD,
               szUniLabel,       /* use chopped source string as label */
...
BOOL FTogglePasteType(HWND hDlg, LPPASTESPECIAL lpPS, DWORD dwOption)
{
   ...
   if (lpPS->dwFlags & PSF_SELECTPASTE)
   {
      ...
      W2A (lpPS->szSourceOfDataOD, szAnsiStr, 256);
      SetDlgItemText(hDlg, ID_PS_SOURCETEXT, szAnsiStr);
   ...
   {
      // Set the Source of the object in the clipboard
      W2A (lpPS->szSourceOfDataLSD, szAnsiStr, 256);
      SetDlgItemText(hDlg, ID_PS_SOURCETEXT, szAnsiStr);
...
void ChangeIcon(HWND hDlg, LPPASTESPECIAL lpPS)
{
   ...
   uRet = UStandardHook(lpPS, hDlg, uMsgChangeIcon, 0, (LONG)&ci);
...
void SetPasteSpecialHelpResults(HWND hDlg, LPPASTESPECIAL lpPS)
{
   ...
   LPOLESTR    szFullUserTypeName = (lpPS->fLink) ?
               lpPS->szFullUserTypeNameLSD : lpPS->szFullUserTypeNameOD;
   LPOLESTR    szInsert;
   char        szAnsiStr[256];
...
#ifdef UNICODE
      W2A((LPOLESTR)lpOPS->arrPasteEntries[nPasteEntriesIndex].lpstrFormatName, szAnsiStr, 256);
#else
      strncpy(szAnsiStr, lpOPS->arrPasteEntries[nPasteEntriesIndex].lpstrFormatName, 256);
#endif

      fIsObject = FHasPercentS(szAnsiStr, lpPS);
   }
   else return;
   ...
   *psz1=0;
   ...
   if (0!=LoadString(ghInst, iString, psz1, cch))
   {
      ...
      W2A (szInsert, szAnsiStr, 256);
#ifdef UNICODE
      W2A ((LPOLESTR)lpOPS->arrPasteEntries[nPasteEntriesIndex].lpstrResultText, szAnsiStr2, 256);
#else
      strncpy(szAnsiStr2, lpOPS->arrPasteEntries[nPasteEntriesIndex].lpstrResultText, 256);
#endif

      wsprintf(psz3, (LPSTR)szAnsiStr2, (LPSTR)szAnsiStr);
      ...
      LPMALLOC pIMalloc, LPSTR lpszBuf, LPSTR lpszFullUserTypeName)
{
   ...
   strncpy(szAnsiStr,lpOPS->arrPasteEntries[nPasteEntriesIndex].lpstrFormatName, 256);
#endif
   wsprintf((LPSTR)lpszBuf, (LPSTR)szAnsiStr, (LPSTR)lpszFullUserTypeName);
...
BOOL FFillPasteList(HWND hDlg, LPPASTESPECIAL lpPS)
{
   ...
   for (i = 0; i < lpOPS->cPasteEntries; i++)
   {
      ...
      if (lpOPS->arrPasteEntries[i].dwScratchSpace) {
         ...
         W2A (lpPS->szFullUserTypeNameOD, szAnsiStr, 256);
   ...
   for (i = 0; i < lpOPS->cPasteEntries; i++)
   {
      ...
      if (fLinkTypeSupported && lpOPS->arrPasteEntries[i].dwScratchSpace) {
         if (nDefFormat < 0)
          ...
          W2A (lpPS->szFullUserTypeNameLSD, szAnsiStr, 256);
         if (!FAddPasteListItem(hList, fInsertFirst, i, lpPS, pIMalloc,
                  lpszBuf, szAnsiStr))
            goto error;
         nItems++;
...
HGLOBAL AllocateScratchMem(LPPASTESPECIAL lpPS)
{
   ...
   if (lpPS->szFullUserTypeNameOD)
      nSubstitutedText = OLESTRLEN(lpPS->szFullUserTypeNameOD);
   if (lpPS->szFullUserTypeNameLSD)
      nSubstitutedText = __max(nSubstitutedText, OLESTRLEN(lpPS->szFullUserTypeNameLSD));
   if (lpPS->szAppName)
      nSubstitutedText = __max(nSubstitutedText, OLESTRLEN(lpPS->szAppName));
