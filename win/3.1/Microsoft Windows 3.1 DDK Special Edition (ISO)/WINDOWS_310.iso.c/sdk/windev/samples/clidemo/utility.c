...
void FAR WaitForObject(                //- ENTRY:
   ...
   while (OleQueryReleaseStatus(paItem->lpObject) == OLE_BUSY)
   {
      if (!i)
      {
         ToggleBlockTimer(TRUE,paItem);//- set timer
         i++;
      }
...
APPITEMPTR FAR GetTopItem()
{
   ...
   if (hwnd = GetTopWindow(hwndFrame))
      return ((APPITEMPTR)GetWindowWord(hwnd,0));
   else
      return NULL;
...
BOOL FAR ReallocLinkData(              //- ENTRY:
   ...
   handle = (HANDLE)GlobalHandle(HIWORD(pItem->lpLinkData));
   GlobalUnlock(handle);

   if (!(pItem->lpLinkData = GlobalLock(GlobalReAlloc(handle, lSize, NULL)))) 
   {
...
void FAR FreeLinkData(                 //- ENTRY:
   ...
   if (lpLinkData)
   {
      handle = (HANDLE)GlobalHandle(HIWORD(lpLinkData));
      GlobalUnlock(handle);
      GlobalFree(handle);
   }
}
...
void FAR ToggleBlockTimer(BOOL bSet, APPITEMPTR lpaItem)
{     
   lpaItemHold = lpaItem;
   ...
   {
      if (!lpfnTimerProc)
         lpfnTimerProc = MakeProcInstance(fnTimerBlockProc, hInst);
      iTimerID = SetTimer(hwndFrame,1, 3000,lpfnTimerProc);
   }
   else if (iTimerID)
   {
...
BOOL FAR PASCAL fnTimerBlockProc(      //- ENTRY: 
   HWND     hWnd,
   WORD     wMsg,
   int      iTimerID,
   DWORD    dwTime
