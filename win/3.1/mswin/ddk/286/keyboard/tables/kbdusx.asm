; 'kbdusx.asm' Windows 3.00 keyboard table file based on:
;  'us1.wk2',  'us4.wk2'
...
; USA IBM layout -- with extended character support.

; History
;	14 jan 90	peterbe		Put full range transpatch back in.
;	08 jan 90	peterbe		ifdef NOKIA
;	20 nov 89	peterbe		Add mu on M key
;	08 nov 89	peterbe		Add O-slash, degree, fractions, etc.
;	30 oct 89	peterbe		Add single quote chars
;	17 oct 89	peterbe		More punct., multiply, divide
;	28 sep 89	peterbe		Use apostrophe for acute deadkey.
;	06 sep 89	peterbe		Commented out VK_EXECUTE translation.
;	31 aug 89	peterbe		Added dead key for Y acute.
;	18 aug 89	peterbe		acute is deadkey for cedilla now.
;					Also, acute, circumflex, and umlaut
;					deadkeys (double quote = umlaut)
;	17 aug 89	peterbe		Now works for all IBM-compatible
;					keyboards.  Other keyboards must
;					assume IBM layout.
;					Added Iceland thorn, eth, AE
;	14 jul 89	peterbe		SHIFT + VK_TAB ==> ascii TAB now.
;					Also added n-tilde, a-ring, section,
;					generic currency, yen signs.
;
; This version is for testing and development work.
; It provides some control-alt and dead keys on the enhanced keyboard.
;	Accent-grave and tilde keys are dead keys.
;	Keys A, E, I, O, and U + Ctrl-Alt or Shift-Control-Alt should
;	produce the corresponding letters with Acute accent.
;	(shift) control-alt Y is U-umlaut.
;	(shift) control-alt P is O-umlaut.
;	(shift) control-alt Q is A-umlaut.
; It interprets the left ALT key as CTRL-ALT.
...
DUMLAUT	equ	1
DTILDE	equ	1
	; No cedilla deadkeys (Special for this: use acute for cedilla)
...
umlaut		equ	022h ; double quote)
acute		equ	"'"	; change 28 sep 89
grave		equ	060h
circumflex	equ	'^'
tilde		equ	'~'
cedilla		equ	0b8h
...
PatchIndices label byte
    ...
    ; on this keyboard, all keys in this range are same as US.
...
TransPatch4 label byte		; This is for type 3

	; these are the 'normal' US IBM keyboard values:
	db	VK_OEM_MINUS	; 0ch 12
...
PatchATran3 label word		; AT table
ifdef NOKIA
PatchATran5 label word		; Nokia table
PatchATran6 label word		; Nokia table
endif
	...
	db	'\',	'|'	; VK_OEM_5    
	db	']',	'}'	; VK_OEM_6    
	db	-1,	-1	; VK_OEM_7    ; quotes are dead keys
	db	-1,	-1	; VK_OEM_8    
	db	-1,	-1	; VK_OEM_102
	db	'.',	'.'	; VK_DECIMAL
...

; ********************************************************
;	Macros for Control-Alt, Shift-Control-Alt tables
;	The patches are the same as the type 4 table, but
;	in a different segment, so we use these macros.
; ********************************************************

CtlAltInsert macro VKF
					; Lower case acute accent keys
	ktrans	VK_A,	225,	VKF	; a'
	ktrans	VK_E,	233,	VKF	; e'
	ktrans	VK_I,	237,	VKF	; i'
	ktrans	VK_O,	243,	VKF	; o'
	ktrans	VK_U,	250,	VKF	; u'

	ktrans	VK_N,	241,	VKF	; n~
	ktrans	VK_W,	229,	VKF	; W -> a ring
	ktrans	VK_L,	248,	VKF	; L -> o slash
	ktrans	VK_C,	169,	VKF	; Copyright (c)
	ktrans	VK_R,	174,	VKF	; Registered (r)

	ktrans	VK_Y,	252,	VKF	; Y -> u-umlaut'
	ktrans	VK_Q,	228,	VKF	; q -> a-umlaut'
	ktrans	VK_P,	246,	VKF	; p -> o-umlaut'

	ktrans	VK_S,	223,	VKF	; German sharp S (lower case).
	ktrans	VK_T,	254,	VKF	; Icelandic thorn LC
	ktrans	VK_D,	240,	VKF	; Icelandic eth LC
	ktrans	VK_Z,	230,	VKF	; ae
	ktrans	VK_M,	181,	VKF	; Mu

	ktrans	VK_1,	161,	VKF	; inverted ! (1, ! key)
	ktrans	VK_2,	178,	VKF	; 2 super
	ktrans	VK_3,	179,	VKF	; 3 super
	ktrans	VK_4,	164,	VKF	; generic currency ! (4 key)
	ktrans	VK_6,	188,	VKF	; 1/4
	ktrans	VK_7,	189,	VKF	; 1/2
	ktrans	VK_8,	190,	VKF	; 3/4
	ktrans	VK_9,	145,	VKF	; open single quote
	ktrans	VK_0,	146,	VKF	; close single quote

	ktrans	VK_OEM_COMMA,231,VKF	; c cedilla (LC only)
	ktrans	VK_OEM_MINUS,165,VKF	; YEN sign (- key)
	ktrans	VK_OEM_PLUS,215,VKF	; multiply sign

	ktrans	VK_OEM_1, 182,	VKF	; Para sign on semicolon key
	ktrans	VK_OEM_2, 191,	VKF	; inverted ? (? key)
	ktrans	VK_OEM_4, 171,	VKF	; <<
	ktrans	VK_OEM_5, 172,	VKF	; not sign
	ktrans	VK_OEM_6, 187,	VKF	; >>
	ktrans	VK_OEM_7, 180,	VKF	; acute accent (instead of apostrophe)
endm

ShiftCtlAltInsert macro VKF
					; Upper case acute accent keys
	ktrans	VK_A,	193,	VKF	; A'
	ktrans	VK_E,	201,	VKF	; E'
	ktrans	VK_I,	205,	VKF	; I'
	ktrans	VK_O,	211,	VKF	; O'
	ktrans	VK_U,	218,	VKF	; U'

	ktrans	VK_N,	209,	VKF	; N~
	ktrans	VK_W,	197,	VKF	; A ring
	ktrans	VK_L,	216,	VKF	; O slash

	ktrans	VK_S,	167,	VKF	; section sign ("german para. sign")
	ktrans	VK_T,	222,	VKF	; Icelandic thorn UC
	ktrans	VK_D,	208,	VKF	; Icelandic eth UC
	ktrans	VK_Z,	198,	VKF	; AE
	ktrans	VK_C,	162,	VKF	; cent sign (C key)

	ktrans	VK_Y,	220,	VKF	; Y -> U-umlaut'
	ktrans	VK_Q,	196,	VKF	; q -> A-umlaut'
	ktrans	VK_P,	214,	VKF	; p -> O-umlaut'

	ktrans	VK_1,	185,	VKF	; 1 exponent
	ktrans	VK_4,	163,	VKF	; Sterling (pound)

	ktrans	VK_OEM_PLUS,247,VKF	; divide sign
	ktrans	VK_OEM_COMMA,199,VKF	; C cedilla

	ktrans	VK_OEM_1, 176,	VKF	; degree sign on semicolon key
	ktrans	VK_OEM_5, 166,	VKF	; alternate vertical bar
	ktrans	VK_OEM_7, 168,	VKF	; umlaut: double quote key
endm

MPatchMorto macro VKF	;define dead keys

	; grave/tilde key
	deadtrans	vk_oem_3,	0,	grave,	VKF
	deadtrans	vk_oem_3,	1,	tilde,	VKF

	; 6/circumflex key
	deadtrans	vk_6,		1,	circumflex, VKF

	; single/double quote keys
	deadtrans	vk_oem_7,	0,	acute,	VKF
	deadtrans	vk_oem_7,	1,	umlaut,	VKF
endm

;
; ********************************************************
...
; Patch Shift-Control-Alt table

    ShiftCtlAltInsert	VKFLAG	; macro
...
;	Variable entries in AscTab[]:
	...
	ktrans2	VK_6      ,	'6',	-1,	VKFLAG ; '^' is dead key
...
;;;	ktrans2	VK_OEM_3    ,	'`',	'~',	VKFLAG
	;; This key is a dead key for this keyboard
	ktrans2	VK_OEM_3    ,	-1,	-1,	VKFLAG

	ktrans2	VK_OEM_4    ,	'[',	'{',	VKFLAG
	ktrans2	VK_OEM_5    ,	'\',	'|',	VKFLAG
	ktrans2	VK_OEM_6    ,	']',	'}',	VKFLAG
	ktrans2	VK_OEM_7    ,	-1,	-1,	VKFLAG
...
IRP VKFLAG, <0, 1>
	...
	deadtrans	' ', acute,	acute,	VKFLAG
	; Special for this keyboard -- use acute as deadkey for c-cedilla
	deadtrans	'c', acute,	231,	VKFLAG
	deadtrans	'C', acute,	199,	VKFLAG
    endif
...
IRP VKFLAG, <0, 1>
	...
	ShiftCtlAltInsert	VKFLAG		; see macro above

    klabdef AscShCtlAltEnd, VKFLAG

	; Padding not needed -- all tables same size
