...
non_justified_text proc near
	...
	jz	short sc_no_left_clipping     ;No clipping needed on lhs
...
spcl_current_is_visible:
	...
	mov	ebx, dptr fsCharOffset[bx][PROP_OFFSET]
...
spcl_step_clipped_char:
	...
	add	ebx,lfd.font_height	 ;Move to next column of character
	sub	dx,8			;Is current byte visible?
...
scc_good_char:
	...
	mov	bx, ax
	shl	ax, 1
	add	bx, ax			;Index into width/offset table
	mov	ax,wptr fsCharOffset[bx][PROP_WIDTH]
	mov	ebx, dptr fsCharOffset[bx][PROP_OFFSET]
...
scc_clip_enters_here:
	...
	cmp	di,clip.right
	jge	short scc_char_is_clipped  ;Clipped (or first pixel of next is)
...
scc_still_wide:
	...
	add	ebx,lfd.font_height	 ;  of the remaining width
	cmp	ax,8
...
scc_width_ok:
	...
	jl	short scc_restart	;Not enough stack for another character
...
jcl_have_tot_extra:
	...
	or	dx,dx
	jnz	short jcl_extra_pixels	;Extra pixels required
	mov	ax,wptr fsCharOffset[bx][PROP_WIDTH]
...
jc_see_if_next:
	cmp	sp,min_stack		;Stack compare must be unsigned
	jl	short jc_restart	;Not enough stack for another character
	dec	cx
...
;-----------------------------------------------------------------------;
	...
	mov	di,clip.left
	cmp	di,ax
	jl	short wccl_dummy_is_visible ;Some part of dummy char became visible
...
wcc_have_adj_width:
	...
	add	di,ax			;DI = next char's X position
	cmp	di,clip.right
	jge	short wcc_char_is_clipped     ;Clipped (or first pixel of next is)
...
wcc_see_if_next:
	cmp	sp,min_stack		;Stack compare must be unsigned
	jb	short wcc_restart	;Not enough stack for another character
	dec	cx
...
wcc_char_is_clipped:
	...
	or	dx,dx
	jl	short wcc_might_need_restart  ;Might need a restart
...
wcc_might_need_restart:
	...
	jle	short wcc_clipped_no_restart  ;  then no restart needed
	add	dx,di			;Compute next starting x
	sub	dx,clip.right
	jge	short wcc_clipped_no_restart  ;Rest of string is clipped
...
plhs_add_padding:
	...
	push	dptr null_char_offset	;Offset in font of null character
	jmp	dx
...
pad_right_hand_side	proc near
	...
	jz	short prhs_new_rhs	;  move text_bbox to byte boundary
	mov	bl,excel
	not	bl
	test	bl,OPAQUE_RECT+BOUNDED_IN_Y
...
; output using the supplied dispatch tables (located in psmartpr.asm)
;
; Entry:
; Returns:
;	None
...
;	If the clip mask is non-zero, then this is the case.
	...
	or	al,al
	mov	ss_clip_mask,al
	jz	short psd_not_clipped	;No partial first byte
...
psd_not_clipped:
	...
	jnz	short psd_collect_chars ;Need to cross two
...
psd_collect_chars:
	...
	cmp	ch,al
	ja	short psd_have_more_than_enough
	je	short psd_have_exact_fit
...
psd_unlikely_cases:
	...
	cmp	ch,al
	ja	short psd_have_more_than_enough
	jmp	short psd_have_exact_fit;It had to have fit!
...
prepare_for_overlap proc near
	...
	jz	short pfo_exit		;Did not step backwards
