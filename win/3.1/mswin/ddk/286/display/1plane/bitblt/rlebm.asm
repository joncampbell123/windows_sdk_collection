...
;									     ;
;  ************************************************************************  ;
;  NOTE: RLEBM.ASM IN DISP20 & DISP30 TREES ARE IDENTICAL.  AFTER  CHANGING  ;
...
;									     ;
;									     ;
;          Added a new IFDEF label called CGAHERCMONO which will be true for ;
;          the CGAHERC mono family and the EGAVGA mono family of drivers. For;
;          these drivers we must not touch any device registers.             ;
;          -by-  Amit Chatterjee [amitc]   on   Apr-15-1990   13:58:45       ;
;									     ;
;          Mon Dec-11-1989  09:18:35.                                        ;
;          For 4 bit RLE encoded runs, if the two pels values are different, ;
;          we will now do the stretch in two passes. The first pass will set ;
;          all pels in the stretch to the first color. The second pass will  ;
;          overwrite alternating pels in the stretch with the second color.  ;
;          This significantly speeds up display onto devices and is much     ;
;          better than treating the stretch as if it is in an absolute mode. ;
;          -by- Amit Chatterjee [amitc]					     ;
;									     ;
;          Fri Oct-27-1989  17:57:30.					     ;
;          Merged in CGAHERC code with proper IFDEFs. This RLEBM.ASM is now  ;
;          do be used both in the DISP20 and the DISP30 trees, for both the  ;
;          EGA & the CGAHERC family of drivers.				     ;
;          -by-  Amit Chatterjee  [amitc]				     ;
;									     ;
;	   Wed May-03-1989  06:51:30.					     ;
;          Implemented 4 bit RLE recording and playback.		     ;
;          -by-  Amit Chatterjee  [amitc]				     ;
;								    	     ;
;          Fri Apr-26-1989  09:44:08.					     ;
;          Modified the encoding to the new structure.			     ;
;          -by-  Amit Chatterjee  [amitc]				     ;
;									     ;
;	   Mon Feb-27-1989  08:00:50.					     ;
;          Created.  -by-  Amit Chatterjee  [amitc]			     ;
;----------------------------------------------------------------------------;
...
;----------------------------------------------------------------------------;
; The following is the color table that is to be returned to the caller from ;
; the GETRLEBITS calls.							     ;
;----------------------------------------------------------------------------;

if NUMBER_PLANES eq 4

own_color_table label byte
...
; ES:DI point to info block
	...
	mov	ax,256			;the default color table size
...
get_table_for_color:
	...
	lea	si,own_color_table	;ds:si points to oen color table
