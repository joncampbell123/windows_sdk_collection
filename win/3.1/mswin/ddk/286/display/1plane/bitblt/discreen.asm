...
;									     ;
;  ************************************************************************* ;
;  NOTE: DISCREEN.ASM IN DISP20 & DISP30 TREES ARE IDENTICAL. AFTER CHANGING ;
...
;									     ;
;       Merged in CGAHERC code with proper IFDEFs. This DISCREEN.ASM is now  ;
;       to be used both in the DISP20 and the DISP30 trees, for both the     ;
;       EGA & the CGAHERC family of drivers.				     ;
;       -by-  Amit Chatterjee  [amitc]	 on   Oct-27-1989  18:00:00          ;
;									     ;
;     . DIB Info Block enriched. Clipping now being done by the driver. A ptr;
;       to a DRAWMODE structure is also passed in and the destination rect.  ;
;	is painted with the background color in OPAQUE mode (only for RLE).  ;
;	-by-  Amit Chatterjee  [amitc]   on   Mar-08-1989  10:01:30          ;
;								             ;
;     . Created.					                     ;
...
; the start scan be s and the number of can in the copy be n.		     ;
; 								             ;
...
; fill in the actual rectangle coordinates
	...
	add	ax,MapHeight		; bottom can is exclusive
	sub	ax,StartScan		; last scan on the BLT RECT on surface
	mov	[si].bottom,ax
	sub	ax,NumScan		; the number of can in the blt area
	mov	[si].top,ax
