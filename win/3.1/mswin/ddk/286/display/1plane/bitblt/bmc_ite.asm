...
;									     ;
;     . Modified to suite 3/4 plane EGA/VGA drivers  for Windows 286/386     ;
;	                        by Amit Chatterjee [amitc]  on 30-Nov-1988   ;
;									     ;
;     . Bitmap Info Header structure enriched.				     ;
;				by Amit Chatterjee [amitc]  on 13-Mar-1989   ;
;									     ;
;     . if lp_bits is NULL fill in the biSizeImage field any way             ;
;                               by Todd Laney      [ToddLa] on 18-Aug-1989   ;
;                                                                            ;
; Last Modified -by-  Amit Chatterjee [amitc]     13-Mar-1989   07:28:40     ;
;----------------------------------------------------------------------------;
...
;----------------------------------------------------------------------------;
; The following table will be used to convert 3,4 bits/pel internal format to;
; 24 bits per pel device independent format. Actually we will have 2 8 entry ;
; parts which will basically be same.					     ;
;----------------------------------------------------------------------------;
...
ColorToMono label byte
	...
	db  1		;map low gray to 0

if NUMBER_PLANES eq 4

	db  0		;map high gray to 1
	db  0		;map high red to 1
	db  0		;map high green to 1
	db  1		;map high yellow to 1
	db  0		;map high blue to 1
...
;----------------------------------------------------------------------------;
; Fixed color table is the default color table which the device is programmed;
; with.	This is the 24 bits/pel format of the above colors .		     ;
;----------------------------------------------------------------------------;
...
fixed_color_table label byte
	...
	db	000h,000h,080h,0	;Red
	db	000h,080h,000h,0	;Green
	db	000h,080h,080h,0	;Yellow
	db	080h,000h,000h,0	;Blue
	db	080h,000h,080h,0	;Magenta
	db	080h,080h,000h,0	;Cyan
	db	080h,080h,080h,0	;gray

if NUMBER_PLANES eq 4

	db	0c0h,0c0h,0c0h,0	;special gray
	db	000h,000h,0ffh,0	;Red
	db	000h,0ffh,000h,0	;Green
	db	000h,0ffh,0ffh,0	;Yellow
	db	0ffh,000h,000h,0	;Blue
	db	0ffh,000h,0ffh,0	;Magenta
	db	0ffh,0ffh,000h,0	;Cyan
	db	0ffh,0ffh,0ffh,0	;White
...
create_color_color_table proc near
	...
	mov	si,DIMapSegOFFSET fixed_color_table
	mov	cx,SIZE_FIXED_COLOR_TABLE/2
	rep	movs word ptr es:[di], word ptr cs:[si]
if	SIZE_FIXED_COLOR_TABLE and 1
