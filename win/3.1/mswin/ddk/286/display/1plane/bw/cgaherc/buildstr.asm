...
;----------------------------------------------------------------------------;
; the following macro is a prt of the code to add a word to ebx. The word is ;
; added to bx and this macro adds 10000h to ebx if a carry is generated.     ;
;----------------------------------------------------------------------------;
...
;
; History:
;	Mon 30-Jan-1989 09:31:40 -by-  Amit Chatterjee [amitc]
;       Added code to handle > 64k fonts. All these codes are within
;       ifdef BIGFONT
;
...
;----------------------------------------------------------------------------;
; for normal fonts the header has 2 byte pointer to bits and the node size is;
; 4 bytes per char, for bigfonts pointers are 4 bytes and the node size is 6 ;
; bytes per character.							     ;
;----------------------------------------------------------------------------;
...
;----------------------------------------------------------------------------;
ifdef BIGFONT
; add 10000h to ebx if carry is generated above
	updc_ebx
endif
;----------------------------------------------------------------------------;
...
; the following macro will add 10000h to EBX if carry is set in add above
	updc_ebx
endif
;----------------------------------------------------------------------------;
...
; the following macro will add 10000h to EBX if a carry is generated above
	updc_ebx
endif
;----------------------------------------------------------------------------;
...
jcl_have_tot_extra:
	...
	xchg	ax,bx
;----------------------------------------------------------------------------;
; for normal fonts the header has 2 byte pointers and the entry size is 4    ;
; bytes per node. For BIGFONTS the header size is 6 bytes per node with 4    ;
; byte pointers to bits.						     ;
;----------------------------------------------------------------------------;
ifdef BIGFONT
...
;----------------------------------------------------------------------------;
ifdef BIGFONT
; the following macro will add 10000h to EBX if carry has been set above
	updc_ebx
endif
;----------------------------------------------------------------------------;
...
;----------------------------------------------------------------------------;
ifdef BIGFONT
;the following macro wil add 10000h to EBX if carry has been set above
	updc_ebx	
endif
;----------------------------------------------------------------------------;
	sub	dx,8			;Is current byte visible?
...
;----------------------------------------------------------------------------;
ifdef BIGFONT
; the following macro will add 10000h to EBX if carry is generated above
	updc_ebx
endif
;----------------------------------------------------------------------------;
...
;----------------------------------------------------------------------------;
ifdef BIGFONT
; the following macro adds 10000h to EBX if a carry is generated above
	updc_ebx
endif
;----------------------------------------------------------------------------;
;	Instead of incrementing the current position by 8 and
...
wcc_neg_dda:
	add	brk_err,bx		;Run dda and subtract an extra pixel
	jl	wcc_have_tot_extra	;  if needed.  JL to be consistent with
	mov	bx,brk_count		;  wccl_... case above
	sub	brk_err,bx		;SUB instead of ADD for same reason
...
;----------------------------------------------------------------------------;
ifdef BIGFONT
; the following macro wil add 10000h to EBX if carry is generated above
	updc_ebx
endif
;----------------------------------------------------------------------------;
...
process_stack_data proc near
	...
	call	get_dest_scan		;compute scan start address
	mov	di,ax			;
	add	di,scan_start		;di complete
	mov	es,seg_lp_surface	;es:di --> first byte to modify
