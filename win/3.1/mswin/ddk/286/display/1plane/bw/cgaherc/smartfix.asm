...
SPECIAL_CASE_DEV_OBW   =1	;Device, opaque black on white, non-clipped
SPECIAL_CASE_DEV_TB    =0	;Device, transparent black,	non-clipped
SPECIAL_CASE_DEV_OWB   =1	;Device, opaque white on black, non-clipped
...
XOR_SCAN	equ	TOGGLE_EVEN_ODD
endif


sBegin	Code
	externNP comp_byte_interval	;Interval calculations
	externNP get_dest_scan		;Dest address calculations
...
; dev_spec_case
;
;	dev_spec_case is the macro invoked to define a special case
;	for the device.
;
;	Warning: This special case code cannot access the frame in
;		 it's innermost loop.
;
;	Usage:
;		dev_spec_case name,logic,draw
;	Where
...
ifdef HERCULES
dev_spec_case	macro	name,logic,draw

	logic				;;Define required macros

;	The inner loop for outputting to destination bits will be
;	expanded based on the hercules interleaving of scanlines.

name&_disp_table label word
	dw	name&_0000h	  ;;Dispatch table used to enter
	dw	name&_2000h	  ;;  the innermost loop.  Based
	dw	name&_4000h	  ;;  on the Hercules interleaving.
	dw	name&_6000h

	public	name
name	proc	near
	mov	bx,di			;;Compute innerloop entry based
	xchg	bl,bh			;;  on which 4K bank of memory
	and	bx,0060h		;;  the character starts in
	shiftr	bx,4			;;
	mov	dx,name&_disp_table[bx]
...
name&_0000h:
	draw	<fetch_logic>
	add	di,SCAN_INC-DRAW_ADJUST      ;;DRAW_ADJUST is 1 if STOSB used
	dec	ch			     ;;  else it is 0
	jz	name&_done_char
...
name&_6000h:
	draw	<fetch_logic>
	add	di,SCAN_INC_PLUS_WRAP-DRAW_ADJUST
	dec	ch
	jnz	name&_inner_loop
...
name&_outer_loop proc near
	loop_setup			;;Char offset fetching
	mov	dx,EVEN_SCAN_INC	;;Compute initial increment
	cmp	di,dx			;;If in first half of display
	jb	name&_have_init_inc	;;  this is the correct increment
	mov	dx,ODD_SCAN_INC 	;;else this is it
name&_have_init_inc:
	mov	ch,byte ptr clipped_font_height
...
; bm_spec_case
;
;	bm_spec_case is the macro invoked to define a special case
;	for a monochrome memory bitmap.  The special case loop will
;	be unrolled by 8.
;
;	Usage:
;		bm_spec_case name,logic,draw,ur
;	Where
;		name	is the name of the procedure being defined
;		logic	is the name of the macro which will define
;			the following macros:
...
bm_spec_case	macro	name,logic,draw
if1					;;
name&_DEST_ADJ = 0
endif
	logic				;;Define required macros
...
INDEX_DEV_TB	= offset $ - offset non_clipped_drawing_functions
	dw	CodeOFFSET dev_trans_black
INDEX_DEV_TB_ALT = offset $ - offset non_clipped_drawing_functions
	dw	CodeOFFSET dev_trans_black
	dw	CodeOFFSET dev_trans_white
	dw	CodeOFFSET dev_trans_white
	dw	CodeOFFSET dev_opaque_black_on_black
INDEX_DEV_OBW	= offset $ - offset non_clipped_drawing_functions
	dw	CodeOFFSET dev_opaque_black_on_white
INDEX_DEV_OWB	= offset $ - offset non_clipped_drawing_functions
...
if SPECIAL_CASE_DEV_OBW eq 1
	mov	ax,CodeOFFSET dev_obw_one_in_cell
	mov	cx,CodeOFFSET dev_obw_two_in_cell
	cmp	bl,INDEX_DEV_OBW
	je	preset_fix_text_finish
endif


if SPECIAL_CASE_DEV_TB eq 1
	mov	ax,CodeOFFSET dev_tb_one_in_cell
	mov	cx,CodeOFFSET dev_tb_two_in_cell
	cmp	bl,INDEX_DEV_TB
	je	preset_fix_text_finish
	cmp	bl,INDEX_DEV_TB_ALT
	je	preset_fix_text_finish
endif

if SPECIAL_CASE_DEV_OWB eq 1
	mov	ax,CodeOFFSET dev_owb_one_in_cell	;Old App support for
	cmp	bl,INDEX_DEV_OWB			;  8-wide byte-aligned
	je	preset_fix_text_finish_gtic		;  text only
endif


;	The middle bytes are not to be special cased.

	mov	ax,next_scan		;If a bitmap, then will need index
	mov	ss_next_scan,ax 	;  to next scan of the bitmap
	mov	ax,CodeOFFSET gen_one_in_cell

preset_fix_text_finish_gtic:
	mov	cx,non_clipped_drawing_functions[bx]
	mov	ss_draw,cx
...
;	outputing the text.  They will call through the stack
...
if SPECIAL_CASE_DEV_OBW eq 1		;Device, opaque black on white
	dev_spec_case	<dev_obw_one_in_cell>,<one_in_cell>,<obwnc>
	dev_spec_case	<dev_obw_two_in_cell>,<two_in_cell>,<obwnc>
endif


if SPECIAL_CASE_DEV_TB eq 1		;Device, transparent black, non-clipped
	dev_spec_case	<dev_obw_one_in_cell>,<one_in_cell>,<tbnc>
	dev_spec_case	<dev_obw_two_in_cell>,<two_in_cell>,<tbnc>
endif

if SPECIAL_CASE_DEV_OBW eq 1		;Device, opaque white on black
	dev_spec_case	<dev_owb_one_in_cell>,<one_in_cell>,<owbnc>
endif

ifdef	PUBDEFS
	include smartfix.pub
endif
