        page    ,132
;-----------------------------Module-Header-----------------------------;
; Module Name:	plasma.ASM
;
;   This module contains functions and definitions specific to
;   the Compaq Portable III 640 x 400 plasma display.
;
; Created: 19-Aug-1987
...
incDevice = 1				;Include control for gdidefs.inc
	...
	externNP dev_initialization


;	The following label provides some error checking at link time
;	as to whether a module was compiled for the CGA or for the
;	Hercules card (or a card using the hercules interleaving).
;	This label enables extern statements which the linker will
;	catch in case of incorrect target adapters.
...
globalW is_protected,0			;not on protected mode

entrymode	db	?		;Old video mode
...
;	The GDIInfo data Structure.  The specifics of the Display
...
info_table_base label byte
	...
	dw	130			;Vertical size in millimeters
	...
	dw	13000			;  VertSize * 100
	dw	640			;  HorizRes
	dw	-400			;  -VertRes


	errnz	dpELoWin-dpMHiWin-8	;English Lo res WinX,WinY,VptX,VptY
        dw      325		        ;  HorzSize * 1000 scaled (/640)
        dw      325		        ;  VertSize * 1000 scaled (/400)
        dw      254		        ;  HorizRes * 254  scaled (/640)
        dw      -254			;  -VertRes * 254 scaled (/400)


	errnz	dpEHiWin-dpELoWin-8	;English Hi res WinX,WinY,VptX,VptY
        dw      1625			;  HorzSize * 10000 scaled (/1280)
        dw      1625			;  VertSize * 10000 scaled (/800)
        dw      127			;  HorizRes * 254 scaled (/1280)
        dw      -127			;  -VertRes * 254 scaled (/800)


	errnz	dpTwpWin-dpEHiWin-8	;Twips		WinX,WinY,VptX,VptY
        dw      2340		 	;  HorzSize * 14400 scaled (/1280)
        dw      2340		        ;  VertSize * 14400 scaled (/800)
        dw      127			;  HorizRes * 254 scaled (/1280)
        dw      -127			;  -VertRes * 254   scaled (/800)
...
;	Constants and macro for changing the screen mode of the display

SET_MODE	equ	0
READ_MODE	equ	15
TEXT_MODE	equ	3
GRAPHICS_MODE	equ	64	; Compaq III 640 x 400 mode.
...
; physical_enable
;
;   Compaq III 640x400 graphics mode is enabled.
;
; Entry:
...
;	INT 10h
; History:
;	Wed 19-Aug-1987 14:48:34 -by-  Walt Moore [waltm]
;	Created.
;-----------------------------------------------------------------------;
...
;-----------------------------------------------------------------------------;
	mov	al,entrymode		;Get mode saved in SaveOldMode
	stosb				;Save in structure
...
; physical_disable
;
;   Compaq III 640x400 graphics mode is exited.  The text mode of the
;   adapter is restored.
;
; Entry:
...
; SaveOldMode
;
;   Compaq III 640x400 load time initialization.
;
; Entry:
;	DS:    =  Data
; Returns:
...
cProc	SaveOldMode,<PUBLIC,FAR>

cBegin

	mov	ah,0Fh			; get current screen mode,
	int	10h
	mov	entrymode,al		; and save it.

	call	dev_initialization	;patch up segs with selectors
