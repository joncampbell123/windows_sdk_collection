...
; General Description:
;
; History:		Nov. 15, 1989: Changed to new color dither scheme.
...
; History:
;
;	Tue 29-Aug-1989 13:53:45 -by-  Amit Chatterjee [amitc]
;         .  On a VGA an RGB of (192,192,192) creates a brush with index 8
;	     whereas on an EGA the index used is 7 (since index 8 is now 
;	     unreacheable except by inverting index 7). The routine 
;	     'RGB192Brush'defined in EGAHIRES.ASM and VGA.ASM does this.
;
;       Wed 12-Apr-1989 17:44:53 -by-  Amit Chatterjee [amitc]
;         .  The color for index 8 changed to (128,128,255)
;         .  GREY_SCALE accelrator set only when we have high intensity colors
;	     and special case the bad dithers - for it is only here that we  
;            can gurantee that the pattern is same in all 4 planes.
;
...
sum_RGB_colors_alt proc near
	push	ds
	cCall	rgb_to_ipc		;makes DS=Data
	pop	ds
sum_RGB_colors_exit:
	ret
...
;	None
; History:
;	Fri 20-Jan-1989 14:30:20 -by-  Amit Chatterjee [amitc]
;	number of planes = 4 + mono, instead of 3 + mono.
;
;	Thu 19-Feb-1987 14:06:41 -by-  Walt Moore [waltm]
;	Created.
;-----------------------------------------------------------------------;
