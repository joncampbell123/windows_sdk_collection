...
;   Author: MDW
;
;   (C) Copyright MICROSOFT Corp. 1986, 1987, 1988
;
;   January 1988
;
;DESCRIPTION:
...
BeginProc   VDD_Int_10,PUBLIC
	...
	test	[edi.VDD_Flags],fVDD_MEna   ; Q: Is memory enabled?
	jz	VI10ExitEmNo		    ;	N: Don't emulate
...
BeginProc VDD_I10_Mode_Switch
	...
	test	[Vid_Flags], fVid_SysVMI    ; Q: SYS VM initialized?
...
VI10_Not2:
	...
	or	[edi.VDD_Flags],fVDD_NTSave
	and	[edi.VDD_Flags],NOT fVDD_NoTrap
VI10_TrapEnable:
...
%OUT what about timeout on adjusted exec priority during mode switch
...
; Assume RAM not saved and clear "Needs 256k mem" flag
	and	[edi.VDD_Flags],NOT (fVDD_MdSvRam+fVDD_old256)
...
VI10_SetSvRAMBit:
	or	[edi.VDD_Flags],fVDD_MdSvRam+fVDD_Save ;Y: Remember RAM saved

VI10_exit:
	test	[edi.VDD_Flags],fVDD_Win	; Q: Running in a window?
        jz      SHORT VI10_00
        test    [edi.VDD_Flags],fVDD_ModeChange 
        jnz     SHORT VI10_00
	call	VDD_Clr_VM_Time_Out		; Clear VM timeout
        bts     [edi.VDD_Flags],fVDD_ModeChangeBit; Delay mode change
VI10_00:
...
BeginProc VDD_I10_Mode_Sw_Done
	...
	test	[edi.VDD_Flags],fVDD_NTSave
	jz	SHORT VIR_00
	and	[edi.VDD_Flags],NOT fVDD_NTSave
	or	[edi.VDD_Flags], fVDD_NoTrap
VIR_00:
...
VIR_01:
	...
	test	[edi.VDD_Flags],fVDD_256	;Q: 256 bit set now?
	jnz	SHORT VIR_exit			;   Y:
	test	[edi.VDD_Flags],fVDD_old256	;Q: 256 bit set before mode sw?
	jz	SHORT VIR_exit			;   N:
