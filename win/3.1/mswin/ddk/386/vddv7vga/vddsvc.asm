...
;******************************************************************************
;
;VDDCTL - Virtual Display Device Control Routines
;
;   Author: MDW
;
...
Assert_Msg_Mode_VM ClrScrn
	...
	test	[Vid_Flags], fVid_MsgA		;Q: message mode attached?
...
BeginProc VDD_Msg_InitScreen
	...
	test	[Vid_Flags],fVid_MsgA		; Q: Already init screen?
	...
	or	[Vid_Flags], fVid_MsgR		; msg mode state programmed
	xor	eax, eax
...
ENDIF
	...
	test	[Vid_Flags], fVid_MsgR	    ;Q: msg mode state programmed?
...
; Hi res graphics mode in background require attached VM to be benign...
	...
	add	edi,[VDD_CB_Off]
	test	[edi.VDD_Flags],fVDD_256	; Q: Is attached VM video hog?
	pop	ebx
...
;
	test	[edi.VDD_Flags],fVDD_ModeChange ; Q: During mode change?
...
Changed_Since_Getmod:
        test    [edi.VDD_Flags],fVDD_ScrollModf ; Q: Scroll modf returned on previous Get_mod?
        jz      SHORT VCM_00                    ;  N: skip copy mem update
        or      [edi.VDD_Flags],fVDD_UpdAll1    ;  Y: Force complete screen update
        jmp     SHORT Update_CopyMem            ;     copy mem update

VCM_UpdAll1Set:
        or      [edi.VDD_Flags],fVDD_UpdAll2
        jmp     SHORT VCM_Exit
...
%OUT Should we have mode where we don't warn user?
