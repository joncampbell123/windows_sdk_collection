...
ELSE
	mov	eax,10h
ENDIF
	test	[edi.VDD_PIF],fVidHghRsGrfxMd	; Q: Alloc hi res mode mem?
	jnz	SHORT VMVI_ModeSet              ;   Y: 
	mov	eax,4
	test	[edi.VDD_PIF],fVidLowRsGrfxMd	; Q: Alloc lo res mode mem?
	jnz	SHORT VMVI_ModeSet              ;   Y:
...
;Lastly, disable access to VM's video memory
	or	[edi.VDD_Flags],fVDD_MEna	; Force mem disable
...
BeginProc VDD_SetEGAMemSize,PUBLIC
	...
	test	[ebx.CB_VM_Status],VMStat_Exclusive ; Q: Exclusive VM?
	jz	SHORT VSE_Exit			    ;   N: Proceed
	test	[ebx.CB_VM_Status],VMStat_Background ; Q: Does it run bkgrnd?
...
BeginProc VDD_CheckMmgrCallOK
        ...
        test    [Vid_Flags],fVid_PageSwapInst ; Q: PageSwap device installed?
...
Assert_VDD_ptrs ebx,edi
	...
	test	[edi.VDD_PIF],fVidTextMd	    ; Q: Alloc text mode mem?
	jz	SHORT VMAM_A0
	or	ecx,30003h
VMAM_A0:
	test	[edi.VDD_PIF],fVidLowRsGrfxMd	    ; Q: Alloc lo res mode mem?
...
VMAM_R0:
    ...
    ;
	test	[edi.VDD_PIF],fVidRetainAllo	    ; Q: Skip shrinking?
	jnz	SHORT VMAM_MemRetained		    ;	Y: retain old mem handle

	test	[edi.VDD_Flags],fVDD_MdSvRam	    ; Q: RAM save?
...
;Look at fVDD_Save only if we are in a mode set
	test	[edi.VDD_Flags],fVDD_ModeSet        
        jz      SHORT VMAM_PreserveOldPages         
	test	[edi.VDD_Flags],fVDD_Save	    ; Q: Data saved in mem?
...
Assert_VDD_ptrs ebx,edi
IFDEF	PEGA
	test	[edi.VDD_Stt.V_Flag_PEGA],fVPa_PShdw  ; Q: Is mem enabled?
	jnz	SHORT VDD_MDis_Enabled		;   Y: disable it
ENDIF
	test	[edi.VDD_Flags],fVDD_MEna	; Q: Is mem enabled?
...
ENDIF
	and	[edi.VDD_Flags],not fVDD_MEna	; Mem not enabled
VDD_MDis_Ex:
	ret
...
VMP_NoUnmap:
	or	[edi.VDD_Flags],fVDD_MEna	; Memory is enabled
	and	[edi.VDD_Flags],NOT fVDD_MonoMem
	pop	edx
	pop	ecx
	ret
...
IFDEF	VGAMONO
	...
	jnz	SHORT VMUnm_01
ENDIF
	and	[edi.VDD_Flags],not fVDD_MEna	; No Mem enabled
VMUnm_01:
	ret
VDD_U_CGA:
...
VDD_U_Grap:
	test	[edi.VDD_Flags],fVDD_256	; Q: 256k VM?
...
; B8000-BFFFF not mapped
	...
	VMMCall _ModifyPageBits,<ebx,0B8h,8,<NOT P_AVAIL>,eax,PG_HOOKED,eax>
IFDEF	VGAMONO
	test	[Vid_Flags],fVid_Mono		; If mono supported, unmap
...
VMV_CGA:
	test	[edi.VDD_Flags],fVDD_Font	; Q: mem mapped for font load?
...
IFDEF	VGAMONO
	...
	test	[Vid_Flags],fVid_Mono		;   N: Q: Mono mapping support?
	jnz	SHORT VMV_Ex			;	N: Return
	test	[edi.G_Misc],04h		; Q: B000 mapping in text mode?
	jnz	SHORT VMV_NotMono		;   N: Make sure mem at B8000
	test	[edi.VDD_Flags],fVDD_MonoMem	;   Y: Make sure mem at B0000
	jz	SHORT VMV_Invalid
	jmp	SHORT VMV_Ex
VMV_NotMono:
	test	[edi.VDD_Flags],fVDD_MonoMem	; Q: Mem at B0000
...
VMV_EGA:
	test	BYTE PTR [edi.VDD_Flags],fVDD_Font ; Q: Mapped for font load?
...
VDD_MR_CGA0:
	and	[edi.VDD_Flags],not fVDD_Font
	jmp	SHORT VDD_MR_DoIt

;*******
; VRAM was in unmappable mode, return error
VDD_MR_VGAGrap:
	or	[edi.VDD_Flags],fVDD_256	    ; Indicate needs all of mem
	call	VDD_Msg_Exclusive
	stc
	ret
...
VDD_MR_ToEGA:
	...
	or	[edi.VDD_Flags],fVDD_Font	    ;	Y: mapped for font load
...
VDD_MR_DoIt:
	...
	test	[edi.VDD_Flags],fVDD_ModeSet	    ; Q: During mode set?
...
VMRMI_0:
	...
	test	al,al
	jns	SHORT VMRMI_1
	test	[edi.VDD_Flags], fVDD_ModeSet	;Q: in INT 10 mode switch?
...
Assert_VDD_ptrs ebx,edi
VEB_MapNull:
	or	[edi.VDD_Flags],fVDD_MEna	; Mem is enabled and null
	VMMCall _GetNulPageHandle
	mov	edx,0A0h
	push	eax
...
IFDEF	VGA
	...
	call	VDD_SaveCtlr			; Save controller state
        and     [Vid_Flags],NOT fVid_DOff       ; cancel display off
        clc
        call    VDD_RestIndx
VMI_00:
ENDIF
...
ENDIF
	...
	test	[edi.VDD_Flags],fVDD_MdSvRam	; Q: RAM saved?
...
ENDIF
	...
	and	[edi.VDD_Flags], NOT fVDD_Save	;   Y: Mem not saved yet
VMI_01:
...
Assert_VDD_ptrs ebx,edi
	test	[Vid_Flags],fVid_PEGA		    ; Q: PEGA card?
...
Assert_VDD_ptrs ebx,edi
	test	[Vid_Flags],fVid_PEGA		; Q: PEGA?
	jz	SHORT VDDDP_Ex
IFDEF	DEBUG
...
Assert_VDD_ptrs ebx,edi
	...
	test	DWORD PTR [edi.VDD_Stt.VC_PShdw],fShdwEna ; Q: Shadow enabled?
	stc
	jz	SHORT VMQW_Ex			;   N: Return CF=1, ZF=1
