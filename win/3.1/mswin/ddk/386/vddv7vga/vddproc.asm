...
VPF_Ok:
	...
	test	[VDD_PT_Buff],P_PRES		; Q: Did we map some mem here?
...
VPF_256:
	bts	[edi.VDD_Flags],fVDD_256Bit	; Only run forground
...
BeginProc VPF_SuspendNow_Event
	mov	edi,edx
	add	edi,[VDD_CB_Off]
	and	[edi.VDD_Flags],NOT fVDD_SuspEvent ; Suspension dequeued
...
ENDIF
	jc	SHORT VPFF_Exit
	or	[edi.VDD_Flags],fVDD_WaitAtt	; VM is Suspended
...
Trace_Out "VDD: Suspending #EDX on VM #EBX"
	...
	or	[edi.VDD_Flags],fVDD_WaitAtt    ; Suspended
	mov	ebx,edx
	VMMCall Suspend_VM
VPFF_Exit:
	ret
...
BeginProc VDD_VMIdle,SERVICE,High_Freq
	...
	jz	SHORT VMI_00
	test	[edi.VDD_Flags],fVDD_ModeChange ; Q: In a mode change?
...
BeginProc VDD_VMTimeOut,PUBLIC,High_Freq
	mov	edi,edx 			; EDI = VM's VDD CB ptr
        btr     [edi.VDD_Flags],fVDD_ModeChangeBit
...
BeginProc VDD_Attach2,PUBLIC
	...
	test	[edi.VDD_Flags],fVDD_256	; Q: Need 256k video RAM?
	jnz	SHORT VA2_256			;   Y: Don't run this VM bckgnd
	push	edi
	mov	edi,[VDD_Focus_VM]
	add	edi,[VDD_CB_Off]
	test	[edi.VDD_Flags],fVDD_256	; Q: Attached VM a RAM hog?
	pop	edi
...
BeginProc VDD_Detach2,PUBLIC
	...
	or	[edi.VDD_Flags],fVDD_Save	; Indicate state saved
