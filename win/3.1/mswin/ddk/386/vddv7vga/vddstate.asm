...
;	or	[Vid_Flags],fVid_450		;   Y: Allow 2nd half EGA
ELSE
	xor	eax,eax
IFDEF	CTVGA
...
;VN_SYS_2EGA:
IFDEF	VGA
	or	[esi.VDD_Flags],fVDD_256	; SYS VM needs 256k RAM
ENDIF
...
;	and scan lines beyond the max scan line.
	...
	jnz	VSGS_None 		            ;	Y: return no cursor
	...
	ja	VSGS_None 		            ;	Y: set to null cursor
VSGS_2:
...
; Return the screen ON/OFF flag
	...
	or	[esi.VDA_EGA_Flags],fVDA_V_ScOff    ;	Y: Set screen off flag
VSGS_6:
        test    [edi.VDD_Flags],fVDD_MInit          ; Q: VMInit done yet?
        jnz     SHORT VSGS_MainMemCheck             ;   Y: continue
	or	[esi.VDA_EGA_Flags],fVDA_V_ScOff    ;	N: Set screen off flag
        jmp     SHORT VSGS_7
VSGS_MainMemCheck:        
...
; Return the horizontal cursor tracking flag
	...
	or	[esi.VDA_EGA_Flags],fVDA_V_HCurTrk  ;	Y: Set HCurTrk flag
VSGS_8:
	ret
VSGS_None:
...
; First check whether screen is turned off
	...
	test	[edi.VDD_Flags],fVDD_ModeSet	    ; Q: Mode change ongoing?
	jnz	SHORT VCC_NoChg 		    ;	Y: report screen off
	test	[edi.VDD_Stt.A_Indx],fVAI_ScOn
	jnz	SHORT VCC_0
VCC_Off:
	or	[esi.VDD_Mod_Flag],fVDD_M_ScOff     ; Set screen off flag
...
VMC_EGAGraphics:
	...
	or	[esi.VDD_Mod_Flag],fVDD_M_Type_Page+fVDD_M_VRAM
...
;	jz	short VSQ_Exit			;   N: report no change
        ...
        test    [edi.VDD_Flags],fVDD_UpdAll2Bit
        jnz     VSQ_Change
...
; assume no scroll modfs
        and     [edi.VDD_Flags],NOT fVDD_ScrollModf
        test    [edi.VDD_Flags],fVDD_UpdAll2    ; Q: Force a full screen Upd?
...
ENDIF
        pop     esi
        pop     ecx
        or      [ebx.VDD_Flags],fVDD_ScrollModf
    ;
    ; ScrCnt = 7 - ECX + 1
    ;
        sub     ecx,7
