;--------------------------------------------------------------------
;
;   Screen Grabber for HERCULES adaptor - HERC.ASM
;
...
;
VDD_Type db	"VIDEOMDA"		; HERCULES 720x348
...
;   and the grabber. HERC grabbers do not use this.
;
; ENTRY:
...
cBegin
    ...
    ;
	test	[si.EPGrabDTA.CntrlSt.VDA_HERC_Flags],fVDA_V_ScOff
	jnz	short NulScreen
	mov	ah,[si.EPGrabDTA.CntrlSt.VDA_HERC_Mode]
	cmp	ah,HERC_MODE_Grfx	;Text mode(0)?
...
;
;   FOR THE HERC grabbers this routine is a NOP.
;
; ENTRY:
;	lpPntStruc Extended paint structure
; EXIT:
...
TextSel:
	mov	cx,[bx.AltFnt1.FontWid] ; Assume 80 col
	mov	si,[bx.AltFnt1.FontHgt]
    ;
	...
	inc	ax			; LF
ifdef	DBCS
;added two for DBCS on each side(left and right)
	cmp	[bx.EPGrabDTA.CntrlSt.VDA_HERC_Mode],HERC_MODE_Text
	jnz	@F
...
RenderFromGrabState:
	...
	test	[si.EPGrabDTA.CntrlSt.VDA_HERC_Flags],fVDA_V_ScOff
	jnz	RS_Error
	mov	cl,[si.EPGrabDTA.CntrlSt.VDA_HERC_Mode]	  ; Get mode
	cmp	cl,HERC_MODE_Grfx
	jb	RSText
        ja      short RS_Error
...
;	lpDBCSRect - Points to a RECT data that specifies adjusted region.
;		     
; EXIT:
;	AX = TRUE Continue
;	   = FALSE End
...
ADR2:
	...
	mov	dx,[di.rcTop]
	call	AdjustDBCSBound ;adjust left and right bound
	mov	[di.rcLeft],ax
	mov	[di.rcRight],cx
...
cBegin
    ...
    ; Now ds:bx,ds:si -> EXTPAINTSTRUC structure
    ;
	mov	dl,[bx.EPGrabDTA.CntrlSt.VDA_HERC_Mode]
	cmp	dl,HERC_MODE_Grfx
	jb	short FontYes
	xor	ax,ax
	mov	[bx.FntHgt],ax
	mov	[bx.FntWid],ax
...
; us (actually only need one extra, but the EXTPAINTSTRUC has room for six).

FontList	Label	byte
F1	db	"HERCWOA.FON",0,0	; Name	   80 column font
.erre ($-F1) EQ 13
	dw	13			; Height
	dw	9			; Width
...
.erre	fVDD_M_Curs	    EQ	 fDisp_Curs
	...
	test	[si.EPGrabDTA.CntrlSt.VDA_HERC_Flags],fVDA_V_HCurTrk
	jz	short NoHTrk
...
JustCursor:
	...
	dec	dx
	mov	cl,[si.EPGrabDTA.CntrlSt.VDA_HERC_Mode]
	cmp	cl,[si.EPGrabDTA.CurCursMode]	; Change in cursor mode?
	jnz	short MkNCur		; Yes
	mov	cx,[si.EPGrabDTA.CntrlSt.VDA_HERC_CurEnd]
	cmp	cx,[si.EPGrabDTA.CurCursEnd]	; Change in cursor shape?
	jnz	short MkNCur		; Yes
	mov	cx,[si.EPGrabDTA.CntrlSt.VDA_HERC_CurBeg]
...
NoAdjB:
	...
	cmp	[si.EPGrabDTA.CntrlSt.VDA_HERC_Mode],HERC_MODE_Grfx	; Text mode?
...
NtTxt1:
	...
	idiv	cx
	xor	cx,cx
	mov	cl,[si.EPGrabDTA.CntrlSt.VDA_HERC_Rows]
...
;*****************************************************************************
cProc TranslateWinToFScreen,<FAR,PUBLIC>
	parmD	lpPntStruc
cBegin
    assumes ds,nothing
    assumes es,nothing
