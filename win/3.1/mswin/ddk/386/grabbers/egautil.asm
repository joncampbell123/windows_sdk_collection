...
hgtok1:
IFNDEF	VGA
    ; On EGA, the VDA_EGA_CurEnd value is one greater than the
    ; bottom line of the cursor.
	dec	ax
	or	ax,ax
	jnz	hgtok2
	inc	ax
...
ENDIF
    ...
    ; AX = height
    ;
	call	GetFontHeight			; cx = font height.
    ;
    ; The height now depends on font also... cx = fontheight
    ; height = height*fntheight/dos_cursor_size
    ;
	mul	cx				; ax = height*fntheight
	call	GetDosCursorSize
	div	cx
	shr	cx,1				; cx = fntheight/2
	cmp	dx,cx
...
;	    If Graphics planar mode(D,E,10)
...
; GetFontHeight
;
; ENTRY: DS:SI -> EXTPAINTSTRUC structure
; EXIT:  CX = height of current font.
; USES:  NONE
;
;
...
CPText:
	...
	mul	cx
	mov	dx,[si.EPGrabDTA.CntrlSt.VDA_EGA_CurBeg]
	cmp	dx,[si.EPGrabDTA.CntrlSt.VDA_EGA_CurEnd]
	jae	SHORT CPR		; cursor height <=0, no cursor
  ;
  ; Now we need to calculate the X coordinate...
  ; X coord = CurBeg*FontHgt/doscursize (rounded off)
  ; No rounding off for cursorpos
  ;
	push	ax
	mov	ax,dx
	mul	cx
...
IFDEF VGA
	cmp	[si.EPGrabDTA.CntrlSt.VDA_EGA_Rows],25
	ja	short CursorStartOK
	sub	ax,2			; Adjust
        jns     short CursorStartOK     ; Q: AX negative?
        mov     ax,0                    ;   Y: coerce to 0
CursorStartOK:
ENDIF
	push	ax
