...
*   WIN32_FIND_DATA structure. The values are supposedly in system time and
*   system time is ambiguously defined (it is UTC for Windows NT, local time
*   for Win32S and probably local time for Win32C). Therefore, these values
*   must be converted to local time before than can be used.
*
*Entry:
...

{
    ...
    if ( findhandle == INVALID_HANDLE_VALUE ) {
        ...
        buf->st_mtime = __loctotime_t(1980,1,1,0,0,0);
...
unsigned short _CALLTYPE1 _dtoxmode ( CInfoPBPtr pcinfoPB );
...
unsigned short _CALLTYPE1 _dtoxmode (
    CInfoPBPtr pcinfoPB
    )
{
    REG1 unsigned short uxmode;
