...
typedef Rect			RECT;			
typedef Rect			*LPRECT;
typedef EventRecord		MSG;
typedef EventRecord		*LPMSG;
...
{
#ifndef _MSC_VER
#pragma unused(lpszPrefix, lpszMsg, nIndent);
#endif
...
)
{
	...
	sprintf(szBuf, "[obj=(0x%lx) cnt=%ld] %s", lpObj, refcnt, lpszMsg);
	OleDbgPrintAlways(lpszPrefix, szBuf, 0);
#else
#ifndef _MSC_VER
	#pragma unused(lpszPrefix, lpszMsg, lpObj, refcnt);
#endif
#endif
}
...
#else
#ifndef _MSC_VER
	#pragma unused(lpszPrefix, lpszMsg, lpRect);
#endif
#endif
}
...
#define CASE_SCODE(sc)  \
		case sc: \
			strcpy((LPSTR)szErrName, (LPSTR)#sc); \
			break;
...
STDAPI_(void) OleDbgPrintScodeAlways(LPSTR lpszPrefix, LPSTR lpszMsg, SCODE sc)
{
	...
	switch (sc) {
		...
		default:
			strcpy(szErrName, "UNKNOWN SCODE");
	}
...
#else
#ifndef _MSC_VER
	#pragma unused(lpszPrefix, lpszMsg, sc)
#endif
