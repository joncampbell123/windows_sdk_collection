   ...
   )
{
     ...
     if (!hPrevInstance) {
         ...
         wndclass.hIcon         = LoadIcon(hInst, MAKEINTRESOURCE(IDI_SHOWICON));
         wndclass.hCursor       = LoadCursor (NULL, IDC_ARROW) ;
         wndclass.hbrBackground = GetStockObject (BLACK_BRUSH) ;
         wndclass.lpszMenuName  = MAKEINTRESOURCE(SHOWDIB) ;
     ...
     /*	The Macintosh dosn't have extensions in WIN.INI file */
#if !defined(_MAC)
     ...
     hWnd = CreateWindow( szAppName,
                          ...
                          NULL) ;
#if defined(_MAC)
	// stick "about" menu into Apple menus
	{
		char szHelpText[MAX_BUFFER];
		LoadString(hInstance, IDS_HELPMENUTEXT, szHelpText, MAX_BUFFER);
		ModifyMenu(GetSystemMenu(hWnd, FALSE), 0, MF_BYPOSITION, 
				IDM_ABOUT, szHelpText);
	}
#endif   // _MAC
...
#if defined(_MAC)
		// handle About command on apple menu
        case WM_SYSCOMMAND:
...
#if !defined(_MAC)
         //
         // WLM dosn't support AnimatePalette, because you can't shift a
         // realized palette on the mac.
         //
         case WM_TIMER:
...

{
    ...
    HPALETTE         hpal;
#if !defined(_MAC)
    WORD             i;       // used only on !_MAC compilations
#endif
    CHAR             Name[40];
    BOOL             bSave;
    ...
    switch (id) {
        ...
        case IDM_OPEN:
               //
               // Porting Note:
               // On the Macintosh, DlgDirList, DlgDirSelect,
               // GetCurrentDirectory, SetCurrentDirectory aren't
               // implemented due to differences in the file system.
               // The original File/Open File/Save relied on these functions,
               // so I replaced them with a call to Common Dialog
               // GetOpenFileName (which would also work for the
               // Windows side of things, but I'm preserving as much
               // of the original code as possible).
               //
               {
                  ...
                  // buffer for OPENFILENAME structure, passed to commdlg
                  hofn     = GlobalAlloc(GHND, sizeof(OPENFILENAME));

                  // buffer for namke of selected file on return from commdlg
                  hszFile  = GlobalAlloc(GHND, MAX_BUFFER);

                  // buffers for dialog title, and filter strings
                  hszFilter  = GlobalAlloc(GHND, MAX_BUFFER);
                  hszTitle   = GlobalAlloc(GHND, MAX_BUFFER);

                  // make sure everything got allocated ok
                  if (hofn && hszFile && hszFilter && hszTitle)
                  {
                     // lock memory down and boogy on it
                     lpofn    = GlobalLock(hofn);
                     lpszFile = GlobalLock(hszFile);
                     lpszFilter = GlobalLock(hszFilter);
                     lpszTitle  = GlobalLock(hszTitle);

                     LoadString(hInst, IDS_OPENDLGFILTER, lpszFilter, MAX_BUFFER);
                     LoadString(hInst, IDS_OPENDLGTITLE,  lpszTitle,  MAX_BUFFER);

                     iBytes = lstrlen(lpszFilter);

                     // normalize filter, replacing '|' with NULL
                     for(idx=0; idx < iBytes; idx++)
                     {
                        if(lpszFilter[idx] == '|')
                           lpszFilter[idx] = '\0';
                     }
                     ...
                     lpofn->lpstrFile     = lpszFile;
                     lpofn->nMaxFile      = MAX_BUFFER;
                     lpofn->lpstrTitle    = lpszTitle;
                     lpofn->Flags         = OFN_FILEMUSTEXIST | OFN_HIDEREADONLY |
                                            OFN_NOCHANGEDIR;
                     ...
                     if (GetOpenFileName(lpofn))
                     {
                        ...
                        fh = OpenFile(lpszFile, &ofs, OF_READWRITE);
                        lstrcpy(achFileName, lpszFile);
                     ...
                     // unlock memory
                     GlobalUnlock(hszTitle);
                     GlobalUnlock(hszFilter);
                     GlobalUnlock(hszFile);
                     GlobalUnlock(hofn);
                  }

                  GlobalFree(hszTitle);
                  GlobalFree(hszFilter);
                  GlobalFree(hszFile);
                  GlobalFree(hofn);
               }
               break;
       ...
       case IDM_SAVE:
               {
                  ...
                  if (hofn && hszFile && hszFilter && hszTitle)
                  {
                     ...
                     LoadString(hInst, IDS_SAVEDLGFILTER, lpszFilter, MAX_BUFFER);
                     LoadString(hInst, IDS_SAVEDLGTITLE,  lpszTitle,  MAX_BUFFER);
                     ...
                     if (GetSaveFileName(lpofn))
                     {
                        ...
                        if (fh != HFILE_ERROR)
                        {
                           ...
                           // Realize a DIB in the specified format and
...
#if !defined(_MAC)
        //
        // Disable animated palette on Macintosh because you
        // can't shift a realized palette on the Mac.
        //
        case IDM_ANIMATE0:
                if (!hpalSave)
                    break;
