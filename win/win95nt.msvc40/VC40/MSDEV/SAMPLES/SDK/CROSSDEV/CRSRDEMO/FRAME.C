...
BOOL INTFUNC DoConnect(HWND hwnd)
{
	...
	if (SUCCESS(rc)) {
		...
		rc = SQLAllocStmt(g_hdbc, &hstmt);
		if (!SUCCESS(rc))
			g_fAsyncSupported = FALSE;

		else {
			g_fAsyncSupported = SUCCESS(SQLSetStmtOption(hstmt,
														SQL_ASYNC_ENABLE, 1));
...
BOOL INTFUNC DoFrameMenu(HWND  hwnd, WPARAM  wparam, LPARAM  lparam)
{
	...
	switch (GET_WM_COMMAND_ID(wparam, lparam)) {
		...
		case IDM_HELP_HELP:
			WinHelp(g_hwnd, szHELPFILE, HELP_CONTEXT, HLP_CRSRDEMO);
...
--------------------------------------------------------------------------*/
LRESULT CALLBACK FrameProc(HWND    hwnd,
							UINT   msg,
							WPARAM wparam,
							LPARAM lparam)
{
...
#ifdef _MAC
		case WM_SYSCOMMAND:
			if (lparam ==0 && LOWORD(wparam) == IDM_HELP_ABOUT)
	        {
