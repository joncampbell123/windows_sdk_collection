...
//RODROD changed handle to hinstance for hinst...
...
VOID 	InitializeMenu (HANDLE);
VOID 	CommandHandler (HWND, UINT, LONG);
BOOL 	SetWrap (HWND,BOOL);
BOOL 	QueryCloseAllChildren ( VOID );
INT  	QueryCloseChild (HWND);
...
#if defined(_MAC)
		case WM_SYSCOMMAND:
			if (lParam ==0 && LOWORD(wParam) == IDM_HELPABOUT)
	            {
	                SendMessage(hwnd,WM_COMMAND,IDM_HELPABOUT,0);
	                return 0;
	            }
			else
            hWndClient = (HWND)GetWindowLong(hwndFrame,CLIENTWND);
            return(DefFrameProc(hwndFrame,hWndClient,msg,wParam,lParam));
 ...
 *                                                                          *
 ****************************************************************************/
LONG CALLBACK MPMDIChildWndProc
(
    HWND    hwnd,
    UINT    msg,
    WPARAM  wParam,
 ...
 *                                                                          *
 ****************************************************************************/
VOID InitializeMenu (register HANDLE hmenu)
{
...
BOOL SetWrap(HWND hwnd, BOOL fWrap)
{
    LONG    dws;
    HANDLE  hT;
    HANDLE  hTT;
    ...
    SetWindowWord (hwnd, GWL_WORDWRAP, (WORD)fWrap);
