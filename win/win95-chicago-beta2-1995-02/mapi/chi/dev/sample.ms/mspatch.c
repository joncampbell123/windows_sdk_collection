...
#define ATCH_ValidateParameters(pobj, eMethod)      \
        OBJ_ValidateParameters(pobj, eMethod, sizeof(IATCH), &vtblIATCH)
...
STDMETHODIMP IATCH_SaveChanges(PIATCH piatch, ULONG ulFlags)
{
    ...
    ATCH_ValidateParameters(piatch, IMAPIProp_SaveChanges);
    ...
    ULONG *pcval, LPSPropValue *ppval)
{
    ...
    ATCH_ValidateParameters(piatch, IMAPIProp_GetProps);
...
IATCH_GetPropList(PIATCH piatch, ULONG ulFlags, LPSPropTagArray *lppPropTagArray)
{
    ...
    ATCH_ValidateParameters(piatch, IMAPIProp_GetPropList);
    ...
    ULONG ulInterfaceOptions, ULONG ulFlags, LPUNKNOWN * lppUnk)
{
    ...
    ATCH_ValidateParameters(piatch, IMAPIProp_OpenProperty);
    ...
    {
        hr = NewIMSGInIATCH((LPMESSAGE) lpunk, (POBJ) piatch, &pimsg);
        if (hr != hrSuccess)
            goto exit;
 ...
 *      MAPI_E_NO_ACCESS    The caller does not have access
 *                                  to the requested properties.
 *      MAPI_E_MODIFY_FAILURE       A general problem affecting
    ...
    LPSPropProblemArray *pprba)
{
    ...
    ATCH_ValidateParameters(piatch, IMAPIProp_SetProps);
    ...
    LPSPropProblemArray * pprba)
{
    ...
    ATCH_ValidateParameters(piatch, IMAPIProp_DeleteProps);
    ...
    LPSPropProblemArray * pprba)
{
    ...
    ATCH_ValidateParameters(piatch, IMAPIProp_CopyTo);
    ...
    LPSPropProblemArray * pprba)
{
    ...
    ATCH_ValidateParameters(piatch, IMAPIProp_CopyProps);
 ...
 *      values for the base properties of the an attachment object:
 *      takes as input parameters the values of those properties
 *      that are specific to this attachment and derives the values
 *      of those properties that are identical for all attachments
 *      in the Sample Store.  Also sets attributes.
 *
 *  Arguments:
 *      piatch      Internal IAttach object instance.
 *
 *  Returns:
 *      HRESULT
 *
 *  Side effects:
 *      Allocates a property value array, property tag array, and
 ...
 *                                  the property arrays.
 */
HRESULT HrInitIATCHProps(PIATCH piatch)
{
    HRESULT hr = hrSuccess;
    SCODE sc = S_OK;
    ...
    AssertSz(piatch, "Bad piatch\n");

    /* Allocate the property arrays. */
    hr = HrAllocPropArrays(cInitIATCHProps, &pval, &ptaga, &patra);
    if (hr != hrSuccess)
        goto exit;

    /* Initialize property value array and all property tags. */

    ptaga->cValues = patra->cValues = cInitIATCHProps;

    pval[0].ulPropTag = ptaga->aulPropTag[0] = PR_OBJECT_TYPE;
    pval[0].Value.l = MAPI_ATTACH;

    /* next two properties are initialized to illegal values since */
    /* these must be set by the client */
    pval[1].ulPropTag = ptaga->aulPropTag[1] = PR_ATTACH_METHOD;
    pval[1].Value.l = 0L;
    pval[2].ulPropTag = ptaga->aulPropTag[2] = PR_RENDERING_POSITION;
    pval[2].Value.l = -1;

    /* Initialize the property attribute array. */

    patra->aPropAttr[0] = grfInitIATCHPropAttr;
    patra->aPropAttr[1] = grfInitIATCHPropAttr | PROPATTR_WRITEABLE;
    patra->aPropAttr[2] = grfInitIATCHPropAttr | PROPATTR_WRITEABLE;

    /* Set the property values. */

    hr = piatch->lpattach->lpVtbl->SetProps(piatch->lpattach,
        cInitIATCHProps, pval, &pprba);
    ...
    hr = SetAttribIMsgOnIStg(piatch->lpattach, ptaga, patra, &pprba);

    /* if (hr != hrSuccess || pprba) fall through to exit; */

exit:
    FreePropArrays(&pval, &ptaga, &patra);
    if (pprba)
    {
        LMFree(&piatch->pims->lmr, pprba);
        hr = ResultFromScode(MAPI_E_CALL_FAILED);
    }

    DebugTraceResult(HrInitIATCHProps, hr);
    return hr;
}
