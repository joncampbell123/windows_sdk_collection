...
Basic Forms interface for exe server.

Written: 9-8-1994 18:03:00

Author: kylel

CodeBase: \\malibu\src\slm1\src\boomer\spm\mailint.cxx
          \\malibu\src\slm2\src\capone\sx\sf.c
...
HrStartOleAndRegisterClassFactory(void)
{
    ...
    if (S_OK != hr)
        {
        TraceTag(tagForm,"OleInit failed 0x%08lx",hr);
        return hr;
        }
    ...
    DWORD dwRegMyForm = 0;
    hr = CoRegisterClassObject (CLSID_MyFormsClsId, (LPUNKNOWN)pfrmfmr,
...
FRM::ScGetRecipientAdrlist(LPMESSAGE pmsg, LPADRLIST * ppal)
{
    ...
    hr = HrQueryAllRows(pmt, ptaga, NULL, NULL, 0, (LPSRowSet *) ppal);
    if (hr)
    {
...
FRM::SendForm(VOID)
{
    ...
    if (hr = pMessageSite->SubmitMessage(0) )
    {
        ...
        #ifndef GOOD_FORM_BEHAVIOR //$REVIEW: sending myself is easier to debug when something fails

        // ----- No harm in trying to send it myself is there?
...
//$TODO return cases leak memory
    ...
    static SizedSPropTagArray(24,tagaRemoveFromNewReply) =
        { 24,
    ...
    /*
       ...
       subclass the reply note for IPM.Checkers.
    ...
    */
    ...
    for (itaga = 1; itaga < tagaRepliee.cValues; itaga++)
        ...
        TraceTag(tagForm,"0x%08lx <0x%08lx>",al.aEntries[0].rgPropVals[itaga].ulPropTag,al.aEntries[0].rgPropVals[itaga].Value.ul);

        AssertSz(SUCCEEDED(al.aEntries[0].rgPropVals[itaga].Value.ul),"Failure to get PR_SENDER* RAID: 10961 ems_server");
        }

    #ifdef RAID_10961_FIXED //$REVIEW
    // ----- Resolve the name in case the entry id is missing
    {
        LPADRBOOK               pAdrBook;
        LONG                    ulUIParam = 0;
        ULONG                   itaga = 1;

        AssertSz(pSession,"pSession not available ... ");
        Assert(al.aEntries && al.aEntries[0].rgPropVals);
        if (FAILED(al.aEntries[0].rgPropVals[3].Value.ul))
        {
            hr = pSession->OpenAddressBook(ulUIParam, NULL, 0, &pAdrBook);
            AssertSz(pAdrBook,"pAdrBook not available ... ");
            if (SUCCEEDED(hr))
            {
                hr = pAdrBook->ResolveName(0,0,al.aEntries[0].rgPropVals[itaga].Value.LPSZ,&al);
                AssertSz(SUCCEEDED(hr),"Failed to Resolve Name:: Please use the address book to select the recipient of this message");
                pAdrBook->Release();
            }
        }
    }
    ...
    // Set PR_SENT_REPRESENTING_XXX to PR_RCVD_REPRESENTING_XXX if present //$BUG
    // pNewMessage SetProps
    ...
    pNewFormIPersist->
    #endif
    Load(pNewMessageSite,pNewMessage,0,MSGFLAG_UNSENT); // $REVIEW parameters?
    if (FAILED(hr))
        {
    ...
    MAPIFreeBuffer(al.aEntries[0].rgPropVals);

    // ----- Close down the read form (that's me)
    #ifdef ALLOW_SUBCLASS_IPM
    hr = Close(OLECLOSE_NOSAVE);
    if (FAILED(hr))
        {
        TraceTag(tagForm,"failure Closeing read form 0x%08lx",hr);
        return hr;
        }
    #endif // ALLOW_SUBCLASS_IPM
...
FRM::AddressForm(HWND hwnd, BOOL fDontShowIfRecipsExist)
{
    ...
    if (hwnd != hMainWnd)
    {
        TraceTag(tagNull,"ADDRESSFORM: pAdrBook->Address changed it's out parameter even though DIALOG_MODAL"); //$REVIEW
    }
    #endif
    if (FAILED(hr))
    {
    ...
    //$BUG this doesn't seem to always work (ie- centries is bogus sometimes)
...
cleanuppal:
    Assert(pal);
    MAPIFreeBuffer( (LPSRowSet) pal);
    if (fCloseForm) Close(OLECLOSE_PROMPTSAVE);
...
FRM::Release(void)
{
    ...
    if (!(--cRef))
        {
        // ----- be for damn sure our ui is gone when we leave
        #ifdef DEBUG
        if (IsWindow(hMainWnd))
            {
            TraceTag(tagForm,"Last Release called, but IsWindow(hMainWnd).  Close called?");
            }
...
FRM::Save(LPMESSAGE pmsg, ULONG fSameAsLoad)
{
    ...
    //$BUG: when we fix save, we can remove this ...
    // ----- set our message class
    prop.ulPropTag  = PR_MESSAGE_CLASS;
    prop.Value.LPSZ = TEXT("IPM.Checkers");
    hr = pMessage->SetProps(1, &prop, NULL);
...
FRM::SaveCompleted(LPMESSAGE pmsg)
{
    ...
    AssertSz(pmsg,"pmsg in savecompleted missing");
    AssertSz(cRef > 0,"excuse me.  are you refering to me?");

    //$BUG: we are supposed to replace our pMessage with pmsg
 ...
 */
STDMETHODIMP
FRM::Close(DWORD dwSaveOptions)
{
    HRESULT hr = NOERROR;

    TraceTag(tagFormFunc,"FRM::Close dwSaveOptions=%d",dwSaveOptions);
    Assert(pMessageSite);
    AssertSz(cRef > 0,"excuse me.  are you refering to me?");
...
FRM::Unadvise(ULONG ulConnection)
{
    ...
    AssertSz(afAdvisee[ulConnection],"never wanted ::Advise in ::Unadvise?");

    // ----- forget about advising this guy
    afAdvisee[ulConnection] = 0;
    aAdvisePtrs[ulConnection]->Release();
    return NOERROR;
}
...
FRM::FRM(REFCLSID clsid)
{
    ...
    TraceTag(tagFormFunc,"FRM::FRM ................. Happy Happy Joy Joy");

    cRef = 1;
    this->clsid = clsid;

    //$REVIEW: need to use a memset or something instead
