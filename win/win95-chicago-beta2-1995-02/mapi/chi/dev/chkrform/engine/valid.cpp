...
int GameOver(BOARD b, int t)
{
    ...
    if (move_is_possible)
    {
        int i=rConfig.iMaxMoves; //REVIEW: max moves 200? Nope, rConfig.iMaxMoves
        int t=0;
        SQUARE b[SQRS_MAX];

        if (CMoves.GetFirstBoard(b,&t)) // REVIEW: this dependancy on movelist should not exist because it is not part of the engine
