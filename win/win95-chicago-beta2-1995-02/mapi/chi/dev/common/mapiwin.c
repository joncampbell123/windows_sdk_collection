    ...
    LPSTR lpTempFileName)
{
    ...
    AssertSz( lpTempFileName && IsBadWritePtr( lpTempFileName, MAX_PATH ),
...
MulDiv32(long nMultiplicand, long nMultiplier, long nDivisor)
{
    ...
    lProd = nMultiplicand * nMultiplier;    /* no overflow protection */

    if (!lProd)
        return 0L;

    lQuot = lProd / nDivisor;
    lRem = lProd % nDivisor;

    if (ABS(lRem) >= ABS(nDivisor >> 1))
        lQuot += (lQuot > 0 ? 1 : -1);
