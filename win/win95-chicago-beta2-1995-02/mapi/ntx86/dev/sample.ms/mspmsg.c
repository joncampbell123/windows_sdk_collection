...
#define MSG_ValidateParameters(pobj, eMethod)       \
        OBJ_ValidateParameters(pobj, eMethod, sizeof(IMSG), &vtblIMSG)
 ...
 *
 *  Errors:
 *      Stream writing/committing errors (MAPI_E_WRITE_FAULT,
 *      etc.).
 */
STDMETHODIMP IMSG_SaveChanges(PIMSG pimsg, ULONG ulFlags)
{
    ...
    MSG_ValidateParameters(pimsg, IMAPIProp_SaveChanges);
    ...
    ULONG *pcval, LPSPropValue *ppval)
{
    ...
    MSG_ValidateParameters(pimsg, IMAPIProp_GetProps);
...
STDMETHODIMP IMSG_GetPropList(PIMSG pimsg, ULONG ulFlags, LPSPropTagArray *pptaga)
{
    ...
    MSG_ValidateParameters(pimsg, IMAPIProp_GetPropList);
 ...
 *
 *  Errors:
 *      MAPI_E_OPEN_CALL_FAILED     An error occurred opening a
 *                              supported interface.
    ...
    ULONG ulInterfaceOptions, ULONG ulFlags, LPUNKNOWN *lppUnk)
{
    ...
    MSG_ValidateParameters(pimsg, IMAPIProp_OpenProperty);
    ...
    LPSPropProblemArray *lppProblems)
{
    ...
    MSG_ValidateParameters(pimsg, IMAPIProp_SetProps);
    ...
    LPSPropProblemArray *ppErr)
{
    ...
    MSG_ValidateParameters(pimsg, IMAPIProp_DeleteProps);
    ...
    LPSPropProblemArray *ppErr)
{
    ...
    MSG_ValidateParameters(pimsg, IMAPIProp_CopyTo);
    ...
    LPSPropProblemArray *ppErr)
{
    ...
    MSG_ValidateParameters(pimsg, IMAPIProp_CopyProps);
...
IMSG_GetAttachmentTable(PIMSG pimsg, ULONG ulFlags, LPMAPITABLE *lppTable)
{
    ...
    MSG_ValidateParameters(pimsg, IMessage_GetAttachmentTable);
    ...
    ULONG ulFlags, LPATTACH *lppAttach)
{
    ...
    MSG_ValidateParameters(pimsg, IMessage_OpenAttach);
    ...
    ULONG *lpulAttachmentNum, LPATTACH *lppAttach)
{
    ...
    MSG_ValidateParameters(pimsg, IMessage_CreateAttach);
    ...
    ULONG ulUIParam, LPMAPIPROGRESS lpProgress, ULONG ulFlags)
{
    ...
    MSG_ValidateParameters(pimsg, IMessage_DeleteAttach);
    ...
    LPMAPITABLE *lppTable)
{
    ...
    MSG_ValidateParameters(pimsg, IMessage_GetRecipientTable);
...
STDMETHODIMP IMSG_ModifyRecipients(PIMSG pimsg, ULONG ulFlags, LPADRLIST lpMods)
{
    ...
    MSG_ValidateParameters(pimsg, IMessage_ModifyRecipients);
...
#define NUM_RECIP_COLS  2
    ...
    MSG_ValidateParameters(pimsg, IMessage_SubmitMessage);
    ...
    while (TRUE)
    {
        ...
        while (prw < prwMac)
        {
            ...
            AssertSz(!IsBadReadPtr(pval, (UINT) prws->aRow->cValues
                * sizeof(SPropValue)), "Bad pval array");
    
            /* PR_ROWID is in column zero. Leave it alone, and start with */
            /* the next column. */
            ++pval;
    
            /* We don't ever want responsibility for any recipient. */
            if (PROP_TYPE(pval->ulPropTag) == PT_NULL)
            {
                pval->ulPropTag = PR_RESPONSIBILITY;
                pval->Value.b = FALSE;
            }
    ...
    /* If SubmitMessage has the FORCE_SUBMIT flag set, then the provider */
    /* should clear the CHECK_SENDER flag, so that PrepareSubmit won't */
    /* check for additional queuing time. */

    ulPrepareFlags = (ulFlags & FORCE_SUBMIT) ? 0L : CHECK_SENDER;

    /* Call this first. It will handle a resubmitted message properly. */
    /* Follow the PrepareSubmit call immediately by a call to */
    /* ExpandRecips. If PrepareSubmit returns NON_STANDARD, then return */
    /* MAPI_E_NON_STANDARD to the caller. */
    ...
    if (ulPrepareFlags & NON_STANDARD)
    {
        hr = ResultFromScode(MAPI_E_NON_STANDARD);
        goto exit;
    }

    /* ExpandRecips checks for no recipients, and will return an error. */
    /* Therefore, this code doesn't need to check itself for that case. */
    ...
    /* for any recipients that it wishes to handle. (For example, if it */
    /* is tightly coupled to a transport.) */

    /* Get the current time to add to the message as PR_CLIENT_SUBMIT_TIME */
    GetSystemTime(&st);
...
STDMETHODIMP IMSG_SetReadFlag(PIMSG pimsg, ULONG ulFlags)
{
    ...
    MSG_ValidateParameters(pimsg, IMessage_SetReadFlag);
 ...
 *                                  the IMSG instance.
 */
HRESULT NewIMSGInIATCH(LPMESSAGE lpmsg, POBJ pobj, PIMSG *ppimsg)
{
    PIMSG pimsgNew = NULL;
    SCODE sc = S_OK;
    PIMS pims;
