...
typedef struct tagUPDATELINKS
{
   ...
   LPOLESTR                lpszTitle;          // caption for dialog box
                      ...
                      HMENU FAR *lphMenu)
{
   ...
   OLEVERB             oleverb;
   LPUNKNOWN           lpUnk;
   LPOLESTR            lpszShortTypeName = lpszShortType;
   LPOLESTR            lpszVerbName = NULL;
   HRESULT             hrErr;
   BOOL                fStatus;
   BOOL                fIsLink = FALSE;
   BOOL                fResult = TRUE;
   ...
   *lphMenu=NULL;
   ...
   if (! lpszShortTypeName) {
      ...
      hrErr = lpOleObj->lpVtbl->GetUserType(
            lpOleObj,
            USERCLASSTYPE_SHORT,
            (LPOLESTR FAR*)&lpszShortTypeName
      );
      OLEDBG_END2
   ...
   while (lpEnumOleVerb != NULL) {         // forever
      ...
      */
      ...
      if ( 0 == uIDVerbMax ||
         ...
         W2A (lpszVerbName, szAnsiStr, 256);
         fStatus = InsertMenu(
               *lphMenu,
               (UINT)-1,
   ...
   if (fAddConvertItem) {
      ...
      if (0 == cVerbs) {
         ...
         lpszVerbName = OleStdMalloc(sizeof(szConvert));
         lpsz = szAnsiStr;
         ...
         A2W (szAnsiStr, lpszVerbName, OLEUI_CCHVERBMAX);
      }
   ...
   if (cVerbs == 0) {
      ...
      W2A (lpszShortTypeName, szAnsiStr, 256);
      wsprintf(
         szBuffer,
         (fIsLink ? (LPSTR)szLinkCmdNVerb:(LPSTR)szObjectCmdNVerb),
         (szAnsiStr ? szAnsiStr : (LPSTR)"")
      );
      uFlags |= MF_GRAYED;
      ...
      *lphMenu = NULL;
      ...
      LPSTR       lpsz = (fIsLink ? szLinkCmd1Verb : szObjectCmd1Verb);
      char        szAnsiBuf[256];
      LPSTR       lpsz1 = szAnsiBuf;  //lpszVerbName;

      W2A (lpszVerbName, szAnsiBuf, 256);
      ...
      if (lpsz1 && !fAddConvertItem) {
            DWORD       cch=OLESTRLEN(lpszVerbName)+1;
            LPOLESTR lpszVerbName2 = OleStdMalloc(cch*sizeof(OLECHAR));
            char  szAnsiBuf2[256];
            LPSTR lpsz2 = szAnsiBuf2; //lpszVerbName2;
            ...
            if (lpsz2) {
                ...
                *lpsz2 = '\0';
                A2W (szAnsiBuf2, lpszVerbName2, cch);
                OleStdFreeString(lpszVerbName, NULL);
      ...
      if (*lpsz == '0') {
         W2A (lpszShortTypeName, szAnsiStr, 256);
         W2A (lpszVerbName, szAnsiBuf, 256);
         wsprintf(szBuffer, lpsz+1, szAnsiBuf,
            (szAnsiStr ? szAnsiStr : (LPSTR)"")
         );
      }
      else {
      ...
      case WM_INITDIALOG:
      {
         ...
         if (lplpUL) {
            *lplpUL = (LPUPDATELINKS)lParam;
            W2A ((*lplpUL)->lpszTitle, szAnsiStr, 256);
 ...
 *      FALSE                   otherwise
 */
STDAPI_(BOOL) OleUIUpdateLinks(LPOLEUILINKCONTAINER lpOleUILinkCntr, HWND hwndParent, LPOLESTR lpszTitle, int cLinks)
{
   LPUPDATELINKS lpUL = (LPUPDATELINKS)OleStdMalloc(sizeof(UPDATELINKS));
   BOOL          fError;
   char          lpszAnsiTitle[256];

   W2A (lpszTitle, lpszAnsiTitle, 256);
   OleDbgAssert(lpOleUILinkCntr && hwndParent && lpszAnsiTitle && (cLinks>0));
   OleDbgAssert(lpUL);
