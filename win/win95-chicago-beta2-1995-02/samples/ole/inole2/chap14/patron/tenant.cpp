...
BOOL CTenant::FLoad(LPSTORAGE pIStorage, PTENANTINFO pti)
    ...
    if (m_fInitialized)
        ...
        //This should release all holds on oure IStorage as well.
        if (NULL!=m_pIViewObject2)
            {
...
void CTenant::Repaint(void)
     ...
     * The check om m_fRepaintEnable prevents multiple repaints
     * during conversion/emulation.
     */
    //CHAPTER14MOD
...
void CTenant::ObjectClassFormatAndIcon(LPCLSID pClsID
...
#if defined(WIN32) && !defined(UNICODE)
    LPOLESTR pwcsType;
    hr=ReadFmtUserTypeStg(m_pIStorage, pwFormat, &pwcsType);
    if (SUCCEEDED(hr))
    {
            *ppszType=(TCHAR *)OleStdMalloc(wcslen(pwcsType) + 1);
            wcstombs(*ppszType, szType, wcslen(pwcsType) + 1);
            OleStdFree(pwcsType);
    }
#else
    ...
    if (FAILED(hr))
        ...
        *ppszType=NULL;
        ...
        if (OleStdGetUserTypeOfClass(*pClsID, szType, sizeof(szType) / sizeof(OLECHAR)
            , NULL))
#if defined(WIN32) && !defined(UNICODE)
            *ppszType=(TCHAR *)OleStdMalloc(wcslen(szType) + 1);
            wcstombs(*ppszType, szType, wcslen(szType) + 1);
#else
