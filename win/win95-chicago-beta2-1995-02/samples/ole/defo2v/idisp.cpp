...
void IDispDlg::DoGetInfo()
{
    ...
    if (m_pDispatch != NULL)
    {
        ...
        if (FAILED( hr ))
        {
            wsprintf( szTemp, _T("IDispatch::GetTypeInfoCount( ) FAILED: %s"), (LPTSTR)HRtoString( hr ) ) ;
        ...
        if (FAILED( hr ))
        {
            wsprintf( szTemp, _T("IDispatch::GetTypeInfo( 0 ) FAILED: %s"), (LPTSTR)HRtoString( hr ) ) ;
            MessageBox( m_hDlg, szTemp, _T("Could not get type information"), MB_OK ) ;
            return ;
        }
    ...
    for (UINT n = 1 ; n <= uiTypeInfoCount ; n++)
    {
        ...
        if (SUCCEEDED(hr))
        {
...
#else
                wsprintf( szTemp, _T("%s (%u)"), bstrName ? bstrName : _T("(null name)"), n-1 ) ;
#endif
            ...
            if (SUCCEEDED( hr ))
            {
                UINT ui ;
#if defined(WIN32) && !defined(OLE2ANSI)
                wsprintf( szTemp, _T("%ws (%u)"), (LPOLESTR)rgbstrNames[0], n-1 ) ;
#else
...
void IDispDlg::OnSelChangeTypeInfo()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {
#if defined(WIN32) && !defined(OLE2ANSI)
            wsprintf( szTemp, _T("Name = %ws"), (LPOLESTR)(bstrName == NULL ? OLESTR("n/a") : bstrName) ) ;
#else
            wsprintf( szTemp, _T("Name = %s"), (LPTSTR)(bstrName == NULL ? _T("n/a") : bstrName) ) ;
#endif
...
#else
            wsprintf( szTemp, _T("DocString = %s"), (bstrDoc == NULL ? _T("n/a") : bstrDoc) ) ;
#endif
            ListBox_AddString( m_hwndTypeAttr, szTemp ) ;
            ...
            ListBox_AddString( m_hwndTypeAttr, szTemp ) ;
#if defined(WIN32) && !defined(OLE2ANSI)
            wsprintf( szTemp, _T("Help File = %ws"), (LPOLESTR)(bstrHelpFile == NULL ? OLESTR("n/a") : bstrHelpFile) ) ;
#else
            wsprintf( szTemp, _T("Help File = %s"), (LPTSTR)(bstrHelpFile == NULL ? _T("n/a") : bstrHelpFile) ) ;
#endif
            ListBox_AddString( m_hwndTypeAttr, szTemp ) ;
            SysFreeString( bstrName ) ;
    ...
    for ( iIndex = 0 ; iIndex < cFuncs ; iIndex++)
    {
        ...
        if (SUCCEEDED( hr ))
        {
            ...
            if (rgbstrNames[0])
            {
                ...
                if (szName)
                {
                    wcstombs(szName, rgbstrNames[0], wcslen(rgbstrNames[0]) + 1);
                }
            }
...
//    lstrcpy( szFuncName, _T("<???>") ) ;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED( hr ))
        {
            ...
            {
#if defined(WIN32) && !defined(UNICODE)
                wcstombs(szFuncName, rgbstrNames[0], wcslen(rgbstrNames[0]) + 1);
#else
            ...
            for ( UINT n = 0 ; n < max(cNames-1,cParams) ; n++ )
            {
                ...
                if (n+1 < cNames)   // saftey check
                {
                    if (n < cParams)
                        lstrcat( szfn, _T(" ") ) ;
#if defined(WIN32) && !defined(UNICODE)
                    wcstombs(&szfn[strlen(szfn) + 1], rgbstrNames[n+1], wcslen(rgbstrNames[n+1]) + 1);
#else
...
//            ListBox_AddString( m_hwndInfo, bstrName ) ;    stevebl - this lookes like it was a bug
           ...
           if (SUCCEEDED(hr))
           {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    if (bstrName)
                    {
                        ...
                        if (szName)
                        {
                            wcstombs(szName, bstrName, wcslen(bstrName) + 1);
                        }
                    }
