...
void initNumSurface( void )
{
    ...
    if (bFullscreen)
        len = wsprintf(szInfo, szINFO,  GameSize.cx, GameSize.cy, GameBPP,bStretch ? " x2" : "");
    else
        len = wsprintf(szInfo, szINFOW, GameSize.cx, GameSize.cy, GameBPP,bStretch ? " x2" : "");
...
long FAR PASCAL MainWndProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    case WM_ACTIVATEAPP:
        bIsActive = (BOOL)wParam && GetForegroundWindow() == hWnd && !IsIconic(hWnd);
...
void __cdecl Msg( LPSTR fmt, ... )
{
    ...
    wvsprintf( &buff[lstrlen(buff)], fmt, (LPVOID)(&fmt+1) );
    lstrcat( buff, "\r\n" );
