    ...
    switch( uMsg )
        ...
        case WM_DSSTREAM_PROGRESS:
            if( wiWave.dwProgress > wiWave.mmckInRIFF.cksize )
                wiWave.dwProgress -= wiWave.mmckInRIFF.cksize;
            fPercent = (float)((wiWave.dwProgress * 100)
...
int StreamBufferSetup( void )
...
#ifdef DEBUG
    wsprintf( szDebug, "First BufferSize = %lu", wiWave.dwBufferSize );
    ListBox_AddString( hWndList, szDebug );
#endif

    wiWave.dwBufferSegSize = wiWave.dwBufferSize / NUM_BUFFER_SEGMENTS;

#ifdef DEBUG
    wsprintf( szDebug, "BufferSegSize = %lu", wiWave.dwBufferSegSize );
    ListBox_AddString( hWndList, szDebug );
#endif

    wiWave.dwBufferSize = wiWave.dwBufferSegSize * NUM_BUFFER_SEGMENTS;

#ifdef DEBUG
    wsprintf( szDebug, "Final BufferSize = %lu", wiWave.dwBufferSize );
    ListBox_AddString( hWndList, szDebug );
#endif
    /*
     ...
     */
                ...
                wiWave.nRemainingSegments = (int)(uChkErr /
                                                    wiWave.dwBufferSegSize);
                DPF( 3,"Setting bFoundEnd in load, nRemSegs = %i, dwBuffSegSize = %u",
                        wiWave.nRemainingSegments, wiWave.dwBufferSegSize );
    ...
    dsRetVal = lpDSBStreamBuffer->lpVtbl->Unlock( lpDSBStreamBuffer,
                                        (LPVOID)lpWrite1, dwLen1,
                                        (LPVOID)lpWrite2, 0 );
    wiWave.dwNextWriteOffset = wiWave.dwPlayLast = wiWave.dwProgress = 0;
...
void ResetWavePlayer( void )
     ...
     */
                ...
                {
                wiWave.bFoundEnd = TRUE;
                wiWave.nRemainingSegments = (int)(uChkErr / wiWave.dwBufferSegSize);
                DPF( 3,"Setting bFoundEnd in load, nRemSegs = %i, dwBuffSegSize = %u",
                        wiWave.nRemainingSegments, wiWave.dwBufferSegSize );
