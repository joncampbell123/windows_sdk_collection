...
//      lpDstPalette->lpVtbl->Release(lpDstPalette);
    ...
    if (!(ddsd.ddsCaps.dwCaps & DDSCAPS_VIDEOMEMORY))
        d3dappi.bTexturesInVideo = FALSE;
...
{
    if (d3dappi.ThisDriver.bIsHardware && !d3dappi.bTexturesInVideo) {
        D3DAppISetErrorString("Could not put all the textures in video memory for this hardware device.\n");
        goto exit_with_error;
    }
...
D3DAppILoadAllTextures(void)
{
    ...
    if (d3dappi.ThisDriver.bDoesTextures) {
        d3dappi.bTexturesInVideo = TRUE;
        for (i = 0; i < d3dappi.NumTextures; i++) {
            ATTEMPT(D3DAppILoadTextureSurf(i));
