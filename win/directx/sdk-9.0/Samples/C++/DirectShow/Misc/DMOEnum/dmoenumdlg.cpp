...
void CDMOEnumDlg::GetTypeSubtypeString(TCHAR *szCLSID, DMO_PARTIAL_MEDIATYPE& aList)
{
    ...
    wsprintf(szCLSID, TEXT("%s\t%s\0"), (LPCTSTR) strType, (LPCTSTR) strSubtype);
}
...
void CDMOEnumDlg::AddTypeInfo(const GUID *pCLSID)
{
    ...
    TCHAR szNum[10];
    wsprintf(szNum, TEXT("%d\0"), ulNumInputsSupplied);
    m_nInputTypes.SetWindowText(szNum);
    wsprintf(szNum, TEXT("%d\0"), ulNumOutputsSupplied);
...
void CDMOEnumDlg::GetGUIDString(TCHAR *szString, GUID *pGUID, int nStringSize)
{
    ...
    // If we got here, there was no match
    _tcscpy(szString, TEXT("GUID_NULL\0"));
}
