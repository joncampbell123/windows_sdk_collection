...
HRESULT CDemonstration::Perform()
{
    ...
    try
    {
        ...
        if( FAILED(hr) )
        {
            _stprintf( m_szMsg, _T("Failed to run VMR, method returned %s\n"), 
...
HRESULT CDemonstration::UpdateStreams(clock_t tStart)
{
    ...
    for( i=0; i< this->m_MList.Size(); i++)
    {
        ...
        if( !SUCCEEDED(hr))
        {
            _stprintf( m_szMsg, _T("Failure in CDemonstration::UpdateStreams, GetMixerControl()->SetAlpha, method returned %s\n"),
                     hresultNameLookup(hr));
            DbgOutString(m_szMsg);
        }
        ...
        if( !SUCCEEDED(hr))
        {
            _stprintf( m_szMsg, _T("Failure in CDemonstration::UpdateStreams, GetMixerControl()->SetOutputRect, method returned %s\n"),
                     hresultNameLookup(hr));
            DbgOutString(m_szMsg);
        }
        ...
        if( !SUCCEEDED(hr))
        {
            _stprintf( m_szMsg, _T("Failure in CDemonstration::UpdateStreams, GetMixerControl()->GetOutputRect, method returned %s\n"),
                     hresultNameLookup(hr));
            DbgOutString(m_szMsg);
        }
        else
        {
            ...
            {
                _stprintf( m_szMsg, _T("Failed invalidation of SetOutputRect(): required [l,t,r,b] = [%f,%f,%f,%f] and returned [%f,%f,%f,%f]\n"),
