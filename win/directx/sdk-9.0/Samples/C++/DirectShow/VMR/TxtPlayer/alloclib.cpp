...
{
    CheckPointer(pDDrawSurface,E_POINTER);
    ...
    if (hr == DD_OK)
    {
        ...
        CheckPointer(pDst,E_UNEXPECTED);
...
{
    CheckPointer(pdds,E_POINTER);

    AMTRACE((TEXT("YUV16PaintSurfaceBlack")));
    HRESULT hr = NOERROR;
    DDSURFACEDESC2 ddsd;

    // Lock the surface so we can start filling the surface with black
    ddsd.dwSize = sizeof(ddsd);
