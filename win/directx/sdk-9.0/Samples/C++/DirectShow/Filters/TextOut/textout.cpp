...
// termimated text. The AVI parser reads these and sends them packaged up
// as DirectShow media samples. We get the buffer from these and cast
...
// each time the filter is paused. Into this window we will ExtTextOut the
...
void CTextOutFilter::DrawText(IMediaSample *pMediaSample)
{
    ...
    const int nBufSize=129; // 128 chars plus terminating NULL
    ...
    TCHAR strNumber[nBufSize];
    _tcsncpy(strNumber, A2T((char *) pText), nBufSize-1);

    strNumber[nBufSize-1] = 0;  // Ensure NULL-termination of string
