...
BOOL CPATProcessor::MapPmtPid(DWORD dwPmtPid)
{
    ...
    RETURN_FALSE_IF_FAILED(TEXT(" CPATProcessor::MapPmtPid():: QI the IMPEG2PIDMap inf on PSI out pin failed"), hr);
...
BOOL CPATProcessor::UnmapPmtPid()
{
    ...
    IMPEG2PIDMap * pIPmtPIDMap;
    HRESULT hr = m_pDemuxPsiOutputPin->QueryInterface(IID_IMPEG2PIDMap, (void **) &pIPmtPIDMap);
    RETURN_FALSE_IF_FAILED(TEXT(" CPATProcessor::SetupMPEGDeMux():: QI the IMPEG2PIDMap inf on PSI out pin failed"), hr);
