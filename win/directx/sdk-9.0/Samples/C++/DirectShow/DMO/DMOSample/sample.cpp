...
HRESULT CSample::InternalGetInputMaxLatency(DWORD dwInputStreamIndex, 
                                            REFERENCE_TIME *prtMaxLatency)
{
    return E_NOTIMPL;
}

HRESULT CSample::InternalSetInputMaxLatency(DWORD dwInputStreamIndex, 
                                       ...
                                       DWORD *pdwStatus)
{
    ...
    HRESULT hr = pOutputBuffers[0].pBuffer->GetBufferAndLength(&pbData, &cbCurrent);
    if (FAILED(hr)) {
        return hr;
    }
    ...
    TCHAR szBuffer[20];
    wsprintf(szBuffer, TEXT("%2.2d:%2.2d:%2.2d:%2.2d\0"),
             (dwTimeCode >> 19) & 0x1F,
             (dwTimeCode >> 13) & 0x3F,
             (dwTimeCode >> 6) & 0x3F,
              dwTimeCode & 0x3F);
