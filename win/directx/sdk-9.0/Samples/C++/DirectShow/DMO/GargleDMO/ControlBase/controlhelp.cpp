        ...
        bool        fDiscrete)
{
    ...
    // Lots of ticks become less useful as guides.  Use quarters for fine-grained sliders.
    short sTicks = 4; 
...
LRESULT CSliderValue::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    {
        case WM_HSCROLL:
            if (reinterpret_cast<HWND>(lParam) == m_hwndSlider && 
                LOWORD(wParam) >= TB_LINEUP && LOWORD(wParam) <= TB_ENDTRACK)
            {
        ...
        case WM_COMMAND:
            if (HIWORD(wParam) == EN_KILLFOCUS && 
                reinterpret_cast<HWND>(lParam) == m_hwndEdit)
            {
                UpdateSlider();
                bHandled = TRUE;
            }
...
void CSliderValue::UpdateEditBox(float fPos)
{
    ...
    if (m_fDiscrete)
    {
        ...
        wsprintf(szText, TEXT("%hd\0"), sPos);
    }
    else
    {
        wsprintf(szText, TEXT("%.3hf\0"), fPos);
    }
...
LRESULT CRadioChoice::MessageHandler(UINT uMsg, WPARAM wParam, 
...
LRESULT MessageHandlerChain(Handler **ppHandlers, UINT uMsg, 
