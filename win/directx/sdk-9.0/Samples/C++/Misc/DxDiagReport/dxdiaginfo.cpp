...
HRESULT CDxDiagInfo::GetSystemInfo( SysInfo** ppSysInfo )
{
    ...
    *ppSysInfo = pSysInfo;
    ...
    for( i=0; i<16; i++ )
    {
        WCHAR strName[256];
        WCHAR strName2[256];
        swprintf( strName, L"ExtendedCPUFunctionBitmasks_0x800000%0.2x_bits", i+0x09 );

        wcscpy( strName2, strName ); wcscat( strName2, L"0_31" );
        if( FAILED( hr = GetUIntValue( pObject, strName2, &pSysInfo->m_ExtFuncBitmasks[i].dwBits0_31 ) ) )
            goto LCleanup; nCurCount++;
        wcscpy( strName2, strName ); wcscat( strName2, L"32_63" );
        if( FAILED( hr = GetUIntValue( pObject, strName2, &pSysInfo->m_ExtFuncBitmasks[i].dwBits32_63 ) ) )
            goto LCleanup; nCurCount++;
        wcscpy( strName2, strName ); wcscat( strName2, L"64_95" );
        if( FAILED( hr = GetUIntValue( pObject, strName2, &pSysInfo->m_ExtFuncBitmasks[i].dwBits64_95 ) ) )
            goto LCleanup; nCurCount++;
        wcscpy( strName2, strName ); wcscat( strName2, L"96_127" );
