...
Namespace Tut08_Voice
    ...
    Public Class VoiceApp
        ...
        ' generate a new guid.
        Private Shared m_AppGuid As New Guid("91F570F2-CE88-428a-8025-25A1E4918B44")
        Public Shared DefaultPort As Integer = 2608 ' Default port number
        ...
        ' DirectPlay Voice
        Private m_VoiceClient As Voice.Client = Nothing ' DirectPlay Voice client
        Private m_VoiceServer As Voice.Server = Nothing ' DirectPlay Voice server
        ' Application 
        ...
        Private m_NumPlayersTalking As Integer = 0 ' Number of talking players
        ...
        '/ </summary>
        Private Sub InitDirectPlayVoice()
            ' Release any existing resources
            If Not (m_VoiceClient Is Nothing) Then
                m_VoiceClient.Dispose()
                m_VoiceClient = Nothing
            End If

            If Not (m_VoiceServer Is Nothing) Then
                m_VoiceServer.Dispose()
                m_VoiceServer = Nothing
            End If

            ' Initialize UI variables
            Me.m_NumPlayersTalking = 0
            ...
            Select Case Connection
                Case ConnectionType.Hosting
                    ...
                    Dim desc As New Voice.SessionDescription()
                    ...
                    GoTo CaseConnectionTypeDotConnected

                Case ConnectionType.Connected
CaseConnectionTypeDotConnected:
                    ' Test Direct Voice
                    If TestDirectVoice() = False Then
                        Return
                    End If
                    ...
                    ' Add event handlers
                    AddHandler m_VoiceClient.PlayerStarted, AddressOf PlayerStartedHandler
                    AddHandler m_VoiceClient.PlayerStopped, AddressOf PlayerStoppedHandler
                    AddHandler m_VoiceClient.RecordStarted, AddressOf RecordStartedHandler
                    AddHandler m_VoiceClient.RecordStopped, AddressOf RecordStoppedHandler
                    ...
                    Dim clientConfig As New Voice.ClientConfig()
                    clientConfig.Flags = Voice.ClientConfigFlags.AutoVoiceActivated Or Voice.ClientConfigFlags.AutoRecordVolume
                    clientConfig.RecordVolume = CInt(Voice.RecordVolume.Last)
                    clientConfig.PlaybackVolume = CInt(Voice.PlaybackVolume.Default)
                    ...
                    Dim targets(0) As Integer
                    targets(0) = Voice.PlayerId.AllPlayers
                    m_VoiceClient.TransmitTargets = targets

                Case ConnectionType.Disconnected
                    Return
            End Select
        End Sub 'InitDirectPlayVoice
        ...
        Private Function TestDirectVoice() As Boolean
            ' Create a new Test object for DirectPlay Voice
            Dim VoiceTest As New Voice.Test()

            Try
                ' Check the current audio configuration
                VoiceTest.CheckAudioSetup(DSoundHelper.DefaultVoicePlaybackDevice, DSoundHelper.DefaultCaptureDevice, Voice.TestFlags.QueryOnly) ' Guid of the default playback hardware
                ' Guid of the default capture hardware
                ' Read-only flag
            Catch dex As DirectXException
                If dex.ErrorCode = CInt(Voice.ResultCode.RunSetup) Then
                    Try
                        ' Test setup again, this time allowing user configuration
                        VoiceTest.CheckAudioSetup(DSoundHelper.DefaultVoicePlaybackDevice, DSoundHelper.DefaultCaptureDevice, m_Form, Voice.TestFlags.AllowBack) ' Guid of the default playback hardware
                        ' Guid of the default capture hardware
                        ' Current window handle for UI display
                        ' Enable back button flag
                    Catch ex As Exception
            ...
            Return True
        End Function 'TestDirectVoice

        '/ <summary>
        '/ Event handler for the DirectPlay Voice RecordStarted message
        '/ </summary>
        Public Sub RecordStartedHandler(ByVal sender As Object, ByVal args As Voice.RecordStartedEventArgs)
            ' Show active record icon
            m_Form.BeginInvoke(New AsyncCall(AddressOf m_Form.ActiveRecordIcon.Show))
        End Sub 'RecordStartedHandler

        '/ <summary>
        '/ Event handler for the DirectPlay Voice RecordStoppedmessage
        '/ </summary>
        Public Sub RecordStoppedHandler(ByVal sender As Object, ByVal args As Voice.RecordStoppedEventArgs)
            ' Hide active record icon
            m_Form.BeginInvoke(New AsyncCall(AddressOf m_Form.ActiveRecordIcon.Hide))
        End Sub 'RecordStoppedHandler


        '/ <summary>
        '/ Event handler for the DirectPlay Voice PlayerStarted message
        '/ </summary>
        Public Sub PlayerStartedHandler(ByVal sender As Object, ByVal args As Voice.PlayerStartedEventArgs)
            m_NumPlayersTalking += 1

            ' Show active playback icon
            m_Form.BeginInvoke(New AsyncCall(AddressOf m_Form.ActivePlayIcon.Show))
        End Sub 'PlayerStartedHandler


        '/ <summary>
        '/ Event handler for the DirectPlay Voice PlayerStopped message
        '/ </summary>
        Public Sub PlayerStoppedHandler(ByVal sender As Object, ByVal args As Voice.PlayerStoppedEventArgs)
            m_NumPlayersTalking -= 1

            ' If no players are talking, hide active playback icon
            If m_NumPlayersTalking = 0 Then
                m_Form.BeginInvoke(New AsyncCall(AddressOf m_Form.ActivePlayIcon.Hide))
            End If
        End Sub 'PlayerStoppedHandler
        ...
        Public Sub UpdateUI()
            ...
            Select Case m_Connection
                ...
                Case ConnectionType.Disconnected
                    ...
                    If m_Form.IsHandleCreated Then
                        m_Form.BeginInvoke(New AsyncCall(AddressOf m_Form.ActiveRecordIcon.Hide))
                        m_Form.BeginInvoke(New AsyncCall(AddressOf m_Form.ActivePlayIcon.Hide))
                    End If
