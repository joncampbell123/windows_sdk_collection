										...
										LPVOID lpContext)
{
	...
	if (DPPLAYERTYPE_GROUP == dwPlayerType)
	{
		...
		else
		{
			...
			if (ets.bRecursive)
			{
				hr = lp->lpTree->m_lpDP4A->EnumGroupsInGroup(	dpId, NULL, 
															TV_EnumPlayersCallback, 
															&ets, DPENUMPLAYERS_ALL );

				hr = lp->lpTree->m_lpDP4A->EnumGroupPlayers(	dpId, NULL, 
															TV_EnumPlayersCallback, 
															&ets, DPENUMPLAYERS_ALL );
			}
...
CGroupTree::CGroupTree()
{
	...
	// Prepare tree icons
	m_hImageList = ImageList_LoadImage (m_hInst, 
...
///////////////////////////////////////////////////////////////////////////////////////
BOOL CGroupTree::Init( HWND hWnd, LPDIRECTPLAY4A lpDP4A, DPID dpidPlayer )
{
	if ( (hWnd) && (lpDP4A) && (dpidPlayer) )
	{
		m_hwndTreeView = hWnd;
...
HRESULT CGroupTree::GetPlayerName( DPID dpidPlayer, LPDPNAME * lplpn)
{
	...
	if (DPERR_BUFFERTOOSMALL == hr )
	{
		...
		if ( NULL != lpData )
		{
			hr = m_lpDP4A->GetPlayerName( dpidPlayer, lpData, &dwSize );
...
HRESULT CGroupTree::GetGroupName( DPID dpidGroup, LPDPNAME * lplpn)
{
	...
	hr = m_lpDP4A->GetGroupName( dpidGroup, lpData, &dwSize );
...
BOOL CGroupTree::Update(LPVOID lpvMsg)
{
	...
	case DPSYS_CREATEPLAYERORGROUP:
        ...
        case DPSYS_ADDPLAYERTOGROUP:
		{
			...
			DWORD	dwFlags;
			HRESULT hr = IDirectPlayX_GetPlayerFlags( m_lpDP4A, lp->dpIdPlayer, &dwFlags );
        ...
        case DPSYS_ADDGROUPTOGROUP:
		{
			...
			DWORD	dwFlags;
			HRESULT hr = IDirectPlayX_GetGroupFlags( m_lpDP4A, lp->dpIdGroup, &dwFlags );
...
void CGroupTree::GetGroupIcons( DPID dpID, DWORD dwFlags, int * lpnNormal, int * lpnSelected )
{
	if (DPGROUP_STAGINGAREA & dwFlags)
	{
		...
		hr = IDirectPlayX_GetGroupConnectionSettings( m_lpDP4A, 0, dpID, NULL, &dwSize );
		if (DPERR_BUFFERTOOSMALL == hr )
		{
			...
			if (lp)
			{
				hr = IDirectPlayX_GetGroupConnectionSettings( m_lpDP4A, 0, dpID, lp, &dwSize );
...
///////////////////////////////////////////////////////////////////////////////////////
void CGroupTree::SetGroupOwner( DPID dpidGroup, DPID dpidNewOwner, DPID dpidOldOwner )
{
	HRESULT	hr = DPERR_GENERIC;
	DWORD	dwFlags;
	...
	if (dpidOldOwner)
	{
		...
		hr = IDirectPlayX_GetPlayerFlags( m_lpDP4A, dpidOldOwner, &dwFlags );
	...
	hr = IDirectPlayX_GetPlayerFlags( m_lpDP4A, dpidNewOwner, &dwFlags );
