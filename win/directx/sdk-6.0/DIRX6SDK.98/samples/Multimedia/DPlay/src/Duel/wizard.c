...
DWORD WINAPI DoWizard(LPVOID pv)
{
	...
	LoadString(ghinst, IDS_WIZARD_TITLE_GS, tszTitle1, MAX_WINDOWTITLE); 
	LoadString(ghinst, IDS_WIZARD_TITLE_SP, tszTitle2, MAX_WINDOWTITLE); 
    ...
    psp[0].pszIcon = NULL; 
#endif
    psp[0].pfnDlgProc = DlgProcGameSetup;
    ...
    psp[1].pszIcon = NULL; 
#endif
    psp[1].pfnDlgProc = DlgProcChooseProvider;
	...
	if (gbWaitForLobby || IsDPlay())
	{
		// success
		CoUninitialize();
		PostMessage(ghWndMain, UM_LAUNCH, (WPARAM)gbWaitForLobby, 0);
...
#ifdef UNICODE
	iIndex = SendMessage(hWnd, LB_ADDSTRING, 0, (LPARAM) lpName->lpszShortName);
#else
...
BOOL CALLBACK DlgProcChooseProvider(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NOTIFY:
		...
		case PSN_QUERYCANCEL:
			ReleaseSPData();
			DPlayRelease();		// Indicates no game session
			return(TRUE);
		}
		break;
    ...
    case WM_INITDIALOG:
        ...
        IDirectPlayX_EnumConnections( glpDP, glpGuid, DPEnumConnectionsCallback, hWndCtl, 0);
        SetFocus(hWndCtl);
...
BOOL CALLBACK DlgProcGameSetup (HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NOTIFY:
		switch (((NMHDR FAR *) lParam)->code) 
		{
			...
			case PSN_WIZNEXT: 
				if (gbIsHost)
					gbUseProtocol = (SendDlgItemMessage(hDlg, IDC_USEPROTOCOL,
													    BM_GETCHECK, 0, 0)
									 == BST_CHECKED);
				else
					gbUseProtocol = FALSE;
    ...
    case WM_INITDIALOG:
		...
		SendDlgItemMessage(hDlg, IDC_USEPROTOCOL, BM_SETCHECK, gbUseProtocol, 0);
        return(TRUE);
...
BOOL CALLBACK DlgProcJoinSession (HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NOTIFY:
		switch (((NMHDR FAR *) lParam)->code) 
		{
			...
			case PSN_WIZFINISH:
				...
				// open session
				if (FAILED(hr = DPlayOpenSession(lpGuid)))
				{
					// fail finish
				...
				// create player
				if (FAILED(hr = DPlayCreatePlayer(&gOurID, tszPlayerName, NULL, NULL, 0)))
				{
					// fail finish
			...
			case PSN_QUERYCANCEL:
				...
				DPlayEnumSessions(0, EnumSession, (LPVOID)hWndCtl,
	...
	case WM_TIMER:
		...
		// enum sessions and let dplay decide the timeout
		hr = DPlayEnumSessions(0, EnumSession, (LPVOID) hWndCtl,
