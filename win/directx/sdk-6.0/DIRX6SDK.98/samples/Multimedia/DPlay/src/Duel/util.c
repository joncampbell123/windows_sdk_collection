...
#ifdef DEBUG
static const TCHAR DEBUG_MODULE_STR[] = TEXT("DUEL: ");
#endif
...
void	ConvertField(LPBYTE lpByte, LPSTR * ppStr, int iFieldSize, 
					 BOOL fRightToLeft);
/*
 * StringFromGUID
 * 
 ...
 */
int StringFromGUID(GUID* lpguid, LPWSTR lpwsz)
{
    int i;
...
/*
 * dtrace - diagnostic trace to OutputDebugString() (UNICODE supported)
 */
void dtrace(LPTSTR szFormat, ...)
{
	...
	offset = wsprintf((LPTSTR)szBuf, DEBUG_MODULE_STR);
	offset += wvsprintf(szBuf+offset, szFormat, ap);
