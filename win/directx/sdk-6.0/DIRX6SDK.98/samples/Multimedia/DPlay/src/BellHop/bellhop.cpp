...
BOOL CALLBACK LobbyWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
        if(lpDPInfo->bSplitMove)
        {
            ...
            lpDPInfo->xPaneSplit = (x - (lpDPInfo->xSpacing));
            OnSize(hWnd, lpDPInfo);
        }
		else
		{
...
HRESULT ShutdownConnection(LPDPLAYINFO lpDPInfo)
{
	...
	if (lpDPInfo->lpDirectPlay4A)
	{
		...
		{
			IDirectPlayX_DestroyPlayer(lpDPInfo->lpDirectPlay4A, lpDPInfo->dpidPlayer);
			lpDPInfo->dpidPlayer = 0;
		}
		IDirectPlayX_Close(lpDPInfo->lpDirectPlay4A);
		IDirectPlayX_Release(lpDPInfo->lpDirectPlay4A);
		lpDPInfo->lpDirectPlay4A = NULL;

		IDirectPlayLobby_Release(lpDPInfo->lpDirectPlayLobby3A);
		lpDPInfo->lpDirectPlayLobby3A = NULL;
...
HRESULT ReceiveMessage(LPDPLAYINFO lpDPInfo)
{
	...
	do
	{
		...
		do
		{
			...
			hr = IDirectPlayX_Receive(lpDPInfo->lpDirectPlay4A, &idFrom, &idTo, DPRECEIVE_ALL,
						 ...
						 DPID idFrom, DPID idTo)
{
	...
	case DPSYS_DELETEPLAYERFROMGROUP:
        {
			...
			// get pointer to player name
			hr = GetPlayerName(lpDPInfo->lpDirectPlay4A, dpidPlayer, &lpPlayerName);
			if FAILED(hr)
			{
			...
			hr = GetGroupName(lpDPInfo->lpDirectPlay4A, dpidGroup, &lpGroupName);
			if FAILED(hr)
			{
	...
	case DPSYS_DELETEGROUPFROMGROUP:
        {
			...
			// get pointer to player name
			hr = GetGroupName(lpDPInfo->lpDirectPlay4A, dpidParentGroup, &lpParentGroupName);
			if FAILED(hr)
	...
	case DPSYS_SETGROUPOWNER:
		{
			LPDPMSG_SETGROUPOWNER	lp = (LPDPMSG_SETGROUPOWNER) lpMsg;
			LPSTR							lpszGroupName = NULL, 
											lpszNewOwnerName = NULL,
											lpszOldOwnerName = NULL;
			LPDPNAME						lpGroupName = NULL, 
											lpNewOwnerName = NULL,
											lpOldOwnerName = NULL;
			LPSTR							lpszDisplayFormat = "Owner changed for group \"%s\" to \"%s\" from \"%s\"\r\n";
            HRESULT							hr;


			// get pointer to Group name
			hr = GetGroupName(lpDPInfo->lpDirectPlay4A, lp->idGroup, &lpGroupName);
			if FAILED(hr)
				lpGroupName = NULL;

			if ((lpGroupName) && 
				(lpGroupName->lpszShortNameA))
			{
				lpszGroupName = lpGroupName->lpszShortNameA;
			}
			else
			...
			hr = GetPlayerName(lpDPInfo->lpDirectPlay4A, lp->idNewOwner, &lpNewOwnerName);
			if FAILED(hr)
				lpNewOwnerName = NULL;

			if ((lpNewOwnerName) && 
				(lpNewOwnerName->lpszShortNameA))
			{
				lpszNewOwnerName = lpNewOwnerName->lpszShortNameA;
			}
			else
			{
				lpszNewOwnerName = "unknown";
			}

			// get pointer to old owner name
			hr = GetPlayerName(lpDPInfo->lpDirectPlay4A, lp->idOldOwner, &lpOldOwnerName);
			if FAILED(hr)
				lpOldOwnerName = NULL;

			if ((lpOldOwnerName) && 
				(lpOldOwnerName->lpszShortNameA))
			{
				lpszOldOwnerName = lpOldOwnerName->lpszShortNameA;
			}
			else
			{
				lpszOldOwnerName = "unknown";
			}

			// allocate space for string
			lpszStr = (LPSTR) GlobalAllocPtr(GHND, lstrlen(lpszDisplayFormat) +
												   lstrlen(lpszGroupName) +
												   lstrlen(lpszNewOwnerName) +
												   lstrlen(lpszOldOwnerName) + 1);
			// build string
			if (lpszStr)
				wsprintf(lpszStr, lpszDisplayFormat, lpszGroupName, lpszNewOwnerName, lpszOldOwnerName );

			// free data we allocated
			if (lpGroupName)
				GlobalFreePtr(lpGroupName);
			if (lpNewOwnerName)
				GlobalFreePtr(lpNewOwnerName);
			if (lpOldOwnerName)
				GlobalFreePtr(lpOldOwnerName);
		}
		break;
...
///////////////////////////////////////////////////////////////////////////////////////
HRESULT GetPlayerName(LPDIRECTPLAY4A lpDirectPlay4A, DPID dpidPlayer,
					  LPDPNAME *lplpName)
{
	...
	hr = IDirectPlayX_GetPlayerName(lpDirectPlay4A, dpidPlayer, NULL, &dwNameSize);
	if (hr != DPERR_BUFFERTOOSMALL)
		goto FAILURE;
	...
	hr = IDirectPlayX_GetPlayerName(lpDirectPlay4A, dpidPlayer, lpName, &dwNameSize);
	if FAILED(hr)
		goto FAILURE;
...
///////////////////////////////////////////////////////////////////////////////////////
HRESULT GetGroupName(LPDIRECTPLAY4A lpDirectPlay4A, DPID dpidGroup,
					 LPDPNAME *lplpName)
{
	LPDPNAME	lpName = NULL;
	DWORD		dwNameSize;
	...
	// get size of player name data
	hr = IDirectPlayX_GetGroupName(lpDirectPlay4A, dpidGroup, NULL, &dwNameSize);
	if (hr != DPERR_BUFFERTOOSMALL)
		goto FAILURE;
	...
	// get player name data
	hr = IDirectPlayX_GetGroupName(lpDirectPlay4A, dpidGroup, lpName, &dwNameSize);
	if FAILED(hr)
		goto FAILURE;
...
void InitializeLobbyGroupWindow(HWND hWnd, LPLOBBYGROUPCONTEXT lpContext)
{
	...
	lpContext->lpDPInfo->lpDirectPlayLobby3A->EnumLocalApplications(EnumApp, hWnd, 0);
	...
	hr = IDirectPlayX_EnumConnections(	lpContext->lpDPInfo->lpDirectPlay4A, 
										&BELLHOP_GUID, 
...
HRESULT CreateLobbyGroup(HWND hWnd, LPLOBBYGROUPCONTEXT lpContext)
{
	...
	if (lpContext->dpidRoom == 0)
	{
		hr = IDirectPlayX_CreateGroup(lpContext->lpDPInfo->lpDirectPlay4A,
	...
	{
		hr = IDirectPlayX_CreateGroupInGroup(	lpContext->lpDPInfo->lpDirectPlay4A,
												lpContext->dpidRoom, 
												&dpID, &dpName, 
												NULL, 0, dwFlags);
	}
	...
	if ( DPGROUP_STAGINGAREA & dwFlags )
	{
		...
		hr = IDirectPlayX_SetGroupConnectionSettings(lpContext->lpDPInfo->lpDirectPlay4A,
													 0, dpID, &dplconn );
...
HRESULT DoDeleteRoom(HWND hWnd, LPDPLAYINFO lpDPInfo)
{
	...
	// delete the group
	hr = IDirectPlayX_DestroyGroup(lpDPInfo->lpDirectPlay4A, dpidRoom);
...
HRESULT DoDeletePlayerFromGroup(HWND hWnd, LPDPLAYINFO lpDPInfo)
{
	...
	hr = IDirectPlayX_DeletePlayerFromGroup(lpDPInfo->lpDirectPlay4A, dpidRoom, dpidPlayer );
...
///////////////////////////////////////////////////////////////////////////////////////
HRESULT DoSetGroupOwner(HWND hWnd, LPDPLAYINFO lpDPInfo)
{
	DPID dpidOwner = lpDPInfo->lpGroupTree->GetDPIDOfCurrentSelection();
	DPID dpidGroup = lpDPInfo->lpGroupTree->GetDPIDOfCurrentSelectionParent();

	return lpDPInfo->lpDirectPlay4A->SetGroupOwner( dpidGroup, dpidOwner );
}
...
HRESULT DoDeleteTable(HWND hWnd, LPDPLAYINFO lpDPInfo)
{
	...
	// delete the player from the group
	hr = IDirectPlayX_DeleteGroupFromGroup(lpDPInfo->lpDirectPlay4A, dpidRoom, dpidShortcut );
	return (hr);
}
...
HRESULT DoLaunch(HWND hWnd, LPDPLAYINFO lpDPInfo)
{
	...
	hr = IDirectPlayX_StartSession(lpDPInfo->lpDirectPlay4A, 0, dpidRoom); 
...
HRESULT InitializeLobby(HWND hWnd, LPDPLAYINFO lpDPInfo)
{
	...
	{	
		...
		hr = IDirectPlayX_CreateGroup(lpDPInfo->lpDirectPlay4A, &dpID, &name, NULL, 0, 0);
		if FAILED(hr)
			goto FAILURE;
...
HRESULT SendChatMessage(HWND hWnd, LPDPLAYINFO lpDPInfo)
{
	...
	hr = IDirectPlayX_SendChatMessage(lpDPInfo->lpDirectPlay4A, lpDPInfo->dpidPlayer,
					  ...
					  LPSTR lpszMsg, LPSTR *lplpszStr)
{
	...
	// get name of player
	hr = GetPlayerName(lpDirectPlay4A, dpidPlayer, &lpName);
	if FAILED(hr)
		goto FAILURE;
...
HRESULT WaitForRunMsg( DWORD dwAppId, HANDLE hReceiveEvent, LPDWORD lpdwStatus, LPDPLAYINFO lpDPInfo )
{
	...
	*lpdwStatus = 0;
	...
	while (bNotFinished)
	{
		...
		do
		{
			hr =IDirectPlayLobby_ReceiveLobbyMessage(	lpDPInfo->lpDirectPlayLobby3A, 
...
void HandleStartSession(LPDPLCONNECTION lpConn, LPDPLAYINFO lpDPInfo )
{
	...
	if (hEvent)
	{
		...
		hr = GetLocalAppName(lpDPInfo->lpDirectPlayLobby3A, 
		...
		// Call RunApplication
		hr = IDirectPlayLobby_RunApplication(lpDPInfo->lpDirectPlayLobby3A, 0, &dwAppID, lpConn, hEvent );
