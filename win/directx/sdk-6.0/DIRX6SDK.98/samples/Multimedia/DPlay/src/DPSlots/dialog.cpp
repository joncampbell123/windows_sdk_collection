...
HRESULT                 GetSessionDesc(LPDIRECTPLAY4A lpDirectPlay4A, LPDPSESSIONDESC2 *lplpSessionDesc);
...
BOOL CALLBACK ConnectWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	lpDPInfo = (LPDPLAYINFO) lParam;
		...
		lpDirectPlay4A->EnumConnections(&DPSLOTS_GUID, DirectPlayEnumConnectionsCallback, hWnd, 0);
	...
	switch(LOWORD(wParam))
	{
		...
		case IDC_HOSTBUTTON:
			...
			hr = HostSession(lpDirectPlay4A, &serverConfig, lpDPInfo);
			if FAILED(hr)
				goto HOST_FAILURE;
	...
	case IDC_JOINBUTTON:
			...
			// join this session
			hr = JoinSession(lpDirectPlay4A, &guidSessionInstance, &clientLogin, lpDPInfo);
					...
					LPDPLAYINFO lpDPInfo)
{
	...
	// host the session
	hr = lpDirectPlay4A->SecureOpen(&sessionDesc, DPOPEN_CREATE, lpSecurityDesc, NULL);
	if FAILED(hr)
		goto FAILURE;

	// create a server player with no name
	hr = lpDirectPlay4A->CreatePlayer(&dpidPlayer, NULL, 
					...
					LPDPLAYINFO lpDPInfo)
{
	...
	// join the session 
	hr = lpDirectPlay4A->SecureOpen(&sessionDesc, DPOPEN_JOIN, NULL, lpCredentials);
	...
	hr = GetSessionDesc(lpDirectPlay4A, &lpSessionDesc);
	if FAILED(hr)
		goto FAILURE;
...
HRESULT GetSessionDesc(LPDIRECTPLAY4A lpDirectPlay4A, LPDPSESSIONDESC2 *lplpSessionDesc)
{
	...
	hr = lpDirectPlay4A->GetSessionDesc(NULL, &dwSessionDescSize);
	...
	// get the session desc
	hr = lpDirectPlay4A->GetSessionDesc(lpSessionDesc, &dwSessionDescSize);
	if FAILED(hr)
		goto FAILURE;
