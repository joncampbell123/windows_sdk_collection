...
// Borland CBuilder3 is missing a _strupr(), so here's one:
void
__strupr(char *psz)
{
    while (*psz)
...
FAILURE:
	...
	// send the message
    return (lpDPInfo->lpDirectPlay4A->Send(lpDPInfo->dpidPlayer,
...
HRESULT GetSecureAccountDesc(LPDPLAYINFO lpDPInfo, DPID idPlayer, LPDPACCOUNTDESC *lplpAccountDesc)
{
	...
	hr = lpDPInfo->lpDirectPlay4A->GetPlayerAccount(idPlayer, 0,
	...
	__strupr(lpAccountDesc->lpszAccountIDA);
...
HRESULT GetUnsecureAccountDesc(LPDPLAYINFO lpDPInfo, DPID idPlayer, LPDPACCOUNTDESC *lplpAccountDesc)
{
	...
	hr = lpDPInfo->lpDirectPlay4A->GetPlayerName(idPlayer,
									NULL, &dwAccountDescSize);
	if (hr != DPERR_BUFFERTOOSMALL)
		goto FAILURE;
