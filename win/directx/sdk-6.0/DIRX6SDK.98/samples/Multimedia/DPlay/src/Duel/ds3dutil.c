...
LPWAVEDATA lpWD;
    ...
    lpWD->lpName = (LPTSTR)malloc(sizeof(TCHAR) * lstrlen(lpName) + 1);
    if (lpWD->lpName != NULL)
    {
...
PARAMS:     lplpWD  A pointer to a WAVEDATA struct.
...
RETURNS:    Should always return TRUE, unless something really catastrophic has happened.
