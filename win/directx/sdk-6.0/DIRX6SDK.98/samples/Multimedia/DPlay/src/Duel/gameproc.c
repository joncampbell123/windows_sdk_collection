...
static int         gShipTypesUsed[NUM_SHIP_TYPES];
...
void LaunchGame(void)
{
	...
	gbUseProtocol = (glpdpSD->dwFlags & DPSESSION_DIRECTPLAYPROTOCOL) != 0;
	...
	ZeroMemory(gShipTypesUsed, sizeof(gShipTypesUsed));
...
HRESULT InitOurShip(void)
{
	...
	gShipTypesUsed[ship.byType] = 1;	// our color
...
void AddFrag(LPSHIP lpShip, int offX, int offY)
{
	...
	default: TRACE(TEXT("Unknown ship type\n")); return;
    }
...
void ProcessSoundFlags(LPSHIP lpShip)
{
    ...
    //
    if (lpShip->bDeath)          // nothing but the boom
        {
        bStop [BFIRE]   = TRUE;
        bStop [SBOUNCE] = TRUE;
        bStop [SSTOP]   = TRUE;
        ...
        lpShip->bEngineRunning  = FALSE;
        lpShip->bMoving = FALSE;
        lpShip->dwKeys = 0;       // no point processing input for a dead ship
    ...
    for (i=0; i<MAX_SOUNDS; i++)
        ...
        if (bStop[i])
            ...
            if (bStart[i])
                {
                    bStart[i] = FALSE;
                    TRACE(_T("ASSERT in ProcessSoundFlags: bStart = bStop = TRUE\n"));
                }
            }
        }
...
void DrawShip( LPSHIP lpShip )
{
    ...
    default: TRACE(TEXT("Ship type not specified\n")); return;
    }
...
void DoSystemMessage( LPDPMSG_GENERIC lpMsg, DWORD dwMsgSize, DPID idFrom, DPID idTo )
{
    ...
    case DPSYS_CREATEPLAYERORGROUP:
        {
            ...
            if( gbIsHost)
		    {
				...
				for(i = 0; i < NUM_SHIP_TYPES; i++)
					gHostMsg.usedShipTypes[i] = gShipTypesUsed[i];
			...
			for(i = 0; i < NUM_SHIP_TYPES; i++)
			{
				if ((iLeastAmt < 0) || (gShipTypesUsed[i] < iLeastAmt))
				{
					iLeastAmt = gShipTypesUsed[i];
					iLeastType = i;
				}
			}
			gShipTypesUsed[iLeastType]++; // update our copy of the used ship types
        }
		break;
    ...
    case DPSYS_DESTROYPLAYERORGROUP:
        {
			...
			gShipTypesUsed[lpShip->byType]--; // decrement the leaving players' ship type usage counter
    ...
    case DPSYS_SENDCOMPLETE:
        // Async send status
        {
        	LPDPMSG_SENDCOMPLETE lpComplete = (LPDPMSG_SENDCOMPLETE) lpMsg;

			if (FAILED(lpComplete->hr))
			{
				LPTSTR lpszErr;
				switch (lpComplete->hr)
				{
				case DPERR_CANCELLED:	lpszErr = _T("Cancelled"); break;
				case DPERR_ABORTED:		lpszErr = _T("Aborted");   break;
				case DPERR_TIMEOUT:		lpszErr = _T("Timed out"); break;
				case DPERR_GENERIC:		lpszErr = _T("Generic");   break;
				default:				lpszErr = _T("Unknown");
				}
				TRACE(_T("Async msg %u to %u transmission failed. err = %s %#08X\n"),
						 lpComplete->dwMsgID, lpComplete->idTo, lpszErr,
						 lpComplete->hr);
			}
        }
        break;
    ...
    case MSG_HOST:
		{
			...
			if( !gbIsHost )
			{
				...
				hr = DPlayGetPlayerData(gOurID, &ship, &dwSize, DPGET_LOCAL);
				if (FAILED(hr))
					TRACE(_T("Get Player local data failed for id %d\n"), gOurID);
				...
				if (SUCCEEDED(hr) && dwMsgSize >= sizeof(HOSTMSG))
				{
					...
					for (i = 0; i < NUM_SHIP_TYPES; i++)
					{
						gShipTypesUsed[i] = lpHost->usedShipTypes[i];
						if ((leastUsed < 0) || (gShipTypesUsed[i] < leastUsed))
						{
							leastUsed = gShipTypesUsed[i];
							ship.byType = i;
						}
					}
					gShipTypesUsed[ship.byType]++; // increment it because we used one

					hr = DPlaySetPlayerData(gOurID, &ship, dwSize, DPSET_LOCAL);
					if (FAILED(hr))
						TRACE(_T("Set Player local data failed for id %d\n"), gOurID);
				}
