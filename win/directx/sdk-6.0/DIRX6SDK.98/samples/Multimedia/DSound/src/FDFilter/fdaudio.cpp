...
static BOOL IsDSDeviceInList(const GUID *, PDSDEVICEDESC );
static BOOL EnumDSDevices( void );
static BOOL EnumDSCDevices( void );
static BOOL CALLBACK DSEnumProc( GUID *, LPCSTR, LPCSTR, LPVOID );
...
//
//
static BOOL IsDSDeviceInList( const GUID * lpguidDevice, PDSDEVICEDESC pdsddList )
    {
...
static BOOL EnumDSCDevices( void )
	...
	{
    MessageBox(ghMainWnd, "DSOUND.DLL does not implement DirectSoundCaptureEnumerate(). This application requires DirectX 5 or later.", gszAppName, MB_OK);
	}
	else
	{
...
static BOOL InitInputDevice( void )
	...
	{
        MessageBox(ghMainWnd, "DSOUND.DLL does not implement DirectSoundCaptureCreate(). This application requires DirectX 5 or later.", gszAppName, MB_OK);
		goto IID_ExitError;
	}
	else
	{
...
//
static BOOL CALLBACK DSEnumProc( GUID * lpguidDevice, LPCSTR lpszDesc,
