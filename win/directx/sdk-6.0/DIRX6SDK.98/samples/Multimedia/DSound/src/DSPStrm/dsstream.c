...
HWND    hWndMain, hWndPan, hWndPanText, hWndVol, hWndVolText;
HWND    hWndBar, hWndPlay, hWndStop, hWndLoopCheck, hWndProg;
HWND    hWndProgText;
...
static void UpdateControlText( void );
...
int PASCAL WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance,
	...
	{
// Redundant, since all possible cases in InitInstance will inform the user properly
 ...
 */
	...
	+ PAN_TB_CY + VOL_TB_CY + PROG_TB_CY
	+ GetSystemMetrics( SM_CYMENU ) + 2*CONTROL_SPACE_CY
...
#ifdef DEBUG
    cy = 2*BORDER_SPACE_CY + PAN_TB_CY + VOL_TB_CY + PROG_TB_CY
		+ 2*CONTROL_SPACE_CY;
	...
	{
	ErrorMessageBox( IDS_ERROR_CREATECHILDREN, MB_ICONSTOP );
	return( FALSE );
	}
    ...
    if( FAILED( dsRetVal = lpDS->lpVtbl->GetCaps( lpDS, &dscaps )))
	{
	DPF(0,"DirectSound::GetCaps failed");
	ErrorMessageBox( IDS_ERROR_DSCAPS, MB_ICONSTOP );
	return FALSE;
	}

    if( dscaps.dwFlags & DSCAPS_EMULDRIVER )
    {   
	ErrorMessageBox( IDS_ERROR_EMULATEDCARD, MB_ICONINFORMATION );
	return FALSE;
    }
    ...
    // This function picks a starting directory for the Open box
    // and puts it in the global szCDStartPath for later use.
    GetMediaStartPath();
...
LRESULT CALLBACK MainWindowProc( HWND hWnd, unsigned uMsg,
...
#endif
    DWORD   dwCDErr = 0, dwProgressTemp;
    float   fPercent;
    UINT    uPercent;
    BOOL    bResult = FALSE;
    int     nChkErr;
	...
	case WM_DSSTREAM_PROGRESS:
	{
	    ...
	    // Protect against invalid calls, since it's possible we won't
	    // process this message out of the queue until after the actual
	    // IDirectSoundBuffer object has been released.
	    if( NULL != lpDSBStreamBuffer )
		{
		IDirectSoundBuffer_GetVolume(lpDSBStreamBuffer, &lVol);
		lVol = lVol/VOL_MULT - VOL_SHIFT;
		SendMessage( hWndVol, TBM_SETPOS, TRUE, lVol);

		IDirectSoundBuffer_GetPan(lpDSBStreamBuffer, &lPan);
		lPan = lPan/PAN_MULT - PAN_SHIFT;
		SendMessage( hWndPan, TBM_SETPOS, TRUE, lPan);

		UpdateControlText();
		}

	    dwProgressTemp = (DWORD)lParam;
	    dwProgressTemp  = dwProgressTemp % wiWave.mmckInRIFF.cksize;
	    fPercent = (float)((dwProgressTemp * 100)
     ...
     */
		...
		case IDM_PLAY:
		    ...
		    if( timeBeginPeriod( wiWave.dwTimerPeriod
			...
			{
			if(( uTimerID = timeSetEvent( wiWave.dwTimerPeriod
						       / PLAYBACK_OVERSAMPLE,
		...
		case IDM_STOP:
			...
			timeEndPeriod( wiWave.dwTimerPeriod
					/ PLAYBACK_OVERSAMPLE );
			// Busy wait for timer function to exit
			while (InterlockedExchange(&lInTimer, FALSE)) Sleep(100);
     ...
     */
	    ...
	    if( lpDS )
		dsrval = lpDS->lpVtbl->Release( lpDS );
...
int StreamBufferSetup( void )
    ...
    /* Create the buffer and Set Format properties according to
     * the WAVE file we just opened */
    ...
    if (DS_OK == dsRetVal) {
	dsRetVal = IDirectSoundBuffer_SetFormat( lpDSBStreamBuffer, wiWave.pwfx );
    }

    if (DS_OK == dsRetVal) {
	dsbc.dwSize = sizeof(dsbc);
	dsRetVal = IDirectSoundBuffer_GetCaps( lpDSBStreamBuffer, &dsbc );
    }
     ...
     * buffer segment size.
     */
    wiWave.dwBufferSize = dsbc.dwBufferBytes;
    wiWave.dwTimerPeriod = dsbc.dwBufferBytes * 1000
			   / wiWave.pwfx->nAvgBytesPerSec / NUM_BUFFER_SEGMENTS;
    ...
    // Avoid multiple code paths by calling this mutual function instead of using
    // a copy of the same code.
    FillDataBuffer();
...
/* ResetWavePlayer()                                                         */
/*                                                                           */
/*   This function simply sets up a few things and calls FillDataBuffer to   */
/* refill the sound buffer from the start.  The specific things it does are  */
/* steps like rewinding the read position and clearing any "done" state vars.*/
/*                                                                           */
/*****************************************************************************/
...
void FillDataBuffer( void )
    ...
    DWORD       dwLen1, dwLen2, cbReadLoopTotal;
     ...
     */
		...
		do
		    ...
		    nChkErr = WaveStartDataRead( &wiWave.hmmio,
						    &wiWave.mmck,
						    &wiWave.mmckInRIFF );
		    nChkErr = WaveReadFile( wiWave.hmmio, (UINT)cbReadLoopTotal,
					    lpTemp,
					    &wiWave.mmck, &uActualBytesWritten );
		...
		wiWave.cbBytesRemaining = (DWORD)uActualBytesWritten;
		DPF( 3,"Setting bFoundEnd in load, cbBytesRem = %i",
			wiWave.cbBytesRemaining );
		...
		// exactly on a block boundary and we run a bit into NULL data
				FillMemory( lpWrite1+uActualBytesWritten,
							dwLen1-uActualBytesWritten,
...
void UpdateFromControls( void )
	...
	dsRetVal = lpDSBStreamBuffer->lpVtbl->SetVolume( lpDSBStreamBuffer,
	...
	dsRetVal = lpDSBStreamBuffer->lpVtbl->SetPan( lpDSBStreamBuffer,
...
/*                                                                              */
/*    This function gets all the required values from the DirectSoundBuffer and */
/* updates the screen text controls to reflect those values.  It does not move  */
/* the trackbar settings.                                                       */
/*                                                                              */
/********************************************************************************/
void UpdateControlText( void )
    {
    long        lPan, lVol;
...
void GetMediaStartPath( void )
	...
	if( ERROR_SUCCESS != RegQueryValueEx( hReg, gszRegValueD3DPath,
				NULL, NULL,
				(LPBYTE )szCDStartPath, &cbStartPathLen ))
	    {
	    goto REG_OPEN_FAILED;
	    }
	RegCloseKey( hReg );
