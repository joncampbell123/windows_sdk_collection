...
BOOL FileInfo3D::OnInitDialog( HWND hDlg, WPARAM wParam )
{
    ...
    m_ht3d.hDisable3D		= GetDlgItem( hDlg, IDC_RADIO_3DDISABLE );
    m_ht3d.hHeadRelative	= GetDlgItem( hDlg, IDC_RADIO_HEADRELATIVE );
    m_ht3d.hNormal			= GetDlgItem( hDlg, IDC_RADIO_NORMAL );
...
BOOL FileInfo3D::OnCommand( WPARAM wParam, LPARAM lParam )
{
    ...
    if( HandleOuterVolContext( wParam ) 
        ...
        fRet = TRUE;
    else if( Handle3DModeNotify( wParam, lParam ))
        fRet = TRUE;
    else
...
HRESULT FileInfo3D::Duplicate( FileInfo *pfiSource )
{
    HRESULT	hr = DS_OK;

    hr = FileInfo::Duplicate( pfiSource );
	if(FAILED(hr))
		return hr;
...
BOOL FileInfo3D::Handle3DModeNotify( WPARAM wParam, LPARAM lParam )
{
    HRESULT hr;
    BOOL    fRet = FALSE;

    if( ((HWND)lParam == m_ht3d.hDisable3D ) ||
		((HWND)lParam == m_ht3d.hHeadRelative ) ||
		((HWND)lParam == m_ht3d.hNormal ))
    {
        if( IsDlgButtonChecked(m_ht3d.hDlg, IDC_RADIO_3DDISABLE ) == BST_CHECKED)
        {
            if( m_pDSB3D ) {
            if( FAILED( hr = m_pDSB3D->SetMode( DS3DMODE_DISABLE, DS3D_IMMEDIATE )))
                DPF( 0, "SetMode: %s", TranslateDSError(hr));
            fRet = TRUE;
            }
        }
        else if ( IsDlgButtonChecked(m_ht3d.hDlg, IDC_RADIO_HEADRELATIVE ) == BST_CHECKED)
        {
            if( m_pDSB3D ) {
            if( FAILED( hr = m_pDSB3D->SetMode( DS3DMODE_HEADRELATIVE, DS3D_IMMEDIATE )))
                DPF( 0, "SetMode: %s", TranslateDSError(hr));
            fRet = TRUE;
            }
        }
        ...
        {
            if( m_pDSB3D ) {
            if( FAILED( hr = m_pDSB3D->SetMode( DS3DMODE_NORMAL, DS3D_IMMEDIATE )))
                DPF( 0, "SetMode: %s", TranslateDSError(hr));
            fRet = TRUE;
            }
        }
