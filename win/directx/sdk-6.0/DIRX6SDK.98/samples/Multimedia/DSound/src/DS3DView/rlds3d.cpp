...
extern BOOL     gbFatalError;   // See viewer.cpp
...
BOOL
D3DRM_SUCCEED(HRESULT result, int force_critical, char* info)
{
	if (result == D3DRM_OK)
        return TRUE;
...
static void __cdecl bulletCallback(LPDIRECT3DRMFRAME obj, void* arg, D3DVALUE delta) {
...
void RLDS3D_OrbitSelected(void) {
	...
	HWND mydial = CreateDialogParam((HINSTANCE)hinst, "OrbitBox", hwndParent, (DLGPROC)OrbitDlgProc, (LONG)orbitdat);
	if (mydial) {
		char name_string[50];
...
void RLDS3D_BulletSelected(void) {
	...
	// Create a dialog box and orbitinfo so that this thing has something to control it
	HWND mydial = CreateDialogParam((HINSTANCE)hinst, "BulletBox", hwndParent, (DLGPROC)BulletDlgProc, (LONG)bs);
	if (mydial) {
		char name_string[50];
...
static void __cdecl listenerCallback(LPDIRECT3DRMFRAME obj, void* arg, D3DVALUE delta)
{
	arg = arg;
	if (!lpDS) return;
...
static void __cdecl soundCallback(LPDIRECT3DRMFRAME obj, void* arg, D3DVALUE delay)
{
	arg = arg;
...
void RLDS3D_Render(D3DVALUE time_delta)
{
    ...
    if (D3DRM_SUCCEED(info->scene->Move(time_delta)))
        if (D3DRM_SUCCEED(info->view->Clear()))
            if (D3DRM_SUCCEED(info->view->Render(info->scene)))
