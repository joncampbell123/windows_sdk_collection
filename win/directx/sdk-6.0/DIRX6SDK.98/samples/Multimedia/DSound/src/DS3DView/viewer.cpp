...
BOOL    gbFatalError = FALSE;       // Global flag to exit cleanly
    ...
    (HINSTANCE this_inst, HINSTANCE prev_inst, LPSTR cmdline, int cmdshow)
{
	...
	// Load accelerators
    accel = LoadAccelerators((HINSTANCE)this_inst, "ViewerAccel");
...
static BOOL FirstInstance(HANDLE this_inst)
{
    ...
    wc.hIcon = LoadIcon((HINSTANCE)this_inst, "ViewerIcon");
...
static BOOL AnyInstance(HANDLE this_inst, HWND* window_handle)
{
	...
	(   ViewerClass,                /* class */
	    ...
	    NULL,                       /* parent window */
	    NULL,                       /* menu handle */
	    (HINSTANCE)this_inst,       /* program handle */
	    NULL                        /* create parms */
	);
...
LONG FAR PASCAL WindowProc(HWND win, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	switch( wparam & 0xffff )
	{
		...
		case MENU_SOUND_SELECTEDSOUNDPROPERTIES:
			{
				if (RLDS3D_SoundInitialized()) {
					if (RLDS3D_SoundSelected()) {
						...
						HWND mydial = CreateDialog((HINSTANCE)inst_handle, "SelectedParamBox", win, (DLGPROC)SelectedParamDlgProc);
						if (mydial) {
		...
		case MENU_SOUND_GLOBALPROPERTIES:
			{
				if (RLDS3D_SoundInitialized()) {
					inst_handle = (HANDLE) GetWindowLong(win, GWL_HINSTANCE);
					HWND mydial = CreateDialog((HINSTANCE)inst_handle, "GlobalParamBox", win, (DLGPROC)GlobalParamDlgProc);
					if (mydial) {
						ShowWindow(mydial, SW_SHOW);
						UpdateWindow(mydial);
					}
		...
		case MENU_FILE_ABOUT:
			{
				inst_handle = (HANDLE) GetWindowLong(win, GWL_HINSTANCE);
				DialogBox((HINSTANCE)inst_handle,"AboutBox", win, (DLGPROC)AboutDlgProc);
			}
	    break;
