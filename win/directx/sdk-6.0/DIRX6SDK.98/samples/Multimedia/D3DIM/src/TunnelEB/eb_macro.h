...
// File: EB_Macro.h
//
// Desc: Macros for doing execute buffers
//
//
...
// MAKE_MATRIX to make execute-buffer compatible matrices
#define MAKE_MATRIX( pd3dDevice, hmat, data ) \
{   if( pd3dDevice->CreateMatrix( &hmat ) != D3D_OK ) \
    	return FALSE; \
    if( pd3dDevice->SetMatrix( hmat, &data ) != D3D_OK ) \
    	return FALSE; \
}
