                               ...
                               LPDIRECT3DVIEWPORT3 pvViewport )
{
	...
	*pwIndex++ = 5;  *pwIndex++ = 3;  *pwIndex++ = 7;
    ...
    light.dvPosition.x = light.dvDirection.x = 5.0f;
    light.dvPosition.y = light.dvDirection.y = 5.0f;
    light.dvPosition.z = light.dvDirection.z = 0.0f;
...
HRESULT App_FrameMove( LPDIRECT3DDEVICE3 pd3dDevice, FLOAT fTimeKey )
{
	...
	// direction.
	D3DVECTOR vEyePt    = D3DVECTOR( 5*sin( fTimeKey), 3, 5*cos( fTimeKey ) );
	D3DVECTOR vLookatPt = D3DVECTOR( 4*sin( fTimeKey+.1), 2.5, 4*cos( fTimeKey+.1 ) );
