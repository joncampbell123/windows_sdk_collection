...
// File: Perlin.cpp
// 
// Desc: Support code for the D3D fog demo. Excerpted from SIGGRAPH '92,
//       Course 23. "PROCEDURAL MODELING" by Ken Perlin, New York University
//
//	     3.6.1 The turbulence function
//
//	     The turbulence function, which you use  to  make  marble, clouds,
//       explosions, etc., is just a simple fractal generating loop built on
//       top of the noise function. It is not a real turbulence model at all.
//       The key trick is the use of the fabs() function, which makes the
//       function have gradient discontinuity "fault lines" at all scales. This
//	     fools the eye into thinking it is seeing the  results of turbulent
//       flow. The turbulence() function gives the best results when used as a
//       phase shift, as in the familiar marble trick:
//	        sin(point + turbulence(point) * point.x);
//
//	     Note the second argument below, lofreq, which sets the lowest desired
//       frequency component of the turbulence. The third, hifreq, argument is
//       used by the function to ensure that the turbulence effect reaches down
//       to the single pixel level, but no further. I usually set this argument
	...
	for( i=0; i<B; i++ )
	{
		do {	// Choose uniformly in a cube */
			v = D3DVECTOR((float)(rnd() - rnd()),
						 ...
						 (float)(rnd() - rnd()));
			s = DotProduct(v,v);
		} while( s > 1.0f );	// If not in sphere try again
		s = (float)sqrt(s);
		g[i] = v / s;		// normalize
	}
