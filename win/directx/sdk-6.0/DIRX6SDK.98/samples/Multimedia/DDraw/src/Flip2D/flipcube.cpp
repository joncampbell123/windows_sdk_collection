...
static DWORD      g_dwCurrentModeMenuItemData;
...
void AppPause(BOOL f)
{
    if (f)
    {
		// Before drawing the menu, let's store info on the menuitem for 
		// the current mode, so that we can place a checkmark by it. Note,
		// we have to do this here, in case we are in modex and need to
		// switch modes to display the menu.
		DDSURFACEDESC ddsd;
		ZeroMemory( &ddsd, sizeof(ddsd) );
		ddsd.dwSize = sizeof(ddsd);
        FrontBuffer->GetSurfaceDesc(&ddsd);
        g_dwCurrentModeMenuItemData = MAKEMENUITEMDATA( ddsd.dwWidth,
 ...
 BOOL bModeMatch = FALSE;
            for (int imi = 0; imi < cmi; imi++) {
                ...
                if (GetMenuItemInfo(hmenuPopup, imi, MF_BYPOSITION, &mii)) {
                    ...
                    if (mii.dwItemData == g_dwCurrentModeMenuItemData) {
