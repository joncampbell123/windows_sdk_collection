...
BOOL BoidMusic::LoadSection()

/*	This loads a precomposed section segment.
*/

{
	if (m_pSection) m_pSection->Release();

	DMUS_OBJECTDESC ObjectDescript;
	ObjectDescript.guidClass = CLSID_DirectMusicSegment;
	wcscpy(ObjectDescript.wszFileName, L"BoidsD.sec");
	ObjectDescript.dwValidData = DMUS_OBJ_CLASS | DMUS_OBJ_FILENAME ;
	ObjectDescript.dwSize = sizeof(DMUS_OBJECTDESC);
	if (SUCCEEDED(m_pLoader->GetObject(&ObjectDescript, IID_IDirectMusicSegment, (void**)&m_pSection)))
	{
		m_pSection->SetParam(
...
BOOL BoidMusic::LoadPersonality()

/*	Load the Boids personality.
*/

{
	if (m_pPersonality) m_pPersonality->Release();

	DMUS_OBJECTDESC ObjectDescript;
	ObjectDescript.guidClass = CLSID_DirectMusicPersonality;
	wcscpy(ObjectDescript.wszFileName, L"Boids.per");
	ObjectDescript.dwValidData = DMUS_OBJ_CLASS | DMUS_OBJ_FILENAME;
	ObjectDescript.dwSize = sizeof(DMUS_OBJECTDESC);
	if (SUCCEEDED(m_pLoader->GetObject(&ObjectDescript, IID_IDirectMusicPersonality, (void**)&m_pPersonality)))
	{
        return TRUE;
	}
...
void BoidMusic::ComposeSection(DWORD dwIndex)

/*	Use the composer to compose a section segment from a previously loaded
	template segment, the boids personality and style. Place the
	segment in the m_pPrimarySegments[] array.
*/

{ 	

	if (m_pComposer && m_pStyle && m_pPersonality && m_pTemplateSegments[dwIndex])
	{
...
void BoidMusic::StartMusic(HWND hWnd)
...

{
	...
	{
		m_pPerformance->PlaySegment( 
					m_pSection, DMUS_SEGF_BEAT , 0, NULL);
	}
...
BOOL BoidMusic::LoadMusic()
{
    ...
    {
        OutputDebugString("Couldn't get SDK media path from registry\n");
		return FALSE;
    }

	m_pLoader->EnableCache(GUID_DirectMusicAllTypes, TRUE);
	m_pLoader->SetSearchDirectory(GUID_DirectMusicAllTypes, dir,FALSE);
	...
	bResourcesOk = bResourcesOk && LoadPersonality();
	...
	bResourcesOk = bResourcesOk && LoadSection();
	...
	hr = m_pDMusic->Activate(NULL,FALSE);
    if (FAILED(hr))
    {
        OutputDebugString("Couldn't IDirectMusic::Activate\n");
        return FALSE;
    }
	...
	for (DWORD index = 0; ; index++)
	{
		...
		{
			switch(dmpc.dwFlags & DMUS_PC_SOFTWARESYNTH)
			{
				case DMUS_PC_SOFTWARESYNTH:
    ...
    hr = m_pPerformance->Init(&m_pDMusic);
    if (FAILED(hr))
    {
