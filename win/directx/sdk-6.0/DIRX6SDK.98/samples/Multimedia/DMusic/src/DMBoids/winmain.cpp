...
BOOL InitMain (void)
{
	...
	if (!g_Music.LoadMusic())
    {
        MessageBox(NULL, "Failed to initialize DirectMusic", NULL, MB_OK | MB_ICONEXCLAMATION);
        return FALSE;
    }
...
void RunMain (void)
{
	...
	g_Music.StartMusic(g_hMainWindow);
    ...
    while (TRUE)
    {
		...
		Sleep(10);	// Multitasking seems to be unreliable without this. 
					// Even the mmtimer thread is blocked out. Not good.
		g_Music.HandleNotifies();
    } // End While
