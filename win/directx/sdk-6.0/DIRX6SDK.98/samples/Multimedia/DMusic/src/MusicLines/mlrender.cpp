...
HWND GraphicsEngine::EnterFullScreen()
{
    ...
    {
        // Already exists, just reset the style bits and size
        //
...
HRESULT GraphicsEngine::RenderToSurface()
{
    ...
    else
    {
        ...
        if (FAILED(hr))
        {
            TraceMsg("backbuffer blt -> black fill", hr);
            return hr;
        }
    }
...
HRESULT GraphicsEngine::FlipWindowed()
{
    ...
    long Width  = min(GAME_WIDTH,  ClientRect.right);
    long Height = min(GAME_HEIGHT, ClientRect.bottom);
