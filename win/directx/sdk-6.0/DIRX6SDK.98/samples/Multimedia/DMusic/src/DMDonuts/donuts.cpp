...
#define SECTION_1			0
#define SECTION_2			1
#define SECTION_TRANS_1		2
#define SECTION_TRANS_2		3
...
IDirectMusicPersonality*	gapPersonality[NUM_STYLES] [NUM_PERSONALITIES];
IDirectMusicSegment*		gapMotif[NUM_STYLES] [NUM_MOTIFS];
IDirectMusicSegment*		gapSection[NUM_SECTIONS] = {NULL, NULL, NULL, NULL};
...
IDirectMusicSegment*		gapShieldBand[NUM_STYLES] = {NULL, NULL};
IDirectMusicSegment*		gapDefaultBand[NUM_STYLES] = {NULL, NULL};

// Global variables
BOOL						bAnyHits = FALSE;
int							gnCurrentStyle = 0;
int							gnCurrentPersonality = 0;
int							gnLastStyle = 0;
int							gnLastPersonality = 0;
...
/////////// DMUSIC BEGIN /////////////
void ComposeNewSections()
{
	// Set up new sections
	snSubLevel = 1;
	snLastTempo = 0;
	gnLastStyle = gnCurrentStyle;
	gnLastPersonality = gnCurrentPersonality;
	gnCurrentStyle = level % NUM_STYLES;
	gnCurrentPersonality = ((level - 1) / 2) % NUM_PERSONALITIES;
	HRESULT hr = S_OK;
	MUSIC_TIME mtSectionLength;
	if (gapSection[SECTION_1])
	{
		gapSection[SECTION_1]->Release();
	}
	hr = gpComposer->ComposeSegmentFromTemplate(
		gapStyle[gnCurrentStyle],
		gpIntroTemplate, 0, 
		gapPersonality[gnCurrentStyle][gnCurrentPersonality],
		&gapSection[SECTION_1]
	);
	if (!SUCCEEDED(hr))
	{
		CleanupAndExit("Section 1 composition failed");
	}
	gapSection[SECTION_1]->GetLength(&mtSectionLength);
	gapSection[SECTION_1]->SetLength(mtSectionLength - MEASURE_LENGTH);
	gapSection[SECTION_1]->SetRepeats(999);
	if (gapSection[SECTION_2])
	{
		gapSection[SECTION_2]->Release();
	}
	hr = gpComposer->ComposeSegmentFromTemplate(
		gapStyle[gnCurrentStyle],
		gpGameTemplate, 0, 
		gapPersonality[gnCurrentStyle][gnCurrentPersonality],
		&gapSection[SECTION_2]
	);
	if (!SUCCEEDED(hr))
	{
		CleanupAndExit("Section 2 composition failed");
	}
	gapSection[SECTION_2]->GetLength(&mtSectionLength);
	gapSection[SECTION_2]->SetLength(mtSectionLength - MEASURE_LENGTH);
	gapSection[SECTION_2]->SetRepeats(999);
	bAnyHits = FALSE;
}

BOOL TransitionDone() // only called if music is enabled...
{
	if (gapSection[SECTION_TRANS_2]) // NULL on startup
	{
		return (gpPerformance->IsPlaying(gapSection[SECTION_1], NULL) == S_OK);
	}
	else
	{
...
void setup_game(void)
{
	...
	gpPerformance->PlaySegment(
		gapSection[SECTION_1], 0, 0, NULL);
	gpPerformance->PlaySegment(
		gapDefaultBand[gnCurrentStyle], DMUS_SEGF_SECONDARY, 0, NULL);
	/////////////// DMUSIC END /////////////////
				// set the palette
...
long FAR PASCAL MainWndproc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    case WM_COMMAND:
		...
		case IDC_MUSIC:
			...
			{
				short nSection = (snSubLevel == 1) ? SECTION_1 : SECTION_2;
				gpPerformance->PlaySegment(gapSection[nSection], 0, 0, NULL);
			}
                        ...
                        int nCmdShow )
{
    ...
    while( 1 )
    {
		...
		/////////////////// DMUSIC BEGIN //////////////////
		if( hNotifyEvent && gpPerformance)
		{
			DMUS_NOTIFICATION_PMSG* pEvent;
			if ( S_OK == gpPerformance->GetNotificationPMsg(&pEvent))
			{
...
BOOL InitializeGame( void )
{
	...
	for(DWORD index = 0; hr == S_OK; index++)
	{
		...
		if(hr == S_OK)
		{
			if ( (dmpc.dwClass == DMUS_PC_OUTPUTCLASS) && 
				 (dmpc.dwFlags & DMUS_PC_SOFTWARESYNTH) )
			{
				...
				hr = gpDirectMusic->CreatePort(guidSynthGUID, guidZero, &dmos, &pPort, NULL);
				break;
			}
		}
	}
...
#endif
	gpDirectMusic->Activate(hWndMain, TRUE);
	...
	gpLoader->SetSearchDirectory(GUID_DirectMusicAllTypes, wszSearchPath, FALSE);
	...
	WCHAR awszPersonalities[NUM_STYLES][NUM_PERSONALITIES][64];
	wcscpy(awszPersonalities[0][0], L"minaeo.per");
	wcscpy(awszPersonalities[0][1], L"minfunc.per");
	wcscpy(awszPersonalities[0][2], L"mipedpt.per");
	wcscpy(awszPersonalities[0][3], L"tension.per");
	wcscpy(awszPersonalities[1][0], L"dianoble.per");
	wcscpy(awszPersonalities[1][1], L"minpedpt.per");
	wcscpy(awszPersonalities[1][2], L"mippjazz.per");
	wcscpy(awszPersonalities[1][3], L"minjazz.per");

	ObjectDescript.guidClass = CLSID_DirectMusicPersonality;
	ObjectDescript.dwValidData = DMUS_OBJ_CLASS | DMUS_OBJ_FILENAME;
	for (short n = 0; n < NUM_STYLES; n++)
	{
		for (short m = 0; m < NUM_PERSONALITIES; m++)
		{
			if (hr == S_OK)
			{
				wcscpy(ObjectDescript.wszFileName, awszPersonalities[n][m]);
				hr = gpLoader->GetObject(&ObjectDescript, IID_IDirectMusicPersonality, (void**)&gapPersonality[n][m]);
			}
		}
	}
	...
	if (hr != S_OK)
	{
		return CleanupAndExit("Couldn't load a personality");
	}

	BSTR bstrDefault = SysAllocString(L"Default 2");
	BSTR bstrShields = SysAllocString(L"Shields");
	IDirectMusicBand* apShieldBand[NUM_STYLES] = {NULL, NULL};
	IDirectMusicBand* apDefaultBand[NUM_STYLES] = {NULL, NULL};

	for (n = 0; n < NUM_STYLES; n++)
	{
		if (hr == S_OK)
		{
			hr = gapStyle[n]->GetBand(bstrShields, &apShieldBand[n]);
		}
		if (hr == S_OK)
		{
			hr = gapStyle[n]->GetBand(bstrDefault, &apDefaultBand[n]);
		}
	}
	...
	if (hr != S_OK)
	{
		for (n = 0; n < NUM_STYLES; n++)
		{
			if (apShieldBand[n]) apShieldBand[n]->Release();
			if (apDefaultBand[n]) apDefaultBand[n]->Release();
		}
	...
	for (n = 0; n < NUM_STYLES; n++)
	{
		if (hr == S_OK)
		{
			hr = apShieldBand[n]->CreateSegment(&gapShieldBand[n]);
		}
		if (hr == S_OK)
		{
			hr = apDefaultBand[n]->CreateSegment(&gapDefaultBand[n]);
		}
	...
	if( !hNotifyEvent )
	{
		return CleanupAndExit("Couldn't create an event handle");
	}
	GUID guid;
	gpPerformance->SetNotificationHandle( hNotifyEvent, 0 );
...
void CleanUpDMusic()
{
	...
	for (short n = 0; n < NUM_STYLES; n++)
	{
		...
		for (short m = 0; m < NUM_PERSONALITIES; m++)
		{
			if (gapPersonality[n][m])
			{
				gapPersonality[n][m]->Release();
			}
		}
    ...
    switch( ProgramState )
    {
        ...
        case PS_ACTIVE:
            ...
            if ( isDisplayListEmpty() )
            {
				...
				if (bMusicEnabled)
				{
					// Do an Autotransition with ending to the new first section
					if (gapSection[SECTION_TRANS_2]) 
					{
						gapSection[SECTION_TRANS_2]->Release();
					}
					gpComposer->AutoTransition(
						...
						gapPersonality[gnLastStyle] [gnLastPersonality], 
						&gapSection[SECTION_TRANS_2],
						NULL, NULL
					);
				}
				///////////// DMUSIC END //////////////
            ...
            if( hit )
            {
				...
				if (!bAnyHits)
				{
					if (bMusicEnabled && !gbShieldsOn)
					{
						...
						if (gapSection[SECTION_TRANS_1])
						{
							gapSection[SECTION_TRANS_1]->Release();
						}
						gpComposer->AutoTransition(
							gpPerformance,
							gapSection[SECTION_2], DMUS_COMMANDT_FILL, 
							DMUS_COMPOSEF_MODULATE | DMUS_COMPOSEF_MEASURE,
							gapPersonality[gnCurrentStyle] [gnCurrentPersonality], 
							&gapSection[SECTION_TRANS_1],
							NULL, NULL
						);
						bAnyHits = TRUE;
					}
...
void DrawDisplayList( void )
{
    ...
    do
    {
        while( 1 )
	{
	    //ddrval = lpBackBuffer->lpVtbl->BltFast( lpBackBuffer, thiss->dst.left, thiss->dst.top, this->surf, &(this->src), dwTransType );
