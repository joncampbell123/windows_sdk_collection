...
#ifdef DISABLE
        case WM_SYSCHAR:     /* hooking for Alt+BkSp = UNDO 20-Mar-1987 */
            if (wParam==VK_BACK && (GetKeyState(VK_MENU) < 0)) {
                HiliteMenuItem(hIndexWnd, GetMenu(hIndexWnd), UNDO, MF_HILITE);
                SendMessage(hIndexWnd, WM_COMMAND, UNDO, 0L);
                HiliteMenuItem(hIndexWnd, GetMenu(hIndexWnd), UNDO, MF_UNHILITE);
            } else
                goto PassMessageOn;
            break;
#endif
...
FAR AddCurCard()
	...
	for(pch1 = CurCardHead.line, lpch2 = lpTCards->line; *pch1; ++pch1,++lpch2)
            if ((char)AnsiUpper((LPSTR)(DWORD)(BYTE)*pch1) != (char)AnsiUpper((LPSTR)(DWORD)(BYTE)*lpch2))
		break;
        if ((char)AnsiUpper((LPSTR)(DWORD)(BYTE)*pch1) <= (char)AnsiUpper((LPSTR)(DWORD)(BYTE)*lpch2))
	    break;
	lpTCards++;
