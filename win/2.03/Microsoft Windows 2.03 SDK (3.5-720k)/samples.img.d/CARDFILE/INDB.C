...
LONG lParam;
	...
	case WM_INITDIALOG:
            ...
            lstrcat((LPSTR)szExtension, (LPSTR)rgchFileExtension);

            SetDlgItemText(hwnd, ID_EDIT, (LPSTR)szExtension);
            SendDlgItemMessage(hwnd, ID_EDIT, EM_LIMITTEXT, PATHMAX-8, 0L);
            DlgDirList(hwnd, (LPSTR)szExtension, ID_LISTBOX, ID_PATH, ATTRDIRLIST);
	    break;
			...
			case 1:
				...
				if (DlgDirSelect(hwnd, (LPSTR)szNewName, ID_LISTBOX))
					...
					lstrcpy((LPSTR)szNewName + cchDir, (LPSTR)pchFile);
                                        ...
                                        {
                                        lstrcpy((LPSTR)szNewName + cchDir, (LPSTR)"*.");
                                        lstrcat((LPSTR)szNewName, (LPSTR)rgchFileExtension);
                                        }
				    }
				SetDlgItemText(hwnd, ID_EDIT, (LPSTR)szNewName);
...
BOOL	fSearching;
{
    ...
    BOOL    fDone = FALSE;
    int     cchEdit;
    char szExtSave[10]; /* will get "\\*.CRD" */
    ...
    lstrcpy((LPSTR)szExtSave, (LPSTR)"\\*.");
    lstrcat((LPSTR)szExtSave, (LPSTR)rgchFileExtension);
...
LONG lParam;
	...
	case WM_INITDIALOG:
		...
		/* rgch gets current directory string, terminated with "\\\0" */
                len = GetDlgItemText(hwnd, ID_PATH, (LPSTR)rgch, PATHMAX);
		if (rgch[len-1] != '\\')
		    {
		    rgch[len] = '\\';
		    rgch[++len] = 0;
                ...
                /* must have path and current file in ansi char set. 28-Oct-1987. */
                OemToAnsi((LPSTR)CurIFile, (LPSTR)rgchAnsi);

		/* Now see if path in reopen buffer matches current directory. */
                for (pchFileName = rgchAnsi,
                     pchTest = PFileInPath(rgchAnsi),
		     pchCmp = rgch;
		     pchFileName < pchTest;
