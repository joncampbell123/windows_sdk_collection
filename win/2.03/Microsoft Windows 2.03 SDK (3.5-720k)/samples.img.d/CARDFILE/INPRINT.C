...
HDC SetupPrinting()
    ...
    /* 21-Aug-1987. larryf. on second print job, this was getting used
     * before the dialog was created for the second time.
     */
    hAbortDlgWnd = (HWND)NULL;


    GetProfileString((LPSTR)rgchWindows, (LPSTR)rgchDevice, (LPSTR)"", (LPSTR)fileandport, 80);
...
CantPrint:
    ...
    Escape(hPrintDC, SETABORTPROC, 0, (LPSTR)(LONG)(FARPROC)lpfnAbortProc, (LPSTR)0);
    BuildCaption(buf);
...
LONG lParam;
	...
	case WM_INITDIALOG:
	    ...
	    if (CurIFile[0])
		{
                for (pchTmp = rgchAnsi + lstrlen((LPSTR)rgchAnsi) - 1;
                    pchTmp >= rgchAnsi && *pchTmp != '\\' && *pchTmp != ':';
		    pchTmp--)
		    ;
		pchTmp++;
