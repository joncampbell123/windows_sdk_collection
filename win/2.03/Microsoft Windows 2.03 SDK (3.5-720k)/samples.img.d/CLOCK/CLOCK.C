...
#if TEFTI
extern FAR PASCAL ClearCounter(int);
extern FAR PASCAL StartCounter(int);
extern FAR PASCAL StopCounter(int);
extern FAR PASCAL LatchCounter(int);
extern FAR PASCAL Pre_devCounter(int, int, int);
extern FAR PASCAL Get_cntCounter(int);
#endif
...
WORD  SizeWord;
{
    ...
    if(SizeWord == SIZEICONIC) {
        ...
        KillTimer( hWnd, TimerID );
        SetTimer( hWnd, TimerID, (unsigned) ICON_TLEN, (long)0 );
        bIconic = TRUE;
        }
    else if (bIconic) {
        ...
        KillTimer( hWnd, TimerID );
        SetTimer( hWnd, TimerID, OPEN_TLEN, (long)0 );
        bIconic = FALSE;
        }
}
...
WORD msg;                 /* timer ID */
{
    ...
    StopCounter(0);
    LatchCounter(0);
    usecs = (LONG)(unsigned)Get_cntCounter(0) * 64;
    ClearCounter(0);
    StartCounter(0);
#endif
...
int cmdShow;
{
    ...
    hWnd = CreateWindow(
        ...
        CW_USEDEFAULT,
        0,                  /*y not used*/
        (HorzRes/3) + GetSystemMetrics(SM_CXFRAME)*2,
        (((HorzRes/3)*aspectN)/aspectD)+GetSystemMetrics(SM_CYCAPTION)+GetSystemMetrics(SM_CYFRAME)*2,
    ...
    if ( !SetTimer(hWnd, TimerID, OPEN_TLEN, (long)0) ) {
...
HANDLE hInstance;
{
    ...
    LoadString( hInstance, IDS_DATA, (LPSTR)szData, 5 );
    hRes = FindResource( hInstance, (LPSTR)szBuffer, (LPSTR)szData );
    if (hRes) {
