...
PSTR pchName;
    ...
    cBytes = mylmul((cCards+cMergeCards),sizeof(CARDHEADER));
...
PSTR pchName;
    ...
    /* limit data to < 64k. 26-Jun-1987. */
    cBytes = mylmul(cNewCards,sizeof(CARDHEADER));
    if (cBytes >= 0x0000FFFF)
        goto Fail;
...
PSTR pchBuf;
    ...
    lstrcpy((LPSTR)pchBuf, (LPSTR)pchName);
    pch1 = pchBuf + lstrlen((LPSTR)pchBuf);
    while ((ch = *pch1) != '.' && ch != '\\' && ch != ':' && pch1 > pchBuf)
	pch1 = (char *)AnsiPrev((LPSTR)pchBuf, (LPSTR)pch1);
    if (*pch1 != '.') {
        lstrcat((LPSTR)pchBuf, (LPSTR)".");
        lstrcat((LPSTR)pchBuf, (LPSTR)rgchFileExtension);
    }
    AnsiUpper((LPSTR)pchBuf);
    }

FAR WriteCardFile(pchName)
PSTR pchName;   /* filename in OEM character set */
    {
    ...
    OemToAnsi((LPSTR)pchName, (LPSTR)pchName);
    AppendExtension(pchName, buf);
    AnsiToOem((LPSTR)buf,(LPSTR)buf);

    if (fSameFile = !lstrcmp((LPSTR)buf, (LPSTR)CurIFile))
	{
        if(!GetTempFileName(CurIFile[0] | TF_FORCEDRIVE, (LPSTR)rgchFileExtension, 0, (LPSTR)bakName))
	    goto SaveTempProblem;
	}
    else
	lstrcpy((LPSTR)bakName, (LPSTR)buf);

    OemToAnsi((LPSTR)bakName, (LPSTR)szAnsi);
    fhBak = OpenFile((LPSTR)szAnsi, (LPOFSTRUCT)&bakofStruct, OF_CREATE);
    /* open files */
    if (CurIFile[0])
    ...
    OemToAnsi((LPSTR)pchFileName, (LPSTR)szAnsi);
    if(fReadOnly || !(fhDollar = OpenFile((LPSTR)szAnsi, (LPOFSTRUCT)&tmpreopen, OF_PROMPT | OF_REOPEN | 2)) == -1)
	{
SaveTempProblem:
    ...
    if (CurIFile[0]) {
        OemToAnsi((LPSTR)CurIFile, (LPSTR)szAnsi);
        fhOld = OpenFile((LPSTR)szAnsi, (LPOFSTRUCT)&mainreopen, OF_PROMPT | OF_REOPEN);
    }
...
CantMakeFile:
	_lclose(fhDollar);
	LoadString(hIndexInstance, ECANTMAKEFILE, (LPSTR)dolName, PATHMAX);
        OemToAnsi((LPSTR)buf, (LPSTR)szAnsi);
        MergeStrings((LPSTR)dolName, (LPSTR)szAnsi, (LPSTR)bakName);
    ...
    lCardData = _llseek(fhBak, 0L, 1) + (unsigned long)((unsigned)cCards * (unsigned)sizeof(CARDHEADER));
...
WFDiskFull:
	...
	Frename(bakName, buf);
        OemToAnsi((LPSTR)buf, (LPSTR)szAnsi);
        fhOld = OpenFile((LPSTR)szAnsi, (LPOFSTRUCT)&mainreopen, 2);
	}
    else
	{
...
FAR MakeTempFile(hInstance)
HANDLE hInstance;
    {
    int  fh;

    if(GetTempFileName(CurIFile[0], (LPSTR)rgchFileExtension, 0, (LPSTR)TempFile))
	{
        if ((fh = OpenFile((LPSTR)TempFile, (LPOFSTRUCT)&tmpreopen, OF_CREATE)) < 0)
	    goto CantMakeTmp;
	else
	    _lclose(fh);
	}
