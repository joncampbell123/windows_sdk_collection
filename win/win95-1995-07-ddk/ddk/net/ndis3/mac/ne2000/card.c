...

{
	...
	//
	// Write to and read from CR to make sure it is there.  This is a chip stabilization
	// technique.  If you push the chip to fast, it tends to corrupt data.
	//
...
{
	PUCHAR CurBufAddress, OddBufAddress, XmitBufAddress;
	UINT CurBufLen, PacketLength;
	UINT OddBufLen = 0;
	...
	// Get address and length of the first buffer
	...
	//
	// Set up the card.  This code used to be in CardCopyDown, and we used
	// to set up the registers on the card for every buffer in a packet.  When your
	// protocol uses at the most 3 to 4 buffers per packet, this worked fine.  Under
	// our TESTPROT tests, we linked together packets with up to 13 small buffers.
	// The overhead associated with setting up all the registers on the card for each
	// buffer caused the chip to become unstable and corrupt data.  By touching the
	// card registers only once per packet, which is what we do here, we reduce the
	// instability of the chip.  In essence, on 486/50+ machines, we are pushing the 
	// hardware to its limits, and any unnecessary touching of the hardware must be
	// eliminated.
	// 
	
	{
		...
		NdisRawWritePortUchar( Adapter->IoPAddr + NIC_RMT_ADDR_LSB, LSB(ReadBuffer) );
		NdisRawWritePortUchar( Adapter->IoPAddr + NIC_RMT_ADDR_MSB,    MSB(ReadBuffer) );
	...
	//
	// Copy the packet down to the card
	//
	...
	if (OddBufLen) { // Write trailing byte (if necessary)
		NdisRawWritePortUchar(
			Adapter->IoPAddr + NIC_RACK_NIC,
			*OddBufAddress
			);
		...
		if ((ULONG)TargetBuffer & 0x1) {
			...
			while (Count) {
				...
				if (!(Tmp & ISR_DMA_DONE)) {
					...
					DbgPrint("CopyDown DMA didn't finish!");
...

{
	...
	//
	NdisReadPortUchar(Adapter->NdisAdapterHandle, Adapter->IoPAddr+NIC_COMMAND, &Temp);
