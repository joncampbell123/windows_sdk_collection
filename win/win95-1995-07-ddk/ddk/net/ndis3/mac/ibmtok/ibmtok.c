        ...
        if (!TrNoteFilterOpenAdapter(
            ...
            if (Adapter->AdapterNotOpen) {
                ...
                if (!Adapter->OpenInProgress) {
                    ...
                    TR_COPY_NETWORK_ADDRESS((PCHAR)OpenSrb->NodeAddress,
                                            Adapter->NetworkAddress
                                           );
...

{
    ...
    switch (Oid) {
        ...
        case OID_GEN_CURRENT_LOOKAHEAD:
            ...
            //
            // For now, if someone asks for a lookahead of a
            // certain size, convert that to that size plus
            // the maximum possible header.
            //
...
--*/
{
    ...
    //
    // First the transmit queue.
    //

    Packet = Adapter->FirstTransmit;

    while (Packet) {

        Reserved = PIBMTOK_RESERVED_FROM_PACKET(Packet);
        Next = Reserved->Next;
        Open =
        ...
        NdisInterlockedAddUlong((PULONG)&Adapter->FrameTransmitErrors, 1, &Adapter->Lock);
    ...
    //
    // Now TransmittingPacket if it exists.
    //

    if (Adapter->TransmittingPacket != NULL) {

        Reserved = PIBMTOK_RESERVED_FROM_PACKET(Adapter->TransmittingPacket);
    ...
    for (i=0; i<MAX_COMMAND_CORRELATOR; i++) {
        ...
        Packet = Adapter->CorrelatorArray[i];
...

{
    ...
    NDIS_STRING PacketSizeStr = NDIS_STRING_CONST("MAXPACKETSIZE");
