...
@@:     in      ax,dx
        and     eax,FIFO_3_EMPTY        ;IO_FIFO_WAIT(3)
        jnz     short @b

        mov     edx,[esi].pdev_ioFrgd_color
        mov     eax,[esp].sf_color
        out     dx,ax                   ;IO_FRGD_COLOR(color)

        cmp     [esi].pdev_iBitmapFormat,BMF_32BPP
        jne     short io_not_32bpp

        shr     eax,16
        out     dx,ax                   ;IO_FRGD_COLOR32(color)

io_not_32bpp:
        mov     edx,[esi].pdev_ioFrgd_mix
        mov     eax,[esp].sf_ulHwMix
        or      eax,FOREGROUND_COLOR
        out     dx,ax                   ;IO_FRGD_MIX(FOREGROUND_COLOR | ulHwMix)
...
io_int_current_position_already_set:
        mov     edx,[ebp].pdev_ioGp_stat_cmd
@@:     in      ax,dx
        and     eax,FIFO_5_EMPTY        ;IO_FIFO_WAIT(5)
        jnz     short @b
...
@@:     in      ax,dx
        and     eax,FIFO_4_EMPTY        ;IO_FIFO_WAIT(4)
        jnz     short @b
...
io_int_radial_continue_figure::
        ...
        mov     edx,[edi].pdev_ioGp_stat_cmd
@@:     in      ax,dx
        and     eax,FIFO_2_EMPTY        ;IO_FIFO_WAIT(2)
        jnz     short @b
...
io_int_radial_skip_length::
        mov     edx,[edi].pdev_ioGp_stat_cmd
@@:     in      ax,dx
        and     eax,FIFO_1_EMPTY        ;IO_FIFO_WAIT(1)
        jnz     short @b
...
@@:
        mov     edx,[ecx].pdev_ioDesty_axstp
        mov     eax,esi
        out     dx,ax                   ;IO_AXSTP(dM)

        mov     edx,[ecx].pdev_ioDestx_diastp
        sub     eax,edi
        out     dx,ax                   ;IO_DIASTP(dM - dN)
...
;---------------------------Public-Routine------------------------------;
; vMmFastLine(ppdev, ppo, prclClip, apfn, flags)
;
; Draws fast lines.  Or at least attempts to.  Uses memory-mapped I/O.
; Twin to 'vIoFastLine'.
;
; Input:
;
...
; Make local copies of some PDEV variables, so that we don't have to carry
; a PDEV pointer around everywhere:

        mov     eax,[esi].pdev_xOffset
        mov     ebx,[esi].pdev_yOffset
        mov     edx,[esi].pdev_ioGp_stat_cmd
        mov     edi,[esi].pdev_pjMmBase

        mov     [esp].sf_xOffset,eax
        mov     [esp].sf_yOffset,ebx
        mov     [esp].sf_ioGp_stat_cmd,edx
        mov     [esp].sf_pjMmBase,edi
...
@@:     in      ax,dx
        ...
        mov     eax,[esp].sf_color
        mov     [edi+FRGD_COLOR],ax     ;MM_FRGD_COLOR(color)

        cmp     [esi].pdev_iBitmapFormat,BMF_32BPP
        jne     short mm_not_32bpp

        shr     eax,16
        mov     [edi+FRGD_COLOR],ax     ;MM_FRGD_COLOR32(color)

mm_not_32bpp:
        mov     eax,[esp].sf_ulHwMix
        or      eax,FOREGROUND_COLOR
        mov     [edi+FRGD_MIX],ax       ;MM_FRGD_MIX(FOREGROUND_COLOR | ulHwMix)
...
mm_check_for_closefigure:
        test    [esp].sf_pd.pd_flags,PD_CLOSEFIGURE
        jnz     mm_handle_closefigure
        mov     esi,[esp].sf_bMore
        or      esi,esi
        jnz     mm_next_record
...
; actually be defined as dwords in the PDEV...
        ...
        public  mm_int_output_line
mm_int_output_line::
        mov     ebp,[esp].sf_pjMmBase   ;ebp = ppdev->pjMmBase
        cmp     [esp].sf_bSetCP,0
        je      short mm_int_current_position_already_set

        mov     edx,[esp].sf_ioGp_stat_cmd
@@:     in      ax,dx
        and     eax,FIFO_7_EMPTY        ;IO_FIFO_WAIT(7)
        jnz     short @b

        add     ebx,[esp].sf_xOffset
        mov     [ebp+CUR_X],bx          ;MM_CUR_X(x)

        add     ecx,[esp].sf_yOffset
        mov     [ebp+CUR_Y],cx          ;MM_CUR_Y(y)

        mov     [esp].sf_bSetCP,0       ;the next integer line in this figure
                                        ; will have the CP set correctly

        jmp     short mm_int_output_common

mm_int_current_position_already_set:
        mov     edx,[esp].sf_ioGp_stat_cmd
@@:     in      ax,dx
        and     eax,FIFO_5_EMPTY        ;IO_FIFO_WAIT(5)
        jnz     short @b

        public  mm_int_output_common
mm_int_output_common::
        mov     [ebp+MAJ_AXIS_PCNT],si  ;MM_MAJ_AXIS_PCNT(dx)

        mov     [ebp+AXSTP],di          ;MM_AXSTP(dy)

        sub     edi,esi
        mov     [ebp+DIASTP],di         ;MM_DIASTP(dy - dx)

        mov     edx,[esp].sf_ulErrorTerm
        sar     edx,1
        add     edi,edx
        mov     [ebp+ERR_TERM],di       ;MM_ERR_TERM(dy - dx +
                                        ;            floor((dx - bias) / 2))

        mov     eax,[esp].sf_ulCmd
        mov     [ebp+CMD],ax            ;MM_CMD(ulCmd)
...
;/////////////////////////////////////////////////////////////////////
        ...
        sub     esi,ebx
        jl      mm_int_2_3_4_5
        jz      mm_int_radial_90_270
        sub     edi,ecx
        jl      mm_int_6_7
        jz      mm_int_radial_0
        cmp     esi,edi
        jl      mm_int_1
        je      mm_int_radial_315
...
mm_int_1::
        ...
        public  mm_int_2_3_4_5
mm_int_2_3_4_5::
        neg     esi
        sub     edi,ecx
        jl      mm_int_4_5
        jz      mm_int_radial_180
        cmp     esi,edi
        jl      mm_int_2
        je      mm_int_radial_225
...
;/////////////////////////////////////////////////////////////////////
        ...
        public  mm_int_radial_45
mm_int_radial_45::
        mov     ebp,DRAW_LINE + DRAW + DIR_TYPE_RADIAL + WRITE + \
                    LAST_PIXEL_OFF + MULTIPLE_PIXELS + DRAWING_DIRECTION_45
        jmp     short mm_int_output_radial

        public  mm_int_radial_135
mm_int_radial_135::
        mov     ebp,DRAW_LINE + DRAW + DIR_TYPE_RADIAL + WRITE + \
                    LAST_PIXEL_OFF + MULTIPLE_PIXELS + DRAWING_DIRECTION_135
        jmp     short mm_int_output_radial

        public  mm_int_radial_225
mm_int_radial_225::
        mov     ebp,DRAW_LINE + DRAW + DIR_TYPE_RADIAL + WRITE + \
                    LAST_PIXEL_OFF + MULTIPLE_PIXELS + DRAWING_DIRECTION_225
        jmp     short mm_int_output_radial

        public  mm_int_radial_315
mm_int_radial_315::
        mov     ebp,DRAW_LINE + DRAW + DIR_TYPE_RADIAL + WRITE + \
                    LAST_PIXEL_OFF + MULTIPLE_PIXELS + DRAWING_DIRECTION_315
        jmp     short mm_int_output_radial

        public  mm_int_radial_90_270
mm_int_radial_90_270::
        mov     ebp,DRAW_LINE + DRAW + DIR_TYPE_RADIAL + WRITE + \
                    LAST_PIXEL_OFF + MULTIPLE_PIXELS + DRAWING_DIRECTION_270
        mov     esi,edi
        sub     esi,ecx
        jg      short mm_int_output_radial ;if top-to-bottom vertical line
        jz      mm_next_line               ;if zero length line
...
; ebp = drawing command
        ...
        public  mm_int_output_radial
mm_int_output_radial::
        mov     edi,[esp].sf_pjMmBase   ;edi = ppdev->pjMmBase
        cmp     [esp].sf_bSetCP,0
        je      short mm_int_radial_continue_figure
...
@@:     in      ax,dx
        ...
        add     ebx,[esp].sf_xOffset
        mov     [edi+CUR_X],bx          ;MM_CUR_X(x)

        add     ecx,[esp].sf_yOffset
        mov     [edi+CUR_Y],cx          ;MM_CUR_Y(y)

        mov     [esp].sf_bSetCP,0       ;the next integer line in this figure
                                        ; will have the CP set correctly

        mov     [edi+MAJ_AXIS_PCNT],si  ;MM_MAJ_AXIS_PCNT(dx)

        mov     [edi+CMD],bp            ;MM_CMD(ulCmd)

        jmp     mm_next_line

; Jump to here if we don't have to update the current position first:

        public  mm_int_radial_continue_figure
mm_int_radial_continue_figure::
        mov     edx,[esp].sf_ioGp_stat_cmd
@@:     in      ax,dx
        and     eax,FIFO_2_EMPTY        ;IO_FIFO_WAIT(2)
        jnz     short @b
...
;/////////////////////////////////////////////////////////////////////
        ...
        public  mm_non_integer
mm_non_integer::
        sub     esi,ebx
        jl      mm_not_int_2_3_4_5
        sub     edi,ecx
        jl      mm_not_int_6_7
        cmp     esi,edi
        jl      mm_not_int_1
        je      mm_not_int_0_slope_one
...
mm_not_int_0_common:
        ...
        public  mm_not_int_x_major
mm_not_int_x_major::
        mov     ecx,[esp].sf_pjMmBase   ;ecx = ppdev->pjMmBase
        mov     edx,[esp].sf_ioGp_stat_cmd
@@:     in      ax,dx
        and     eax,FIFO_7_EMPTY        ;IO_FIFO_WAIT(7)
        jnz     short @b
...
; position already set correctly, so we explicitly set the CP each time:
        ...
        mov     eax,[esp].sf_cPels
        mov     [ecx+MAJ_AXIS_PCNT],ax  ;MM_MAJ_AXIS_PCNT(cPels)

        mov     [ecx+AXSTP],di          ;MM_AXSTP(dN)

        sub     edi,esi
        mov     [ecx+DIASTP],di         ;MM_DIASTP(dN - dM)

        mov     eax,[esp].sf_ulErrorTerm
        mov     [ecx+ERR_TERM],ax       ;MM_ERR_TERM(ulErrorTerm)

        mov     eax,[esp].sf_ulCmd
        mov     [ecx+CMD],ax            ;MM_CMD(ulCmd)
...
; and y=1/2 rounding down:
        ...
        public  mm_not_int_0_slope_one
mm_not_int_0_slope_one::
        or      esi,esi
        jz      mm_next_line            ;quick check for a zero length GIQ line

        cmp     esi,MAX_GIQ_DELTA
        jg      mm_punt_line

        GIQ     ROUND_Y_DOWN_SLOPE_ONE
        jmp     mm_not_int_0_common
...
; position already set correctly, so we explicitly set the CP each time:
        ...
        mov     [ecx+AXSTP],si          ;MM_AXSTP(dM)

        sub     esi,edi
        mov     [ecx+DIASTP],si         ;MM_DIASTP(dM - dN)
...
mm_not_int_2::
        ...
        jmp     mm_not_int_y_major

        public  mm_not_int_4_5
mm_not_int_4_5::
        neg     edi                     ;dN = -dN (now positive)
        neg     ecx                     ;N0 = -N0
        cmp     esi,edi
        jl      mm_not_int_5
        je      mm_not_int_4_slope_one
...
; Lines of slope one have a special rounding rule.
        ...
        public  mm_not_int_4_slope_one
mm_not_int_4_slope_one::
        cmp     esi,MAX_GIQ_DELTA
        jg      mm_punt_line

        GIQ     ROUND_X_DOWN_SLOPE_ONE
        jmp     mm_not_int_4_common
...
mm_not_int_5::
        ...
        public  mm_not_int_6_7
mm_not_int_6_7::
        neg     edi                     ;dN = -dN (now positive)
        neg     ecx                     ;M0 = -M0
        cmp     esi,edi
        je      mm_not_int_7_slope_one
        jg      mm_not_int_7
...
mm_not_int_7_common:
        ...
        public  mm_not_int_7_slope_one
mm_not_int_7_slope_one::
        cmp     esi,MAX_GIQ_DELTA
        jg      mm_punt_line

        GIQ     ROUND_X_DOWN_SLOPE_ONE
        jmp     mm_not_int_7_common
