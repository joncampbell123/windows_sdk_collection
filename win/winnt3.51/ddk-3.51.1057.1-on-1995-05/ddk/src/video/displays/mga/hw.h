...
#define     opcode_RESERVED_1       0x00000005
#define     opcode_RESERVED_2       0x00000006
#define     opcode_RESERVED_3       0x00000007
#define     opcode_BITBLT           0x00000008
#define     opcode_ILOAD            0x00000009
#define     opcode_IDUMP            0x0000000a
#define     opcode_RESERVED_4       0x0000000b
#define     opcode_RESERVED_5       0x0000000c
#define     opcode_RESERVED_6       0x0000000d
#define     opcode_RESERVED_7       0x0000000e
#define     opcode_RESERVED_8       0x0000000f
...
#define     pwidth_RESERVED         0x00000003
...
#define     plnwt_MASK_24BPP        0x00ffffff
...
#define TVP3026_PAL_ADDR_WR         ExtDev+RamDac+0x000  //Palette RAM Address Write
#define TVP3026_CUR_ADDR_WR         ExtDev+RamDac+0x000  //Cursor RAM Address Write
#define TVP3026_INDIRECT_INDEX      ExtDev+RamDac+0x000  //Indirect Index
#define TVP3026_PAL_DATA            ExtDev+RamDac+0x004  //Palette RAM Data
#define TVP3026_PIX_RD_MSK          ExtDev+RamDac+0x008  //Pixel Read Mask
#define TVP3026_PAL_ADDR_RD         ExtDev+RamDac+0x00c  //Palette RAM A.address Read
#define TVP3026_CUR_ADDR_RD         ExtDev+RamDac+0x00c  //Cursor RAM Address Read
#define TVP3026_CUR_COLOR_ADDR_WR   ExtDev+RamDac+0x010  //Cursor Color Address Write
#define TVP3026_OVS_COLOR_ADDR_WR   ExtDev+RamDac+0x010  //Overscan Color Address Write
#define TVP3026_CUR_COLOR_DATA      ExtDev+RamDac+0x014  //Cursor Color Data
#define TVP3026_OVS_COLOR_DATA      ExtDev+RamDac+0x014  //Overscan Color Data
#define TVP3026_RESERVED_0          ExtDev+RamDac+0x018  //Reserved
#define TVP3026_CUR_COLOR_ADDR_RD   ExtDev+RamDac+0x01c  //Cursor Color Address Read
#define TVP3026_OVS_COLOR_ADDR_RD   ExtDev+RamDac+0x01c  //Overscan Color Address Read
#define TVP3026_RESERVED_1          ExtDev+RamDac+0x020  //Reserved
#define TVP3026_RESERVED_2          ExtDev+RamDac+0x024  //Reserved
#define TVP3026_INDEXED_DATA        ExtDev+RamDac+0x028  //Indexed Data
#define TVP3026_CUR_DATA            ExtDev+RamDac+0x02c  //Cursor RAM Data
#define TVP3026_CUR_X_LSB           ExtDev+RamDac+0x030  //Cursor Position X LSB
#define TVP3026_CUR_X_MSB           ExtDev+RamDac+0x034  //Cursor Position X MSB
#define TVP3026_CUR_Y_LSB           ExtDev+RamDac+0x038  //Cursor Position Y LSB
#define TVP3026_CUR_Y_MSB           ExtDev+RamDac+0x03c  //Cursor Position Y MSB
...
#define CP_READ(pjBase, addr)                                       \
    ...
    _inline CHAR GET_FIFO_SPACE(BYTE* pjBase)                       \
    {                                                               \
        CP_EIEIO();                                                 \
    ...
    // The Alpha has a nifty feature where it caches I/O and will
    // 'collapse' writes to the same address (see the CP_MEMORY_BARRIER()
    // note, above).  When we're doing monochrome or bitmap transfers
    // to the MGA's pseudo-DMA area, we typically are constantly writing
    // to the same address in the DMA space.  To avoid having to do a
    // memory barrier after every write, we simply use a different address
    // in the window every time (since they're all equivalent):

    #define ALPHA_MASK 0xff

    #define CP_WRITE_DMA(ppdev, pjDma, dw)                          \
        CP_WRITE((pjDma), ((ppdev->cjDmaOffset += 4) & ALPHA_MASK), (dw))
    ...
    // Neither the x86, Mips or PowerPC have problems doing I/O to the
    // same address:
...
#define START_DIRECT_ACCESS(pjBase)                             \
    CP_EIEIO()

#define END_DIRECT_ACCESS(pjBase)                               \
    CP_EIEIO()
...
{                                                               \
    do {} while ((CP_READ_STATUS(pjBase) & (dwgengsts_MASK >> 16)) != 0); \
}
    ...
    {                                                                   \
        LONG    i      = (LONG) (cdSrc) >> 2;                           \
        ULONG*  pulSrc = (ULONG*) (pjSrc);                              \
                                                                        \
        CP_MEMORY_BARRIER();                                            \
        while (i-- != 0)                                                \
        {                                                               \
            CP_WRITE_DIRECT(pjBase, DWG_SRC0, *(pulSrc));               \
            CP_WRITE_DIRECT(pjBase, DWG_SRC1, *(pulSrc + 1));           \
            CP_WRITE_DIRECT(pjBase, DWG_SRC2, *(pulSrc + 2));           \
            CP_WRITE_DIRECT(pjBase, DWG_SRC3, *(pulSrc + 3));           \
            pulSrc += 4;                                                \
            CP_MEMORY_BARRIER();                                        \
        }                                                               \
                                                                        \
        ...
        case 3:                                                         \
            CP_WRITE_DIRECT(pjBase, DWG_SRC0, *pulSrc++);               \
        case 2:                                                         \
            CP_WRITE_DIRECT(pjBase, DWG_SRC1, *pulSrc++);               \
        case 1:                                                         \
            CP_WRITE_DIRECT(pjBase, DWG_SRC2, *pulSrc);                 \
        }                                                               \
    }
