...
typedef struct
{
    // FLONG  fl;     // flags, for future expansion, regulate if this font should be sent to printer on a per job basis
    WCHAR *pwcPFB;    // path to pfb file, may not be full path if in system dir
    NTFM  *pntfm;     // pointer to ntfm structure for this font;
} SOFTFONTENTRY, *PSOFTFONTENTRY;

// flags for softlist, dirty when out of date.

#define SOFTNODE_DIRTY   1


typedef struct _SOFTNODE
{
    struct _SOFTNODE *psnodeNext;
    FILETIME       ftLastWriteTime;
    DWORD          cPDEV;           // reference count, # of pdevs currently using this softlist
    DWORD          cSoftFonts;
    FLONG          fl;
    SOFTFONTENTRY  asfe[1];
} SOFTNODE;
...
typedef struct _PARSEDATA
{
    ...
    char        rgbLine[PPD_LINEBUF_SIZE];       // The current line of text being processed
