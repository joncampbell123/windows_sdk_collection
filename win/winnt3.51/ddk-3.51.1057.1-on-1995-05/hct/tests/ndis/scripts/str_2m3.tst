...
friendly_script_name "Two Machine Stress Tests with Test Card as Client/Server"

print "TITLE:  2 machine stress:  test card is client/server, other card is server\n"
...
#
# Open test card, which will be the stress client card
#
...
#
# server will act as stress test server
#
...
while ($Count < 10)
{
   ...
   $result = startstressserver $G_ServerOpen
   ...
   $result = startstress $ClientOpen $PacketCount[$Count] $Iterations[$Count] $PacketSize[$Count] 0 $OptionMask[$Count]
   if ($result == TEST_SUCCESSFUL)
   {
      $result = waitstress $ClientOpen
      if ($result != TEST_SUCCESSFUL)
      {
         fail_variation
         print "FAILURE:  failure in waitstress"
      }
      $result = getstressresults $ClientOpen
   ...
   $result = stopstress $G_ServerOpen
   if ($result != TEST_SUCCESSFUL)
   {
      fail_variation
