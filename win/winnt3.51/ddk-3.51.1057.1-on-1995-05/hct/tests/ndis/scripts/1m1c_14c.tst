...
# File 1m1c_14c.tst
#
# This script tests the ability to send, receive, and resend group
# packets with various receive filters set..
###############################################################


friendly_script_name "Single Open Send/Receive/Resend Group Tests"

print "TITLE:  Test send, receive, and resend with group packets\n"
...
$result = setgroup $OpenInstance $FuncAddress
if ($result != TEST_SUCCESSFUL)
{
   block_variation
   print "BLOCKED:  failed to set group address"
   close $OpenInstance
   goto endvar0
}

#------------------------------------------------
# variation:   send group packets of various sizes with
...
$result = setgroup $OpenInstance $NullAddress
if ($result != TEST_SUCCESSFUL)
{
   fail_variation
   print "FAILURE:  failed to clear group address"
}
