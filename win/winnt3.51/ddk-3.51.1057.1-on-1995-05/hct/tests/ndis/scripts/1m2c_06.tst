...
friendly_script_name "Two Card Send/Receive/Resend Broadcast Tests"

print "TITLE:  Test send, receive, resend with broadcast packets, 2 cards on same machine\n"
      ...
      while ($Count < $MaxTests)
      {
         ...
         if ($result == TEST_SUCCESSFUL)
         {
            ...
            while (TRUE)
            {
               ...
               $result = send $OpenOne $BroadcastAddr $PacketSize $NumPackets $ResendAddr
               if ($result != TEST_SUCCESSFUL)
               {
                  fail_variation
