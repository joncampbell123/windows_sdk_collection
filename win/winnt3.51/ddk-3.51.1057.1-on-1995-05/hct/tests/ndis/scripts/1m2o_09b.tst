...
# FILE:  1m2o_09b.tst
#
# These tests verify the ability to send functionally-addressed packets from
# one open instance and receive them on the other, with various
# packet filters set
#######################################################

friendly_script_name "Two Open Send/Receive Functional Tests"

print "TITLE:  Test ability to receive functional packets sent from other open instance\n"
...
# combine these with group filter to save time...
...
while ($Count < $MaxTests)
{
   ...
   if ($result == TEST_SUCCESSFUL)
   {
      ...
      {
         #-----------------------------------
         # no functional address set--open 2 should receive when PROMISCUOUS, ALLFUNCTIONAL
         #-----------------------------------
         ...
         #--------------------------------------------------
         # wrong functional address -- open 2 should receive when PROMISCUOUS and ALLFUNCTIONAL
         #--------------------------------------------------
