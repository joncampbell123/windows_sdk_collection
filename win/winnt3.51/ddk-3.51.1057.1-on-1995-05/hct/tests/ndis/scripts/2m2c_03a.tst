...
# FILE:  2m2c_03a.tst
#
# These tests verify the ability to receive multicast packets sent
# from card on another machine, with various
# packet filters set
#######################################################

friendly_script_name "Two Machine Send/Receive Multicast Tests"

print "TITLE:  Test ability to receive multicast packets sent by another card\n"
...
while ($Count < $MaxTests)
{
   ...
   if ($result == TEST_SUCCESSFUL)
   {
      ...
      while (TRUE)
      {
         ...
         $result = send $G_ServerOpen $GoodMulticast $PacketSize $NumPackets
         if ($result != TEST_SUCCESSFUL)
         {
            fail_variation
         ...
         $result = addmulticast $ClientOpen $GoodMulticast
         if ($result != TEST_SUCCESSFUL)
         {
            fail_variation
         ...
         $result = send $G_ServerOpen $BadMulticast $PacketSize $NumPackets
         if ($result != TEST_SUCCESSFUL)
         {
            fail_variation
            print "FAILURE:  send failed to start"
         ...
         #
         $result = deletemulticast $ClientOpen $GoodMulticast
         if ($result != TEST_SUCCESSFUL)
         {
            warn_variation
            print "WARNING:  unable to delete multicast address"
         }
         ...
         {
            if (($CurFilter & MULTICAST) || ($CurFilter == PROMISCUOUS) || ($CurFilter == ALLMULTICAST))
            {
