...
# FILE:  1m2o_08.tst
#
# These tests verify the ability to send broadcast packets from
# one open instance and receive them on the other, with various
# packet filters set
#######################################################

friendly_script_name "Two Open Send/Receive Broadcast Tests"

print "TITLE:  Test ability to receive broadcast packets sent from other open instance\n"
...
while ($Count < $MaxTests)
{
   ...
   if ($result == TEST_SUCCESSFUL)
   {
      ...
      while (TRUE)
      {
         ...
         {
            fail_variation
            print "FAILURE:  stopreceive failed"
         }
         else
         {
