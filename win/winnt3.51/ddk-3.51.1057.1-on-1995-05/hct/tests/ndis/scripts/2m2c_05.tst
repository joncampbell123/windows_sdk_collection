...
friendly_script_name "Two Machine Send/Receive/Resend Directed Tests"

print "TITLE:  Test send, receive, and resend with directed packets between two cards\n"
      ...
      while ($Count < $MaxTests)
      {
         ...
         if ($result == TEST_SUCCESSFUL)
         {
            ...
            while (TRUE)
            {
               ...
               $result = send $G_ServerOpen $G_TestAddress $PacketSize $NumPackets $ResendAddr
               if ($result != TEST_SUCCESSFUL)
               {
                  fail_variation
