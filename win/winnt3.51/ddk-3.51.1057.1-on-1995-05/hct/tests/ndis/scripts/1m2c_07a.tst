...
# packets between two different cards on the same machine
#######################################################

friendly_script_name "Two Card Send/Receive/Resend Multicast Tests"

print "TITLE:  Test send, receive, and resend with multicast packets, 2 cards in one machine\n"
      ...
      while ($Count < $MaxTests)
      {
         ...
         if ($result == TEST_SUCCESSFUL)
         {
            ...
            while (TRUE)
            {
               ...
               $result = send $OpenOne $MulticastAddr $PacketSize $NumPackets $ResendAddr
               if ($result != TEST_SUCCESSFUL)
               {
                  fail_variation
