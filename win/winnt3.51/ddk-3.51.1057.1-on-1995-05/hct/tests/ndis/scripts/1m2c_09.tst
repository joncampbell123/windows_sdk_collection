...
friendly_script_name "Multiple Open Receive Directed Tests"

print "TITLE:  Test ability to receive directed packets with multiple different opens\n"
...
elseif ($G_Media == MEDIUM_TOKENRING)
{
   ...
   $Filters[11] = DIRECTED | BROADCAST | FUNCTIONAL
   $Filters[12] = DIRECTED | BROADCAST | GROUP

   if ($G_Filters & PROMISCUOUS)
   {
      $Filters[13] = PROMISCUOUS
      $Filters[14] = PROMISCUOUS | DIRECTED
      $Filters[15] = PROMISCUOUS | BROADCAST
      $Filters[16] = PROMISCUOUS | FUNCTIONAL
      $Filters[17] = PROMISCUOUS | GROUP
      $Filters[18] = PROMISCUOUS | BROADCAST | DIRECTED | FUNCTIONAL
      $MaxLoops = 12
   }
   else
   {
...
else                 #if $G_Media == MEDIUM_ETHERNET || $G_Media == MEDIUM_FDDI
{
   ...
   $Filters[7]  = DIRECTED | BROADCAST | MULTICAST

   if ($G_Filters & ALLMULTICAST)
   {
      $Filters[8]  = ALLMULTICAST
      $Filters[9]  = ALLMULTICAST | DIRECTED
      $Filters[10] = ALLMULTICAST | BROADCAST
      $Filters[11] = ALLMULTICAST | MULTICAST
      ...
      if ($G_Filters & PROMISCUOUS)
      {
         ...
         $Filters[16] = PROMISCUOUS | ALLMULTICAST
   ...
   else     # no allmulticast
   {
      if ($G_Filters & PROMISCUOUS)
      {
         ...
         $Filters[12] = PROMISCUOUS | DIRECTED | BROADCAST | MULTICAST
         $MaxLoops = 6
      }
      else
      {
...
while ($Count < $MaxLoops)
{
   ...
   while (TRUE)
   {
      ...
      $result = send $SendOpen $G_TestAddress $PacketSize $NumPackets
      if ($result != TEST_SUCCESSFUL)
      {
         fail_variation
