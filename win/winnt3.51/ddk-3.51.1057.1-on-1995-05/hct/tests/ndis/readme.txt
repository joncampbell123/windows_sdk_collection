...
is basically a script interpreter used in much of our internal testing.
    ...
    nd2mach.bat  -- used to run just the 2-machine tests
    ndperf.bat   -- used to run just the performance tests
    ...
    nd2mach.tst  -- scripts file called by nd2mach.bat, which runs just the
                    2 machine functional and stress tests.
    perf.tst     -- main script for performance tests.
    ...
    perf*.tst    -- performance test scripts
...
"ndfunc", "nd2mach", and "ndstress" are run the same as ndtest.  "repro" 
is similar, except that you must first enable the desired test(s) by un-
commenting it/them in file scripts\repro.tst.  '#' is the script file comment
character.  Just remove the '#' before the appropriate tests, then run
"repro".

The performance test script "ndperf" may be run with either just the test
card, or with both the test and trusted card.  It is recommended that both
test and trusted card be used.  The server must be started first.  The 
performance tests will not run unless the server can be contacted.
...
Some cards hang during the performance tests.  For this reason, the
performance tests are currently commented out in script ndis.tst.
We recommend that you don't run the performance tests until your card
passes both the functional and stress tests.  Use "ndperf.bat" to run
the performance tests.  If the performance tests hang for you, please
let us know where--what script, and what variation.
