...

{
    ...
    if (IbmtokDbg)
                    DbgPrint("IBMTOK: Offsets: SRB %x  SSB %x  ARB %x  ASB %x\n",
                                IBMSHORT_TO_USHORT(OpenResponseSrb->SrbPointer),
                                IBMSHORT_TO_USHORT(OpenResponseSrb->SsbPointer),
                                IBMSHORT_TO_USHORT(OpenResponseSrb->ArbPointer),
                                IBMSHORT_TO_USHORT(OpenResponseSrb->AsbPointer));
#endif
...
#if DBG
                    if (IbmtokDbg) DbgPrint("IBMTOK: Command %x, code %x\n",
                            GenericSrb->Command, GenericSrb->ReturnCode);
#endif
...

{
    ...
    if (IbmtokDbg)
                    DbgPrint("IBMTOK: DLC Status %x\n",
                            IBMSHORT_TO_USHORT(
                                ((PARB_DLC_STATUS)Adapter->ArbAddress)->Status));
#endif
                ...
                case ARB_CMD_RECEIVED_DATA:
                    ...
                    if (FrameData[8] & 0x80) {
                        ...
                        HeaderLength = (FrameData[14] & 0x1f) + 14;
    ...
    if (IbmtokDbg)   DbgPrint("W_ASB R\n");
                        ...
                        if (Adapter->ReceiveWaitingForAsbEnd == (USHORT)(-1)){
                            ...
                            *((SRAM_PTR *)PEnd) = ReceiveBufferPointer;
                ...
                case ARB_CMD_TRANSMIT_DATA_REQUEST:
                    ...
                    if (TransmitPacket == NULL) {
                        ...
                        // are we getting this ARB request??
                        //

                        ASSERT(TransmitPacket != (PNDIS_PACKET)NULL);

                        //
                        // This will be FALSE if the transmit SRB has not
                        // yet been processed, but if so why are we
                        // getting this ARB request??
                        //
                        ASSERT(PacketRemoved);
    ...
    if (IbmtokDbg) DbgPrint("ASB R\n");
                ...
                if (Adapter->ReceiveWaitingForAsbList == Adapter->ReceiveWaitingForAsbEnd){
                    ...
                    Adapter->ReceiveWaitingForAsbList = *((SRAM_PTR *)PFront);
...
if (IbmtokDbg) DbgPrint("IBMTOK: RESET done\n");
            ...
            OpenSrb->Command = SRB_CMD_OPEN_ADAPTER;
            OpenSrb->OpenOptions = OPEN_CONTENDER;
...
if (IbmtokDbg) DbgPrint("IBMTOK: OPEN done\n");
            ...
            if (OpenResponseSrb->ReturnCode != 0) {
                ...
                while (CurrentLink != &(Adapter->OpenBindings)){
                    ...
                    NdisIndicateStatus(TempOpen->NdisBindingContext,
                                       NDIS_STATUS_CLOSED,
                                       (PVOID)&IndicateStatus,
                                       sizeof(IndicateStatus)
                                      );
...
#if DBG
if (IbmtokDbg) DbgPrint("IBMTOK: Offsets: SRB %x  SSB %x  ARB %x  ASB %x\n",
                        IBMSHORT_TO_USHORT(OpenResponseSrb->SrbPointer),
                        IBMSHORT_TO_USHORT(OpenResponseSrb->SsbPointer),
                        IBMSHORT_TO_USHORT(OpenResponseSrb->ArbPointer),
                        IBMSHORT_TO_USHORT(OpenResponseSrb->AsbPointer));
...
#if DBG
if (IbmtokDbg) DbgPrint("IBMTOK: Transmit failed in SRB: %x for %x\n",
                TransmitSrb->ReturnCode,
                TransmitSrb->Command);
#endif
...
#if DBG
if (IbmtokDbg) DbgPrint("IBMTOK: SRB corr %x\n", TransmitSrb->CommandCorrelator);
#endif
...
--*/
{
    ...
    StatsSrb->Command = SRB_CMD_DLC_STATISTICS;
    StatsSrb->StationId = USHORT_TO_IBMSHORT(0x00);
    StatsSrb->ReturnCode = 0x80;                      // Resets counters
...

{
    ...
    Counters = (PDLC_COUNTERS) (((PUCHAR)(Adapter->SrbAddress)) +
                                IBMSHORT_TO_USHORT(StatsSrb->CountersOffset));

    Adapter->FramesTransmitted += IBMSHORT_TO_USHORT(Counters->TransmitCount);
    Adapter->FramesReceived += IBMSHORT_TO_USHORT(Counters->ReceiveCount);
    Adapter->FrameTransmitErrors += Counters->TransmitErrors;
    Adapter->FrameReceiveErrors += Counters->ReceiveErrors;
...

{
    ...
    Adapter->LineErrors += ReadLogSrb->LineErrors;
    Adapter->InternalErrors += ReadLogSrb->InternalErrors;
    Adapter->BurstErrors += ReadLogSrb->BurstErrors;
    Adapter->AcErrors += ReadLogSrb->AcErrors;
    Adapter->AbortDelimeters += ReadLogSrb->AbortDelimeters;
    Adapter->LostFrames += ReadLogSrb->LostFrames;
    Adapter->ReceiveCongestionCount += ReadLogSrb->ReceiveCongestionCount;
    Adapter->FrameCopiedErrors += ReadLogSrb->FrameCopiedErrors;
    Adapter->FrequencyErrors += ReadLogSrb->FrequencyErrors;
    Adapter->TokenErrors += ReadLogSrb->TokenErrors;
}

STATIC
VOID
...
--*/
{
    ...
    ReadLogSrb->Command = SRB_CMD_READ_LOG;
}

STATIC
NDIS_STATUS
...
--*/
{
    ...
    if ((Adapter->WakeUpTimeout) &&
        ...
        if (Adapter->TransmittingPacket != NULL) {
            ...
            TmpOpen = PIBMTOK_OPEN_FROM_BINDING_HANDLE(Reserved->MacBindingHandle);
