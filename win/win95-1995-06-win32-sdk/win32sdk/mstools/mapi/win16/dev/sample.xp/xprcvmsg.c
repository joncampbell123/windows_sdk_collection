...
HrIMsgFromTextMsg(LPXPL lpxpl, LPSPropValue lpPropArray, LPMESSAGE lpMessage, LPSTREAM lpSof)
{
    ...
    while (TRUE)
    {
        /* fHaveTagAndToken gets set only when we return from
           HrGetStreamedProp (because this call actually gets
    ...
    LPTSTR * lppszToken)
{
    ...
    if (hr)
    {
        sc = lpxpl->AllocateMore(lstrlen(lpszLine) + 1, lpMsgProps, &lpsz);
    ...
    if (ulPropTag == PR_SUBJECT)
    {
        ...
        {
            TraceFailedWrite(lpStrm->lpVtbl->Write(lpStrm, lpszLine,
