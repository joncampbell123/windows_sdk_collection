...
WizardPage_INITDIALOG (HWND hdlg, HWND hwndFocus, LPARAM lParam)
{
    ...
    if (!FAILED (MAPIInitialize ((LPMAPIINIT)&mapiinit)) &&
        !HR_FAILED (lpmpWiz->lpVtbl->GetProps (lpmpWiz,
                                        ...
                                        &lpvalWiz)))
    {
        ulFlags = (lpvalWiz[ipSMHFlags].ulPropTag == PR_SMH_FLAGS)
            ? lpvalWiz[ipSMHFlags].Value.l
            : 0;
        ...
        lpvalWiz[ipSMHFlags].ulPropTag = PR_SMH_FLAGS;
        lpvalWiz[ipSMHFlags].Value.l = ulFlags;
      ...
      case WIZ_NEXT:
        ...
        switch (ipgCur)
        {
          ...
          case cpgMax:
            ...
            {
                lpvalWiz[ipSMHFlags].Value.l |= SMH_FILTER_SENTMAIL;
                if (IsDlgButtonChecked (hdlg, ID_SentMailYr))
                    lpvalWiz[ipSMHFlags].Value.l |= SMH_FILTER_SENTMAIL_YR;
                else
                    lpvalWiz[ipSMHFlags].Value.l &= ~SMH_FILTER_SENTMAIL_YR;
            }
            else
                lpvalWiz[ipSMHFlags].Value.l &= ~(SMH_FILTER_SENTMAIL | SMH_FILTER_SENTMAIL_YR);

            if (IsDlgButtonChecked (hdlg, ID_Deleted))
            {
                lpvalWiz[ipSMHFlags].Value.l |= SMH_FILTER_DELETED;
                if (IsDlgButtonChecked (hdlg, ID_DeletedYr))
                    lpvalWiz[ipSMHFlags].Value.l |= SMH_FILTER_DELETED_YR;
                else
                    lpvalWiz[ipSMHFlags].Value.l &= ~SMH_FILTER_DELETED_YR;
            }
            else
                lpvalWiz[ipSMHFlags].Value.l &= ~(SMH_FILTER_DELETED | SMH_FILTER_DELETED_YR);

            if (IsDlgButtonChecked (hdlg, ID_Inbound))
                lpvalWiz[ipSMHFlags].Value.l |= SMH_FILTER_INBOUND;
            else
                lpvalWiz[ipSMHFlags].Value.l &= ~SMH_FILTER_INBOUND;

            if (IsDlgButtonChecked (hdlg, ID_Unread))
                lpvalWiz[ipSMHFlags].Value.l |= SMH_UNREAD_VIEWER;
            else
                lpvalWiz[ipSMHFlags].Value.l &= ~SMH_UNREAD_VIEWER;
