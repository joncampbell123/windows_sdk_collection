...

{
    ...
    switch( DataType ) {
        case DMP_EXCEPTION:
            *DumpData = &CrashdumpInfo->ExceptionInfo->ExceptionRecord;
            *DumpDataLength = sizeof(EXCEPTION_RECORD);
            break;
        ...
        case DMP_THREAD_CONTEXT:
            if (!CrashdumpInfo->ptctx) {
                return FALSE;
            }
            if (!CrashdumpInfo->ptctx->next) {
                return FALSE;
            }
            *DumpData = &CrashdumpInfo->ptctx->context;
            *DumpDataLength = sizeof(CONTEXT);
            CrashdumpInfo->ptctx = CrashdumpInfo->ptctx->next;
            break;
...

{
    ...
    CrashdumpInfo.ptctx = dp->tctxHead;
    SymGetModuleInfo( dp->hProcess, 0, &CrashdumpInfo.mi );
