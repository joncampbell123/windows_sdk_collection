...
PostMortemDump( PDEBUGPACKET dp, LPEXCEPTION_DEBUG_INFO ed )
{
    ...
    if (SymGetModuleInfo( dp->hProcess, 0, &mi )) {
        strcpy( crash.szAppName, mi.ImageName );
    }
...
#error( "unknown target machine" );
    ...
    for (i=0; i<100; i++) {
        ...
        psym = SymGetSymFromAddr( dp->hProcess, stk.AddrPC.Offset, &dwDisplacement );
        if (psym) {
            szSymName = SymUnDName( psym );
            if (!szSymName) {
        ...
        if (psym && (psym->flags & SYMF_OMAP_GENERATED || psym->flags & SYMF_OMAP_MODIFIED)) {
            lprintfs( "[omap] " );
        }
...
LogDisassembly( PDEBUGPACKET dp, PCRASHES pCrash )
{
    ...
    psym = SymGetSymFromAddr( dp->hProcess, dp->tctx->pc, &dwDisplacement );
