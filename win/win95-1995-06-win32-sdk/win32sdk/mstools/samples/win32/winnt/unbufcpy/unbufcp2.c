    ...
    )
{
    ...
    //
    InitialFileSize.QuadPart = (FileSize.QuadPart + PageSize - 1) & ~(PageSize - 1);
    ...
    //
    if (ver.dwMinorVersion >= 51)                                //we are running on NT 3.51 or greater
      //                                                       
      ...
      ReadPort = CreateIoCompletionPort(INVALID_HANDLE_VALUE,    //file handle to associate with I/O completion port
                                        ...
                                        (DWORD)SourceFile,       //completion key
                                        1);                      //# of threads allowed to execute concurrently

    else if (ver.dwMinorVersion == 50)                           //we're running on NT 3.5 - Completion Ports exists                       
    ...
    //completion port.
    //
    if (ver.dwMinorVersion >= 51)
      {
        ReadPort = CreateIoCompletionPort(SourceFile,
                                          ReadPort,
                                          (DWORD)SourceFile,  //should be the previously specified key.
                                          1);
    ...
    )
{
    ...
    //
    //Check to see if we've copied the complete file, if so decrement PendingIO
    // 
    if (CompletedOverlapped->InternalHigh >= FileSize.QuadPart)
      --PendingIO;
    }
}
