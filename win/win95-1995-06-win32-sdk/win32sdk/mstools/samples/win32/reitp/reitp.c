...
WORD wVertical = 0;
static DWORD dwFE = 0;
static CHAR	szLead[512]="";
static CHAR	szFollow[512]="";
static CHAR	szWord[128];
#endif
...
#ifdef DBCS
	240,				// height, 360 twips == 18 points
#else	// DBCS
...
#ifdef DBCS
static CHAR *szJpnFont = "‚l‚r –¾’©";
static CHAR *szKorFont = "±¼¸²Ã¼";
#endif	// DBCS
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_MOVE:
		if(predoc->hwndRE)
		{
			SendMessage(predoc->hwndRE, WM_MOVE, wparam, lparam );
		}
		break;
	...
	case WM_IME_REPORT:
		if(predoc->hwndRE)
		{
			SendMessage(predoc->hwndRE, WM_IME_REPORT, wparam, lparam );
		}
		break;
#endif	// DBCS
...
#endif	// DEBUG
#ifdef DBCS
		CheckMenuItem(hmenu, IDM_VERTICAL, uiVertical);
#endif	// DBCS
	}
		break;
	...
	case IDM_LANGINFO:
		{
			CHAR 	sz[256];
			ULONG	ul = UlGGetLang();
			wsprintf(sz, "LANG ID = %X \r\nCODE PAGE = %u", HIWORD(ul), LOWORD(ul));
			MessageBox(hwndMain, sz, "Language", MB_OK);
		}
		break;
#endif
	case IDM_VERTICAL:
	case TBI_FE_Vertical:
		SendMessage(predoc->hwndRE, EM_SETOPTIONS, ECOOP_XOR, ECO_VERTICAL);
		ToggleCheck(predoc, IDM_VERTICAL);
		break;

	case IDM_WORDREG:
		{
		LRESULT lRst;
		lRst = SendMessage(predoc->hwndRE, EM_GETSEL, 0, 0L);
		if ((HIWORD(lRst) - LOWORD(lRst)) < 128 && lRst != -1) 
			SendMessage(predoc->hwndRE, EM_GETSELTEXT, 0, (LPARAM)szWord);
		else
			lstrcpy(szWord, TEXT(""));

		FIMEWordRegister(predoc->hwndRE, szWord, NULL);
		}
		break;
...
#ifdef DBCS
	LANGID	LangID;
	LangID = HIWORD(UlGGetLang());
	switch(LangID)
	{
		case LangJpn:
			lstrcpy(cfDefault.szFaceName, szJpnFont);
			cfDefault.bCharSet = SHIFTJIS_CHARSET;
			break;
		case LangKor:
			lstrcpy(cfDefault.szFaceName, szKorFont);
			cfDefault.bCharSet = HANGEUL_CHARSET;
			break;
	}
#endif	// DBCS
	if(predoc->hwndRE)
				...
				HGLOBAL hMetaPict)
{
	...
	if(pitpcall->predoc->fReadOnly)
	{
		...
		if(fDoIt)
		{
			if(fReally)
			{
				...
				hr = preole->lpVtbl->ImportDataObject(preole, pdataobj,
														*pcfFormat, hMetaPict);
				TraceError("IRichEditOle::ImportDataObject", GetScode(hr));
				if(SUCCEEDED(GetScode(hr)))
					hr = ResultFromScode(S_FALSE);	// we did it ourselves
			}
			else
			{
		...
		TraceError("IPCALL_QueryAcceptData", GetScode(hr));
		return hr;
	}
		...
		if(fDoIt)
		{
			...
			*ppdataobj = NULL;
			hr = preole->lpVtbl->GetClipboardData(preole, pchrg, reco,
													ppdataobj);
			TraceError("IRichEditOle::GetClipboardData", GetScode(hr));
			if(SUCCEEDED(GetScode(hr)) && *ppdataobj)
				hr = ResultFromScode(S_FALSE);	// we did it ourselves
			TraceError("IPCALL_GetClipboardData", GetScode(hr));
			return hr;
		}
