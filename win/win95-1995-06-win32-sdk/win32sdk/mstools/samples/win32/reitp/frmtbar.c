...
static TBBUTTON rgtbbutton[] =
{
	...
	{ 0, 0L, TBSTATE_ENABLED, TBSTYLE_SEP, 0, 0, 0L, -1},
#ifdef DBCS
	{ tbVertical, TBI_FE_Vertical, TBSTATE_ENABLED, TBSTYLE_BUTTON, 0, 0, 0L, -1}, 
	{ 0, 0L, TBSTATE_ENABLED, TBSTYLE_SEP, 0, 0, 0L, -1}
#endif
};
...
#ifdef DBCS
extern TAG tagReDBCS;
extern WORD wVertical;
#endif
								...
								INT nFontType, LPARAM lParam)
{
	...
	if (!(lr == CB_ERR || lr == CB_ERRSPACE))
	{
		...
		dw |= nFontType & 0xFFFF;
#ifdef DBCS
		TraceTag(tagReDBCS, "Font:%s : CHARSET:%u",plf->lfFaceName, plf->lfCharSet);
#endif
...
LOCAL void FB_OnCommand(HWND hwnd, INT nID, HWND hwndCtl, INT nNotify)
{
...
#ifdef DBCS
		TraceTag(tagReDBCS, ">Font:%s : CHARSET:%u",pfbs->cf.szFaceName, pfbs->cf.bCharSet);
#endif
			}
	...
	case TBI_FE_Vertical:
		if (wVertical)
			...
			wVertical = TRUE;
		SendMessage(pfbs->hwndToolbar, TB_CHECKBUTTON, TBI_FE_Vertical, MAKELONG(wVertical, 0));
#endif
