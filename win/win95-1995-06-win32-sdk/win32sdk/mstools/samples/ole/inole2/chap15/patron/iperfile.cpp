...
STDMETHODIMP CImpIPersistFile::Load(LPCOLESTR pszFile, DWORD grfMode)
    ...
    wcstombs(szFile, pszFile, MAX_PATH);
    uRet=m_pDoc->ULoad(TRUE, szFile);
#else
    uRet=m_pDoc->ULoad(TRUE, (LPOLESTR)pszFile);
#endif
...
STDMETHODIMP CImpIPersistFile::Save(LPCOLESTR pszFile, BOOL fRemember)
    ...
    TCHAR szFile[MAX_PATH];
    wcstombs(szFile, pszFile, MAX_PATH);
    uRet=m_pDoc->USave(0, szFile);
#else
    uRet=m_pDoc->USave(0, (LPOLESTR)pszFile);
#endif
...
STDMETHODIMP CImpIPersistFile::GetCurFile(LPOLESTR * ppszFile)
    ...
    *ppszFile=NULL;
...
#if defined(WIN32) && !defined(UNICODE)
    TCHAR pszTemp[OLEUI_CCHPATHMAX];
    uRet=m_pDoc->FilenameGet(pszTemp, OLEUI_CCHPATHMAX);
    mbstowcs(psz, pszTemp, OLEUI_CCHPATHMAX);
#else
    ...
    if (0==uRet)
#if defined(WIN32) && !defined(UNICODE)
        mbstowcs(psz, (*m_pDoc->m_pST)[IDS_EXTENSION], OLEUI_CCHPATHMAX);
#else
