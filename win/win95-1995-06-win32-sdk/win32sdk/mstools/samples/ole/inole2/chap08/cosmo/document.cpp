...
BOOL CCosmoDoc::FInit(PDOCUMENTINIT pDI)
    ...
    if (NULL!=m_pDropTarget)
        ...
        /*
         * Connecting to a drag & drop target and disconnecting
         * from it creates a sequence where the "stub manager"
         * object created on behalf of the source will do an
         * AddRef and Release on some internal pointers.  The
         * Release causes it to disconnect from the target, in
         * which case your target can only be used once.  Calling
         * CoLockObjectExternal prevents the stub manager from
         * entering such a 'zombie' state.
         */
