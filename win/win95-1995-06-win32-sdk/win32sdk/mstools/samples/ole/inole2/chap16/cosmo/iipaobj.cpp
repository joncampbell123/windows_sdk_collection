...
STDMETHODIMP CImpIOleInPlaceActiveObject::OnDocWindowActivate
    ...
    hWndGB=m_pObj->m_pGB->Window();

    if (fActivate)
        {
#if defined(WIN32) && !defined(UNICODE)
        OLECHAR pwcsTemp[256];
        mbstowcs(pwcsTemp, (*m_pObj->m_pST)[IDS_INPLACETITLE], 256);
        m_pObj->m_pIOleIPFrame->SetActiveObject(this, pwcsTemp);
#else
        ...
        if (m_pObj->FInPlaceToolsRenegotiate())
            {
            RECT    rc;

            m_pObj->m_pIOleIPFrame->GetBorder(&rc);
            SetWindowPos(hWndGB, NULL, rc.left, rc.top
                , rc.right-rc.left, rc.top+m_pObj->m_cyBar
                , SWP_NOZORDER);

            ShowWindow(hWndGB, SW_SHOW);
            }
        }
    else
        {
...
STDMETHODIMP CImpIOleInPlaceActiveObject::ResizeBorder(LPCRECT pRect
    ...
    SetWindowPos(m_pObj->m_pGB->Window(), NULL, pRect->left
