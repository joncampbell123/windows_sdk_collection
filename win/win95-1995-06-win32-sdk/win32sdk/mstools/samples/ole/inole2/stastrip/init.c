...
BOOL WINAPI StatStripMessageMap(HWND hWnd, HWND hWndOwner
    ...
    //   This next call does not work under Windows95.
    // VirtualProtect(pST->pSMM, cbRes, PAGE_READWRITE, &dwPrevProt);
    //   Instead, we'll copy the string resources into
    //   our own buffer so that they can be modified as
    //   needed.
    {
        STATMESSAGEMAP * pTemp = LocalAlloc(LMEM_FIXED, cbRes);
        if (pTemp == NULL)
        {
            StatStripClean(pST, FALSE);
            return FALSE;
        }
        memcpy(pTemp, pST->pSMM, cbRes);
