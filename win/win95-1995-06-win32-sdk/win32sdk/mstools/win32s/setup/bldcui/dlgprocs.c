...
BOOL FAR PASCAL FInfoDlgProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
	switch (wMsg)
		{
    case WM_INITDIALOG:
        if (OleOnly && !Ole16RunApp) {
            HWND hItem;

        	SetDlgItemText(hdlg, 1401, "Press Exit to quit Setup without "
        		"installing Freecell.");
            if (hItem = GetDlgItem(hdlg, 1001))
    ...
    case WM_INITDIALOG:
        {
            ...
            if (OleOnly && !Ole16RunApp)
    	        SetDlgItemText(hdlg, 100, "Setup will now return to Microsoft "
        	    	"Windows.");

    		if (hWnd = GetDlgItem(hdlg, ID_APP_FILE_NAME)) {
                ...
                szAppName = &szAppName1[10];
                if (!Win32sGetRunningApp(szAppName, sizeof(szAppName1), NULL)) {
                    EnableWindow(GetDlgItem(hdlg, ID_APP_FILE_NAME1), FALSE);
            		return(fTrue);
                }

                hDC = GetDC(hWnd);
				GetCharWidth(hDC, 0, 255, aiCharW);
                ReleaseDC(hWnd, hDC);
                ...
                while (l && w1 > 0) {
                     if (szAppName[--l] == '/')
                         szAppName[l] = '\\';
                     w1 -= aiCharW[szAppName[l]];
                }
				if (l) {
    				reserve = aiCharW[szAppName[0]] + aiCharW[':'] +
                    	aiCharW['\\'] + aiCharW['.'] + aiCharW['.'] +
                        aiCharW['.'] + aiCharW['\\'];
                    w1 -= reserve;
                    while (w1 < 0 || szAppName[l] != '\\')
						w1 += aiCharW[szAppName[++l]];
                    szAppName[--l] = '.';
                    szAppName[--l] = '.';
                    szAppName[--l] = '.';
                    szAppName[--l] = '\\';
                    szAppName[--l] = ':';
                    szAppName[--l] = szAppName[0];
                }
                SetDlgItemText(hdlg, ID_APP_FILE_NAME, &szAppName[l]);
            }
        }
		return(fTrue);
...
int FAR PASCAL SetCuiFlags(int InOleOnly, int InOle16RunApp)
{
    OleOnly = InOleOnly;
    Ole16RunApp = InOle16RunApp;
